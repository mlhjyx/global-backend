...
- **模型与 embedding 网关 = new-api 中转站**（单一 OpenAI 兼容端点）。2026-07-17 本机 `/v1/models` 令牌视图返回 39 个可调用型号，已包含 DeepSeek/方舟原路由，以及 `gpt-5.6-terra/sol/luna`、`claude-sonnet-5`、`gemini-3.5-flash`、`gpt-image-2` 和自托管 `bge-m3`；这只证明通道/型号可调用，**不等于任何 Site Builder task 已切路由或通过 task-shaped 评测**。现役唯一真值仍是 `task-routes.ts`；ADR-020 记录质量优先目标组合，逐 task 晋级仍服从 ADR-016。`gemini-3.1-flash-image`、`gemini-3-pro-image`、`gemini-omni-flash-preview` 当前不在该令牌可调用清单，不能冒充已接。旧别名 `deepseek-chat`/`deepseek-reasoner` 官方 2026-07-24 关停，一律用显式 V4 型号。
