...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）、R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2 与 R4-A1** 均已闭环，不把 DOC-12 重新描述为一项代码能力。**下一施工顺序**：R4-A2 Claim/Evidence truth bridge → R4-B-min，再进入 M1-d 多语种事实受限文案；R1-min 的生产对象存储 Release、跨节点恢复/回收与 unknown component 门须在 M1-e 可见预览前完成。R3-B2 已实现本地 durable artifact + 原子 symlink pointer，但不等于生产 R1-min；R4-A1 只冻结来源并建立 EvidenceRef v2 精确 quote/hash/provenance，不包含 Claim 真值桥、value/quote 语义对齐、snippet 发布降级或认证发布门。上述开发验证均不代表生产部署；M1-c 也仍没有公开 process/select API 或 Renderer `<picture>`。
>
