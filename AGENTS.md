...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）、R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c 与 R3-A** 均已闭环，不把 DOC-12 重新描述为一项代码能力。**下一施工顺序**：R3-B → R4-A1 Evidence 2.0 → R4-A2 Claim/Evidence truth bridge → R4-B-min，再进入 M1-d 多语种事实受限文案；R1-min 原子预览安全门须在 M1-e 可见预览前完成。R3-A 只表示 BuildRun 数据库背书已在 Ubuntu 开发环境验证，不代表生产部署；M1-c 已接纯 Sharp writer/refurbish P2，但没有公开 process/select API 或 Renderer `<picture>`，后两者不能冒充完成。
>
