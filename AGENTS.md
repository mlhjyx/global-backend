...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）、R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min 与 DI-0（#164）** 均已闭环，不把 DOC-12 重新描述为一项代码能力。DI-0 只交付净室设计合同与冻结的空静态 Catalog，不包含真实 Family、SiteSpec 1.1、Renderer/assembly 消费或 DesignEvaluation 运行时生产。**下一施工顺序**：继续 M1-e-A 55 型封闭组件与变体（#165 只落提取、组件 props Zod 与 dev gallery 基线；首批 `HeroBanner`/`StatsBand`/`CtaBanner` 已完成技术网格基线与机器可验七件套，状态为 `m1_e_a_qualified`。原 R1 冻结名单仍为 10 型 provenance，其中余下 7 型维持 `transitional_release` 兼容债，另 45 型为 `gallery_only`；资格引用同时复核证据 JSON、fixture 与三断点截图的 SHA-256 字节）→ M1-e-B 六个 Family、DesignBrief 与受控组装；`template-distillation` 只可选择性提取，不能整包反向改写批准合同。R4-A2 已建立内部 Claim/Evidence truth bridge；R4-B-min 已建立 BuildRun DB hard cap、task attempt fencing、model/tool spend ledger、终态 `site-builder-cost-summary/v1` 与取消/状态/预算 kill switch，未知结算不继续 fallback，estimate 不冒充真实成本。上述开发验证均不代表生产部署；M1-c 仍没有公开 process/select API 或 Renderer `<picture>`。
>
