> 【2026-07-10 文档体系重构】单一事实源体系已建立：定位/边界看 [docs/product-scope.md](docs/product-scope.md)（边界=止于 LeadQualifiedPackage，QGO 归 SaaS；身份归 A+ADR-011）· as-built 架构与 8 项缺口看 [docs/architecture/current.md](docs/architecture/current.md) · 决策看 [docs/adr/registry.md](docs/adr/registry.md) · 当前状态看 [docs/status/current.md](docs/status/current.md) · 收口与路线看 [docs/roadmap/release-plan.md](docs/roadmap/release-plan.md)。**先收口（六项）再加 provider。** 本文件其余内容与 CLAUDE.md 同步维护，冲突以上述权威层为准。

# AGENTS.md — 项目记忆（每会话自动加载）

> 给接手本仓库的 Codex 会话（本地 Mac 或远端 WSL）。随 git 同步，是跨会话的权威上下文。
> 详细进度看 [docs/roadmap/changelog.md](docs/roadmap/changelog.md)。
...
- **提交**：Conventional Commits（`feat(scope):` / `fix` / `docs` / `refactor` / `test` / `chore`）；正文说清「为什么 + 实测」；末行 `Co-Authored-By: Codex …`。
