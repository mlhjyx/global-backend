...
- **模型与 embedding 网关 = new-api 中转站**（单一网关，按模型显式选择已验证的 OpenAI Chat/Responses/Anthropic Messages 协议）。2026-07-17 本机通用令牌 `/v1/models` 返回 39 个可调用型号，已包含 DeepSeek/方舟原路由，以及 `gpt-5.6-terra/sol/luna`、`claude-sonnet-5`、`gemini-3.5-flash`、`gpt-image-2`；自托管 BGE-M3 只以私有别名 `site-builder-bge-m3-local` 暴露，专用令牌只允许该别名。#140 合并并完成 Ubuntu 开发环境切换后，真实 `EmbeddingsClient` 经 new-api 返回两组 1024 维有限向量，专用令牌 `/v1/models` 仅返回该别名；这不代表生产部署。现役文本路由唯一真值仍是 `task-routes.ts`：2026-07-18 只有 `site_builder.brand_profile` 经同形 6×2 评测成为代码级 `promotedRoute`=`gpt-5.6-terra`（Responses）→`claude-sonnet-5`（Messages），有 EvidenceRef v2 任务硬门、证据 id 与一键回 DeepSeek Pro→GLM；其他 6 个文本 task 仍是原 currentRoute。ADR-020 记录整组质量优先目标，逐 task 晋级仍服从 ADR-016。`gpt-image-2` 的 `/images/generations` 单次真探已成功，但 edit/mask、生产 MediaGateway 与消费者未落，绝不冒充上线；`gemini-3.1-flash-image`、`gemini-3-pro-image`、`gemini-omni-flash-preview` 当前不在该令牌可调用清单。旧别名 `deepseek-chat`/`deepseek-reasoner` 官方 2026-07-24 关停，一律用显式 V4 型号。
...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）、R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2 与 R4-A1/A2** 均已闭环，不把 DOC-12 重新描述为一项代码能力。**下一施工顺序**：R4-B-min，再进入 M1-d 多语种事实受限文案；R1-min 的生产对象存储 Release、跨节点恢复/回收与 unknown component 门须在 M1-e 可见预览前完成。R4-A2 已建立内部 Claim/Evidence truth bridge：事实键只接受严格 lower_snake_case，非 canonical 输入不静默归一；origin Claim 审批必须仍有 exact Site bridge，应用锁 Claim、数据库受限 security-definer trigger 锁 bridge，直写同样 fail-closed；Workspace 在 Site/对象清理编排落地前禁止物理级联删除；cert Asset 反向扫描由 partial ordered covering index 支撑。A2 未新增 endpoint、SiteSpec/CopyBundle 或正式 `PublishableClaimSnapshot`，也不代表生产部署；M1-c 仍没有公开 process/select API 或 Renderer `<picture>`。
>
...
- **worktree 生命周期**：合并后删除只是可选清理，不是开发必需。仅当 PR 已合并、提交已进入 `main`、worktree 干净且未跟踪文件均已归属时才可删除；仍需回查、继续维护或保留现场时可以继续留在 `/global/wt/`，定期审计即可。
