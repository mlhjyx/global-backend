...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）、R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2 与 R4-A1/A2/B-min** 均已闭环，不把 DOC-12 重新描述为一项代码能力。**下一施工顺序**：M1-d 多语种事实受限文案；R1-min 的生产对象存储 Release、跨节点恢复/回收与 unknown component 门须在 M1-e 可见预览前完成。R4-A2 已建立内部 Claim/Evidence truth bridge；R4-B-min 已建立 BuildRun DB hard cap、task attempt fencing、model/tool spend ledger、终态 `site-builder-cost-summary/v1` 与取消/状态/预算 kill switch，未知结算不继续 fallback，estimate 不冒充真实成本。两者均不代表生产部署，也未改变 MODEL-1 路由证据；M1-c 仍没有公开 process/select API 或 Renderer `<picture>`。
>
