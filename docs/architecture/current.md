...
- 离线候选评测现为代码级 `site-builder-model-evaluation-harness/2026-07-28-v2`：evaluation-only executor 只通过依赖注入的 wire/cost seam 连接 canonical harness request，文本协议固定映射 Responses、Messages，并以隔离入口保留 legacy Chat comparator；其 transport admission 与生产 `VERIFIED_GATEWAY_MODEL_TRANSPORTS` 分离。trusted probe/run 只接受 factory 私有 WeakSet 品牌化并冻结的 target execute，不暴露 register/test seam；任意 callback/wrapper 在预算 reserve 与 client dispatch 前拒绝。媒体、embedding、preview、deferred、legacy-only target、无消费者或无 suite 同样在 client 前拒绝；当前唯一完整 suite 是 BrandProfile。task/protocol/envelope、canonical suite、矩阵、排序、预算与完整 provenance 合同只认[生成评测基线](../site-builder/model-evaluation-harness.md)。当前测试仅使用 fake wire/fetch/settlement，未接生产 `ModelGateway`、Temporal 或媒体消费者；这仍不是 capability 证据、真实评测报告、`evaluatedCandidate` 或路由晋级。
