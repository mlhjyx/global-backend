...
- R4-A2 已在 BrandProfile 持久化边界加入确定性 value/quote 关键值对齐与 `research_hint` fail-closed；新 intake 原子创建 CompanyProfile→Site 复合租户关系，`Site.companyProfileId` 对历史行保持 nullable，未关联旧站禁止按显示名猜身份。bridge 的三列 Site FK 强制 company 与 Site 已绑定 company 相同；公开 Evidence 的 origin key 包含 snapshot/role/asset，插入 trigger 再逐字段核对 EvidenceRef 的 fact index、snapshot/hash/quote/selector 与 cert provenance。通过门的事实以稳定、分域 SHA-256 key 复用共享 Claim/Evidence，并写入不可变、FORCE RLS 的 `BrandProfileClaimBridge` 精确边；生成 Claim 只进 `NEEDS_REVIEW`。认证事实须按 UUID 稳定顺序在同一事务锁定同 workspace/site 的 ready cert Asset。人工审批以 status+version CAS 只允许一个并发赢家，proof 的 SHA-256 digest 绑定 Claim 身份/正文/有效期/批准版本；bridged/终态 Claim 身份与 bridged Evidence provenance 均不可改写。内部 approved-effective 读取对所有新桥接 Claim 重算审批 proof，legacy 认证也按 type+statement 分类并要求完整审计。该内部桥没有新增公开 HTTP API、SiteSpec/CopyBundle/正式 `PublishableClaimSnapshot` 或 M1-d 消费者，也不代表生产部署；两步迁移在生产前仍须以恢复快照测表规模/索引与锁时长，超出 5 秒锁预算则安排维护窗口。
- 模型面须分层描述：现役只认 `task-routes.ts`；7 个文本 task 中仅 `site_builder.brand_profile` 经真实 new-api 协议探针、同形 6×2 Golden 与成本/质量门成为代码级 `promotedRoute`=`gpt-5.6-terra`（Responses）→`claude-sonnet-5`（Messages），并有任务硬门和回 DeepSeek Pro→GLM 开关；其余 6 个仍是 DeepSeek/GLM/MiniMax/Doubao `currentRoute`。本机 new-api 已可见 GPT/Claude/Gemini 3.5/GPT Image 2。#140 合并后，Ubuntu 开发环境的应用已把 Ollama BGE-M3 以私有别名 `site-builder-bge-m3-local` + 专用模型受限令牌统一经 new-api 调用，并通过真实 `EmbeddingsClient` 两条 1024 维有限向量验证；这不代表生产部署。ADR-020 是整组质量优先目标，其他逐 task promotion 仍受 ADR-016 约束；图片/视频无生产 MediaGateway/消费者；BGE 路由迁移不换模型、维度或 `embed_version`，因此不触发重嵌。
- 当前关键路径：**R4-B-min → M1-d**。R4-A2 已关闭内部 Claim/Evidence truth bridge；BrandProfile 整体重试幂等和持久成本真值仍属 B-min，正式 Copy/locale/SiteSpec 消费仍属 M1-d。R1-min 的生产 Release 边界仍须在 M1-e 可见预览前完成。
