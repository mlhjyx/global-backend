...
- 离线候选评测新增代码级 `site-builder-model-evaluation-harness/2026-07-27-v1`，但没有接入生产 `ModelGateway`、Temporal 或媒体消费者。其 task/protocol/envelope、canonical suite、矩阵、排序、预算与完整 provenance 合同只认[生成评测基线](../site-builder/model-evaluation-harness.md)；没有 suite 的 task 不可 dispatch。这仍不是 capability 证据、真实评测报告、`evaluatedCandidate` 或路由晋级。
