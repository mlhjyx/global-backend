...
本项目采用一套面向非开发者的“AI 软件开发操作系统”：产品负责人提供业务目标、优先级和验收判断；Claude Code 担任需求编译器、架构师与最终集成者；仓库和机器合同保存项目权威真值；研究、记忆、文档检索和代码图只提供受控的认知辅助。
...
flowchart LR
    ...
    J["文档检索<br/>候选 QMD"] --> B
    K["普通静态代码认知<br/>候选 CodeGraph"] --> D
    K --> E
    M["动态契约认知<br/>仓库 ContractGraph"] --> D
    M --> E
    L["Claude Code 主代理<br/>架构师与集成者"] --> B
    L --> D
    L --> E
    L --> F
```
...
默认情况下，Claude Code 在开始工作和发生模式切换时，用一句话报告“当前模式、将产生什么、明确不做什么”。
...
产品负责人不需要负责数据库选择、接口细节、测试框架或逐行代码审查。Claude Code 必须把需要决定的内容翻译为业务影响、选项、推荐和后果。
...
Claude Code 不能代替产品负责人批准产品范围，也不能代替隐私、安全、法务、运营或发布 Owner 签字。AI 自评、测试通过或截图都不能单独证明真实用户可用。
...
共享 ADR、公共类型、数据库 Schema 和跨模块合同默认由 Claude Code 主代理控制。Claude Code 负责最终集成并关闭 CI、审查和冲突门。常规 PR 的 squash 合并遵循 `DEC-AIDEV-004`：开发代理独立审查通过、必需检查在当前 head 全绿、审查线程清零，且与最新 main 的组合已在本地复核后可合并；不包括 ruleset/仓库设置/权限变更、部署发布与付费调用。`DEC-AIDEV-003` 的四 PR 批准保留为历史授权；现行范围与撤回方式见[决策登记](../governance/conflict-register.md#11-aidev-gate-1-已批准决策)。CI、静态检查、审查机器人和检索工具是证据或闸门，不是产品批准者。

### 3.4 合并就绪审查代理与产品负责人说明

产品负责人不承担代码审查。每个进入合并候选的 PR 在正文「给产品负责人的说明」区段用业务语言回答四件事：你会得到什么、明确没有改变什么、风险与回退、建议（合并 / 暂缓 / 需要你决定）。中高风险、跨模块、发布相关或产品负责人指定的 PR，再由一个不修改文件、不回复审查、不执行合并的独立审查代理复核，只给出 `RECOMMEND_MERGE / RECOMMEND_HOLD / NEED_USER_DECISION`，不能代替真实 Owner 批准。

如果说明仍包含产品负责人无法理解的术语，不能要求其盲批。开发代理必须先用业务影响和具体例子解释；解释后仍无法判断的事项保持待决或升级到相应真实 Owner。

说明是作者自述，不经机器解析，也不构成授权：CI 全绿、审查代理建议或正文里的任何表述都不自动构成合并授权。合并前由开发代理对当前 exact head 分别回读真实 CI、审查结论与未解决讨论；合并授权只来自产品负责人：对该 PR 的明确指示，或 `DEC-AIDEV-004` 的常设授权。技术门、独立审查、用户授权三者分离的原则不变。

> 2026-09-21 起取代原「非技术合并决策卡」（14 个单行字段、绑定 head SHA、由 `pull_request_target` 工作流解析并作为 `nontechnical decision card freshness` 必需检查）。原因：按其自身规则，卡片内容只是未验证声明、授权字段恒为 `NOT_AUTHORIZED`，机器检查既不能验证真实性也不承载授权；却让每次推送都使卡片失效，两天内产生 123 次运行、40 次失败与 42 次取消。原合同见 git 历史与[真实验证记录](decision-card-live-validation.md)。

## 4. 什么问题 Claude Code 应自行解决

在不改变已批准目标、风险边界和用户承诺时，Claude Code 应自行处理：
...
出现以下情况时，Claude Code 必须给出证据和推荐后请求决定：

| 需要决定的事项                         | 为什么不能由 Claude Code 补猜  |
| -------------------------------------- | ------------------------ |
...
升级不是把技术问题原样丢给产品负责人。Claude Code 必须先完成安全的只读调查，并使用以下格式：
