...
共享 ADR、公共类型、数据库 Schema 和跨模块合同默认由 Codex 主代理控制。Codex 可以完成最终集成并关闭 CI、审查和冲突门，但每个 PR 的最终合并都必须等待产品负责人对该 PR 的明确授权；不能从 CI 全绿、阶段批准或以往合并授权中推定。CI、静态检查、审查机器人和检索工具是证据或闸门，不是产品批准者。
