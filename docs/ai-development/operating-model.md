...
共享 ADR、公共类型、数据库 Schema 和跨模块合同默认由 Codex 主代理控制。Codex 可以完成最终集成并关闭 CI、审查和冲突门。默认情况下，每个 PR 的最终合并都必须等待产品负责人明确授权，不能从 CI 全绿、阶段批准或以往合并授权中推定；只有产品负责人针对精确实施计划另行批准自动合并门时，Codex 才能在该范围内按“独立只读审查通过、CI 全绿、线程清零、基于最新 main”自动合并。`DEC-AIDEV-003` 只为其四 PR 建立这一例外，不外推到生产发布或其他 PR；2026-09-21 起 `DEC-AIDEV-004` 把同一合并门扩展为常规 PR 的常设授权，范围与撤回方式见[决策登记](../governance/conflict-register.md#11-aidev-gate-1-已批准决策)。CI、静态检查、审查机器人和检索工具是证据或闸门，不是产品批准者。
