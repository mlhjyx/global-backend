...
`DEC-AIDEV-003` 已批准仓库原生 ContractGraph：它用 TypeScript Compiler API 与确定性配置提取器把业务 Registry、NestJS、Temporal、Outbox、Prisma、workspace、Astro 和部署关系连接为当前 worktree 的派生图。它只提供第一张影响地图；查询必须先核对 worktree、commit 和 source hash，且动态注册、运行时 DI、外部消费和数据库真实行为仍须回到当前源码、测试和运行证据。
...
| CodeGraph              | 当前 worktree 的普通静态调用关系试点 | 用 main 图回答分支、把图当运行事实       | 30 题评测证明比 ContractGraph+源码基线有净收益 |
