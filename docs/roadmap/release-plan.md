...
3. **MF-0-thin（连续两个独立交付，总范围不缩水）**：
   - **MF0-A ✅ 2026-07-17 完成**：`AssetVariant` + RLS/FORCE RLS、单输出 recipe/checksum/复合 provenance、Variant 专属对象键与不可改写来源账本、`derivedKeys` 响应式共享合同及纯兼容投影；真 PostgreSQL A/B/unset 隔离、并发 unique、增量/空库 40 migrations 与 schema diff=0 已验证。仅为 Ubuntu 开发环境，不代表生产部署。
