...
16. **盲评视觉校准首轮证据 🟡 2026-07-25**：六 Family 的确定性来源图与冻结劣化图形成 6 对，每对同断点双图 × 3 次固定左右顺序；保留原始 PNG，以质量与稳定性优先。固定提交 `e6320d6` 的 [campaign report](../evidence/model-routing/blind-visual-calibration-20260725-final-pair-matrix-v1/campaign-report.json) 记录四候选串行、隔离的真实调用与稳定 source bundle。Gemini 因实际型号 `gemini-default` 不匹配冻结型号而 unavailable；Sonnet、Terra、Sol 各完成 18/18 格式/provenance 成对调用，但已知问题命中为 12/18、13/18、13/18，均未到 ≥17/18 单模型门，且无两个不同 upstream family 通过者，故不产生双模型组合结论。120 秒只为延迟观察，`maxTokens=4096` 是请求容量而非评分上限；这次结论来自完整成对矩阵。按观测 token 与冻结价表归集的已知计算成本为 $1.078987；网关账单未完整返回，Gemini probe 明确未知。后验原图成本预测 $6.8345，仅供下一轮预算判断。**P4/DesignEvaluation v2/ReleaseManifest/Temporal/路由完全未改**；源图不称 aesthetic Gold，结论为 `no_promotion_or_runtime_route_change`。任何运行时双模型门仍需两个稳定候选、独立 Gold 与用户另行批准。
