...
- R1-safety 两个小 PR 与 R2-A1 Asset 正确性门已于 2026-07-17 完成；Asset 已具备 CAS/fencing、retry/duplicate 收敛、tombstone 与 parked cleanup Outbox，开发环境真实 PostgreSQL/RLS/MinIO 和空库 migration 全绿。
...
2. **R2-A 正确性门（拆分 PR）**：R2-A1 Asset ✅；当前做 R2-A2 KB lease/fencing/retry/幂等，随后 R2-A3 Profile schema/乐观并发、R2-A4 Outbox/Temporal cleanup/redrive/真实集成。禁止合成一个 migration/API/状态机 mega PR。
