...
- 对 Phase 1–5 文档运行非破坏性校验，并在内容稳定后创建本地 docs-only checkpoint commit；不推送、不建 PR、不合并。

### 1.2 当前未授权

- 不进入第 6–8 阶段；Gate 4 只授权独立站模块文档，不把它升级为视觉定稿、前端施工或用户可用实现。
- 不创建其他产品域 Capability Pack、OSS adoption cards、真实用户/管理员发布指南或 Release Bundle。
...
当前状态：Phase 4 已形成 [全局前端规范](../frontend/README.md)、[设计治理入口](../design/README.md)和 [Gate 4 评审包](saas-frontend-phase-4/gate-4-review.md)，并由产品负责人按 `DEC-FE-P4-001..011` 批准。正式前端 repo、设计事实源、Workspace/allowed actions、Claim review、指标/隐私和实际 QA/运营/安全责任继续作为 blocker。
...
4. Gate 4 已批准 Workspace 管理员、内容审批人、运营和平台管理员的权限/数据社会属性基线；Phase 5 已映射 Site 动作，实际 SaaS 角色与 allowed actions 合同仍待定。
5. 站点发布、域名、询盘、Cookie/隐私和数据保留的首发市场要求。
6. 哪些冻结产品域只保留产品地图，哪些需要同步做到 Dev-Ready。
7. Word 中 AiToEarn、Chatwoot、媒体和知识图谱候选的业务优先级。
8. Gate 4 已批准统一任务/通知/审批/异常模式；首期合同、读模型和实施范围仍待正式 SaaS Owner 决定。
