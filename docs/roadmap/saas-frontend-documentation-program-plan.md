...
> 版本：v0.7-gate-4
> 状态：`PHASE_4_READY_FOR_GATE_4_REVIEW`
> 批准基线：`v0.2-approved / APPROVED_PLAN`；v0.3–v0.4 回写 Phase 1，v0.5 回写 Phase 2，v0.6 回写 Phase 3，v0.7 只回写用户随后明确授权的 Phase 4 全局前端规范与 Gate 状态，不扩张 Phase 5–8 授权
> 计划批准：2026-07-20（含产品工作方式、Capability Pack、设计资产、场景库和发布学习闭环增补）
> 执行授权：`PHASE_4_GRANTED`——Gate 3 已由产品负责人明确通过，并授权“全局前端规范”；Phase 5–8、模块 Dev-Ready 施工和产品代码实现仍未获授权
> 基线提交：`676c6cdc175326927ec341a2d585168aa0a1a374`
> 适用范围：完整 SaaS 产品前端，其中“独立站管理”是当前优先模块
> 拍板人：产品负责人
> 执行主体：Codex（Phase 1/2/3/4 均于 2026-07-20 分别获明确授权；Gate 4 后须再次取得明确批准）
...
- Gate 3 通过后执行第 4 阶段“全局前端规范”：建立 `docs/frontend/`、设计资产/微文案登记，以及跨模块可复用的 Shell、权限、状态、AI/Evidence/Approval、a11y、性能、集成、测试和发布证据规则；不进入模块 Dev-Ready。
- 对 Phase 1–4 文档运行非破坏性校验，并在内容稳定后创建本地 docs-only checkpoint commit；不推送、不建 PR、不合并。

### 1.2 当前未授权

- 不进入第 5–8 阶段；Gate 3 只授权全局规范，不把它升级为独立站模块 Dev-Ready、视觉定稿、前端技术方案或用户可用实现。
- 不创建 `docs/frontend/modules/`、实施方案、用户/管理员/运营指南或真实 Release Bundle。
...
- 不自行改变 Gate 2 已批准导航，也不替产品负责人决定新的商业承诺、数据权利、OSS 采用、设计工具或跨仓 ownership。

### 1.3 后续启动条件

第 1–3 阶段已完成并分别通过 Gate 1–3；第 4 阶段随后获明确授权。完成 Phase 4 后必须停在 Gate 4，提交完整评审包并等待下一次明确批准；在此之前不得进入第 5–8 阶段。对权限、商业承诺、数据权利、OSS 采用、设计工具、正式前端仓库和跨仓 ownership 的新裁决，执行过程中仍须单独暴露，不得由执行者静默决定。
...
当前状态：计划内容已批准；Phase 1–3 已完成并通过 Gate 1–3；Phase 4 已获授权并形成 Gate 4 评审包；Phase 5–8 仍未授权。
...
当前状态：Phase 3 已建立 [项目文档门户](../README.md)、[治理 Registry](../governance/README.md)和 [Gate 3 评审包](saas-frontend-phase-3/gate-3-review.md)，并于 2026-07-20 由产品负责人以“Gate 3 通过，授权 Phase 4”明确批准。Phase 3 包保持冻结 provenance，current Registry 在后续阶段增量维护。
...
当前状态：Phase 4 已形成 [全局前端规范](../frontend/README.md)、[设计治理入口](../design/README.md)和 [Gate 4 评审包](saas-frontend-phase-4/gate-4-review.md)，状态为 `READY_FOR_GATE_4_REVIEW`。正式前端 repo、设计事实源、Workspace/allowed actions、Claim review、指标/隐私和实际 QA/运营/安全责任仍作为 blocker；本阶段没有创建模块文档、视觉稿、代码、可执行 Fixture 或真实 Release Bundle。
...
4. Gate 4 候选已给出 Workspace 管理员、内容审批人、运营和平台管理员的权限/数据社会属性基线；实际 SaaS 角色与 allowed actions 合同仍待定。
5. 站点发布、域名、询盘、Cookie/隐私和数据保留的首发市场要求。
6. 哪些冻结产品域只保留产品地图，哪些需要同步做到 Dev-Ready。
7. Word 中 AiToEarn、Chatwoot、媒体和知识图谱候选的业务优先级。
8. Gate 4 候选已建议建设统一任务/通知/审批/异常中心；首期合同、读模型和实施范围仍待正式 SaaS Owner 决定。
...
当前 handoff 状态：`PHASE_4_READY_FOR_GATE_4_REVIEW`。

产品负责人已通过 Gate 3 并授权 Phase 4；统一 SaaS 的范围/原则、用户/旅程、IA/Shell、权限、状态恢复、AI/Evidence/Approval、设计系统/资产/微文案、响应式/a11y/性能/i18n、合同接入、分析/测试/发布证据已经形成 [Gate 4 评审包](saas-frontend-phase-4/gate-4-review.md)。后续可选择：

- `Gate 4 通过，按 DEC-FE-P4-001..011 批准全局前端规范，并授权 Phase 5`：只进入独立站管理 Dev-Ready 文档包；
- `Gate 4 有条件通过`：先补指定全局模式、Owner 或追踪 finding，再请求复核；
- `Gate 4 不通过`：修改指定权限、状态、AI、设计、质量或接入规则。

Gate 4 通过也不自动授权 Phase 6–8、正式前端实现、产品代码、Schema、OpenAPI、基础设施、依赖、历史文件移动、push、PR 或合并；正式 repo、设计源、权限/Claim/指标合同等 blocker 也不会被自动关闭。

在收到明确 Gate 4 决策前，继续停止第 5–8 阶段，不编写独立站模块 PRD/UX、前端实施方案、产品代码或实际设计资产。
