...
5. ✅ **跨源身份解析**（待办 2，已落地，见 [decision-maker-cross-source-identity-design.md](decision-maker-cross-source-identity-design.md) · PR #54）：落库前 `resolvePersonIdentity` 前置（同公司 4-Tier：externalId/邮箱精确/归一名/高置信模糊 + 🔴 邮箱冲突守卫 + 严阈值 0.9·margin 0.1，宁欠并不错并），修 email/无-email 桥 + 人名变体重复，建成待办 3 复用缝（`externalIds`→Tier 0）。matchRule 走 field_evidence（零迁移）。**遗留**：待办 3 P1 身份源。
6. ✅ **待办 3 首源 · 公司注册处董事（UK Companies House）**（已落地，见 [decision-maker-p1-companies-house-design.md](decision-maker-p1-companies-house-design.md) · PR #58）：对 fit=match 英国公司 → CH 官方注册处取现任董事 → 高置信对齐公司（🔴 GB country 门 + `pickBestByName` 0.9·margin）→ `externalId(uk-ch-officer)` 走 **Tier 0** 精确并/新建（**兑现待办 2 缝**，同董事跨 Impressum 自动并一条）。🔴 数据最小化（只 name+role+officer_id，不摄 DOB/国籍/住址）+ §8.8 用途门 fail-closed + OGL-UK-3.0 署名 + `hasExternalIdConflict` 防同名不同 id 误并。联系人发现改 fan-out 全 adapter。**遗留**：待办 3 后续源（专利 inventor USPTO/EPO、商标 EUIPO/WIPO；CH 扩德/法）。
7. 🔄 **待办 3 第二源 · 专利发明人（BigQuery Google Patents，替代被封 EPO OPS）**（代码完成·CI 绿·seed **DISABLED** 待 GCP key，见 [decision-maker-p1-google-patents-inventor-design.md](decision-maker-p1-google-patents-inventor-design.md)）：EPO OPS（PR #61）账号被网关封停 → 改走 **BigQuery Google Patents Public Data**（`patents-public-data.patents.publications`，IFI CLAIMS 谐调；仅需 Google 账号，**无审批/无身份墙/无封号**）。assignee 检索近 5 年专利 → 具名 inventor = 技术买家 → 高置信对齐 → **归一名并**（🔴 无 person id → 非 Tier 0，走待办 2 Tier 2/3）。provider 对齐/合规逻辑**源无关移植自 #61**（独家申请人防误挂 + 国别门 + cap 25 + 数据最小化只 name）。🔴 `maximumBytesBilled` 成本护栏护 1TB/月免费额度 + §8.8 用途门 fail-closed + CC BY 署名（⚠️ ENABLE 前核实文案）。⚠️ 规模：publications 无 assignee 分区 → 每查全表扫，适合有界/周期 sweep；高频生产需物化小表（fast-follow）。EPO 代码 PR #61 留档 DISABLED。
8. ✅ **待办 3 第三源 · 法国 dirigeants（INPI RNE 经开放政务网关）**（已落地，见 [decision-maker-p1-inpi-rne-dirigeant-design.md](decision-maker-p1-inpi-rne-dirigeant-design.md)）：对 fit=match 法国公司 → 开放 API Recherche d'entreprises（`recherche-entreprises.api.gouv.fr`，数据源 = INPI RNE + Sirene，**零鉴权** → 本会话真测通过，避开 EPO 审批坑）取 dirigeant（personne physique）= 具名经济买家 → 高置信对齐公司（🔴 FR country 门 + `pickBestByName` 0.9·margin）→ **归一名并**（🔴 无 person id → 非 Tier 0，同 EPO 走 Tier 2/3）。🔴 数据最小化（只 nom+prenoms+qualite，**源头剥离** API 主动吐的 DOB/annee/nationalite）+ §8.8 用途门 fail-closed + Licence-Ouverte-2.0 署名 + 跳过 personne morale/审计师 + 非公示公司网关自动排除。seed **ENABLED**（可真测，不同于 EPO）。

> **已落地实测**（真库真爬真 SMTP、无 sandbox）：
> - 法国 dirigeants（`scripts/verify-inpi-rne.mts`，真库真开放 API，五段全绿）：WILO FRANCE(FR)→对齐 1.00→真拉 4 名 dirigeant（Philippe Marjollet *Président de SAS*→economic_buyer + 3 board→decision_maker）；personalData/Licence Ouverte/**无 DOB·国籍·externalIds**；落库+二次幂等（Tier 2 归一名 merged=4/created=0）；跨源并（Impressum 同名→并同一行 name_exact）；🔴 同名边界（2 同名行→无邮箱同名不误并 2→3，#62 歧义守卫）；§8.8 用途门 DENIED 零落地。质量 29 单测（adapter 数据最小化/类型过滤 + provider FR 门/对齐/角色/去重/fail-safe）。
