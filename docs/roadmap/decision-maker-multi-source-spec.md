...
6. ✅ **待办 3 首源 · 公司注册处董事（UK Companies House）**（已落地，见 [decision-maker-p1-companies-house-design.md](decision-maker-p1-companies-house-design.md) · PR #58）：对 fit=match 英国公司 → CH 官方注册处取现任董事 → 高置信对齐公司（🔴 GB country 门 + `pickBestByName` 0.9·margin）→ `externalId(uk-ch-officer)` 走 **Tier 0** 精确并/新建（**兑现待办 2 缝**，同董事跨 Impressum 自动并一条）。🔴 数据最小化（只 name+role+officer_id，不摄 DOB/国籍/住址）+ §8.8 用途门 fail-closed + OGL-UK-3.0 署名 + `hasExternalIdConflict` 防同名不同 id 误并。联系人发现改 fan-out 全 adapter。
7. ✅ **待办 3 第二源 · 专利发明人（EPO OPS）**（已落地，见 [decision-maker-p1-epo-ops-inventor-design.md](decision-maker-p1-epo-ops-inventor-design.md)）：原定 USPTO PatentsView（有消歧 `inventor_id`=干净 Tier 0），因其 2026-03 迁 USPTO ODP、key 改需 **ID.me 实名**（非美国身份卡死）→ 转 **EPO OPS**（官方 OPS API，OAuth2 client-credentials，零实名，CC BY 4.0）。按 applicant 检索近 5 年专利 → 具名发明人（🔴 数据最小化只取 name）→ 高置信对齐公司（`pickBestByName` 0.9·margin + 国别门）→ `technical_buyer`（技术评估席，上限 25/公司）。**EPO 无消歧人 id → 不走 Tier 0**：经归一名走 **Tier 2/3** 并入（第 3 个源压测跨源名并）；硬凑「公开号+名字」键会触 `hasExternalIdConflict` 误拆同一人，故**不产 externalIds**。🔴 §8.8 用途门 fail-closed + CC BY 署名（顺带补 `person.profile` 证据带**源 license** 缺口——无联系点的源靠此承载署名，CH 亦受益）+ 发明人 personalData。**遗留**：待办 3 后续源（商标 EUIPO/WIPO；CH 扩德/法；PatentsView 若日后可实名）。
...
**P1 —— 扩身份途径（免费绿源优先）**
4. ✅ **专利 inventor**（**EPO OPS 已落地**，见 §5.7；USPTO PatentsView 因 ID.me 实名墙暂缓）—— 绿事实、具名工程决策人。
