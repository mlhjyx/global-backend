...
2. **“独立站管理”是统一 SaaS 的一级产品区域；Astro 公开站只是该区域的版本化输出，不是第二个 SaaS 前端。** 当前后端在 intake、Profile、Asset/KB、Build、成本、取消、Claim/Copy 和开发预览上已深入落地，但正式 SaaS 控制面和生产 Release/Publish/Domain/Inquiry/Analytics 仍缺。
3. **页面、后端和用户可用性的成熟度严重不对称。** 原型展示了域名、发布、分析、询盘、SEO、博客等目标能力，但 main 的 Site Builder OpenAPI 只有 12 个操作，询盘明确禁用，生产 Release 的 R1-min 仍在未合并 worktree。
...
| 退出条件 | 证据 | 结果 |
|---|---|---|
| 所有已知材料已登记 | 66 份基线 Markdown、5 份 Word、1 份 HTML、92 JSON、本地前后端、10 模板、41 worktree、GoodJob 和 OSS 组均在来源总账 | 满足 |
...
| 历史 worktree 不冒充 main | 精确记录 main、R1-min-release、template-distillation、R4-A2 legacy 等差异和 dirty 状态 | 满足 |
| 定位真实 SaaS 前端 | `/global/frontend` 三套代码均审计；边界、规模、接口、质量和风险明确 | 满足 |
| 定位设计事实源 | 正式 Figma/Token/Storybook 等在扫描范围内 `NOT_FOUND`；现有原型/模板已分级 | 满足 |
| 冲突和缺失决策公开 | 23 条冲突、14 项承重决策、9 项缺失输入、15 项风险 | 满足 |
...
- 不请求合并 R1-min-release、template-distillation 或 legacy R4-A2 worktree。
