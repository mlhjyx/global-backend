...
企业资料与知识拥有 Company/Profile/Offering/Claim/Evidence/Asset/KnowledgeSource 的 canonical 工作区，但不会复制这些对象。全局搜索、任务、审批、通知、长任务、事故、帮助和对象 Inspector 是横向能力；管理与设置是第八个侧栏一级入口，但不是第八个业务价值域；公共站、身份、帮助/开发者和平台运营不进入 SaaS 主导航。完整管理员视图为 8 个一级/38 个二级，完整归属与遗漏处理见[信息架构与遗漏覆盖审计](information-architecture-and-coverage-audit.md)。
