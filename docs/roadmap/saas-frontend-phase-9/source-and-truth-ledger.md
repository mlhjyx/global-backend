...
- 组件目标集合为 55 型。当前 `status/current` 与仓内资格证据显示 13 型 `m1_e_a_qualified`、1 型 `transitional_release`、41 型 `gallery_only`；M1-e-A 尚未完成。
...
| `SRC-FE-P9-005` | [当前状态](../../status/current.md) | `main@8dcbbcb`；`L1` | `CROSS_CHECKED_AT_BASELINE` | 顶部日期未列每个 #178–#181，但正文已记录 13 型资格 | 当前主线与完成度 | 状态唯一入口 |
| `SRC-FE-P9-006` | [Release Plan](../release-plan.md) | `main@8dcbbcb`；`L1` | `CROSS_CHECKED_AT_BASELINE` | “进入 M1-e-A”没有表达当前 13/55 进度 | 施工顺序，不定义用户承诺 | 路线唯一入口；进度回 status |
...
| `SRC-FE-P9-022` | `docs/design/*` | Gate 4/5；`L2` | `CROSS_CHECKED_AT_BASELINE` | 设计资产登记、微文案、Site 书面线框 | 无 Figma URL/file key；书面线框不是 `DESIGNED` | [Figma 交付登记](figma-delivery-register.md) |
...
| `SRC-FE-P9-024` | current Phase 9 package | `main@8dcbbcb` 后新增；`L2/L3 proposal` | `CROSS_CHECKED_AT_BASELINE` | 设计范围、交互语言、12 原型旅程、Figma 交付 schema | 全部仍 `DRAFT`；不得表示 Figma 已创建 | 本阶段评审包 |
...
| `SRC-FE-P9-SB02` | [02 架构](../../site-builder/02-architecture.md) | `L2 / CROSS_CHECKED_AT_BASELINE` | 固定 DAG、bounded task、Release/媒体/模型边界 | 生产部署与目标 provider 不等于 as-built | integration/interface proposals |
...
| `SRC-FE-P9-SB04` | [04 SiteSpec](../../site-builder/04-sitespec-contract.md) | `L2 + machine-linked / CROSS_CHECKED_AT_BASELINE` | SiteSpec 1.0、55 型集合、七件套门、1.1 目标 | 资格摘要仍写 9/45，落后当前 13/41；1.1 字段未实现 | `CON-FE-P9-002/003`；机器 Registry 为资格真值 |
| `SRC-FE-P9-SB05` | [05 Hosting](../../site-builder/05-deployment-hosting.md) | `L2 / CROSS_CHECKED_AT_BASELINE` | 不可变 Release/发布目标与 DNS/TLS 风险 | 当前仅开发预览，未有 Publish/Domain 公开合同 | HostingTarget/Deployment 提案 |
| `SRC-FE-P9-SB06` | [06 Security](../../site-builder/06-security-abuse.md) | `L2 / CROSS_CHECKED_AT_BASELINE` | 权利、abuse、网络和发布安全门 | 不构成 BaoTa/渠道法务批准 | Provider准入/发布 Gate |
...
| `SRC-FE-P9-031` | `packages/contracts/src/site-builder/*` | `8dcbbcb / CROSS_CHECKED_AT_BASELINE` | SiteSpec 1.0、DI-0 合同、55 型集合、13 型七件套资格注册 | SiteSpec 1.1、真实 Catalog/Family、全组件资格 | Site设计事实与组件状态只读机器值 |
...
| `SRC-FE-P9-035` | `*.spec.ts`、renderer tests、`docs/evidence/**`、verify scripts | `8dcbbcb / MACHINE_INVENTORIED` | 当前代码已有不同强度测试；13 型各有七件套与 39 张三断点截图 | 本阶段未重跑全 API/DB/真实服务；不证明正式前端 E2E | QA scenario/fixture/release evidence 入口 |
...
|---|---|---|---|---|---|
| `SRC-FE-P9-040` | `git log` / main / origin | `8dcbbcb / CROSS_CHECKED_AT_BASELINE` | #178–#181 已进入 main；当前 worktree 基于同一 origin/main | commit 存在不等于用户发布 | current status + evidence |
...
| Site | 开发预览 substrate as-built；M1-e-A 13/55 qualified | 诚实区分 Build/Version/Release/Preview/Publish/Domain | 把 Build success、Preview 或 BaoTa 部署写成 Publish |
...
| 正式前端/Figma | 两张 FigJam 已完成指定越界节点纠偏；Foundations、九张 SaaS 桌面代表页、五个关键状态、三张移动端、三条原型骨架、产品官网首页和客户生成站首页已有受控 Node；76/76 Page Manifest 已登记 | 五张用户确认界面作为视觉基线；继续完成 Library、逐页 Node、其余状态/断点、a11y、12 条完整 Journey 和用户验证 | 把代表页、部分状态/原型、草稿、截图/导出代码或示例业务内容当全文件已完成或已验证 Design System/能力合同 |
