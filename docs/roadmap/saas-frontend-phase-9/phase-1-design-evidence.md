...
用户确认 Phase 0 范围后，Phase 1 已完成第一轮 Foundations、组件、状态实验室、五类产品模式、十三张 SaaS 桌面代表页、四张产品公共表面代表页，以及客户生成站首页。所有资产仍是设计草稿，不表示正式 SaaS 前端、公共合同、Provider 运行时或生产发布已经存在。
...
| Navigation Components | `26:2` | 一级/二级导航 Variant、双层导航 anatomy、对象 Tab 边界 | `COMPONENT_DRAFT` |
...
- Navigation / Primary Item `26:17`；
- Navigation / Secondary Item `26:30`。
...
- 8 个一级栏目、38 个二级栏目；
- 7 个业务一级/30 个业务二级，以及 1 个管理一级/8 个管理二级；
- 7 个横向 Shell 控件不计入一级/二级；
- 媒体工作室、销售触达、公开互动、指标与数据质量、AI 工作策略、数据导出与关停六项继续显示为规划能力，不提供可执行假入口。

桌面核心页、跨域页与 Cover 中的异常状态页已统一一级顺序和选中态。移动底栏保留“今日、客户、站点、互动、更多”，完整 8/38 结构通过“更多”抽屉 `53:2` 到达；抽屉内规划项继续显示状态。
...
| 经营洞察 | `61:2` | `PAGE-FE-084 / SCN-FE-INSIGHT-001 / Operator / MetricUnavailable / TARGET_NOT_RUNNABLE / v0.1` | 展示指标定义、数据水位、权限、新鲜度和不可用原因；未知不显示为 0，无伪 KPI |
...
|---|---|---|---|
| 产品官网 | [`FIG-P9-005`](https://www.figma.com/design/IMLSUMTQViEEauwBdzbczr) · `3:10` | `CAND-P9-PUBLIC-HOME / CAND-P9-SCN-PUBLIC-001 / Visitor / Normal / PROPOSED / v0.1` | 已改为 7 个业务任务域 + 1 个管理控制区；公共站导航仍与 SaaS Shell 分离，不使用虚构客户、KPI、部署状态或“所有模块已上线”文案 |
...
2026-07-23 通过 Figma 同视口截图逐一检查：

| 证据 | Node | 检查结果 |
|---|---|---|
| 完整 IA 总图 | `FIG-P9-004` · `44:2` | 8 张栏目组卡片、38 个二级入口、六个规划状态和 7 个 Shell 控件无截断/重叠 |
| Today 桌面 Shell | `FIG-P9-004` · `6:2` | 8 个一级顺序稳定；重复“团队与设置”入口已移除；工作区、搜索和工作中心保持横向控制 |
| AI 工作策略管理 Shell | `FIG-P9-004` · `12:2` | “管理与设置”父级与“AI 工作策略”二级同时选中；8 个管理二级入口完整可见 |
| 经营洞察代表页 | `FIG-P9-004` · `61:2` | 与 Today 基线按 1586×992 同视口并排检查；布局密度和 Shell 一致，指标不可用/部分可用/未接通分离，无虚构数字、趋势或归因结论 |
| 移动更多抽屉 | `FIG-P9-004` · `53:2` | 390×844 下 8 个一级均可达；管理 8 项展开无截断；规划能力状态未伪装为可执行 |
| 导航组件与 anatomy | `FIG-P9-003` · `26:2` | Primary/Secondary 的 default、selected、planned 状态分离；对象 Tab 不计入二级 |
| 产品官网 | `FIG-P9-005` · `3:10` | 4×2 卡片完整覆盖 7 个业务域与 1 个管理控制区；旧“六个任务域”已清除 |

本次是作者视觉 QA，不等于键盘、读屏、RTL、德语长文本或目标用户任务验证。
...
1. 完成 Foundations 的文本属性化、发布与迁移策略；导航 Primary/Secondary Variant 已建立，但尚未发布 Library；
...
4. 在已完成的六条原型骨架、三张移动端代表页和移动“更多”抽屉上继续闭合其余六条 Journey 与全断点；
