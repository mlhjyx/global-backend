...
> 生命周期：`REFERENCE_ONLY`
> 生命周期依据：v3.0 买家智能调研，作为输入保留；现行范围见 docs/product-scope.md
...
> 关联：[discovery-sources.md](../backend/discovery-sources.md)（多源蓝图，本文更新其「海关公司级只能付费」旧结论）· [discovery-architecture.md](discovery-architecture.md)（四层/ToolBroker）· [positioning-and-acquisition-backlog.md](positioning-and-acquisition-backlog.md)（六维评分与历史 backlog）。
...
**P0 —— 零/极少新基建，最高 ROI（全靠复用）** ✅ 四件套已全部落地（现状以 [status/current](../status/current.md) 为准）
