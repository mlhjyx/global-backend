...
- New `20260830130500_organization_identity_mainline_constraint_adoption`, after exact-main 130300/130400, verifies the main-owned `canonical_company_workspace_id_id_key` UNIQUE constraint, rebinds every exact foreign key that actually depends on the temporary index, removes that index, and leaves no duplicate final unique index. The allowlist is the five Artifact A CanonicalCompany composite foreign keys plus exact-main `discovery_company_materialization_outcome_company_fkey`; the sixth is dropped/recreated only when its `conindid` points to the temporary index and its complete table/column/reference/action/deferrability contract matches exact main. A seventh dependency or any definition drift is a hard stop.
- New `20260830130600_organization_identity_link_materialization_compat`, after exact-main materialization functions, exact-preflights and replaces `identity_link_company_raw_unique` so uniqueness applies only to `canonical_type='company' AND status='ACTIVE'`. Existing workspace/canonical/raw uniqueness continues to prevent duplicate party links while multiple distinct `PENDING_CONFLICT` parties for one Raw remain valid. The same migration adds an exact private materialization validator requiring every `CANONICALIZED` outcome to reference the exact workspace/Raw/company IdentityLink in `ACTIVE` status with `conflict_id IS NULL`; pending, conflicted, revoked, cross-workspace, wrong-Raw, or wrong-company links fail with the fixed materialization identity-conflict error. Main 130300/130400 history is not rewritten.
