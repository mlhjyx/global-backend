...
`scripts/governance-organization-identity-root-materialization-packet.spec.mjs` challenges every packet/request field, generator identity, packet-review substitution, source/root/HEAD drift, historical schema reuse, fixed seven-role source/destination mismatch, create-exclusive ignored output reuse, and authorization-scope/requestId derivation drift.
