...
- 成本安全合同 `site-builder-model-evaluation-cost-safety/2026-07-28-v1` 在 executor branding 时强制绑定：独立 spend authorization 的批准金额/execution 数必须与 campaign limits 精确一致；凭据 purpose 必须是 `site_builder_model_evaluation`、quota 必须 limited，target/legacy comparator 的 alias+protocol scope 与冻结价格表必须精确覆盖且禁止默认/未配置倍率；resolverId 必须匹配价格快照。仓库绝对止损为 credential 25000¢、campaign 10000¢、500 executions、1000 wire calls、单次 prompt 1048576 bytes 与 output 100000 tokens；具体 campaign 只能更低。
