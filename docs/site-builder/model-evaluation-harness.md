...
- attestation branding 只证明输入满足机器合同，不冒充 new-api 管理面真值。后续真实 evidence runner 必须从固定提交读取并保存脱敏凭据管理快照及其 SHA-256、bearer secret digest、价格快照和 resolver 实现，先核验真实 token quota/scope 再构造 attestation；还必须为 executor 提供 authorization 已绑定 ledgerId + resolved absolute path/device/inode SHA-256 的 append-only、fsync 文件账本，symlink、换目录、remount 或 target identity 漂移都不能重新开户。authorizationId 以 exclusive-create claim，claim file 与其父目录 entry 都在成功返回前 fsync；claim/reserve/settle/freeze 的 JSONL 写入必须循环到完整 payload 落盘，partial write 先失败且不得更新内存状态；进程或主机崩溃后该 authorization 永久 fail-closed，不能重领整份 campaign 额度。本 PR 不提供绕过这些步骤的默认值、环境变量读取或 live introspection。
