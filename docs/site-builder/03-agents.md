...
- **事实真值/内部消费护栏（R4-A2，✅ 2026-07-18 当前交付分支）**：确定性出口门要求 value 中关键数值/单位、认证代码和名称/型号被 quote 支持；不一致项与任何 `research_hint` 均降 gap。通过项以包含 snapshot/role/asset 的稳定分域 key 原子关联共享 Claim/Evidence，并由不可变、FORCE RLS bridge 及逐字段数据库 trigger 锁定精确 BrandProfile/EvidenceRef/source hash/quote selector；三列 Site FK 强制同一 company。机器生成 Claim 永远从 `NEEDS_REVIEW` 开始；status+version CAS 让并发审批只有一个赢家，proof digest 绑定 Claim 身份/正文/有效期/批准版本，bridged/终态 Claim 身份和 Evidence provenance 不可改写。认证分类在语义/持久化/读取门复用；BrandProfile 事务按 UUID 稳定顺序锁定同 workspace/site、ready、未删除的 cert Asset，否则不建 Claim。内部 approved-effective 对所有新桥接 Claim 重算人审 proof，legacy 认证也按 type+statement fail-closed。新 Site 原子关联 CompanyProfile；历史无关系行不按名称回填并 fail-closed。当前没有公开 claim projection API，也没有正式 `PublishableClaimSnapshot`/CopyBundle/SiteSpec 消费者；这些仍随 M1-d 落地。
