...
> 架构详设见 [02-architecture.md](02-architecture.md)。获客侧开发冻结已解除，M1 也已完成阶段收口；获客任务仍须重新审计并明确 owner，不得并入 Site Builder PR 或争抢未明确分配的共享文件 ownership。
...
- 不因独立站优先而删除 AI 获客代码；M1 已完成阶段收口，获客恢复施工仍须独立排期、独立 ownership，不与 Site Builder PR 混改共享核心。
