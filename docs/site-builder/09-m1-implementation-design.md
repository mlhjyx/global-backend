...
| EVAL-bootstrap | 可执行启动集：6 个合成 BrandProfile 文本 fixture（3 行业 × sparse/rich，含 DE/EU locale），保存输入/事实不变量/accepted artifact/token/延迟；当前不含截图或视觉偏好，4/6 成对偏好与 desktop/mobile 截图待对应渲染消费者后才可用于扩 12 | — | M1-g 扩集 | 🟢 | BrandProfile 文本子集已具备 |
