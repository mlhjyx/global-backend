...
> 文档 ID：`SITE-M1-001`
> 生命周期：`CURRENT`
> 当前事实来源：[状态](../status/current.md)、[路线](../roadmap/release-plan.md)、当前代码与候选基线 `site-builder-model-candidate-baseline/2026-08-07-v3`。
...
>
> **2026-07-24 as-built**：R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min、DI-0、M1-e-A/B 与 M1-f 均已落地**。六个 Family、DesignBrief、受控组装、SiteSpec 1.1、ReleaseManifest v2/v3 与真实 Renderer/Release 消费已完成；M1-f 已接 closed-shape DesignEvaluation v2、确定性 QA/SEO/a11y/genericness、三断点截图、最多三次闭合 `optionId` 修复与 replay-safe P4。Gemini task-shaped MODEL-1 超时后明确 unavailable、未晋级。`template-distillation` 未采纳。
>
> **阅读规则**：下文仍标作 M1-e/M1-f/M1-g 待办的段落仅保留施工 provenance，不覆盖上列 as-built；当前 Catalog 已有六个 approved Family，R1-min/DI-0 已被真实消费者使用，M1 已于 2026-08-04 阶段收口。
...
- **生产化审计前置（2026-07-24 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min、DI-0、M1-e-A/B 与 M1-f**均已完成。R1-min 的不可变 Release 与 DI-0 合同已由 M1-e-B/M1-f 真实消费；Catalog 仍恰含六个 approved Family，下一主线为 M1-g，M1-f 未扩大 Family 或切换文本模型路由。

## 1. 承重假设与实测证据（2026-07-14 全部只读真探，无 sandbox）

| #   | 假设                                             | 探法                                                                       | 结果                                                                                                                                                                                                                | 对设计的影响                                                                                                                                   |
| --- | ------------------------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| H1  | ADR-021 candidate baseline 在当前网关的可见边界  | 分域令牌 `/v1/models` 与脱敏渠道状态；本轮不做模型效果实测                 | ✅ GPT 文本、GPT 图片、Gemini preview 图片与五个视频 alias 已按域登记；Gemini 文本目录可见但渠道 fail-closed 禁用；BGE-M3 仍只以私有别名 + 专用令牌可见；所有 Site Builder currentRoute/promotedRoute/rollback 未切 | 型号可见≠能力/质量晋级；精确状态/alias/协议只认生成基线，逐 task capability probe + Golden Set 后才 promotion                                  |
...
| H3  | 图片/视觉/视频能力当下可用                       | 同 H1                                                                      | ⚠️ `gpt-image-2` 经 new-api `/images/generations` 已真生成 1024² PNG，但 `images/edits`/mask、生产 MediaGateway 与真实消费者仍不存在；视频也仍无 current consumer 或 task-shaped 探针                                       | 单次生成探针只登记 capability evidence，不激活 imagePipeline；生成/编辑与视觉质检仍 **capability-gated**，未就绪即显式跳过，绝不拿文本模型硬顶 |
...
- **新表 `brand_profile`**（schema:1137 注释已点名）：`id/workspaceId/siteId/version/valueProps(Json)/tone(Json)/glossary(Json)/keywords(Json)/differentiators(Json)/competitors(Json)/factSheet(Json)/gaps(Json)/researchDegraded(Bool)/createdAt`，版本化追加不覆盖；RLS + FORCE RLS（M0 六表先例）。**evidence 结构分级**：factSheet 每项 `{value, evidence:{sourceType: intake|upload|storefront|web_research, url?, fetchedAt?}}`（合规 D2）。
  - ✅ **R4-4 幂等/provenance（R4-B-min，2026-07-19）**：`SiteBuildTaskAttempt` 以 `(buildRunId,taskId)` 唯一 logical attempt、lease/fence、冻结 input/output/result 收口；`BrandProfile.taskAttemptId` 绑定成功 attempt。同一 run 重放复用成功产物，过期接管从已保存模型输出恢复，不再重复付费调用。
  - 🔴 **R4-A1 Evidence 2.0 基础（✅ 2026-07-17）**：`{sourceType,url}` 升级为共享 `EvidenceRefV2{sourceId,sourceType,sourceRole,contentHash,quote,selector,...}`；模型前冻结经 PII 清洗/规范化/有界截断的 intake/KB/storefront/research source，KB 精确到 chunk hash，web 正文由服务端重算完整 SHA-256。SearXNG 原始 title/snippet/path/query/fragment 不冻结，只按站主公司名 + external origin 生成最小 `research_hint`；持久化边界会再次重建该 hint，并跳过无法证明已最小化的历史 `KbDocument.source=web_research` 正文。所有新模型事实必须有 8–512 code point 精确 quote，并由服务端重新绑定 source/type/hash/selector 后追加进不可变、FORCE RLS 的 provenance 表；旧 v1 行不伪造回填。无公开 OpenAPI/SiteSpec 版本变化。
  - 🔴 **R4-A2 truth bridge（✅ 2026-07-19 当前交付分支）**：公共企业事实/PII/value-quote 门与 `research_hint` fail-closed 已落；fact key 只接受严格 lower_snake_case，schema、gate、投影和 EvidenceRef/Claim 数据库边界拒绝非 canonical 输入。新 intake 原子创建 CompanyProfile→Site，旧未关联 Site 禁止名称猜测。稳定 origin key 复用公共 Claim/Evidence，不可变 FORCE RLS bridge + exact trigger 锁定同公司 EvidenceRef；认证另锁 live ready cert Asset。origin Claim 审批由 Claim 行锁、exact precheck 与 security-definer trigger bridge 行锁收口，孤立 Claim/API 绕过均 fail-closed，manual/legacy null identity 兼容。Workspace 尚有 Site 时拒绝物理删除；cert Asset 扫描走 partial ordered covering index。无公开 API、SiteSpec/CopyBundle/正式 `PublishableClaimSnapshot` 变化。
...
- `SiteBuildRun`：`kind='refurbish'`、`phase='P1_understanding'…'P5_publish'`、`scope/steps/costSummary/temporalRunId` 列全部 M0 已建未写，M1 零迁移直接写。🔴 **R3-4 修（M1-d 前）**：`temporalRunId` 从未写入且 `AlreadyStarted`/ACK 模糊——launcher 须回写 `firstExecutionRunId`，`WorkflowExecutionAlreadyStarted` 对同 `buildRunId` 视为幂等成功，不误标 failed。✅ **R4-6 修（R4-B-min，2026-07-19）**：模型与 ToolBroker physical operation 进入同一持久 spend ledger，reported/calculated/estimated/unknown 分层；终态 reconcile 后把闭合的 `site-builder-cost-summary/v1` 写入 run。成功、失败、unknown、released、模型/工具 calls 与 token usage 都来自账本，不把预算 reservation 或 estimate 冒充真实成本。
...
> **2026-07-19 路由裁决/首个逐任务晋级 fast-follow（历史）**：本节旧 targetCandidate/“四通道”清单保留为施工历史；2026-07-27 起非运行时候选由[生成候选基线](model-candidate-baseline.md)覆盖，现役唯一认 `task-routes.ts`。final-code 同一 6×2 source bundle 下，BrandProfile 候选 Terra/Responses 12/12、Sonnet/Messages 12/12；完整 legacy DeepSeek Pro→GLM 路由也为 12/12（GLM probe 通过、正文 fallback 0 次）。因此不再用旧 10/12 基线失败作晋级理由，而以三路全过硬门后的 accepted-artifact 成本、原生 transport/provenance、任务失败门与 rollback 保持首个代码级 `promotedRoute`=`gpt-5.6-terra → claude-sonnet-5`。active evidence id=`model1-brand-profile-20260719-v20`；旧失败/诊断报告全部保留。**2026-08-12 Copy 更新**：Copy 已独立经过 Git-reviewed quality、promotion 与 route adoption（#398）；active route 采用 Sonnet/Messages/`medium`/no-fallback，DeepSeek Pro→GLM/`low` 仅留显式 rollback。此处两条代码级 route 都不等于已生产部署或真实流量 canary。

旧“四通道升级位”表已从施工入口撤下，只在 [10 §0](10-model-selection-study.md#0-2026-07-14-dated-结论按任务路由保留为历史实测证据) 保留 dated provenance。下面抄录 `task-routes.ts` 的 active route；新候选只看 `site-builder-model-candidate-baseline/2026-08-07-v3`，两者之间必须逐 task 经过 ADR-016 promotion 门。
...
| `site_builder.copy`           | `claude-sonnet-5`（Messages） | —                                 | `promotedRoute`；`medium`；`SITE_BUILDER_MODEL_ROLLBACK_COPY=true` → DeepSeek Pro→GLM/`low` |
...
**剩余通道/能力清单（不阻塞 R4/M1）**：精确 alias、状态和预期协议只认生成候选基线，不在施工图再维护第二份表。当前关键缺口是 Gemini 文本上游激活、GPT Image 2 edits/mask task-shaped probe，以及图片/视频的真实消费者与协议 harness；任何目录可见型号仍必须逐 task 评测后才翻 active registry。
...
- **D 文案**：D0【🔴 红线 ADR-017 NO-FABRICATED-IDENTITY / R0-3】**demo-spec 与文案 agent 只用 intake 事实**——对未知企业类型**禁止**默认写 manufacturer/engineering team/production/QC/export packaging/认证/产能/年限/客户名单等身份声明；缺 = 留空 + 提示补录，绝不虚构。✅ #123 已把无证据措辞改为中性事实安全（`supplier/supply/requirement review/delivery & support`），并加 sanitizer/提示词/CI 守卫；仅 BrandProfile 明确证实企业类型后才升级措辞。D1/D2 已由 R4-A1/A2 的 EvidenceRef/approved-effective bridge 与 M1-d 的 immutable PublishableClaimSnapshot/CopyBundle 确定性闸收口；空事实集合只产中性文案。D3 只引 snapshot Claim 且禁绝对化宣称；D4 kbDigest 来源标注+截断；D5 预览产物过 L1 确定性筛查（词库资产先建，L2 模型审查挂 M2 发布门）。✅ **R0-4 #124**：`intakeToMarkdown` 已不再写 `businessEmail`；联系信息只留 `Site.intake` 受控结构区，并提供幂等存量脱敏脚本（各部署环境仍须执行并留证）。
...
| D-M1-2 | 文本模型逐 task 从 currentRoute 晋级  | **是**：BrandProfile 的 final-code 同形 6×2 基线证明 Terra/Sonnet 与完整 legacy DeepSeek Pro→GLM 路由均通过，随后只按该 task 的 accepted-artifact 成本、失败门和 rollback 保持 Terra→Sonnet；其他任务继续保持历史 currentRoute，必须各自补消费者、评测、失败门和 rollback 后再切                                                             |
...
1. **模型任务形状与主档集中风险**：部分主任务仍优先 DeepSeek，方舟 fallback 已降低单 provider 风险；GPT/Claude/Gemini 通道扩容也只增加候选面，“通道连通”仍不代表长结构化任务稳定。网关配置漂移、超时或空 content 可让 build fail-closed。缓解：AiTask 探活+显式失败+run 可重跑；按 task-shaped eval 晋级当前 candidate baseline 候选。
...
> **与 §11 生产化审计 PR 的交织（2026-07-24 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；R0 contract、R1-safety、R2-A 四项、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min、DI-0、M1-e-A/B 与 M1-f 均已完成。下一步按 M1-g 推进。完整依赖与风险见 §11。
...
> **2026-08-04 M1-g as-built**：统一入口、6→12、36 张三断点、事实零虚构、10 站 genericness 与开发真机链均已完成。206-execution campaign 经复审降级为诊断：62 accepted、99 validator rejected、45 transport failed；因缺 canonical execution provenance、resolved upstream/channel 和逐 wire settlement，所有候选均不可 rank/promotion，本次不切运行路由。M1-g 以确定性产品门和“不晋级”决策完成，不把诊断产物、30+ 系统集、MODEL-2 或生产部署冒充成功。
>
...
**R3**：R3-A 已完成 SiteBuildRun 数据库背书；**R3-B1/B2 已在 2026-07-17 当前交付分支完成**。B1 收口有界 targetId、严格 BuildOptions、请求指纹幂等账本与 Temporal workflowId/firstExecutionRunId ACK；取消等待执行链关闭与补偿完成后才释放 active 单飞门。B2 新增 FORCE RLS `SiteBuildStep` attempt 真值，按 Activity/图片批次单调写 phase/progress/read-model，旧 attempt 迟到不覆盖，begin replay 不重写 startedAt，成功/失败/取消均终态化；page/section/pages 冻结请求时 active base version，只替换目标 page/block 与引用 copy keys，发布以 pointer CAS 防止覆盖期间人工编辑，缺失/歧义/脏 spec fail-closed，stylePreset 不逃逸局部 scope。B2 当时非 en 继续 422；M1-d 现已开放 `en/de-DE` 并接入 Renderer locale path。Ubuntu 开发环境以独立真 PostgreSQL/app_user/FORCE RLS + 隔离真 Temporal 验证，**不代表生产部署**。

**R4-A1（✅ 2026-07-17）**：共享 `EvidenceRefV2`、冻结 source snapshot、完整 SHA-256、精确 quote/Unicode selector、KB chunk provenance、append-only RLS/FK 基础已落；SearXNG 原始页面元数据不冻结，只生成最小 origin hint；旧 v1 兼容读取、不伪造回填，公开 OpenAPI 不变。**R4-A2（✅ 2026-07-19 当前交付分支）**：公共企业事实/PII/value-quote 与 `research_hint` 门、strict lower_snake_case fact key、nullable Site→CompanyProfile、稳定 Claim/Evidence origin、同公司 exact bridge、live cert Asset、审批 proof/CAS 已落。审批必须由 Claim 锁+exact precheck+数据库 bridge 行锁证明 surviving Site bridge；孤立 origin Claim 与直写 fail-closed，manual/legacy null identity 兼容。Workspace 有 Site 时禁止物理删除，cert Asset 引用扫描使用 partial ordered covering index。所有生成 Claim 仍先 `NEEDS_REVIEW`；A2 自身没有新增 endpoint、SiteSpec/CopyBundle 或消费者，后续 M1-d 已接入正式消费链。**R4-B-min（✅ 2026-07-19 当前交付分支）**：每个 BuildRun 原子创建 DB 硬预算；logical task attempt 用 lease/fence 与冻结 input/output/result 恢复，physical model/tool operation 用稳定 key 幂等。reserve/settle/reconcile 由数据库函数串行；ACK 不明标 unknown 并停止 fallback。预算耗尽、取消、run 状态和人工停用均 fail-closed；终态写 `site-builder-cost-summary/v1`。进程内 `BudgetLedger` 不再承担 Site Builder 的多 worker 真值；Redis/跨 run 高级配额仍可随部署拓扑后补，但不得绕过数据库 hard cap。

**R4-A1 迁移/兼容/回滚与验证**：schema 只 additive 增 `evidenceSchemaVersion`、两张 provenance 表和复合租户 FK；旧 BrandProfile 默认 v1，新写显式 v2，读取兼容两个版本，不做无法证明的历史回填。source snapshot 按 site+dedupe key 幂等复用，事实 ref 随新 BrandProfile 追加；BrandProfile workflow 重试复用现由 R4-B-min 的 task attempt/operation ledger 承接。两表 `FORCE RLS`，`app_user` 仅 `SELECT/INSERT`，复合 FK 绑定 workspace/site/profile/source/hash，UPDATE trigger 拒绝篡改；URL 去 credentials/fragment/敏感 query，Prompt 不暴露 URL/title/联系信息。迁移 forward-only：如需回退，先部署 v1 reader/writer，再仅在证明无 v2 行后用新的 forward migration 删除未用结构；不反向伪造数据。A1 当时 head 真机 verifier 已完整通过：真 PostgreSQL 两账本 A/B/unset RLS、跨租户/UPDATE 负例、SearXNG/Crawl4AI origin-only hint、BGE-M3，以及 DeepSeek Pro 两轮 Activity 的全部事实/关系 ref/quote selector/hash 与 snapshot 去重均绿；第二轮一次研究降级被显式标记后安全继续，夹具清零。此前同任务的 DeepSeek 超时/GLM 截断仍作为外部波动证据保留；当前 head Terra 复验见下条。R3-B2 `site_build_step.id/updated_at` 默认表达式漂移已在 M1-f 前置独立 PR 修复，空库/现库/schema diff 均验证为零。

**R4-A2 迁移/验证/部署门**：`20260718170000`–`20260718188000` 共 19 个 migration 全部 forward-only，已应用的历史文件不改写。后段以新 migration 收口 Workspace delete guard、strict factKey、surviving exact bridge 审批、最小权限 security-definer 锁与 cert Asset ordered index。Ubuntu `global_dev` 的旧数据预检和真 PostgreSQL verifier 已覆盖并发审批单赢家/单 outbox、非 canonical ref/claim、Workspace guard、Site bridge 删除后的应用/owner 直写审批、EXPLAIN index、同租户跨公司、Evidence 错配、审批 digest、A/B/unset RLS 与 immutability，并确认夹具清零。`app_user` 对 bridge 仍只有 SELECT+INSERT，未为行锁扩大 UPDATE；trigger 固定 `search_path=pg_catalog,public`。上述只代表开发环境；生产前须在恢复快照逐 migration 量测表规模、索引/约束与锁时长，任一 5 秒 lock gate 不满足即停止并安排维护窗口。

### 10.7 各阶段最终判断

| 阶段     | 判断                                                                                                                                                                  | 处理                                                                                                 |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| M0       | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min 与 DI-0 已完成 | M1-e-A/B 消费已完成前置合同与 Release 基础                                                           |
| M1-a     | 工作流框架保留；R3-A 数据库背书、R3-B1 API/ACK 与 R3-B2 局部 scope/单调进度已完成；M1-d 已开放 en/de-DE                                                               | R1-min 已补齐对象存储原子 Release                                                                    |
| M1-b     | Agent 形状保留；R4-A1 引用完整性、R4-A2 内部事实真值桥与 R4-B-min 付费幂等/成本真值已补；M1-d 已接入正式文案消费者                                                    | DI-0 合同已完成，进入 M1-e                                                                           |
| M1-c     | ✅ 纯 Sharp writer 与 refurbish P2 已落地；生成式、公开 process/select API、Renderer `<picture>` 未做                                                                 | R2-A、MF0-A/B 与 R1-min 前置已完成；固定 Variant 消费归 M1-e-A                                       |
| M1-d     | ✅ 已落地                                                                                                                                                             | 只消费 exact approved Claim snapshot；CopyBundle/locale/renderer/inquiry 边界见实施记录              |
| M1-e     | ✅ 设计主改已完成                                                                                                                                                     | DesignCatalog、6 Family、55 组件、受控组装                                                           |
| M1-f     | ✅ 质量主改已完成                                                                                                                                                     | 三断点、确定性 lint/SEO/a11y/genericness、审美 unavailable 明示、closed repair、最多三轮、Release v3 |
...
| R1-min ✅         | #157：生产对象存储不可变 Release、manifest/digest、原子预览指针、跨节点恢复/回收、版本锁、取消/并发 fencing 与 unknown component fail-closed；retention/GC 默认关闭                                                                                                                          | R3-B2/R1-safety               | M1-e 可见预览          | 🔴 发布/预览路径已专项验证；不代表生产部署                                                               | 已完成                      |
...
| R3-B2 ✅          | active SiteSpec 的 page/section/pages 局部消费 + `SiteBuildStep` 单调 step/progress/attempt/replay/终态化；本地 durable artifact/原子 pointer；非 en 随 M1-d，不伪造 cost                                                                                                                    | R3-B1                         | **R4-B-min ✅ → M1-d** | 🟡 真 PG/RLS + 隔离真 Temporal 已验                                                                      | 已完成                      |
...
| R4-A2 ✅          | Claim/Evidence truth bridge：公共事实/PII/value-quote 门、strict factKey、nullable Site→CompanyProfile、稳定 Claim/Evidence + exact RLS bridge、live cert Asset、surviving bridge 审批、Workspace delete guard 与 cert lookup index；无公开 endpoint/SiteSpec/CopyBundle                     | R4-A1                         | **R4-B-min ✅ → M1-d** | 🔴 事实/合规/迁移门；真 PG 已验但不等于生产部署                                                          | 已完成                      |
| R4-B-min ✅       | BuildRun DB hard cap + task attempt lease/fence + physical spend operation key；BrandProfile 冻结 input/output/result 并重放复用；模型/ToolBroker 共用 reserve/settle/reconcile；reported/calculated/estimated/unknown 分层；取消/状态/预算 kill switch；终态 `site-builder-cost-summary/v1` | —                             | **M1-d 付费 fan-out**  | 🟡 真 PostgreSQL RLS/并发/ACK-loss/replay/kill-switch 已验，不代表生产部署                               | 已完成                      |
...
| DI-0 ✅           | #164：DesignSourceManifest/Observation/Rule/DNA/TemplateFamily/DesignBrief/DesignEvaluation + 初始空静态 DesignCatalog；Tier A/B/C 授权分级、规则聚合≥5 独立贡献组、确定性 digest、approved-family-only resolver、运行时零读原始 Readdy（ADR-019）                                           | DQ-1 已合并                   | M1-e-B 已消费          | 🟢 合同与真实消费者均已验证                                                                              | 已完成                      |
...
| M1-d ✅           | Copy slots/内容预算/最小询盘合同：按 slot/locale/Claim refs 生成，先事实门再文风；最小 inquiry/consent/outbox 合同（实际公开接收随 M2）                                                                                                                                                      | R3、R4-A1/A2/B-min 已完成     | —                      | 🟡                                                                                                       | 已完成                      |
| M1-e-A ✅         | 55 组件与变体：55 型全部 `m1_e_a_qualified`，variant/content/a11y/picture/motion/reduced-motion 与七件套、三断点证据闭环                                                                                                                                                                     | R1-min                        | M1-e-B                 | 🟢 385 份资格证据 + 165 张快照                                                                           | 已完成                      |
| M1-e-B ✅         | 六个 approved Family、DesignBrief、封闭 adapter/copy-slot/四层 validator、三轮 repair/safe fallback、SiteSpec 1.1、资产 overlay、ReleaseManifest v2 与 Temporal patch；模型不写任意 props/Astro/Tailwind                                                                                     | M1-e-A、DI-0                  | M1-f ✅                | 🟢 12 Golden + 36 张整站快照 + 真开发链                                                                  | 已完成                      |
| M1-f ✅           | 确定性 QA/SEO/a11y/genericness、三断点截图、DesignEvaluation v2、ReleaseManifest v3、最多三次 digest-bound closed repair、质量后置 materialize 与 pointer CAS；Gemini timeout 明示 unavailable，未晋级                                                                                       | M1-e-B                        | M1-g                   | 🟢 真 replay + Temporal round/ACK-loss/cancel/pointer 注入矩阵                                           | 已完成                      |
| EVAL-bootstrap    | 可执行启动集：6 个合成 BrandProfile 文本 fixture（3 行业 × sparse/rich，含 EU 目标市场），保存输入/事实不变量/accepted/rejected token/延迟；BrandProfile 当前没有 locale 输入，不能冒充本地化覆盖。当前不含截图或视觉偏好，4/6 成对偏好与 desktop/mobile 截图待对应渲染消费者后才可用于扩 12 | —                             | M1-g 扩集              | 🟢                                                                                                       | BrandProfile 文本子集已具备 |
...
| M1-g ✅ 2026-08-04 | 阶段收口：启动集扩 12 视觉子集、Catalog/事实/安全/a11y/性能/回滚回归、记录未完成 30+ 系统集不冒充覆盖；`verify-site-builder-m1.mts`、OpenAPI 与权威状态同步                                                                                                                         | 各 M1 PR                      | —                      | 🟢 确定性门完成；206-execution 仅诊断，所有候选未晋级                                                    | 已完成                      |
...
## 12. 消费契约与 schema（M1-d/e 已落，后续项仍为施工承载）

> 来源 v3.2 §4/§9/§15/§16/§19/§20/§21 回写。M1-d 的 ClaimSnapshot/CopyBundle、M1-e-B 的 SiteSpec 1.1/DesignBrief/受控组装与 M1-f 的确定性 quality/SEO/DesignEvaluation v2/ReleaseManifest v3 已成为 as-built；GEO、询盘公开接收与维护任务仍是后续目标。当前真值以代码、§2 触点与 03/04/13/14 专题文档为准。
...
- **Refurbish 阶段职责与失败语义**（v3.2 §19.1/§4.4，对齐 §2.3 编排）：
  ...
  | P4 quality       | building candidate + 三断点截图 → closed DesignEvaluation v2 + `repairOptionId` | 确定性 QA/SEO/a11y/genericness；审美 unavailable 明示 | round 0 后最多三次；硬门不过不 materialize Release                            |
...
- **单一真相源**（§9.1，M1-d as-built）：`PublishableClaimSnapshot v1` 只冻结 exact Site bridge 的 approved/current/audited Claim/Evidence，认证另须 live cert Asset；激活事务重验 current set。旧 factSheet 不回填、不直接供 Copy；空 snapshot 走中性 copy。`SiteCopyBundle` 以 SiteVersion/locale 唯一并绑定 snapshot/task attempt/inputHash/digest，新 SiteSpec `copyBundleSet` 权威，legacy 字符串仅供无新集合旧行 dual-read。
...
09 只承载两处**施工序/DoD 门**：① **R0/Demo 兼容**——旧 `demo-spec` 继续固定 SiteSpec 1.0，不反向读取 M1-e-B Catalog；若后续 DV-0 升级 Demo，仍须把 `BusinessArchetype/TemplateFamily/StylePreset` 拆开。② **M1-e-B DoD 已完成**——首批 **6 个 TemplateFamily**（`precision-industrial/technical-catalog/oem-capability/scientific-trust/natural-origin/premium-innovation`）均具至少 2 首页 + 2 内页 Blueprint、2 Hero、2 StylePreset、1 DemoVisualPack 与 sparse/rich Golden；正式版本 `1.0.0/approved`，Catalog=`m1-e-b/1.0.0`。`global-wholesale` 等扩展 Family 留 M1-g 后；完整领域契约仍以 **13 号**为准。
