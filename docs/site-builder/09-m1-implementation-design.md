...
- **生产化审计前置（2026-07-19 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min 与 M1-d**均已完成。R3-B2 已完成本地 durable artifact/原子 pointer；当前关键门是 R1-min 的生产对象存储 Release、跨节点恢复/回收与 unknown component gate，仍须早于 M1-e 可见预览；DI-0 可按 M1-d 稳定合同并行。
...
> **与 §11 生产化审计 PR 的交织（2026-07-19 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；R0 contract、R1-safety、R2-A 四项、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min 与 M1-d 均已完成。R1-min 余项早于 M1-e 可见预览；DI-0 按 M1-d 稳定合同并行。完整依赖与风险见 §11。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min 与 M1-d 已完成 | R1-min 生产 Release/跨节点/unknown component 余项在 M1-e 前完成 |
| M1-a | 工作流框架保留；R3-A 数据库背书、R3-B1 API/ACK 与 R3-B2 局部 scope/单调进度已完成；非 en 随 M1-d | 产物原子性随 R1 |
| M1-b | Agent 形状保留；R4-A1 引用完整性、R4-A2 内部事实真值桥与 R4-B-min 付费幂等/成本真值已补；M1-d 已接入正式文案消费者 | R1-min 后进入 M1-e |
...
| M1-d | Copy slots/内容预算/最小询盘合同：按 slot/locale/Claim refs 生成，先事实门再文风；最小 inquiry/consent/outbox 合同（实际公开接收随 M2） | R3、R4-A1/A2/B-min 已完成；DI-0 可并行且在 M1-e 前收口 | — | 🟡 | M1-d |
