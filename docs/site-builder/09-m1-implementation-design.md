...
>
> **truth-sync 审计基线 as-built（合并前 `main@a306ffa`，#124）**：#121 已去 intake 行为分叉，#123 已收口禁虚构身份，#124 已落业务邮箱隔离、可取消 polish 与失败保站；但 intake 仍返回 `{siteId,mode,status}`，尚缺目标合同的 `buildId`、`Idempotency-Key` 和 Swagger/OpenAPI 收口。因此 **R0 行为/安全子项已落地，R0-contract 仍未闭环**。DQ-1 已由 #117 合并；M1-c/MF-0/R2 均尚未落地。
...
- **生产化审计前置（2026-07-16 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。当前关键顺序是：**truth-sync → R0-contract → R1-safety（`finally` 清理 + Renderer env allowlist + Crawl4AI/robots 完整出站闸）→ 拆分后的 R2-A1/A2/A3/A4 → MF-0-thin → M1-c**；R1 其余原子预览工作在 M1-e 可见预览前完成。R3/R4、MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行，不再把所有修复塞进一个 R2 mega PR。完整依赖见 §11。
...
| H6 | 品牌研究链路容器可用 | curl 健康探测 | ✅ SearXNG:8081=200、Crawl4AI:11235=200；Ubuntu mihomo fake-IP 下 Crawl4AI 当前需 broad allow-internal 才能抓公开站 | P1 可复用 client，但 SSRF **未闭环**；R1-safety 前仅允许本地开发者可信公开 URL，并须覆盖 robots 直连路径 |
...
- `brandProfile`：KB digest（来源标注+截断，D4 注入防线）+ SearXNG/Crawl4AI 研究（**robots 遵守复用 web_watch 先例**；生产 SSRF 守卫尚须按 R1-safety 收口，C1/C3）→ 模型综合 → **确定性出口闸**：factSheet 逐项 evidence 非空 + web_research 单源不支撑认证类断言（降 gaps）；输出 schema 不设个人字段（C4 结构性排除）。
...
- **C 研究**：C1 SSRF 是立即前置的 R1-safety 门，不得把当前 broad allow-internal 当守卫；Crawl4AI 与 robots 直连均需解析后 IP/redirect/metadata/内网/DNS rebinding/上限校验 🔴；C2 抓取内容只进模板变量位（AiTask 基类结构性保证）🔴；C3【补】robots 遵守（web_watch 先例）+ 竞品只做定位参考不搬运；C4 第三方页面具名个人不落库不进 Brief（schema 结构性排除）🔴。
- **D 文案**：D0【🔴 红线 ADR-017 NO-FABRICATED-IDENTITY / R0-3】**demo-spec 与文案 agent 只用 intake 事实**——对未知企业类型**禁止**默认写 manufacturer/engineering team/production/QC/export packaging/认证/产能/年限/客户名单等身份声明；缺 = 留空 + 提示补录，绝不虚构。✅ #123 已把无证据措辞改为中性事实安全（`supplier/supply/requirement review/delivery & support`），并加 sanitizer/提示词/CI 守卫；仅 BrandProfile 明确证实企业类型后才升级措辞。D1 factSheet 零虚构=模型后置**代码闸**（evidence 非空，缺=gaps）🔴；D2【补】evidence 分级溯源（升级为 `EvidenceRef`+quote/value 对齐，见 §2.1 R4-A）；D3 只引 factSheet+禁绝对化宣称；D4 kbDigest 来源标注+截断；D5 预览产物过 L1 确定性筛查（词库资产先建，L2 模型审查挂 M2 发布门）。✅ **R0-4 #124**：`intakeToMarkdown` 已不再写 `businessEmail`；联系信息只留 `Site.intake` 受控结构区，并提供幂等存量脱敏脚本（各部署环境仍须执行并留证）。
...
> **与 §11 生产化审计 PR 的交织（2026-07-16 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；当前先完成 R0-contract 与 R1-safety，再把 R2-A 拆为 Asset/KB/Profile/Integration 四个可审 PR，合并 MF-0-thin 后才进 M1-c。R1 余项早于 M1-e 可见预览；R3、R4-A/B、MODEL-0/EVAL-bootstrap、DI-0 按各自消费者并行。完整依赖与风险见 §11。
...
| M1-b | P1 brandProfile：AiTask 基类+registry、KB digest 组装（D4）、web 研究（robots；生产 SSRF 另受 R1-safety 门约束）、factSheet evidence 分级闸（D1/D2）、gaps→kb/status | agents/base、agents/brand-profile、kb.service gaps | ai-task.spec、brand-profile.spec（含零虚构/单源降 gaps/schema 无个人字段）、kb-digest.spec |
| M1-c | P2 图片**纯 Sharp 确定性管线**（**不含 rembg/生成式**，ADR-018）：sharp 重编码+方向/sRGB+EXIF/GPS 剥离+解码炸弹+质量门+focal crop+多尺寸(320/640/960/1440/1920) AVIF/WebP/fallback、写 AssetVariant+derivedKeys 兼容 manifest、hasPerson/aiEdited/cert 分支闸、putBuffer 直写 generated 键、渲染器 `<picture>` 固定 variantId。**依赖 R2-A1~A4 + MF-0-thin**（见 §11） | agents/image-pipeline、assets.service 分叉、ProductGrid 等 | image-pipeline.spec（EXIF/GPS 剥离/cert 硬分支/幂等/单图失败不阻断/picture 消费） |
...
| R0-2 / R0-contract | M0 | intake.controller/dto/service | ⏳ 响应仍是 `{siteId,mode,status}`，没有 `buildId`；`Idempotency-Key` 未实现；Swagger 仍有旧诊断描述。#121 明确只改行为、未做 OpenAPI drift | 前后端按错误/不完整契约开发 | **下一独立小 PR** |
...
### 10.3 R0 已落子项与 contract 尾巴

**已落地**：#121 完成无条件 Demo 行为；#123 完成中性事实安全模板与防回灌守卫；#124 完成邮箱隔离/存量脱敏工具、polish 真取消和 201 后失败保站/原地重试。对应回归测试已随各 PR 合并。

**仍待 `R0-contract`**：intake 接收并持久幂等处理 `Idempotency-Key`；响应收口为 `{siteId,buildId,status}`（不把内部/未来诊断 `mode` 暴露成建站合同）；controller/Swagger/OpenAPI 与 07 示例同步；断言同 key 重放返回同 Site/Build、不同 payload 冲突用稳定 `error.code`。这条完成前，文档和状态页不得写「R0 全闭环」。
...
**Asset commit 状态机**：`pending_upload → committing → ready/queued ↘ rejected/failed_retryable`。要求：commit 入口 `updateMany` CAS 认领，并给每次认领分配 **attempt/fencing token**；所有完成/失败回写都必须匹配 token，旧 worker 在 lease 被接管后不得 zombie write。canonical key 由 content hash 决定、copy 幂等（storage copy 成功但 DB 未完成可安全重试）；P2002 转**明确 duplicate 状态**不留悬空行；staging 删除放 canonical 提交之后、失败交清扫器。删除先落 DB tombstone/outbox，**不在事务里做 S3 IO**；但在 MF-0-thin 的 `SiteSpecAssetReferenceScanner` 落地前，自动清理只准删 staging，**不得删除 ready canonical object**。scanner 扫 active draft/published SiteSpec，命中默认 **409**；MF-1 后切 `AssetUsage` 不改 API。

**KB 状态机**：`KbDocument.assetId` 加**唯一约束**（迁移前先有 duplicate preflight + 明确 reconciliation，不能只 abort）；processing 加 lease + `processingAttempt/leaseToken/leaseUntil`，超时可重认领且回写必须 fenced；**永久错误与瞬时错误用 typed error code 分类**（不按错误字符串猜），Docling/MinIO/embedding 瞬时故障回 `queued/retry_at`，格式损坏才 `failed_terminal`。Temporal attempts 耗尽后仍须有 sweeper/告警/人工 redrive，不能永久失联；资产 ready 前完成文档+chunk 事务，删除资产级联删 KB。

**Profile 与集成门分开**：Profile patch 的组内 schema、大小/数量/URL 上限与乐观并发是独立 R2-A3；`If-Match`/`baseVersionId` 与 409/412 语义必须先同 07 号收成一个合同。R2-A4 再做 Outbox/Temporal cleanup、稳定 `error.code`、真 PostgreSQL/MinIO/Temporal 故障注入与 redrive 验收。四块不得重新打成一个 mega PR。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124 已修行为/安全子项，API contract 未闭环 | 先做 R0-contract；R1-safety 立即；R2 资产/KB 前置 M1-c；R1 原子预览其余项在 M1-e 前完成 |
| M1-a | 工作流框架保留，生产语义需补强 | 做 R3；产物原子性随 R1 |
| M1-b | Agent 形状保留，证据与成本真值需补强 | 做 R4，**必须早于 M1-d** |
| M1-c | 纯 Sharp 设计成立但尚未落地 | 依赖拆分完成的 R2-A 与 MF-0-thin；R1 原子 Release 不耦合算法但须在 M1-e 预览前完成 |
...
3. truth-sync 审计基线 `main@a306ffa` 尚无 M1-c 实现。若旧 worktree/分支中存在 Sharp 纯函数，只可在核对未提交改动后选择性提取，并在 R2-A4 与 MF-0-thin 合并后 rebase/cherry-pick；不以分支存在推断能力已落地。
...
|---|---|---|---|---|---|
| TRUTH-SYNC | DOC-12 后续真值收口：修 00-14/权威状态漂移与归档 provenance；不重开 v3.2 设计范围 | #119/#120 已合 | 后续施工理解 | 🟢 纯文档 | — |
| R0-contract | intake `Idempotency-Key`、`{siteId,buildId,status}`、去响应 `mode`、Swagger/OpenAPI + stable `error.code`（§10.3） | #121/#123/#124 已合 | API 前后端对接 | 🟡 API 契约 | M0 closeout |
| R1-safety | 两个可审小 PR：① 临时 spec/staging `finally` 清理 + Renderer 子进程 env allowlist；② Crawl4AI + robots 完整 egress gate，移除生产 broad allow-internal，并做公开正向及 private/loopback/metadata/rebinding/redirect 负向验证 | TRUTH-SYNC | 构建与抓取安全 | 🔴 租户内容/密钥/出站边界 → 用户确认 | M0/M1-a |
| IT-0（测试泳道） | Industrial Template 效果验证：pump/auto-parts 各 sparse/rich fixture、1440/768/390 截图、build/axe/性能预算、unknown component/copy/事实风险、输出「保留/改造/丢弃」清单 | 可并行 R0-contract/R2 | 非架构主序列 | 🟡 不整包合并契约 | — |
| R2-A1 Asset | commit CAS + fencing、canonical copy 幂等、duplicate reconciliation、tombstone/outbox；scanner 前只自动删 staging | R0-contract、R1-safety | **MF-0/M1-c** | 🔴 状态机 + 对象删除 → 人审 | M1-c 前置 |
| R2-A2 KB | `assetId` 唯一 preflight/reconciliation、lease+fencing、typed retry、sweeper/alert/redrive | R2-A1 schema 接缝串行 | **MF-0/M1-c** | 🔴 migration + 异步状态机 → 人审 | M1-c 前置 |
| R2-A3 Profile | 组内 schema/限额 + 乐观并发；先统一 `If-Match`/`baseVersionId` 与 409/412 合同 | R0-contract；可与 A1/A2 代码并行，migration 串行 | 公开 intake | 🟡 API 并发合同 | M0 closeout |
| R2-A4 Integration | Outbox/Temporal cleanup、稳定错误码、故障注入、真实 PostgreSQL/MinIO/Temporal、redrive/告警验收 | R2-A1/A2/A3 | **MF-0/M1-c** | 🔴 跨系统一致性 → 人审 | M1-c 前置 |
| MF-0-thin | 最小媒体前置：AssetVariant + RLS/FORCE RLS、recipeHash、checksum/provenance、SiteSpecAssetReferenceScanner、删除 409、derivedKeys 兼容投影；**不做** MediaJob/AssetUsage/生成式 provider/视频 | R2-A4 | **M1-c** | 🔴 migration + RLS → 人审 | M1-c 前置 |
| M1-c | 确定性图片处理：纯 Sharp、方向/sRGB、EXIF/GPS、解码炸弹、质量门、focal crop、AVIF/WebP/fallback、多尺寸、失败隔离、picture 消费契约；**禁** rembg/生成图/Readdy/设计 Agent/MediaJob-AssetUsage 预建/视频（合并门见 14 号 · §20.7） | MF-0-thin | — | 🟡 算法 PR | M1-c |
| R1-min（余项） | per-run staging、不可变 artifact、active pointer 原子切换、版本锁、unknown component fail-closed；`finally`/env allowlist/抓取 egress 已前移 R1-safety | 可并行 M1-c | **M1-e 可见预览** | 🔴 发布/预览路径 → 用户确认 | M1-e 前置 |
