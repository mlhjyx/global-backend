...
- **生产化审计前置（2026-07-16 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。当前关键顺序是：**truth-sync → R0-contract → R1-safety（`finally` 清理 + Renderer env allowlist）→ 拆分后的 R2-A1/A2/A3/A4 → MF-0-thin → M1-c**；R1 其余原子预览工作在 M1-e 可见预览前完成。R3/R4、MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行，不再把所有修复塞进一个 R2 mega PR。完整依赖见 §11。
...
> 来源 v3.2 §26 回写。映射本文 §7 的能力主序列；风险分级对齐 merge-judge 红线（migration/RLS/鉴权/GDPR/对外抓取/大量删除 → 升级人审，见 AGENTS.md §8 与 `docs/backend/ci-merge-automation.md`）。
...
| R1-safety | 临时 spec/staging `finally` 清理 + Renderer 子进程 env allowlist；独立小 PR，不等待原子 Release 全套 | TRUTH-SYNC | 构建安全 | 🔴 租户内容/密钥边界 → 人审 | M0/M1-a |
...
| R1-min（余项） | per-run staging、不可变 artifact、active pointer 原子切换、版本锁、unknown component fail-closed；`finally`/env allowlist 已前移 R1-safety | 可并行 M1-c | **M1-e 可见预览** | 🔴 发布/预览路径 → 人审 | M1-e 前置 |
