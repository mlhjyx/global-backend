...
>
> **2026-07-17 as-built**：#121/#123/#124 已完成 intake 行为与安全修复；#125 已完成 truth-sync；#126 已完成 R0 contract closeout；R1-safety ①+②、**R2-A1 Asset 正确性门与 R2-A2 KB 正确性门均已落地**。DQ-1 已由 #117 合并；R2-A3/A4、MF-0 与 M1-c 尚未落地。
...
- **生产化审计前置（2026-07-17 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②、R2-A1 Asset 与 R2-A2 KB**均已完成；当前关键顺序是：**R2-A3 Profile → R2-A4 cleanup/integration → MF-0-thin → M1-c**。R1-min 的 per-run staging/原子预览工作仍在 M1-e 可见预览前完成；R3/R4、MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行，不再把所有修复塞进一个 R2 mega PR。完整依赖见 §11。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ①+②、R2-A1 Asset 与 R2-A2 KB 已完成 | 当前做 R2-A3 Profile，随后 A4；R2 资产/KB 前置 M1-c；R1-min 原子预览余项在 M1-e 前完成 |
