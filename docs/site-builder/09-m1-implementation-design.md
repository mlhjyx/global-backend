...
**Asset commit/cleanup 状态机（✅ R2-A1/A4 已落地）**：`pending_upload/failed_retryable/expired committing → committing → ready/queued ↘ rejected/duplicate/failed_retryable`。入口 `updateMany` CAS 分配递增 attempt、UUID fencing token 和 lease；所有完成/失败回写匹配 attempt+token，过期 lease 接管后旧持有者不能 zombie write。canonical key 由 content hash 决定、copy 幂等；P2002 转明确 duplicate；canonical DB 真值与 staging cleanup intent 同事务提交。staging cleanup 以 eventId 作 workflowId，先 durable timer 等原 presigned PUT 失效，再回查 Outbox+Asset provenance、幂等 DeleteObject+HEAD；失败有限重试、结构化告警，可由 guarded redrive 只重排 FAILED/超时/取消的严格 staging 命令。DELETE 落 tombstone + Outbox，不在事务里做 S3 IO；若 commit/KB worker 已持有状态则 CAS 返回 `409 ASSET_BUSY`。canonical intent 仍 parked；MF-0-thin 的 `SiteSpecAssetReferenceScanner` 落地前**不得删除 ready canonical object**。scanner 扫 active draft/published SiteSpec，命中默认 **409**；MF-1 后切 `AssetUsage` 不改 API。
