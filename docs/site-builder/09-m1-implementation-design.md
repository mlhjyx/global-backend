...
>
> **2026-07-17 as-built**：#121/#123/#124 已完成 intake 行为与安全修复；#125 已完成 truth-sync；#126 已完成 R0 contract closeout；R1-safety ①+②、**R2-A1 Asset、R2-A2 KB 与 R2-A3 Profile 正确性门均已落地**。DQ-1 已由 #117 合并；R2-A4、MF-0 与 M1-c 尚未落地。
...
- **生产化审计前置（2026-07-17 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②、R2-A1 Asset、R2-A2 KB 与 R2-A3 Profile**均已完成；当前关键顺序是：**R2-A4 cleanup/integration → MF-0-thin → M1-c**。R1-min 的 per-run staging/原子预览工作仍在 M1-e 可见预览前完成；R3/R4、MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行，不再把所有修复塞进一个 R2 mega PR。完整依赖见 §11。
...
> **与 §11 生产化审计 PR 的交织（2026-07-17 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；R0 contract 与 R1-safety 已完成，当前把 R2-A 拆为 Asset/KB/Profile/Integration 四个可审 PR，合并 MF-0-thin 后才进 M1-c。R1-min 余项早于 M1-e 可见预览；R3、R4-A/B、MODEL-0/EVAL-bootstrap、DI-0 按各自消费者并行。完整依赖与风险见 §11。
...
**Asset commit 状态机（✅ R2-A1 已落地）**：`pending_upload/failed_retryable/expired committing → committing → ready/queued ↘ rejected/duplicate/failed_retryable`。入口 `updateMany` CAS 分配递增 attempt、UUID fencing token 和 lease；所有完成/失败回写匹配 attempt+token，过期 lease 接管后旧持有者不能 zombie write。canonical key 由 content hash 决定、copy 幂等；P2002 转明确 duplicate；canonical DB 真值与 staging cleanup intent 同事务提交后才在事务外 best-effort 删除 staging。DELETE 落 tombstone + parked Outbox，不在事务里做 S3 IO；若 commit/KB worker 已持有状态则 CAS 返回 409，避免 copy→DB 窗口产生 canonical 孤儿。**R2-A4 尚未消费/redrive cleanup**；消费 canonical cleanup 时还须重查 active `objectKey` 所有权，在 MF-0-thin 的 `SiteSpecAssetReferenceScanner` 落地前自动清理只准删 staging，**不得删除 ready canonical object**。scanner 扫 active draft/published SiteSpec，命中默认 **409**；MF-1 后切 `AssetUsage` 不改 API。

**KB 状态机（✅ R2-A2 已落地）**：`KbDocument.assetId` 在确定性 duplicate/orphan/cross-scope reconciliation 后加 nullable unique；复合 FK 同时绑定 document→Asset 的 workspace/site 与 chunk→document 的 workspace。`queued(due)/expired processing → processing` 复用 Asset 的 attempt+UUID token+lease，外部 IO 间续租，所有失败/完成回写 fenced；文档+chunks replace 与 Asset ready 在同一事务，接管后的旧 worker 无法 zombie write。Docling/MinIO/embedding 由边界客户端发 typed error，瞬时故障回 `queued/retry_at`，损坏/不支持格式才 `failed_terminal`。commit workflow 已改 asset 粒度；5 分钟 recovery Schedule 有界扫描 due/过期行并发结构化告警，另有人工 redrive 脚本。R2-A4 仍负责 cleanup 消费者、公共稳定错误码与完整跨系统 Temporal 故障验收，不因 A2 真服务验证而提前宣称完成。

**Profile 与集成门分开**：R2-A3 已把 Profile 固化为独立 UUID token 的资源级 CAS：GET/PATCH 同回 `versionId` + 强 ETag，PATCH 必须 `If-Match`/`baseVersionId`，缺失 428、body stale 409、header stale 412；五组严格 schema、分组/总大小、数量、URL 与同站 Asset 引用均有界，保存无外部副作用。真 PostgreSQL 两连接 barrier 必须证明同 base 恰一胜，并由失败方 re-GET/retry 保留两组；合同细节见 07 §2。R2-A4 再做 Outbox/Temporal cleanup、其余公共错误码、跨系统故障注入与 redrive 验收。四块不得重新打成一个 mega PR。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ①+②、R2-A1 Asset、R2-A2 KB 与 R2-A3 Profile 已完成 | 当前做 R2-A4；R2 资产/KB/Profile 前置 M1-c；R1-min 原子预览余项在 M1-e 前完成 |
...
| R2-A1 Asset | ✅ 2026-07-17：commit CAS + attempt/token/lease fencing、canonical copy 幂等、P2002→duplicate、failed_retryable、tombstone + parked cleanup Outbox；scanner 前只自动删 staging | #126、R1-safety | **MF-0/M1-c** | 🔴 开发环境 PostgreSQL/RLS/MinIO、空库 migration、并发/故障专项验证已过 | M1-c 前置 |
