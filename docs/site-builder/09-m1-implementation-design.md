...
>
> **2026-07-16 as-built**：#121/#123/#124 已完成 intake 行为与安全修复；#125 已完成 truth-sync；#126 已完成 R0 contract closeout（持久幂等、`buildId`、去响应 `mode`、稳定错误码、Temporal 启动证据与 OpenAPI）。DQ-1 已由 #117 合并；M1-c/MF-0/R2 均尚未落地。
...
- **生产化审计前置（2026-07-17 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract 与 **R1-safety ①（临时 SiteSpec `finally` 清理 + Renderer 最小 env allowlist）已完成**；当前关键顺序是：**R1-safety ②（Crawl4AI/robots 完整出站闸）→ 拆分后的 R2-A1/A2/A3/A4 → MF-0-thin → M1-c**。R1 其余 per-run staging/原子预览工作仍在 M1-e 可见预览前完成；R3/R4、MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行，不再把所有修复塞进一个 R2 mega PR。完整依赖见 §11。
...
> **与 §11 生产化审计 PR 的交织（2026-07-16 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；R0 contract 已由 #126 完成，当前先做 R1-safety，再把 R2-A 拆为 Asset/KB/Profile/Integration 四个可审 PR，合并 MF-0-thin 后才进 M1-c。R1 余项早于 M1-e 可见预览；R3、R4-A/B、MODEL-0/EVAL-bootstrap、DI-0 按各自消费者并行。完整依赖与风险见 §11。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ① 已完成构建隔离 | 当前做 R1-safety ②；R2 资产/KB 前置 M1-c；R1 原子预览其余项在 M1-e 前完成 |
...
3. 当前 main 尚无 M1-c 实现。若旧 worktree/分支中存在 Sharp 纯函数，只可在核对未提交改动后选择性提取，并在 R2-A4 与 MF-0-thin 合并后 rebase/cherry-pick；不以分支存在推断能力已落地。
...
| R1-safety | 两个可审小 PR：① ✅ 2026-07-17 临时 SiteSpec `finally` 清理 + Renderer 子进程 env allowlist（不含 R1-min 的可见产物 staging）；② **当前** Crawl4AI + robots 完整 egress gate，移除生产 broad allow-internal，并做公开正向及 private/loopback/metadata/rebinding/redirect 负向验证 | #125/#126 已合 | 构建与抓取安全 | 🔴 租户内容/密钥/出站边界 → 用户确认 | M0/M1-a |
...
| R2-A1 Asset | commit CAS + fencing、canonical copy 幂等、duplicate reconciliation、tombstone/outbox；scanner 前只自动删 staging | #126、R1-safety | **MF-0/M1-c** | 🔴 状态机 + 对象删除 → 人审 | M1-c 前置 |
...
| R2-A3 Profile | 组内 schema/限额 + 乐观并发；先统一 `If-Match`/`baseVersionId` 与 409/412 合同 | #126；可与 A1/A2 代码并行，migration 串行 | 公开 intake | 🟡 API 并发合同 | M0 closeout |
