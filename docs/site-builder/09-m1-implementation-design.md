...
>
> **2026-07-17 as-built**：#121/#123/#124 已完成 intake 行为与安全修复；#125 已完成 truth-sync；#126 已完成 R0 contract closeout；R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A 与 R3-B1 均已落地**。DQ-1 已由 #117 合并；下一施工序不是直接写 M1-d，而是 **R3-B2 → R4-A1 → R4-A2 → R4-B-min**，再进入 M1-d。
...
- **生产化审计前置（2026-07-17 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A 与 R3-B1**均已完成；当前关键顺序是：**R3-B2 → R4-A1 → R4-A2 → R4-B-min → M1-d**。R1-min 的 per-run staging/原子预览工作仍在 M1-e 可见预览前完成；MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行。
...
> **与 §11 生产化审计 PR 的交织（2026-07-17 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；R0 contract、R1-safety、R2-A 四项、MF0-A/B、M1-c、R3-A 与 R3-B1 均已完成。下一施工顺序为 R3-B2、R4-A1、R4-A2、R4-B-min，完成后才进入 M1-d。R1-min 余项早于 M1-e 可见预览；MODEL-0/EVAL-bootstrap、DI-0 按各自消费者并行。完整依赖与风险见 §11。
...
| R3-1 ✅ B1 2026-07-17 | M1-a | builds.controller | targetId 已改 1–128 位 SiteSpec 标识符；page/section 在执行器真消费前返回 422 unavailable | 不再用 UUID 假契约或静默整站重建 | R3-B1 已完成；真实局部构建归 B2 |
| R3-2 ✅ B1 2026-07-17 | M1-a | CreateBuildDto | 嵌套 DTO/白名单/数量上限/BCP-47/preset 目录已落；pages 与非 en 在实现前 fail-closed 422 | 无效或未实现 options 不进工作流 | R3-B1 已完成；真实 pages/locales 归 B2 |
...
**R3**：R3-A 已完成 SiteBuildRun 数据库背书。**R3-B1 已在 2026-07-17 当前交付分支完成**：有界字符串 targetId、严格 BuildOptions、共享 preset 目录、请求指纹幂等账本、Temporal workflowId/firstExecutionRunId ACK 持久化；取消等待 Temporal 执行链关闭与补偿完成后才释放 active 单飞门，未确认/未关闭保持 active 并返回 502。page/section/pages/non-en 在执行器真实消费前 fail-closed 422。Ubuntu 开发环境以真 PostgreSQL/app_user/FORCE RLS + 真 Temporal 验证同 key 重放、异请求 409、双 ID 对账、取消与夹具清理，**不代表生产部署**。**R3-B2 仍待完成**：真实 scope/options 消费与每阶段单调 phase/progress/step/replay；costSummary 仍归 R4-B-min，不能在 B2 伪造。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A 与 R3-B1 已完成 | 先做 R3-B2/R4 前置，再进入 M1-d；R1-min 原子预览余项在 M1-e 前完成 |
| M1-a | 工作流框架保留；R3-A 数据库背书与 R3-B1 API/ACK 已完成，真实 scope/options 与进度语义仍需补强 | 做 R3-B2；产物原子性随 R1 |
...
| R3-B1 ✅ | 请求合同 + 持久幂等 + Temporal ACK：targetId/BuildOptions/key 指纹/workflow+run trace 已完成；未实现 scope/options 422 | DQ-1、R3-A | R3-B2 | 🟡 API/ACK；真 PG+Temporal 开发验证 | 已完成 |
| R3-B2 | 真实 scope/options 消费 + 单调 step/progress/replay/终态化；不伪造 cost | R3-B1 | **M1-d** | 🟡 工作流恢复性 | M1-a→M1-d |
...
| M1-d | Copy slots/内容预算/最小询盘合同：按 slot/locale/Claim refs 生成，先事实门再文风；最小 inquiry/consent/outbox 合同（实际公开接收随 M2） | R3、R4-A/R4-B-min；DI-0 可并行且在 M1-e 前收口 | — | 🟡 | M1-d |
