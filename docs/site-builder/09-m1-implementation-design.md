...
>
> **2026-07-17 as-built**：#121/#123/#124 已完成 intake 行为与安全修复；#125 已完成 truth-sync；#126 已完成 R0 contract closeout；R1-safety ①+②、**R2-A1 Asset、R2-A2 KB、R2-A3 Profile 与 R2-A4 Integration 正确性门均已落地**。DQ-1 已由 #117 合并；MF-0 与 M1-c 尚未落地。
...
- **生产化审计前置（2026-07-17 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②与 R2-A1–A4**均已完成；当前关键顺序是：**MF-0-thin → M1-c**。R1-min 的 per-run staging/原子预览工作仍在 M1-e 可见预览前完成；R3/R4、MODEL-0/EVAL-bootstrap、DI-0 等可按各自消费者并行，不再把所有修复塞进一个 R2 mega PR。完整依赖见 §11。
...
| R3-6 | M1-a | SiteBuildRun DB invariant | R2-A4 已统一 re-intake/refurbish site lock；但 run 仍缺 `(siteId,workspaceId)` 复合 FK、status CHECK、每站 active 部分唯一索引 | owner/内部写或新入口可绕过 provenance/单飞 | M1-d 前独立 migration |
...
**Asset commit/cleanup 状态机（✅ R2-A1/A4 已落地）**：`pending_upload/failed_retryable/expired committing → committing → ready/queued ↘ rejected/duplicate/failed_retryable`。入口 `updateMany` CAS 分配递增 attempt、UUID fencing token 和 lease；所有完成/失败回写匹配 attempt+token，过期 lease 接管后旧持有者不能 zombie write。canonical key 由 content hash 决定、copy 幂等；P2002 转明确 duplicate；canonical DB 真值与 staging cleanup intent 同事务提交。staging cleanup 以 eventId 作 workflowId，`notBefore` 只记录原 presigned PUT 到期点；workflow 再执行生产代码固定、命令不可缩短的 **15 分钟在途 grace**，随后回查 Outbox+Asset provenance、首轮 DeleteObject+HEAD，durable settle **5 分钟**后以同一 provenance 二次 DeleteObject+HEAD。它覆盖到期前已获授权且在受支持窗口内完成的 PUT 在首删后复活，不能把“URL 到期”单独描述成绝对防复活；失败有限重试、结构化告警，可由 guarded redrive 只重排 FAILED/超时/取消的严格 staging 命令。DELETE 落 tombstone + Outbox，不在事务里做 S3 IO；若 commit/KB worker 已持有状态则 CAS 返回 `409 ASSET_BUSY`。canonical intent 仍 parked；MF-0-thin 的 `SiteSpecAssetReferenceScanner` 落地前**不得删除 ready canonical object**。scanner 扫 active draft/published SiteSpec，命中默认 **409**；MF-1 后切 `AssetUsage` 不改 API。
...
**R3**：`targetId` 改有界标识符（与 SiteSpec pageId/block id 契约一致，不假装 DB UUID）；BuildOptions 用明确 DTO/schema（stylePreset 命中目录、pages 是已存在 pageId、locales 去重 BCP-47 有上限）；Idempotency-Key 限长/限字符并要求重放先核验/补齐 Temporal launch ACK（后续升显式列+唯一索引）；launcher 回写 `firstExecutionRunId`/workflowId、`AlreadyStarted` 视为幂等成功；SiteBuildRun 补复合 provenance FK、status CHECK 与每站 active 部分唯一索引；cancel 保「DB 先封发布 + Temporal best-effort cancel + 清本 run staging」；每活动完成后增量更新 phase/progress/step/cost。
...
| 阶段 | 判断 | 处理 |
|---|---|---|
| M0 | 主干保留；#121/#123/#124/#126 已闭环 R0 行为、安全与 intake 合同；R1-safety ①+②、R2-A1–A4 已完成 | 当前做 MF-0-thin；R2 正确性门前置 M1-c；R1-min 原子预览余项在 M1-e 前完成 |
...
| R2-A4 Integration | ✅ 2026-07-17：staging-only Outbox/Temporal cleanup、15 分钟晚到上传门、双 provenance、稳定 Asset/KB/Build 错误码、cancel CAS、重试/告警与 guarded redrive；canonical 继续 parked | R2-A1/A2/A3 | **MF-0/M1-c** | 🔴 跨系统一致性 → 人审 | M1-c 前置 |
