...
- **新表 `brand_profile`**（schema:1137 注释已点名）：`id/workspaceId/siteId/version/valueProps(Json)/tone(Json)/glossary(Json)/keywords(Json)/differentiators(Json)/competitors(Json)/factSheet(Json)/gaps(Json)/researchDegraded(Bool)/createdAt`，版本化追加不覆盖；RLS + FORCE RLS（M0 六表先例）。**evidence 结构分级**：factSheet 每项 `{value, evidence:{sourceType: intake|upload|storefront|web_research, url?, fetchedAt?}}`（合规 D2）。
  ...
  - 🔴 **R4-A2 truth bridge（✅ 2026-07-18 当前交付分支）**：value 中关键数值/单位、认证代码与名称/型号须被 quote 支持，否则降 gap；`research_hint` 永不投影。新 intake 原子创建 CompanyProfile→Site 关系，历史无关系 Site 保持 nullable、禁止名称猜测并在 intake/BrandProfile 路径 fail-closed。通过门的事实以包含 snapshot/role/asset 的稳定 origin key 复用公共 Claim/Evidence；三列 Site FK、不可变 FORCE RLS bridge 与逐字段插入 trigger 共同锁定同公司和精确 EvidenceRef。生成 Claim 强制 `NEEDS_REVIEW`；status+version CAS、内容 digest 与 Claim/Evidence immutability 防止双审批和批准后换内容。认证分类共用且 Asset 按 UUID 稳定锁序取得；内部 approved-effective 对所有新桥接 Claim 重算人审 proof。无公开 API、SiteSpec/CopyBundle/正式 `PublishableClaimSnapshot` 变化。
...
**R4-A1（✅ 2026-07-17）**：共享 `EvidenceRefV2`、冻结 source snapshot、完整 SHA-256、精确 quote/Unicode selector、KB chunk provenance、append-only RLS/FK 基础已落；SearXNG 原始页面元数据不冻结，只生成最小 origin hint；旧 v1 兼容读取、不伪造回填，公开 OpenAPI 不变。**R4-A2（✅ 2026-07-18 当前交付分支）**：确定性 value/quote 关键值对齐与 `research_hint` 降 gap 已接入 BrandProfile；nullable Site→CompanyProfile 关系、新站原子写入/旧站 fail-closed、包含 snapshot/role/asset 的 Claim/Evidence origin key、同公司三列 FK、逐字段 exact-evidence trigger、不可变 RLS bridge、稳定 cert Asset 锁序、审批 CAS+内容 digest 与内部 approved-effective proof 重算均已落。所有生成 Claim 仍先 `NEEDS_REVIEW`；A2 没有新增公开 API、SiteSpec/CopyBundle 或 M1-d 消费者。**R4-B-min 预算真值**（v3.2 §24.7 · ADR 无既有条目，属施工承载假设）：M1-c 可继续不消费模型预算；**M1-d 首个付费 fan-out 前须有 DB 持久 `reserve/settle` + 人工停用开关**；`SiteBuildRun.costSummary` 是**持久聚合真值**（含成功/失败/schema repair/timeout/fallback/工具成本）；**进程内 `BudgetLedger` 只可作单测或本地单 worker 适配器，不能被描述为生产预算门**（Redis/多 worker 配额优化随部署拓扑后补）。
...
**R4-A2 迁移/验证/部署门**：首个 additive migration 已进入 Ubuntu `global_dev` 后不再改写，复审 hardening 以第二个 forward migration 增同公司三列 FK、exact-evidence/immutability triggers、quote selector 成对 NULL CHECK 与最小 app_user 权限。value/quote acceptance 语义收紧后，代码 evaluator 升为 `brand-profile-evaluator/3`；既有 MODEL-1 冻结报告继续保留其真实 v2 标识，不回写历史证据。真 PostgreSQL verifier 已覆盖并发双审批单赢家/单 outbox、同租户跨公司、不同 snapshot 同 quote 不误复用、EvidenceRef/Evidence 错配、审批 digest 篡改、Claim/Evidence owner 修改及 A/B/unset RLS；兼容 A1 真服务回归由 Terra 两轮产出 v2 11 facts/8 gaps 与 v3 9 facts/7 gaps，来源去重、PII/selector/hash 和夹具清理均绿。上述只代表开发环境；生产前须在恢复快照量测 site/evidence/claim/bridge 表规模、唯一索引和约束验证时长，5 秒锁预算不满足时安排维护窗口，不能把代码迁移存在写成已部署。
...
| R4-A2 ✅ | Claim/Evidence truth bridge：value/quote 关键值对齐、`research_hint` 降 gap、nullable Site→CompanyProfile + 旧站 fail-closed、稳定 Claim/Evidence key + 不可变 RLS bridge、认证 ready cert Asset 锁、审批审计与内部 approved-effective 门；无公开 API/SiteSpec/CopyBundle | R4-A1 | **R4-B-min → M1-d** | 🔴 事实/合规门；代码与迁移落地不等于生产部署 | 已完成 |
