...
>
> **2026-07-22 as-built**：R1-safety ①+②、**R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min 与 DI-0 均已落地**。DI-0（#164）只交付净室设计合同与冻结的空静态 Catalog；真实 Family、SiteSpec 1.1、Renderer/assembly 消费和 DesignEvaluation 运行时生产仍归 M1-e/M1-f。下一主线门是 M1-e-A，`template-distillation` 只可按批准合同选择性提取。
...
- **生产化审计前置（2026-07-22 as-built 复核）**：M0/M1-a/M1-b 的**主干保留**，已确认问题按消费者与正确性分流。truth-sync、R0 contract、**R1-safety ①+②、R2-A1–A4、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min 与 DI-0**均已完成。R1-min 已提供对象存储不可变 Release、跨节点恢复/回收与 unknown component gate；DI-0 已提供 M1-e 所需的合同 seam，但 Catalog 仍为空。M1-e-A/B 必须补齐批准组件、真实 Family 与消费者后才构成受控组装。
...
- 组件库补齐 04 §5 v1 封闭 **55 型**（D12/ADR-015，17→55；渲染器已注册 **55** 个：AboutBlock/CertWall/CtaBanner/FaqAccordion/HeroBanner/InquiryForm/MapLocation/ProcessTimeline/ProductGrid/StatsBand）：+TrustBar/ProductDetail/FactoryShowcase/CaseStudies/WhatsAppFloat/VideoBlock 等 + 既有组件补变体位；封闭枚举 `type`/`variant`，Section.astro 注册表扩展（🔴 **R1-4 修**：未知 type 从静默 null 改 **fail-closed**——契约漂移不再被掩盖）。55 型清单是 04 契约真值，本文只引不复述。
...
> **与 §11 生产化审计 PR 的交织（2026-07-22 as-built 复核）**：下表 M1-a…M1-g 是**能力主序列**；R0 contract、R1-safety、R2-A 四项、MF0-A/B、M1-c、R3-A、R3-B1/B2、R4-A1/A2/B-min、M1-d、R1-min 与 DI-0 均已完成。M1-e 的两个前置门已解除，下一步按 M1-e-A → M1-e-B 推进。完整依赖与风险见 §11。
...
| M1-e | P3 组装：designSpec+siteAssembly AiTask、三重门校验器（含语义规则）、构建错误回填重试、组件库补齐 **55 型**（M1-e-A：55 型已落地/`<picture>` 固定 variantId/reduced-motion；M1-e-B：DesignBrief + Family/Blueprint/兼容矩阵受控组装）。**依赖 R1-min 已合**（可见预览原子化） | agents/design-spec、agents/assemble、spec-validator、renderer 组件 ×16 | spec-validator.spec（三门逐条）、assemble.spec、design-spec.spec、picture 消费测试 |
...
| DI-0 ✅ | #164：DesignSourceManifest/Observation/Rule/DNA/TemplateFamily/DesignBrief/DesignEvaluation + 冻结的空静态 DesignCatalog；Tier A/B/C 授权分级、规则聚合≥5 独立贡献组、确定性 digest、approved-family-only resolver、运行时零读原始 Readdy（ADR-019） | DQ-1 已合并 | M1-e 受控组装 | 🟡 合规合同已验证；真实 Family/消费者未落 | 已完成 |
...
| M1-e-A | 55 组件与变体：04 的 17->55、variant compatibility、picture、motion/reduced-motion；从 IT-0 选择性提取 | R1-min | — | 🟡 组件契约 | M1-e |
