...
> 机器基线：`site-builder-model-candidate-baseline/2026-08-07-v3`；本文件由机器基线生成并由 `pnpm docs:verify` 校验。

<!-- BEGIN GENERATED MODEL CANDIDATE BASELINE -->

## 边界

本基线 `site-builder-model-candidate-baseline/2026-08-07-v3` 只登记非运行时候选与后续评测顺序。它本身不路由候选、晋级模型、改变 rollback、环境变量、Temporal、P4、DesignEvaluation、ReleaseManifest、MediaGateway 或公共 API，也不证明模型质量、生产可用性或部署状态。
...
|---|---|---|---|---|
| `gpt-5.6-terra` | `text` | `runnable` | `openai-responses`<br>`openai-chat-completions` | Existing BrandProfile promotion remains unchanged; other tasks require their own evaluation. |
...
| `gpt-5.6-sol` | `text` | `runnable` | `openai-responses`<br>`openai-chat-completions` | High-reasoning Copy and multimodal-review candidate; each task still requires independent evaluation and promotion. |
...
| `copy.premium` | `requires_task_evaluation` | `gpt-5.6-terra` (`runnable` · `openai-chat-completions` · `capability_probe`)<br>`gpt-5.6-sol` (`runnable` · `openai-chat-completions` · `capability_probe`)<br>`claude-sonnet-5` (`runnable` · `anthropic-messages` · `capability_probe`) | capability pilot before claim-bound multilingual copy evaluation<br>capability pilot before claim-bound multilingual copy evaluation<br>capability pilot before claim-bound multilingual copy evaluation |
