...
> 🔴 **55 型封闭库（ADR-015 / D12，已落地）**：渲染器 `Section.astro` 实注册 55 个（封闭真值 = `packages/contracts/src/site-builder/site-spec.ts` 的 `SITE_SPEC_COMPONENT_TYPES`），未知组件 fail-closed throw `UNKNOWN_COMPONENT_TYPE`（不再静默 null）。`PuckBlock.type` 已收紧为 `SiteSpecComponentType`。下表为首批组件用途参考，完整 55 型以代码 `SITE_SPEC_COMPONENT_TYPES` 为准。
