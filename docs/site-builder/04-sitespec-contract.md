...
> 文档 ID：`SITE-SPEC-001`
> 生命周期：`CURRENT`
> 当前事实来源：`@global/contracts`、ReleaseManifest reader/writer 与 [状态](../status/current.md)。
...
> 🔴 **as-built 与目标态严格分栏（2026-07-24）**：
> - **as-built 1.0.0**：Demo v0 与 immutable v1 Release 保持 `copyBundles` / legacy `AssetRef{kind,hash}` 的兼容合同。
> - **as-built 1.1.0**：M1-e-B 的受控精装修写 `componentLibraryVersion`、`rendererVersion`、`site.archetype`、`site.familyId`、`site.dirByLocale`，并使用 tenant/catalog 二元 asset manifest；Renderer、Preview 和 ReleaseManifest v1/v2 均按版本收窄读取。
> - **仍是目标**：`claimRefs`、`offeringRefs`、`locks`、`RichTextDoc`、视频/产品富引用和 MediaJob/AssetUsage。凡本文标 **[1.1.0 目标]** 处均不得当作当前接口。
>
> `Reviewed against 12 v3.2` · as-built SHA = #117 合并态 · 组件库 v1 = **55 型**（D12，17→55；ADR-015）。

## 1. 顶层信封

### 1.1 legacy 1.0.0（Demo v0 与 v1 Release）
...
### 1.2 受控 1.1.0 与后续目标字段

DI-0 提供独立设计合同，M1-d 提供 `copyBundleSet`；M1-e-B 已通过 **semver minor** 增量发布 1.1.0（老 spec 恒可渲染）。下列 JSON 同时展示已落 identity 字段和后续目标字段；只有前述 as-built 列表属于当前合同。

```jsonc
{
  "specVersion": "1.1.0",
  "componentLibraryVersion": "m1-e-a/55-qualified", // [新] 组件库资格版本 → 决定组件契约兼容
  "rendererVersion": "renderer-build-identity", // [新] 渲染器 build identity → 决定可重放兼容
  "site": {
    "defaultLocale": "en",
    "locales": ["en", "de"],
    "dirByLocale": { "en": "ltr", "de": "ltr" }, // [新] 显式方向表（替代按 locale 猜 dir）
    "archetype": "industrial-manufacturer",       // [新] BusinessArchetype → 决定业务/页面证据结构
    "familyId": "precision-industrial",            // [新] approved TemplateFamily → 决定全站视觉语法
...
> 🔴 **55 型封闭库（ADR-015 / D12，M1-e-A 已完成）**：渲染器 `Section.astro` 与 dev gallery 已注册 55 个（目标真值 = `packages/contracts/src/site-builder/site-spec.ts` 的 `SITE_SPEC_COMPONENT_TYPES`），未知组件 fail-closed throw `UNKNOWN_COMPONENT_TYPE`（不再静默 null）。55 型均已具备独立 schema、受控 variant、内容预算、a11y、reduced-motion、最小 fixture 与 375/768/1440 三断点视觉证据，并以 `m1_e_a_qualified` 进入 `SITE_SPEC_RELEASE_COMPONENT_TYPES`；不存在 `gallery_only` 或 `transitional_release` 状态。A.13 的复杂 Hero 已移除翻转、揭示和持续动画；既有 HeroFull selector/reveal 输入仅降为静态语义内容，唯一保留的互动状态为原生 radio，CTA 均解析到真实内部页或受控 HTTPS 目标。冻结的原 R1 10 型名单仅保留 provenance/不可注入边界。

各批组件按风险串行进入资格名单；最终 55 型均由 Release 预检验证 CTA 页面引用、证据字节和三断点截图 SHA-256。
...
- **证据门（无证据即删组件，绝不虚构 → ADR-017）**：StatsBand 需 ≥2 个有证据数值否则删；**Testimonials 需用户提供的可验证引用**否则删；**CrewGrid 当前只允许匿名运营角色卡**，不得含姓名、肖像、引语或自我声明授权；具名人物资料（🔴personalData）须等待独立的、数据库可验证的授权台账与发布合同后才可引入；CertWall 需资产或事实证据否则删；MapLocation 地址不可验证时**只显地址文本**（不出交互地图）。
...
> [as-built] #165 为 55 型提供组件 props Zod 门；M1-e-B 已在其上完成 SiteSpec 信封、引用、语义与兼容四层 validator。组件 schema 仍不能单独冒充整体验证；以下保留的是未来扩展字段的目标门。
...
> 本节把 SiteSpec 之外的**衍生共享契约**收进同一处（`packages/contracts/src/site-builder/`）。DI-0/#164 已落地 DesignSourceManifest/Observation/Rule、DesignDNA、TemplateFamily、DesignBrief、DesignEvaluation 与静态 DesignCatalog 的类型、运行时 fail-closed validator 和确定性 digest。M1-e-B 已发布六个 approved Family、Blueprint、StylePreset 与 DemoVisualPack，并由 DesignBrief、assembly 和 Renderer 消费；只有 DesignEvaluation 的运行时生产留在 M1-f。DesignBrief 深入用途见 [03-agents.md](03-agents.md)，落地分期见 [09-m1-implementation-design.md](09-m1-implementation-design.md)。
...
`packages/contracts` **已是 SiteSpec 1.0/1.1 与设计合同的唯一类型源，不得重建第二份**。#164 奠定合同，M1-e-B 已发布真实目录条目和消费者：
