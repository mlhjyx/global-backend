...
> 🔴 **55 型目标库（ADR-015 / D12，提取基线已落地，M1-e-A 尚未完成）**：渲染器 `Section.astro` 与 dev gallery 已注册 55 个（目标真值 = `packages/contracts/src/site-builder/site-spec.ts` 的 `SITE_SPEC_COMPONENT_TYPES`），未知组件 fail-closed throw `UNKNOWN_COMPONENT_TYPE`（不再静默 null）。不可变 Release 另以 `SITE_SPEC_RELEASE_COMPONENT_TYPES` 维护资格；`HeroBanner`、`StatsBand`、`CtaBanner`、`ProductGrid`、`AboutBlock`、`InquiryForm`、`CertWall`、`ProcessTimeline`、`FaqAccordion` 已完成七件套并以 `m1_e_a_qualified` 优先于旧状态，仅 `MapLocation` 仍是 `transitional_release` 兼容债，另 45 型是 `gallery_only`。冻结的原 R1 10 型名单只保留 provenance/不可注入边界，不把已晋级九型重新降级。新增蒸馏组件不得因进入 gallery 自动晋级。
