...
每个组件进入 `SITE_SPEC_RELEASE_COMPONENT_TYPES` 前**必须齐备七件套**，缺一不得晋级：**① schema（props/variant/motion 封闭枚举）② 变体 ③ 内容预算（§5.2）④ a11y 合同 ⑤ reduced-motion 降级 ⑥ fixture ⑦ 三断点视觉回归**。55 型提取或 gallery 可见只代表待改造输入，不代表发布资格。
