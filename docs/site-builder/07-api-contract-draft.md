...
**R2-A1/A4 + MF0-B as-built**：commit 先 CAS 进入 `committing`（递增 attempt + UUID fencing token + lease），事务外做 HEAD/hash；canonical key 的最终门以 PostgreSQL advisory xact lock 串行同 hash producer，在租户事务内拒绝任一 active/unsettled owner，随后 copy 并 fenced 激活，关闭 `copy→旧 cleanup→finalize` 缺失对象窗口。瞬时失败进入 `failed_retryable` 并保留 staging；active owner 收敛为 `ASSET_DUPLICATE`，未 settle tombstone 返回 `ASSET_BUSY`。staging cleanup 仍执行固定 15 分钟在途窗口 + 首删/HEAD + durable 5 分钟 + 二次删除/HEAD。
...
**删除引用语义**（§8.5 回写，失败码 §13 `ASSET_IN_USE`）：当前保护 Profile 三个正式引用面与 `Site.activeVersionId` 指向的唯一当前 SiteVersion；历史版本不阻止删除，但重新切为 active 时必须重新锁定并验证。命中返回 **409** + `details.usages[]`（`source/page/component/fieldPath`，SiteSpec 项可含 `siteVersionId`）；替换流程须**先改 Profile/Spec 再删**。未被引用的对象只做 tombstone + 严格 Outbox plan，不在 DELETE 事务内执行 MinIO IO。
...
| `ASSET_IN_USE`                 | 409                   | 删除被 Profile 或当前 `activeVersionId` SiteSpec 引用的 Asset；`details.usages` 给稳定引用位置清单            | §3 `DELETE /assets/{id}` · ADR-018 删除守卫                   |
