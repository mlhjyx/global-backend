...
> ✅ **as-built（#126）**：Swagger/OpenAPI、DTO 与实现已统一为 `{siteId,buildId,status:"generating_demo"}`；正式客户端必须复用同一 `Idempotency-Key` 进行安全重试。无 key 仅保留兼容，不承诺 Temporal ACK-loss 下的安全重放。
