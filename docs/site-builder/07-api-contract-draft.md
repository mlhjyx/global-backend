...
>
> Reviewed against `12` v3.2（2026-07-16 DOC-12 回写）；truth-sync 审计基线（合并前）= `main@a306ffa`（#124，SiteSpec `1.0.0`）。#121 已完成 intake 无条件 Demo 的**行为**，#123/#124 已完成事实/隐私/失败语义加固；R0 API 契约尾巴仍待独立 closeout。
...
> **as-built vs target**：本稿是**目标契约草案**，不是当前客户端可直接依赖的已发布合同。当前接入一律以 code-first OpenAPI 为准；下方 ⚠️ 标注的 R0-contract/R2-A3/R3 形状，只有代码落地、重导 OpenAPI 并通过 breaking-change 门后才生效。
...
- **HTTP 状态**：401 未认证 / 403 越权 / 404 不存在 / 409 冲突（构建进行中、Asset 被引用、Spec 版本落后）/ 412 前置条件失败（`If-Match` 不匹配）/ 422 校验失败或发布门拒绝 / 429 配额或限流。业务分支一律读 `error.code`（唯一稳定键，见 **§13 统一错误码表**）；`message` 仅供展示、可变。
- **幂等**：目标上 `POST intake`、`POST /sites/{id}/builds`、`POST /sites/{id}/releases`/`publish` 支持 `Idempotency-Key`，同键重放返回首次结果。as-built 仅 builds 已声明该 header；intake 待 R0-contract，release/publish 尚未实现。客户端不得从通用约定推断未在 OpenAPI 声明的端点已幂等。
- **乐观并发**：可变资源（Spec、Profile、Release 草稿）写操作携带版本判据——`If-Match: <ETag>` 头 或 请求体 `baseVersionId`；不匹配返回 **409/412**，**绝不 last-write-wins**（见 §6）。
- 时间一律 ISO 8601 UTC。

## 1. 注册引导（intake）〔🟢 M0 端点/无条件 Demo 行为已存在 · 🎯 R0-contract 目标响应〕
...
> ⚠️ **as-built 待对齐（R0-contract，`12` §24.2 回写）**：#121 已让 `intake.service` 无条件建 Demo，但当前 Swagger 摘要仍写"有站转站点诊断分支"，响应仍是 `{siteId,mode,status}`，没有 `buildId`，也未实现 intake `Idempotency-Key`。目标 `{siteId,buildId,status}` 只在独立 R0-contract 代码与 OpenAPI 合并后生效。
...
> ⚠️ **as-built 待对齐（R2-7，`12` §24.2 回写；公开 intake 前修）**：现 profile patch **只校验五个顶层组名**，不校验组内 schema/数量/URL/总大小，且并发 read-merge-write 可**丢另一组更新**（大 JSON / 脏数据 / 提示注入面 / 向导丢数据）。改为组内 schema + 数量/大小上限校验 + 乐观并发（`baseVersionId`/`If-Match`），冲突 409 不 last-write-wins。

## 3. 素材（Asset）〔🟢 M0：presign/commit/list/delete · 🎯 M1-c：process/select-variant〕
...
> ⚠️ **as-built 现状**：`assets.controller.ts` 的 `DELETE` 注释标"引用检查随 M1 spec 物化补齐"——即 409+usages 守卫是 MF-0-thin 目标、尚未落地；`process`/`select-variant`/`GET /assets/{id}` 为 M1-c 目标端点。
