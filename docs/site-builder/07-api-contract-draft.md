...
>
> Reviewed against `12` v3.2（2026-07-16 DOC-12 回写）；as-built 基线 = main #117（SiteSpec `1.0.0`）。
...
> **as-built vs target**：本稿是**目标契约**。当 as-built 代码与本稿冲突（下方 ⚠️ 标注的 R0-2/R2-7/R3 项），以本稿为准、由对应修复 PR 对齐，**不以旧代码形状为准**。
...
- **幂等**：`POST intake`（§3.2 回写）、`POST /sites/{id}/builds`、`POST /sites/{id}/releases`/`publish` 支持 `Idempotency-Key` 头（客户端生成，如 uuid）；**同键重放返回第一次的结果**，不重复触发流程。键限长/限字符（服务端约束，见 §5 R3-3）。
- **乐观并发**：可变资源（Spec、Profile、Release 草稿）写操作携带版本判据——`If-Match: <ETag>` 头 或 请求体 `baseVersionId`；不匹配返回 **409/412**，**绝不 last-write-wins**（见 §6）。
- 时间一律 ISO 8601 UTC。

## 1. 注册引导（intake）〔🟢 M0 as-built〕
...
> ⚠️ **as-built 待对齐（R0-2，`12` §24.2 回写）**：当前 `intake.controller.ts` Swagger 摘要仍写"有站转站点诊断分支"、`IntakeResultDto.mode` 仍暴露 `builder|diagnosis` 旧分叉——与本稿目标契约冲突，会诱导前后端按错误契约开发。以本稿（**响应 `{siteId,buildId,status}`、无 `mode`**）为准，随 R0 PR 与 `intake.service` 无条件建站同 PR 修正。
