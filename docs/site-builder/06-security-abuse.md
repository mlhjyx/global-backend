...
- **L2 事实硬门（分阶段）**：R4-A1 已提供冻结 source 与精确 EvidenceRef v2 的引用完整性；它不等于 Claim 已批准。认证、数字、客户案例、性能结论必须在 R4-A2 关联公共 Claim/Evidence、完成 value/quote 与 cert Asset/人工 verified 门并达到 **APPROVED** 后才能进站（公共 Claim/Evidence 为单一真相源，见 09；禁自填"ISO"变站点事实；对应 §10 反造假）。
...
- **MapLocation 文本降级**：Demo 不启用地图、坐标或位置推断；注册资料中的地址无论是否已明确，均只作为无外呼文本卡展示（配置见 §11）。
- **事实门收口**：R4-A1 已落冻结 source、完整 SHA-256、精确 quote/selector 与不可变 RLS provenance，但只证明引用完整性；R4-A2 才把通过语义门的事实 upsert/关联公共 Claim/Evidence，并要求认证、数字、客户案例、性能结论 **APPROVED** 后进站（对应 §1 风险 L2）。
...
**MapLocation（04/05 的 D16，2026-07-23 修订）**：发布合同只允许无外呼的地址文本卡；`static` 兼容输入只归一到 `technical-grid`，另有 `quiet` 展示。禁止 iframe、第三方地图、Geocoding、坐标、地理定位、地图 key 与 consent 依赖；CSP 不为该组件开放第三方地图域。地址不确定时只显示提供文本，不推断坐标或位置。
