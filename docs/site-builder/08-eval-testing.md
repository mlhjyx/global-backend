...
>
> **2026-08-05 Copy closed-repair、dispatch preflight 与 operator evidence gate（当前交付分支，zero-call）**：#315/#317 已闭合最多一次 repair、live finite/exact credential、fixed-source 重算、one-shot 3/6/1 real ledger、reservation quota 与 request-bound consume-log settlement。现行 pilot v5 runner 继续把运行结果保持为私有 `copy_gateway_settlement_candidate`，再要求固定 Ed25519 公钥验证 post-execution canonical authorization，并把 authorization/candidate/output/ledger digest 一次性写回同一 hash chain；只有不可 clone 的 authorized handle 才标记 `real_gateway_settled` capability evidence。自签、错误 key/domain、过期、任一 digest 漂移、ledger freeze/unknown 或重复消费均拒绝。所有测试仍只使用 loopback fake 服务，不读取真实凭据或调用模型。
>
...
> **Copy capability fixed-source gate**：[manifest v3](../evidence/site-builder/m1-g-copy-real-capability-manifest-v3.json) 仅固定 #317 当时的 pilot v5 runner；operator gate 合并后必须单独生成固定当前 merge 的 create-only v4，显式覆盖信任公钥、签名 verifier、ledger event 与 runner。v4 产生前扩展后的 runner 不可 dispatch；v4 本身仍是 `NOT_AUTHORIZED`/zero-call。有限凭据、dispatch authorization 与逐 candidate operator authorization 安装前，真实 evidence、晋级、route adoption 和 36/72 Copy 质量矩阵均未发生。
