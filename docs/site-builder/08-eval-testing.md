...
>
> **Copy capability v11 部分成功（2026-08-08 当前 evidence 分支）**：真实固定 v11 共执行 Terra Chat/medium、Sol Chat/high、Sonnet Messages/medium 各 1 wire，无 repair。Terra 与 Sol 输出都通过生产硬门、settlement known，并生成 exact create-only acceptance artifacts；Sonnet 约 4 秒返回，但 AI SDK normalized input=1,501（普通 77 + cache-create 1,424），request-bound resolver 未读取 new-api log metadata，因而把 consume log 判为 invalid 并冻结该子账本。总账本为 3 execution / 3 wire / 2 known / 1 unknown / 2 completed，一次性 token 已停用；见 [partial evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v11-partial-evidence.json)。必须先 Git 接受 Terra/Sol，再合并 fake-gateway 证明的 Anthropic cache usage settlement 修复，并用 Sonnet-only 新 fixed source/授权恢复；不得重跑已成功候选。本结果没有运行 36/72、没有 ranking、promotion 或 route adoption。
>
