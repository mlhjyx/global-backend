...
>
> **2026-08-05 Copy closed-repair 与 dispatch preflight（#315、#317 已合并，zero-call）**：#315 已证明首 wire 输出无效时最多一次 repair、unknown settlement 不 repair、compiled drift 先保留 observation 再冻结；#317 的 pilot v5 把不超过 64 KiB 的 validation-failed prior output 正文作为明确的数据字段重新送入 repair，并与 original input/context、findings、repair context/prompt 和不变的 alias/protocol/reasoning/schema/sampling 一起绑定。测试覆盖 live finite/exact credential、fixed-source 重算、one-shot 3/6/1 real ledger、reservation quota、request-bound consume-log settlement与私有 `copy_gateway_settlement_candidate` 回执；该回执仍不能进入质量评分，只有后续独立的 operator-authenticated authorization/evidence 门才能产生 `real_gateway_settled`。所有测试仍只使用 loopback fake 服务，不读取真实凭据或调用模型。
>
...
> **Copy capability fixed-source gate**：唯一当前 [manifest v3](../evidence/site-builder/m1-g-copy-real-capability-manifest-v3.json) 是固定 #317/v5 runtime 的 create-only 产物：`origin/main@03d701ee15d28254419fa4f04fb865ba4fd44932`、67 个 source blobs、source bundle `e3bcbb3c…`，以及 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium 的 3 executions / 最多 6 wires / 每次最多一次 repair。它仍为 `NOT_AUTHORIZED`，网络/模型调用和实际费用为零；[v2](../evidence/site-builder/m1-g-copy-real-capability-manifest-v2.json) 只作历史审计。有限凭据与 operator-authenticated one-shot authorization 安装前，真实 evidence、晋级、route adoption 和 36/72 Copy 质量矩阵均未发生。
