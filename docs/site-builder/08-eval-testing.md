...
>
> **2026-08-01 zero-cost manifest（覆盖上一句“不含 manifest”状态）**：独立合同 `site-builder-design-spec-evaluation-manifest-prep/2026-08-01-v1` 固定 `origin/main@e493ba1d09fe37feea927f70d12f17aadadc5c6a` 的 `design_spec` suite/harness 源码；prep 分支只新增 create-only writer 和机器 manifest，冻结 73 executions / 146 wire calls、24 个零调用 deterministic comparator case、source bundle、compiled contracts 与停止条件。它不读取 `.env`、OpenOx 价格、余额或凭据，不包含模型 client；dispatch=`NOT_AUTHORIZED`，调用和费用均为零。
