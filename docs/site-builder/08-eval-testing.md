...
>
> **代码级 harness（`design_spec` 尚无真实 evidence）**：`site-builder-model-evaluation-harness/2026-07-30-v7` 从保持不变的 `site-builder-model-candidate-baseline/2026-07-27-v1` 与当前 task binding 生成计划；精确 task/protocol/envelope、canonical suite、结果类、排序、预算、协议 admission 与 provenance 只认[生成评测基线](model-evaluation-harness.md)，不得在本文件再抄矩阵。BrandProfile 既有 suite 保持；`design_spec` v4 suite 固定 12 个六 Family sparse/rich 合成 fixture、3 个 runnable 候选、2 次重复、一次 GPT-5.5 capability probe，以及真实执行并冻结 expected/actual/PASS/digest 的 deterministic catalog baseline。每个 fixture 保存完整合成生产输入，prepare 时调用产品共用的 catalog enumeration、required-role eligibility、ranking 和 top-3 projection 逐字重建；解释只允许 `selectedCandidateId` 与三个冻结指标的封闭 claim，自由文本或自然语言指标均失败。suite 固定 `legacyComparatorAliases=[]`，精确 target-only 凭据按 73 executions / 146 wire calls 绑定，MiniMax/Doubao 额外 scope 在预算与 client 前拒绝；未来真实执行仍由 `site-builder-model-evaluation-cost-safety/2026-07-28-v1` 强制有限凭据、精确 scope、冻结显式价格和 request 级结算。create-only runner 在导入 suite 前删除并本地重建 ignored contracts output，冻结 fixed-commit tracked contracts source tree 和实际加载的 JS tree；同一 21-artifact 树摘要进入 suite、case、probe、run 与汇总，并在每次调用前后复核。Gemini 文本继续 deferred，其余 5 个 task 仍无 suite；当前只用 fake wire/fetch/settlement，未读 `.env`、未发真实模型或媒体请求、未产生费用/evidence、未改 new-api token/channel、P4/DesignEvaluation/ReleaseManifest/Temporal/active route/env、公共 API 或 DB。
>
