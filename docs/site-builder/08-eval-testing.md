...
> **Copy capability fixed-source gate**：[current manifest v4](../evidence/site-builder/m1-g-copy-real-capability-manifest-v4.json) 已固定 #319 merge `00d39b384a03c2144fc04029ec90a3e840550140`，显式覆盖信任公钥、签名 verifier、ledger event/storage 与 runner；69-file source bundle digest=`8eee8729…`，artifact digest=`fa098048…`。它仍是 `NOT_AUTHORIZED`/zero-call；[v3](../evidence/site-builder/m1-g-copy-real-capability-manifest-v3.json) 仅供历史审计。有限凭据、dispatch authorization 与逐 candidate operator authorization 安装前，真实 evidence、晋级、route adoption 和 36/72 Copy 质量矩阵均未发生。
