...
>
> **Copy Sonnet recovery v15 测试不变量**：当前版本与 artifact 状态只认[当前状态](../status/current.md)。#372 已以 merge commit `ef603762359fa20c2fbbef93a62e523ed5b312c7` 合入 v15 create-only 输入，并在合并后再次通过 ancestry、artifact、compiled runtime 与双 fixed-source rebuild；这些仍是零调用技术证明。v15 manifest fixed source 必须精确为 #361 merge `fcb61e3060dd3289fec93bca11d02584f8080791`；manifest/runtime binding 必须分别从干净 GREEN commit 与干净 tracked-manifest commit 生成。canonical path、manifest/artifact/execution key、child slot 与 campaign/global/child/reservation namespace 全部只能使用 v15，v11/v12/v13/v14 wire/authorization 即使重算 dependent digests 也必须在 client/ledger 前拒绝。v14 artifact/binding 只按固定 file SHA、artifact digest、embedded-manifest 与 whole-artifact canonical digest 独立验真，不得交给 v15 validator，也不得成为 live source/admission。compiled tree 必须精确覆盖 #361 后 `ai-sdk-anthropic-messages.adapter`、`ai-sdk-adapter-result`、`types` 与完整 runner/verifier/gateway/recovery allowlist；fake gateway 继续覆盖动态 object schema、64 KiB UTF-8 pre-dispatch schema cap、Messages response compatibility、known/unknown settlement、closed repair 与 Git acceptance。create-only prepare code 不得读取 credential/env、创建 client 或调用 `fetch`。通过这些门只证明固定输入与 admission，不构成真实 capability、质量、晋级或生产路由证据；MiniMax/Doubao 保持 retired。
