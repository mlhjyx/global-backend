...
>
> **Copy Sonnet recovery 测试不变量**：当前版本与 artifact 状态只认[当前状态](../status/current.md)。每版测试必须证明 manifest/runtime binding 可从固定 Git source 重建，canonical path 与 compiled bytes 一致，execution/child-slot 及 campaign/global/child/reservation identity 全部版本隔离，旧 wire/authorization 即使重算 dependent digests 也会在 client/ledger 前拒绝；fake gateway 还须覆盖 Messages response compatibility、known/unknown settlement、closed repair 与 Git acceptance。create-only 通过只证明固定输入和 admission，不构成真实 capability、质量、晋级或生产路由证据。
