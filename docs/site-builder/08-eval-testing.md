...
> **Copy capability fixed-source gate**：[current manifest v6](../evidence/site-builder/m1-g-copy-real-capability-manifest-v6.json) 已固定 #323 merge `55f10fa325916be6dde488ab148d1462175cda12`，覆盖信任公钥、签名 verifier、ledger event/storage、runner、new-api boolean allowlist/token-scoped catalog，以及新增的脱敏 API-call error 保真；69-file source bundle digest=`6e74f4f5…`，artifact digest=`c9a4391e…`。它仍是 `NOT_AUTHORIZED`/zero-call；真实 v5 campaign 已 unknown-settlement 冻结，[v5](../evidence/site-builder/m1-g-copy-real-capability-manifest-v5.json) 及更早版本仅供历史审计。v6 finite credential/settlement preflight 与独立 dispatch authorization 完成前，真实 capability evidence、晋级、route adoption 和 36/72 Copy 质量矩阵均未发生。
