...
> 文档 ID：`SITE-EVAL-001`
> 生命周期：`CURRENT`
> 当前事实来源：资格证据、Golden 资产、[状态](../status/current.md)与候选基线 `site-builder-model-candidate-baseline/2026-08-07-v3`。
>
> **Copy Sonnet recovery v17 successor 测试不变量（当前覆盖规则）**：v16 token `#24` 是唯一允许保留的失败审计记录，且必须精确为名称 `Site Builder Copy Sonnet Recovery v16`、状态 disabled、ID `24`；它不是 live source、admission、credential 或成功 evidence。v17 preflight 必须拒绝 active/malformed/重复 v16、任意既有 v17 或其他同目的 token；失败 cleanup 只可禁用 v17，绝不可修改 v16 `#24`。v17 artifact/secret output 与 v16 路径/ID 分离，仍固定 `NOT_AUTHORIZED`、`dispatchCapable=false`、0 model wire、0 model cost，并要求单 alias/protocol、最多 2 physical wires、≤24 小时。该代码不创建模型 client、不调用 dispatch endpoint；v17 token 只有在 PR/CI/review/merge/post-merge 后才可按单独零调用授权创建，真实 dispatch 仍需新的 v17 明确授权。

> **Copy Sonnet recovery v16 测试不变量**：当前版本与 artifact 状态只认[当前状态](../status/current.md)。v16 manifest fixed source 必须精确为 verifier/runner GREEN commit `dcbbff8dfc34e7f2559327ccbdce0d4049387453`；manifest/runtime binding 必须分别从干净 GREEN commit 与干净 tracked-manifest commit 生成。canonical path、manifest/artifact/execution key、child slot 与 campaign/global/child/reservation namespace 全部只能使用 v16；v11–v15 identity/wire/authorization 即使重算 dependent digests 也必须在 client/ledger 前拒绝。v14/v15 artifact/binding 只按各自固定 file SHA、artifact digest、embedded-manifest 与 whole-artifact canonical digest 独立验真，不得交给 v16 validator，也不得成为 live source/admission。source verifier 必须以逐级 descriptor-anchored、`O_NOFOLLOW`、有界 regular-file read 拒绝 intermediate symlink/替换/增长，runner 必须在 ledger/claim 前和 dispatch 前再次验证 source/compiled guard。compiled tree 必须精确覆盖 `ai-sdk-anthropic-messages.adapter`、`ai-sdk-adapter-result`、`types` 与完整 runner/verifier/gateway/recovery allowlist；fake gateway 继续覆盖动态 object schema、64 KiB UTF-8 pre-dispatch schema cap、Messages response compatibility、known/unknown settlement、closed repair 与 Git acceptance。create-only manifest/binding prepare code 不得读取 credential/env、创建 client 或调用 `fetch`。#378 post-merge eligibility 已重验为 `CURRENT / NOT_AUTHORIZED / BLOCKED`。

> **Copy Sonnet recovery v16 零调用 preflight 工具门**：operator 只能创建一枚 purpose=`site_builder_copy_sonnet_recovery`、alias=`claude-sonnet-5`、protocol=`anthropic_messages`、最多 2 physical wires、最长 24 小时、有限 quota 且不允许 cross-group retry 的 token；允许的网络面只限 New API token/channel/usage/model/log 控制面与 OpenOx 公共价格目录，源码不得包含 `/v1/messages`、`/v1/chat/completions` 或 `/v1/responses` dispatch。New API 控制面和 OpenOx pricing tool 都必须 `redirect=manual` 并在读取重定向 body 前拒绝全部 3xx；OpenOx 目录只能由 `ExecutionBroker` 调用 `openox.pricing_catalog`，经 `ToolBroker` 的 required source policy（domain=`openox.tech`、purpose=`site_builder_copy_sonnet_recovery`）出网，固定 5 秒 timeout、1 MiB UTF-8 JSON 上限与 raw-response SHA-256，禁止 preflight 直接 `fetch` OpenOx。operator 的 `APP_DATABASE_URL` 只允许 `current_user=app_user`、非 superuser、非 BYPASSRLS；角色或 source policy 缺失必须在构造 broker、读取 admin bearer、OpenOx 出网和 token claim 前停止。执行前必须验证 clean exact HEAD 与 v16 runtime binding 固定 file/artifact/compiled-tree digest；route 必须唯一绑定 New API Anthropic channel type `14`、base URL `https://openox.tech` 与显式 identity `model_mapping`，创建后还须重新 readback route digest、单 alias scope（value 必须为 `true`）、quota、完整 `/v1/models`、OpenOx raw/selected-price digest 与 request-bound settlement resolver readiness。价格目录同一 model/group 匹配必须各恰好一行，per-wire quota 上界取 input/cache-read/cache-write 中最高的 input-side rate 再加 output rate；分页 `total` 缺失、变化或 overshoot 以及关键 control-plane observation 缺失都 fail closed。Git 外 bearer 只能写入当前用户私有、无 symlink component 的临时父目录，以 descriptor-anchored `O_EXCL|O_NOFOLLOW` 创建 `0600` 文件；tracked artifact 固定输出路径为 `docs/evidence/site-builder/m1-g-copy-sonnet-recovery-zero-call-preflight-v16.json`（文件未入库；此处仅记录预定输出路径，不作为现存证据），必须拒绝任何 secret 字段并保持 `dispatchAuthorization=NOT_AUTHORIZED`、`dispatchCapable=false`、`observedModelWireCalls=0`。writer 的 create-only publication、existing/raced target 保留、失败 token disable、secret inode/mode readback与删除均须有离线测试；相关 statements/branches/functions/lines 覆盖都必须 ≥80%。截至本页最后核验尚未访问 live credential/control plane、尚未创建 token；即使该门以后 live 通过，也只证明固定 credential、route、scope、quota、价格与零调用 resolver readiness，不证明真实 request-bound settlement、模型 capability、质量、晋级或生产路由。真实 dispatch 仍需新的明确 v16 授权，MiniMax/Doubao 保持 retired。

> **Copy Sonnet recovery v15 不可变历史（2026-08-10，已由 v16 live candidate 覆盖）**：[`v15 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v15.json) 固定 file SHA-256=`0ce90bf7f96b0012b85d410601079e9696305bfeb3e385e16358c5b9c9e4850e`、artifact digest=`b671dbd416b4ff279fd75290f91492bb37e7d5d4d41c29e9f5c86897690d8317`、manifest canonical digest=`789332c4542eadd5e6b534aa18698e7bb4dc0f312d9b081d50d7c1ececc368f5`；[`v15 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v15.json) 固定 file SHA-256=`838121ccf9649b05d9c04b05a1cec7ba094439a8a81a177462e5955a17c2ef7c`、artifact digest=`1c898f79745d14fce431404c0196beb5a69f68286d74a8588bec66d71e432e44`、runtime manifest canonical digest=`b082527a430377a8ccb99401d5c17cc62b89a96500a8cb92222f8f5af6eca85b`。两者均为 `NOT_AUTHORIZED`、0 calls、0 cost，v15 identity/token/authorization/wire 永不复用。

> **Copy Sonnet recovery v14 不可变历史（2026-08-10，已由 v15 live chain 覆盖）**：[`v14 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v14.json) 固定 file SHA-256=`e86f5d17539632f03df008bf9225998c80358ece58f804114bdbe9b593e7cf6f`、artifact digest=`1371fdcafe87aac3ef3ed6dd6fe35230550d5fa68cf235604fcf63ccf8c11c13`、manifest canonical digest=`8eaf961d39b9d6f2cf44d60702ee982433d15da4eb097c1ee1c1429a06a1314f`；[`v14 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v14.json) 固定 file SHA-256=`1ad6088afd6883adeae6824aa2a80ddd8d919b03cfcc90d35ecfe7172213b458`、artifact digest=`4f9fdf0623f854053e337c569760edfe82f643ab31e2034470fc18c26c04e475`、runtime manifest canonical digest=`0a3ddb1dc3d055a6aa60a2b0df1038b3b6de3509eb74fdb52bcc6c5d210799a5`。两者均为 `NOT_AUTHORIZED`、0 calls、0 cost，不含 #361 compiled semantics；v14 campaign/authorization/reservation/wire 永不复用。

> **Copy Sonnet recovery v13 历史输入（2026-08-09，已由 v14/v15 覆盖）**：[`v13 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v13.json) 与 [`v13 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v13.json) 保留为不可变 create-only provenance。其后私有 operator run 发出 2 条 known-settlement wire、执行 1 次 closed repair，但 0 completion 并冻结；该诊断不保存 raw prompt/output，不是 tracked capability/质量 evidence，且 v13 authorization/campaign/wire 永不复用。
>
> **Copy Sonnet recovery v12 历史输入（2026-08-08，已由 v13/v14/v15 覆盖）**：[`v12 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v12.json) 与 [`v12 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v12.json) 首次固定 Sonnet-only 1/2/1；其真实 wire 已停止且 authorization 冻结，不得复用或升级成后续 evidence。
>
> **Copy capability v11 部分成功（2026-08-08 历史执行记录；后续已合并）**：真实固定 v11 共执行 Terra Chat/medium、Sol Chat/high、Sonnet Messages/medium 各 1 wire，无 repair。Terra 与 Sol 输出都通过生产硬门、settlement known，并生成 exact create-only acceptance artifacts；Sonnet 约 4 秒返回，但当时 AI SDK normalized input=1,501（普通 77 + cache-create 1,424），request-bound resolver 未读取 new-api log metadata，因而把 consume log 判为 invalid 并冻结该子账本。总账本为 3 execution / 3 wire / 2 known / 1 unknown / 2 completed，一次性 token 已停用；见 [partial evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v11-partial-evidence.json)。#344/#345 已合并 acceptance 与 cache settlement 修复，当前恢复门以上方 v15 为准；不得重跑 Terra/Sol。本结果没有运行 36/72，没有 ranking、promotion 或 route adoption。
>
> **Copy Chat transport 重基（2026-08-07 当前交付分支，zero-call）**：`copy.premium` 的 Terra medium 与 Sol high 现在精确要求 `openai_chat_completions`，Sonnet medium 仍为 `anthropic_messages`；新 AI SDK adapter 固定 `provider.chat + streamText`、`/v1/chat/completions`、strict JSON Schema、usage/request ID/warnings、`maxRetries=0` 和 AbortSignal。pilot v10 与 Copy Evaluation v7 的 runner 只根据精确协议分派，不会把 Chat 桥接或回退为 Responses；fake gateway 同时覆盖 valid stream、strict request shape、524 + request ID + consume-log miss 导致的 unknown settlement/freeze。本分支不读 credential/new-api、不调用模型、不生成 capability evidence。合并后需新建 v11 create-only fixed-source manifest，真实 3/6/1 pilot 需再次单独授权。
>
> **Copy capability v10 stopped（2026-08-07 历史诊断，已由上方 Chat 重基覆盖）**：fixed-source 与 3/6/1 admission 通过后，只发出 Terra 的一条 `POST /v1/responses`。runner 保留了可在 artifact 中审计的 request ID，网关在 125.723 秒返回 524；new-api 预扣 CNY `0.040092` 后退款，但 consume log 仍为 0 行，逐 wire settlement 必须是 `unknown`。Terra 子账本冻结；Sol/Sonnet、repair、candidate receipt、Git acceptance 均为 0，token #18 已停用并以 401 复核。[`v10 stopped artifact`](../evidence/site-builder/m1-g-copy-capability-pilot-v10-stopped-evidence.json) 不保存 bearer、request/response body，只能证明 Responses transport/settlement 失败，不能评价模型能力。该诊断所需的离线 Chat adapter 与 fake-gateway 协议证明已由上方交付分支完成；下一门是 post-merge v11 manifest 与新授权，仍不得静默 fallback 或重试 Responses 路径。
...
>
> **Copy v10 fixed-source capability manifest（2026-08-07 执行前输入，zero-call）**：[`v10 artifact`](../evidence/site-builder/m1-g-copy-real-capability-manifest-v10.json) 固定 `origin/main@d819455dea736151a4c30d5ffdd0e224d74af917` 与 clean prep `abdc41cb656deca55eec7248b54b5ebb6c7f6868`；68-file bundle=`01e541e1…`，artifact=`ea2783dd…`。它逐字锁定 Terra medium Responses、Sol high Responses、Sonnet medium Messages、3 executions / 6 wires / 1 repair，并包含 #339 的 streaming/settlement 源码；artifact 自身为 `NOT_AUTHORIZED`、`dispatchCapable=false`、0 network/model wire、0 cost。其后真实 dispatch 已按上方 v10 stopped 终止且不得复用。
...
>
> **Copy capability v9 stopped（2026-08-07 历史记录）**：v9 的 fixed-source、compiled runtime、3/6/1 admission、有限精确 token scope 与 `/v1/models` preflight 全部通过；本机 Docker JSON log 的脱敏 operator projection（digest `9079c626…`，不具 settlement authority）记录 Terra/Responses 首 wire 在 channel #17 等待 125.927 秒后收到 OpenOx/Cloudflare 524。new-api 的预扣已全额返还，退款后调用快照为 remain=3,000,000、used=0；随后 token 被置为 status=3/remain=0，网关复核返回 401。但 runner 侧 request ID 为 null、consume log 匹配 0 行，所以逐 wire settlement 仍是 `unknown`，Terra 子账本按 `transport_failed_after_dispatch` 冻结。Sol/Sonnet、repair、candidate receipt、Git acceptance 和质量矩阵均未发生；[`v9 stopped artifact`](../evidence/site-builder/m1-g-copy-capability-pilot-v9-stopped-evidence.json) 只能诊断传输路径，不能判断模型能力或晋级。后续状态以上方 v10 stopped 为准。
>
...
> **Copy capability fixed-source gate（历史 v8）**：[manifest v8](../evidence/site-builder/m1-g-copy-real-capability-manifest-v8.json) 固定源码 `719aacc2ec328870316b71cb0666d22828b89e74` 与准备 head `5e30427a2a08e95e1f4dfa80f039537bd96f5102`，覆盖候选隔离 admission、三子账本 batch runner、信任公钥、签名 verifier、new-api exact scope、脱敏 API-call error 保真以及 v8 的 1,200 max output / 240 秒观察窗；69-file source bundle digest=`0740bb45…`，artifact digest=`1a08697d…`。manifest 本身是 `NOT_AUTHORIZED`/zero-call create-only 输入。v6/v7/v8 campaign 与 authorization 只作不可复用的历史诊断；v9 也已按上方 unknown-settlement 规则停止。任何新调用现在必须先合并上方 Chat 重基，再使用 v11 fixed-source manifest 和新的 campaign-bound 精确授权。

> **2026-08-12 Copy Evaluation native Sonnet route adoption（已 Git review）**：六个生产形态场景各执行两次；schema、Claim provenance、禁用断言、字符预算与 CTA policy 继续由生产 validator 作硬门。通过硬门的输出再按语言质量、品牌语气、CTA 质量、跨 locale 质量与重复稳定性五维评分：前四维只接受盲评的闭合 finding code，模型评审必须与候选供应商分离；稳定性由两次已验证输出确定性计算。`claude-sonnet-5 × anthropic-messages × medium` 已在现有 New API special 通道的 Sonnet-only 矩阵中取得 12/12 accepted outputs（13 条 matrix wire，含同 execution key 的一次恢复）；[quality acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-quality-git-review-acceptance-2026-08-12.json) 固定 #396，随后 [promotion acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-promotion-git-review-acceptance-2026-08-12.json) 固定 #397，最后 [route acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-route-adoption-git-review-acceptance-2026-08-12.json) 固定 #398。Terra/Sol 没有被重放或比较，MiniMax/Doubao 不进入本轮。active task route 为 evidence-bound Sonnet/no-fallback，route resolver 中 Sonnet 为 `medium`；DeepSeek Pro→GLM `low` 只保留在显式 rollback。此次 source-level adoption 不产生新的模型调用、部署或 RuntimeEvidence。
>
...
2. **Judge 固定 ModelProfile 选型**（ADR-016）：需固定模型+snapshot+温度 0，且**尽量异 provider 于被评 candidate**（§3.6 反串谋）。非运行时候选只认 ADR-021 的 `site-builder-model-candidate-baseline/2026-08-07-v3`；Gemini 文本虽仍在上游目录可见，但当前渠道已 fail-closed 禁用并只记 `deferred`。不能由目录可见或一次最小连通直接指定 Judge；多模态与跨 provider Judge 均须后续 harness 用真实任务输入、固定 snapshot 校准，并以独立 evidence/ADR 收口。
