...
>
> **Copy capability v10 stopped（2026-08-07 当前门）**：fixed-source 与 3/6/1 admission 通过后，只发出 Terra 的一条 `POST /v1/responses`。runner 保留了可在 artifact 中审计的 request ID，网关在 125.723 秒返回 524；new-api 预扣 CNY `0.040092` 后退款，但 consume log 仍为 0 行，逐 wire settlement 必须是 `unknown`。Terra 子账本冻结；Sol/Sonnet、repair、candidate receipt、Git acceptance 均为 0，token #18 已停用并以 401 复核。[`v10 stopped artifact`](../evidence/site-builder/m1-g-copy-capability-pilot-v10-stopped-evidence.json) 不保存 bearer、request/response body，只能证明 Responses transport/settlement 失败，不能评价模型能力。当前公开 OpenOx 示例只展示 `/v1/chat/completions`；下一轮必须先离线显式重基 Terra/Sol Chat adapter 并完成 fake-gateway 协议证明，不得把 Chat 当作静默 fallback，也不再重试该 Responses 路径。
...
2. **Judge 固定 ModelProfile 选型**（ADR-016）：需固定模型+snapshot+温度 0，且**尽量异 provider 于被评 candidate**（§3.6 反串谋）。非运行时候选只认 ADR-021 的 `site-builder-model-candidate-baseline/2026-08-04-v2`；Gemini 文本虽仍在上游目录可见，但当前渠道已 fail-closed 禁用并只记 `deferred`。不能由目录可见或一次最小连通直接指定 Judge；多模态与跨 provider Judge 均须后续 harness 用真实任务输入、固定 snapshot 校准，并以独立 evidence/ADR 收口。
