...
**2026-07-24 M1-f Gemini 审美 MODEL-1 记录（未晋级）**：`gemini-3.5-flash` 在 new-api `/v1/models` 的 39 型目录中可见，随后使用 `natural-origin-rich` 的 375/768/1440 三张冻结 PNG 完成真实视觉 capability probe；transport=`openai-chat-completions`，39,903ms，且 requested/reported/resolved 均精确为 `gemini-3.5-flash`。但 12 cases × 2 的矩阵在第 13 次请求达到 120,000ms 硬超时后立即停止，故候选状态为 **`unavailable`，不是 `evaluatedCandidate`**，剩余 12 次不得补称成功。原始不可变报告 [v1/report.json](../evidence/model-routing/model1-aesthetic-review-20260724-v1/report.json) SHA-256=`db17af253b9a6096c573b85f4fa845dfc823ed93dcd56c6c96f8b6103a72bb7f` 保留当时 runner 将 partial matrix 误写为 `skipped 0/24` 的 incident；不覆盖原件。只读 reconciliation [v3/report.json](../evidence/model-routing/model1-aesthetic-review-20260724-v3/report.json) SHA-256=`9a5eaf60585934a5a2838b5a3820536b3cc8e513e0bad740effdb511e7990595` 未再调用模型，逐件重验 12 个已落盘 artifact 后纠正为 `interrupted 12/24`：schema/provenance/禁止字段均 12/12，partial quality 为 4 accepted、6 个 approved false-blocker、seeded issue 4/6。未完成矩阵和明显 false-blocker 已足以否决晋级，不能计算或宣称 24 件硬门、完整 recall、6/6 paired preference 或 accepted-artifact unit cost。M1-f 运行时继续使用 deterministic QA 并显式标记 aesthetic unavailable；不创建 PR7，不启用未经同形评测的模型 fallback。
