...
> **2026-08-05 Copy capability pilot post-merge manifest（当前交付分支）**：#313 已合并；独立 [create-only manifest](../evidence/site-builder/m1-g-copy-real-capability-manifest-v1.json) 固定合并提交 `5287e5c4d95cd1eb78a61e71ba84c1dfa56b632a`、59 个 tracked source blob、source bundle `308c6400…`、v3 pilot 合同及 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium 的精确 3 executions / 最多 6 physical wires / 每次最多一次 repair。生成器要求 fixed commit 同时可由准备 HEAD 与 `origin/main` 到达，并逐文件复核工作树字节；产物固定为 `FIXED_SOURCE_CREATE_ONLY`、`dispatchAuthorization=NOT_AUTHORIZED`、`dispatchCapable=false`，没有读取凭据或连接 new-api/OpenOx，网络/模型调用和实际费用均为零。manifest 不是 compiled runtime attestation 或 operational proof；真实调用前仍须补 purpose-specific 有限凭据、可信 settlement/ledger factory 与品牌回执、post-wire freeze、repair payload binding，并取得单独 dispatch authorization。真实 evidence、晋级和 route adoption 均未发生，36-execution Copy 质量矩阵仍未解锁。
