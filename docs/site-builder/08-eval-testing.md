...
>
> **2026-08-05 Copy closed-repair operational proof（#315 已合并，zero-call）**：测试覆盖首 wire known-settled 且输出无效时恰好一次 repair、第二次仍无效不出现第三条 wire、request ID/usage 不完整时 unknown settlement 立即冻结、compiled artifact 在途漂移时先保留已知 observation 再冻结。repair payload 同时绑定 original input/context、prior output digest 和闭合 findings，但不会把 validation-failed 的 prior output 正文重新送入 prompt，并保持候选 alias/protocol/reasoning/schema/sampling 不变。Copy runner 内部以固定任务/plan/schema/prompt binding 与固定 compiled artifact path 集创建 guard，最终私有回执固定为 `OPERATIONAL_PROOF_ONLY`/`fake_gateway_contract_only`，clone 不受信，不能满足真实质量 evidence。测试只监听 `127.0.0.1`，未读取凭据或调用 new-api/OpenOx；其合并源码已由下述 create-only v2 固定。
>
...
> **2026-08-05 Copy capability pilot fixed-source manifest v2（当前 create-only 产物）**：[manifest v2](../evidence/site-builder/m1-g-copy-real-capability-manifest-v2.json) 固定 #315 merge commit `c167cde19e0d7d415303bc5353e2733480df13da`、60 个 tracked source blob、source bundle `c9ae0a64…`、v3 pilot 合同及 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium 的精确 3 executions / 最多 6 physical wires / 每次最多一次 repair。生成器要求 fixed commit 同时可由准备 HEAD 与 `origin/main` 到达，并逐文件复核工作树字节；产物固定为 `FIXED_SOURCE_CREATE_ONLY`、`dispatchAuthorization=NOT_AUTHORIZED`、`dispatchCapable=false`，没有读取凭据或连接 new-api/OpenOx，网络/模型调用和实际费用均为零。[v1](../evidence/site-builder/m1-g-copy-real-capability-manifest-v1.json) 固定旧 #313 source，仅作历史审计。v2 不是执行时 compiled runtime attestation，也不含 purpose-specific 有限凭据、durable authorization、可信 settlement/ledger factory 或真实品牌回执；真实调用仍需执行时源码复核和单独 dispatch authorization。真实 evidence、晋级和 route adoption 均未发生，36-execution Copy 质量矩阵仍未解锁。
