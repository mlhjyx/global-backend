...
>
> **代码级 harness（`design_spec` 尚无真实 evidence）**：`site-builder-model-evaluation-harness/2026-07-30-v11` 从保持不变的 `site-builder-model-candidate-baseline/2026-07-27-v1` 与当前 task binding 生成计划；精确 task/protocol/envelope、canonical suite、结果类、排序、预算、协议 admission 与 provenance 只认[生成评测基线](model-evaluation-harness.md)，不得在本文件再抄矩阵。BrandProfile 既有 suite 保持；`design_spec` v8 suite 固定 12 个六 Family sparse/rich 合成 fixture、3 个 runnable 候选、2 次重复、一次 GPT-5.5 capability probe，以及真实执行并冻结 expected/actual/PASS/digest 的 deterministic catalog baseline。每个 fixture 保存完整合成生产输入，prepare 时调用产品共用的 catalog enumeration、required-role eligibility、ranking 和 top-3 projection 逐字重建；解释只允许 `selectedCandidateId` 与三个冻结指标的封闭 claim，自由文本或自然语言指标均失败，稳定性按包含 reasons/warnings 的完整标准化输出摘要计算。suite 固定 `legacyComparatorAliases=[]`，精确 target-only 凭据按 73 executions / 146 wire calls 绑定，MiniMax/Doubao 额外 scope 在预算与 client 前拒绝；未来真实执行由 `site-builder-model-evaluation-cost-safety/2026-07-30-v2` 强制把 spend authorization 绑定到准备阶段的 fixed commit、suite、source-bundle contract 与 SHA-256，当前 case digest 漂移时在预算/client 前拒绝，同时保持有限凭据、精确 scope、冻结显式价格和 request 级结算。create-only runner 仅允许 `docs/evidence/site-builder/`；它先拒绝任何已缓存的 compiled contracts runtime，再清除并本地重建 ignored output，随后以私有 build token 将构建回执、构建后 suite 导入回执与最终 attestation 绑定；import-order 检查也只调用模块加载时捕获的 Array intrinsic，运行中原型篡改不能绕过。只有绑定到该次真实导入的 attestation 可写 evidence。运行时同时绑定 21-artifact 文件树、`design_spec` catalog 构造实际使用的全部 contracts 运行时导出代码指纹与已加载模块实例；首调和 repair 的 post-wire guard 覆盖成功、拒绝、畸形响应和 usage 超限，漂移保留已观察 settlement 并冻结后续授权。Gemini 文本继续 deferred，其余 5 个 task 仍无 suite；当前只用 fake wire/fetch/settlement，未读 `.env`、未发真实模型或媒体请求、未产生费用/evidence、未改 new-api token/channel、P4/DesignEvaluation/ReleaseManifest/Temporal/active route/env、公共 API 或 DB。
>
