...
本地入口为 `apps/api/scripts/evaluate-site-builder-blind-visual-calibration.mts`。它默认拒绝付费调用，只有显式设置 `BLIND_VISUAL_CALIBRATION_EXECUTE=1`、候选型号、新的 repository-relative report 路径、gateway image digest 与 gateway URL/key 后才运行单模型 19 次矩阵；先校验 `/models`，再通过 `RouterModelGateway.reviewVision` 调用，报告与 sibling artifacts 均使用 create-only 写入，并绑定固定 commit、source bundle、fixture catalog、实际型号/协议、逐次输入摘要/耗时/token/成本与 source start/end integrity。入口在调用前用 exclusive claim 原子占用证据目标，运行中 source bundle 漂移也会保留已完成调用并写成 `unavailable_source_integrity_changed`，不得进入模型选择。该入口在本 PR 只完成 wiring 和 `--help`/拒绝未授权执行检查，**未被实际武装或运行**。
