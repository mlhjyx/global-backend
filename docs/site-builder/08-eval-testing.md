...
**2026-07-25 双模型盲评视觉校准 harness**：独立模块从六个 approved Family 各取一张 M1-e-B `rich` 的确定性截图，并复用冻结劣化算法组成 6 对；这些源图仍只叫 `deterministic_render_baseline`，不是审美优秀样本或 aesthetic Gold。每对只给模型两张同断点图，按私有哈希固定首轮左右并以 A-B-A 顺序运行 3 次；模型请求看不到 Family、source/degraded、已知问题或其他模型答案。输出严格缩成 `{choice:left|right|tie, findings:[{ruleCode,severity,imageNumber}]}` 且 findings 最多 4 条；总分、维度分、通过结论、修复、代码与自由文本全部由 schema 拒绝。Gemini、Sonnet、Terra、Sol 的候选协议分别冻结为 `google-generate-content`、`anthropic-messages`、`openai-responses`、`openai-responses`，每模型固定为 1 次三图 probe + 18 次成对调用；120 秒只作延迟观察，实际失联回收为 900 秒，请求容量为 4096 tokens。经用户确认，首轮保留原图并以质量、稳定性和完整证据为先；中转站预算与原图成本预测只在实际调用后记录和计算，不作为首轮执行前门。
...
**2026-07-26 首轮完整四模型盲评（只支持 Gold 校准候选判断，不支持晋级）**：完整 create-only campaign 固定在 commit=`c2268b737506ffa6d06391b492d0c09d43962352`、source bundle SHA-256=`720ef1c0f141f53d97c95834ff93bb762368b5a081e5d39c629959b3ba73cd5b`；四型号均在同一 39 型目录 SHA-256=`1ec192e579dd31a22884ed0d0965bfeeee972fa336dfd58db549248c4eba9b6a` 中，执行前后源码与输入哈希稳定。campaign 内的 new-api image digest=`sha256:428018a37c0b26c163a3367c18401161707cd0e08d0f26a3dde9ff0caa05e34c` 是 launcher 记录的 caller assertion，不是 harness 自行检查 Docker 的结果；独立 [gateway runtime attestation](../evidence/model-routing/blind-visual-calibration-20260726-alias-fixed-v2/gateway-runtime-attestation.json) 脱敏记录了执行时间窗、容器 ID、实际 image ID/repo digest、零 restartCount 和 `127.0.0.1:3001→3000/tcp` 绑定，并明确它仍不是逐请求的密码学证明。原交互进程在完成 Gemini/Sonnet/Terra 后收到外部 SIGTERM(143)，其 [interruption.json](../evidence/model-routing/blind-visual-calibration-20260726-alias-fixed-v1/interruption.json) 只保留中断 provenance，不进入模型选择；随后从同一固定提交以 detached service 和全新路径完整重跑，权威汇总为 [campaign-report.json](../evidence/model-routing/blind-visual-calibration-20260726-alias-fixed-v2/campaign-report.json)。

| 候选 | 问题图选择 | 格式/型号/协议 provenance | 冻结已知问题 | Family 三次一致 | 19 次 P95 | token 计算成本 | 结论 |
|---|---:|---:|---:|---:|---:|---:|---|
| Gemini 3.5 Flash | 失败前 4/4 | 后续 1 次缺 `modelVersion` | 不完整 | 不完整 | 不完整 | 未完整结算 | `unavailable` |
| Claude Sonnet 5 | 18/18 | 18/18 | 11/18 | 3/6 | 25,861ms | $0.412406 | 单模型门失败 |
| GPT-5.6 Terra | 18/18 | 18/18 | 13/18 | 5/6 | 9,139ms | $0.217280 | 单模型门失败 |
| GPT-5.6 Sol | 18/18 | 18/18 | 17/18 | 5/6 | 32,322ms | $0.449830 | **单模型门通过** |

完整 campaign 的 `actualKnownCents=114.09215` 是保守记账金额 $1.1409215，其中包含 Gemini 未终态调用按 token 计算的 $0.007854；排除该调用后，已完成调用的 token 计算成本为 $1.1330675。由于该失败调用没有完整 provider cost settlement，`actualCostComplete=false`，两者都不能冒充精确结算账或把未知写成零；4096-token/900-second 配置没有导致任何完整候选截断或超时，三条完整候选的最高实际 output token 仅为 1329。后验原图保守上界 $6.8345 只供下一轮计划，不覆盖实际账。只有 Sol 通过基础门，`rankedPassingModels=["gpt-5.6-sol"]`、`combination=null`，所以不产生双模型结论；最高也只说明 Sol 可进入**独立 aesthetic Gold 校准**，不创建 `evaluatedCandidate`，不改 P4、DesignEvaluation v2、ReleaseManifest、Temporal 或运行时路由。

本轮也给出可执行修复路线，而非只报“不达标”：Gemini 必须由中转站稳定回传原生 `modelVersion`，或切到另一个已实测且能稳定返回实际型号的视觉协议；本地不得用 requested model 伪造 reported model。Sonnet/Terra 的 18/18 问题图选择说明视觉比较有效，主要损失集中在冻结 taxonomy：模糊图常被标为 contrast/hierarchy 而不是 imagery，整页偏色常被标为 contrast 而不是 consistency。下一校准 PR 应先由独立人工 Gold 审定这些劣化是否单义；若不单义，就收紧劣化生成器或冻结经审定的等价代码映射，并把“选对问题图”和“命中精确 ruleCode”作为两个分立指标，绝不事后改写本轮证据或直接放宽 17/18 门。完成 taxonomy 校准后，Terra 可作为高稳定、低延迟/成本的对照复测，但它与 Sol 属同一上游族，不能组成目标双模型；第二个 ensemble 成员必须由 Sonnet 或修复 provenance 后的 Gemini 通过，才重新计算共同命中。
