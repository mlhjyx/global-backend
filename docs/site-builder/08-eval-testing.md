...
>
> **Copy Sonnet recovery v13 create-only manifest/binding（2026-08-09 当前交付分支，zero-call）**：#355 已以 merge commit `a29b222a45ae5fdb4868d5235cc94aeab1574ecd` 合入 v12 暴露的 cache-only settlement 与脱敏 response-shape durable observation 修复。[`v13 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v13.json) 固定该主线、77-file source bundle `c1269c2f…` 与唯一 Sonnet Messages/medium execution（1/2/1）；[`v13 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v13.json) 再固定 post-sync 82-file runtime source `139c8661…`、53-file compiled tree `a7ac0ca8…` 与 binding artifact `caa42cbf…`。Terra/Sol v11 成功 wire及停止的 v12 Sonnet wire/authorization 均不可复用。两个 artifact 都保持 `NOT_AUTHORIZED`、`dispatchCapable=false`、0 network/model wire 与 0 cost；不含 credential、真实 client 调用或 capability evidence。合并后仍须 post-merge ancestry/compiled 重验、purpose-specific finite credential、settlement/ledger 与用户对单次恢复调用的独立授权；36/72、晋级和路由不在本阶段。
>
> **Copy Sonnet recovery v12 历史输入（2026-08-08，已由 v13 覆盖）**：[`v12 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v12.json) 与 [`v12 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v12.json) 首次固定 Sonnet-only 1/2/1；其真实 wire 已停止且 authorization 冻结，不得复用或升级成 v13 evidence。
>
> **Copy capability v11 部分成功（2026-08-08 历史执行记录；后续已合并）**：真实固定 v11 共执行 Terra Chat/medium、Sol Chat/high、Sonnet Messages/medium 各 1 wire，无 repair。Terra 与 Sol 输出都通过生产硬门、settlement known，并生成 exact create-only acceptance artifacts；Sonnet 约 4 秒返回，但当时 AI SDK normalized input=1,501（普通 77 + cache-create 1,424），request-bound resolver 未读取 new-api log metadata，因而把 consume log 判为 invalid 并冻结该子账本。总账本为 3 execution / 3 wire / 2 known / 1 unknown / 2 completed，一次性 token 已停用；见 [partial evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v11-partial-evidence.json)。#344/#345 已合并 acceptance 与 cache settlement 修复，恢复门以上方 v12 为准；不得重跑 Terra/Sol。本结果没有运行 36/72，没有 ranking、promotion 或 route adoption。
>
