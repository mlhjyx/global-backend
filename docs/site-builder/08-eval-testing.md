...
>
> **2026-08-01 source/manifest split（覆盖下方旧 harness 描述）**：当前机器合同为 `site-builder-model-evaluation-harness/2026-08-01-v18` / `design_spec` suite v14；admission、repair、最终评估共同绑定捕获的 validator，单 wire、execution 聚合费用与 probe attestation 分层核验。本 PR 仅含源码，不含 fixed-commit manifest；合并进入 `origin/main` 后须另开零费用 create-only manifest PR。当前没有真实调用、费用、evidence、promotion 或路由变更。
>
> **2026-08-03 zero-cost manifest 源合同**：待合并合同 `site-builder-design-spec-evaluation-manifest-prep/2026-08-03-v2` 绑定 `design_spec` suite/source bundle v15；合并后才由独立 create-only PR 固定新的 `origin/main` source 并生成清单，冻结 73 executions / 146 wire calls、24 个零调用 deterministic comparator case、source bundle、compiled contracts 与停止条件。2026-08-01 v1 的 `origin/main@e493ba1d09fe37feea927f70d12f17aadadc5c6a` 留为历史审计，不能作为新原生双币 execution 输入。v2 不读取 `.env`、OpenOx 价格、余额或凭据，不包含模型 client；dispatch=`NOT_AUTHORIZED`，调用和费用均为零。
> 机器输出只认 [manifest v1](../evidence/site-builder/m1-g-design-spec-evaluation-manifest-v1.json)，file/semantic/source/compiled-contract 摘要见[零费用决策卡](../evidence/site-builder/m1-g-design-spec-evaluation-manifest-prep-decision-card.md)。
>
