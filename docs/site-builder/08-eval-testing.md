...
>
> **2026-08-05 Copy closed-repair 与 dispatch preflight（zero-call）**：#315 已证明首 wire 输出无效时最多一次 repair、unknown settlement 不 repair、compiled drift 先保留 observation 再冻结；当前 v5 分支把不超过 64 KiB 的 validation-failed prior output 正文作为明确的数据字段重新送入 repair，并与 original input/context、findings、repair context/prompt 和不变的 alias/protocol/reasoning/schema/sampling 一起绑定。新增测试覆盖 live finite/exact credential、fixed-source 重算、one-shot 3/6/1 real ledger、reservation quota、request-bound consume-log settlement与私有 `copy_gateway_settlement_candidate` 回执；该回执仍不能进入质量评分，只有后续独立的 operator-authenticated authorization/evidence 门才能产生 `real_gateway_settled`。所有测试仍只使用 loopback fake 服务，不读取真实凭据或调用模型。
>
...
> **Copy capability fixed-source gate**：[manifest v2](../evidence/site-builder/m1-g-copy-real-capability-manifest-v2.json) 是固定 #315/v3 runtime 的完整历史产物；当前 v5 runtime/source 已变化，因此 v2 不再可执行。PR-A 合并后必须另建 create-only v3，固定 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium 的 3 executions / 最多 6 wires / 每次最多一次 repair。v3 生成前、有限凭据与 one-shot authorization/reservation 安装前仍为 `NOT_AUTHORIZED`，真实 evidence、晋级、route adoption 和 36/72 Copy 质量矩阵均未发生。
