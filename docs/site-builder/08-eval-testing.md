...
>
> **Copy capability v7 stopped / v8 bounded retry（2026-08-06 当前覆盖）**：固定 v7 按授权只发出 Terra/Responses 首 wire，120 秒 runner timeout 触发 unknown-settlement freeze；new-api 随后在 127.554 秒记录 OpenOx channel #17 Cloudflare 524 并退回临时预扣。Sol/Sonnet 未调用，0 candidate receipt，质量与排名均 inconclusive；见[脱敏 stopped evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v7-stopped-evidence.json)。v8 的零调用合同将结构化 Copy 的 max output 从 4,000 降至 1,200，并把有界 timeout 提升到 240 秒；候选、协议、reasoning、3 executions / 6 wires、一次 repair、子账本隔离及 unknown settlement fail-closed 均不变。v8 源码合并后仍必须另建 fixed-source manifest 和新授权；这不解锁 36/72、promotion 或生产路由。
...
> **Copy capability fixed-source gate**：[current manifest v7](../evidence/site-builder/m1-g-copy-real-capability-manifest-v7.json) 固定 #326 merge `a0dde9014a4ca22a87191e778303354a96bd7296`，覆盖候选隔离 admission、三子账本 batch runner、信任公钥、签名 verifier、new-api exact scope 和脱敏 API-call error 保真；69-file source bundle digest=`7c23b6bd…`，artifact digest=`2b66605a…`。manifest 本身仍是 `NOT_AUTHORIZED`/zero-call create-only 输入。独立授权的真实 v6 campaign 在 Terra/Responses 首 wire 遭遇 OpenOx HTTP 502，token quota/used 与 consume log 未变化但 provider settlement 仍 unknown，因此该旧账本冻结、Sol/Sonnet 未调用；[v6 stopped evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v6-stopped-evidence.json) 只作诊断，不能进入评分、晋级或 route adoption，也不能为 v7 复用 authorization。后续调用必须基于 v7 确认上游健康并获得新的 campaign-bound 授权。
