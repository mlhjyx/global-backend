...
> **Copy Sonnet recovery v16 零调用 preflight 工具门**：operator 只能创建一枚 purpose=`site_builder_copy_sonnet_recovery`、alias=`claude-sonnet-5`、protocol=`anthropic_messages`、最多 2 physical wires、最长 24 小时、有限 quota 且不允许 cross-group retry 的 token；允许的网络面只限 New API token/channel/usage/model/log 控制面与 OpenOx 公共价格目录，源码不得包含 `/v1/messages`、`/v1/chat/completions` 或 `/v1/responses` dispatch。执行前必须验证 clean exact HEAD 与 v16 runtime binding 固定 file/artifact/compiled-tree digest，创建后必须 readback route/scope/quota、OpenOx raw/selected-price digest 与 request-bound settlement resolver readiness。Git 外 bearer 只能写入当前用户私有、无 symlink component 的临时父目录，以 descriptor-anchored `O_EXCL|O_NOFOLLOW` 创建 `0600` 文件；tracked artifact 固定输出路径为 `docs/evidence/site-builder/m1-g-copy-sonnet-recovery-zero-call-preflight-v16.json`，必须拒绝任何 secret 字段并保持 `dispatchAuthorization=NOT_AUTHORIZED`、`dispatchCapable=false`、`observedModelWireCalls=0`。writer 的 create-only publication、existing/raced target 保留、失败 token disable、secret inode/mode readback与删除均须有离线测试；相关 statements/branches/functions/lines 覆盖都必须 ≥80%。该门即使 live 通过也只证明固定 credential、route、scope、quota、价格与零调用 resolver readiness，不证明真实 request-bound settlement、模型 capability、质量、晋级或生产路由；真实 dispatch 仍需新的明确 v16 授权，MiniMax/Doubao 保持 retired。
