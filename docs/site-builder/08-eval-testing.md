...
>
> **Copy Sonnet recovery v14 create-only manifest/binding（2026-08-09 当前交付分支，zero-call）**：#356/#359 已以 merge commit 进入 `origin/main@2557b991e62ff171aeec60abff33de2ad8f2859f`，其中 #359 修复 Messages HTTP 200 response validation 的有界脱敏诊断。[`v14 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v14.json) 固定该主线、77-file source bundle `3e32bb34…` 与唯一 Sonnet Messages/medium execution（1/2/1）；[`v14 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v14.json) 再固定 `3da93486…`、82-file runtime source `cbec88ad…`、53-file compiled tree `ce806d78…` 与 binding artifact `4f9fdf06…`，并覆盖 canonical binding path、版本化 manifest/artifact/execution/child-slot 身份、v14-only campaign/global authorization/child authorization/reservation namespace 和 recovery Git acceptance。Terra/Sol v11 及停止的 v12/v13 Sonnet wire/authorization 均不可复用。两个 artifact 都保持 `NOT_AUTHORIZED`、`dispatchCapable=false`、0 network/model wire 与 0 cost；不含 credential、真实 client 调用或 capability evidence。合并后仍须 post-merge ancestry/compiled 重验、purpose-specific finite credential、settlement/ledger 与用户对单次恢复调用的独立授权；36/72、晋级和路由不在本阶段。

> **Copy Sonnet recovery v13 历史输入（2026-08-09，已由 v14 覆盖）**：[`v13 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v13.json) 与 [`v13 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v13.json) 保留为不可变 create-only provenance。其后私有 operator run 发出 2 条 known-settlement wire、执行 1 次 closed repair，但 0 completion 并冻结；该诊断不保存 raw prompt/output，不是 tracked capability/质量 evidence，且 v13 authorization/campaign/wire 永不复用。
>
> **Copy Sonnet recovery v12 历史输入（2026-08-08，已由 v13/v14 覆盖）**：[`v12 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v12.json) 与 [`v12 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v12.json) 首次固定 Sonnet-only 1/2/1；其真实 wire 已停止且 authorization 冻结，不得复用或升级成后续 evidence。
>
> **Copy capability v11 部分成功（2026-08-08 历史执行记录；后续已合并）**：真实固定 v11 共执行 Terra Chat/medium、Sol Chat/high、Sonnet Messages/medium 各 1 wire，无 repair。Terra 与 Sol 输出都通过生产硬门、settlement known，并生成 exact create-only acceptance artifacts；Sonnet 约 4 秒返回，但当时 AI SDK normalized input=1,501（普通 77 + cache-create 1,424），request-bound resolver 未读取 new-api log metadata，因而把 consume log 判为 invalid 并冻结该子账本。总账本为 3 execution / 3 wire / 2 known / 1 unknown / 2 completed，一次性 token 已停用；见 [partial evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v11-partial-evidence.json)。#344/#345 已合并 acceptance 与 cache settlement 修复，当前恢复门以上方 v14 为准；不得重跑 Terra/Sol。本结果没有运行 36/72，没有 ranking、promotion 或 route adoption。
>
...
> **2026-08-04 Copy Evaluation v2 评分器与候选（当前计划 v7）**：六个生产形态场景各执行两次；schema、Claim provenance、禁用断言、字符预算与 CTA policy 继续由生产 validator 作硬门。通过硬门的输出再按语言质量、品牌语气、CTA 质量、跨 locale 质量与重复稳定性五维评分：前四维只接受盲评的闭合 finding code，模型评审必须与候选供应商分离；稳定性由两次已验证输出确定性计算。候选、协议、reasoning、无缓存、known settlement 与 output digest 只接受 Model Runtime 完成时生成的品牌元数据，repeat 0/1 的 execution/output digest 必须与评分逐项一致，嵌套评分深冻结。`copy.premium` 候选为 Terra/Sol/Sonnet，candidate admission 为 `READY`；当前 pilot 精确固定 Terra=Chat/medium、Sol=Chat/high、Sonnet=Messages/medium，三者均须先过 `capability_probe`，且不允许回退到 Responses。dispatch 仍是 `NOT_AUTHORIZED`，真实执行、evidence、晋级和路由切换均未发生。
>
