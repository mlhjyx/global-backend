...
> **2026-08-05 Copy capability pilot 耐久 fake-gateway 执行链（当前交付分支）**：v2 pilot 从当前候选表派生 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium，机械上限仍为 3 executions、每个最多一次后续 repair、全局最多 6 physical wires，且只属于 `CAPABILITY_ONLY_NOT_QUALITY_EVIDENCE`。loopback-only、固定非凭据 fixture key 的 runner 使用真实 AI SDK native adapter 对 fake gateway 验证 `/v1/responses` 与 `/v1/messages` 请求形状，并以 `0600` append-only hash-chain ledger 在每个 dispatch 前 claim execution/wire，随后绑定 plan/request/output digest、requested/resolved/reported model、协议、usage 与 request ID；duplicate、tamper、symlink、并发锁、超限、unknown observation 和 validator rejection 均 fail-closed。只有 ledger 完成结果获得 durable attestation，普通 Runtime brand 被评分器拒绝；fake evidence 即使分数通过也带 `TEST_ONLY_EXECUTION_EVIDENCE`，不能晋级。计划状态为 `FAKE_GATEWAY_DURABLE_RUNNER_READY_REAL_DISPATCH_BLOCKED`：runner 拒绝真实凭据，fake usage observation 不是金额结算，真实 fixed-commit manifest、有限凭据 attestation、authorization reservation、gateway settlement observer、post-wire freeze 与 repair payload binding 仍未落。无真实 dispatcher/凭据/费用卡/evidence writer，dispatch=`NOT_AUTHORIZED`，不能解锁 36-execution Copy 质量矩阵。
