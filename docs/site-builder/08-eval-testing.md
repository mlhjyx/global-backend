...
>
> **代码级 harness（`design_spec` 尚无真实 evidence）**：`site-builder-model-evaluation-harness/2026-07-30-v5` 从保持不变的 `site-builder-model-candidate-baseline/2026-07-27-v1` 与当前 task binding 生成计划；精确 task/protocol/envelope、canonical suite、结果类、排序、预算、协议 admission 与 provenance 只认[生成评测基线](model-evaluation-harness.md)，不得在本文件再抄矩阵。BrandProfile 既有 suite 保持；`design_spec` v2 suite 固定 12 个六 Family sparse/rich 合成 fixture、3 个 runnable 候选、2 次重复、一次 GPT-5.5 capability probe，以及真实执行并冻结 expected/actual/PASS/digest 的 deterministic catalog baseline。fixture hash 为独立常量，catalog 数据链和实际 `design-catalog-v2` 合同实现均进入 source bundle；评估器拒绝任何未属于已选 candidate 的 catalog-shaped 标识符，并校验每一次重复指标声明。MiniMax/Doubao legacy comparator 在预算与 client 前拒绝，Gemini 文本继续 deferred，其余 5 个 task 仍无 suite。evaluation-only executor 用注入的 fake wire/cost resolver 覆盖 Responses 与 Messages，实际协议、精确模型 identity、repair usage/callCount、成本依据及调用后 source fingerprint 均 fail-closed。`site-builder-model-evaluation-cost-safety/2026-07-28-v1` 仍强制未来真实执行使用专用有限额度凭据、精确 alias+protocol scope、冻结 OpenOx 显式价格、request 级 `executionId` 结算与 campaign/execution/wire/prompt/output 上限。当前只用 fake wire/fetch/settlement，未读 `.env`、未发真实模型或媒体请求、未产生费用/evidence、未改 new-api token/channel、P4/DesignEvaluation/ReleaseManifest/Temporal/active route/env、公共 API 或 DB。
>
