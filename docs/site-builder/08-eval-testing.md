...
>
> **Copy Sonnet recovery v12 create-only manifest（2026-08-08 当前交付分支，zero-call）**：#344/#345 已分别合入 v11 evidence/acceptance 与 Anthropic cache settlement 修复。新的 [`v12 artifact`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v12.json) 固定修复后的 `origin/main@2db159ed46f7f7c2acbcf2bb53a8c06d573c8382` 与 source bundle `3397a29a…`，只列 Sonnet Messages/medium 的 1 execution / 最多 2 wires / 最多 1 repair，并显式禁止 Terra/Sol 重放。artifact=`c2467ef6…`，保持 `NOT_AUTHORIZED`、`dispatchCapable=false`、0 network/model wire 与 0 cost；本阶段不生成 compiled runtime expectation，不含 credential、client 或 runner。后续必须先合并 manifest，再用 fixed source 补独立 recovery admission/runner 与 compiled binding，之后才可单独授权 Sonnet 恢复调用；本 artifact 不支持 capability 完成、36/72、晋级或路由。
>
