...
**2026-07-25 双模型盲评视觉校准（已实跑）**：独立模块从六个 approved Family 各取一张 M1-e-B `rich` 的确定性截图，并复用冻结劣化算法组成 6 对；这些源图仍只叫 `deterministic_render_baseline`，不是审美优秀样本或 aesthetic Gold。每对只给模型两张同断点图，按私有哈希固定首轮左右并以 A-B-A 顺序运行 3 次；模型请求看不到 Family、source/degraded、已知问题或其他模型答案。输出严格缩成 `{choice:left|right|tie, findings:[{ruleCode,severity,imageNumber}]}` 且 findings 最多 4 条；总分、维度分、通过结论、修复、代码与自由文本全部由 schema 拒绝。Gemini、Sonnet、Terra、Sol 的候选协议分别冻结为 `google-generate-content`、`anthropic-messages`、`openai-responses`、`openai-responses`，每模型执行 1 次三图 probe + 18 次成对调用。经用户确认，首轮保留原图并以质量、稳定性和完整证据为先；中转站预算与原图成本预测只在实际调用后记录和计算，不作为首轮执行前门。
...
**2026-07-25 首轮真实结果（不晋级）**：[固定提交 `e6320d6` 的 create-only campaign report](../evidence/model-routing/blind-visual-calibration-20260725-final-pair-matrix-v1/campaign-report.json) 记录 source bundle 起止哈希相同、四个候选顺序隔离与逐次 artifact。Gemini probe 的 transport 正确但实际 `reported/resolved model=gemini-default`，不等于冻结 `gemini-3.5-flash`，因此 provenance fail-closed、没有进入成对矩阵。Sonnet、Terra、Sol 均在完整 18 对上达到 18/18 closed format 和 provenance；已知问题命中依次为 12、13、13，Family 三次一致依次为 4、5、5，均不达到单模型 ≥17/18 命中门。于是没有通过者、没有跨 upstream family 的双模型共同命中计算，结论固定为 `no_promotion_or_runtime_route_change`。120 秒的延迟观察与 4096 请求容量没有淘汰任何完整结果；Gemini 的不可用仅是型号 provenance，三个完整矩阵的失败仅是质量门。按观测 token 与冻结价表归集的已知计算成本为 $1.078987；网关账单未完整返回，Gemini probe 明确未知。未经缩图的后验下一轮预测为 $6.8345。这些金额只用于后续预算判断，不重写本次质量结果。当前截图仍不是 aesthetic Gold，P4、DesignEvaluation v2、ReleaseManifest、Temporal 与运行时模型路由均未改动。
