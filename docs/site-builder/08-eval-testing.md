...
> **2026-08-05 Copy capability pilot 零调用合同（当前交付分支）**：pilot 计划从当前 Copy 候选表派生 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium，机械上限仍为 3 executions、每个最多一次 closed repair、全局最多 6 physical wires，且只属于 `CAPABILITY_ONLY_NOT_QUALITY_EVIDENCE`。安全复审后没有接入可真实 dispatch 的 native transport：现有 adapter 仍缺 durable authorization/reserve、unique wire claim、所有 post-wire 异常结算、可信 Runtime brand 和全局 3/6 cap，且当前注入式 fake transport 仍可形成仅供单元测试的 Runtime brand，因此计划明确为 `BLOCKED_ON_DURABLE_ADMISSION_AND_BRANDED_RUNNER`，该 brand 不能作为真实 execution/evidence。Runtime 对 validator 失败输出直接冻结，避免使用未绑定原 prompt 发起第二个付费 wire。无真实 dispatcher、凭据、manifest、费用卡或 evidence writer，dispatch=`NOT_AUTHORIZED`，不能解锁 36-execution Copy 质量矩阵。
