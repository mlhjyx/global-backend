...
> **Copy capability candidate isolation（2026-08-06 当前交付分支，zero-call）**：pilot 的总 envelope 仍为 Terra/Sol/Sonnet 各一次、最多 6 wires，但执行账本不再共享冻结域。每个候选固定为独立的 1 execution / 2 wires / 1 repair 子 campaign，并绑定独立 authorization、reservation、ledger identity 与 execution key；候选 unknown settlement 只能冻结自身。batch guard 在每次 dispatch 前复核三方共享 manifest、fixed source、compiled runtime、exact-scope credential 与全局授权，任一共享漂移会持久冻结全部子 campaign。离线 fake-gateway 测试覆盖“Terra unknown 后 Sol repair + Sonnet success”和“共享 quota 漂移后三账本全冻结”。旧 v6 manifest 不包含该执行语义，故仅是 immutable history；新 runner 真实使用前必须以合并提交另行生成 v7 create-only manifest，并重新获得 dispatch 授权。
>
...
> **Copy capability fixed-source gate**：[current manifest v6](../evidence/site-builder/m1-g-copy-real-capability-manifest-v6.json) 固定 #323 merge `55f10fa325916be6dde488ab148d1462175cda12`，覆盖信任公钥、签名 verifier、ledger event/storage、runner、new-api exact scope 和脱敏 API-call error 保真；69-file source bundle digest=`6e74f4f5…`，artifact digest=`c9a4391e…`。manifest 本身仍是 `NOT_AUTHORIZED`/zero-call create-only 输入。独立授权的真实 v6 campaign 在 Terra/Responses 首 wire 遭遇 OpenOx HTTP 502，token quota/used 与 consume log 未变化但 provider settlement 仍 unknown，因此账本冻结、Sol/Sonnet 未调用；[v6 stopped evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v6-stopped-evidence.json) 只作诊断，不能进入评分、晋级或 route adoption。后续调用必须确认上游健康并获得新的 campaign-bound 授权。
