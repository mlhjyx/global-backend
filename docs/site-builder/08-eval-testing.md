...
> **2026-08-12 Copy Evaluation native Sonnet quality（当前单候选收口）**：六个生产形态场景各执行两次；schema、Claim provenance、禁用断言、字符预算与 CTA policy 继续由生产 validator 作硬门。通过硬门的输出再按语言质量、品牌语气、CTA 质量、跨 locale 质量与重复稳定性五维评分：前四维只接受盲评的闭合 finding code，模型评审必须与候选供应商分离；稳定性由两次已验证输出确定性计算。`claude-sonnet-5 × anthropic-messages` 已在现有 New API special 通道的 Sonnet-only 矩阵中取得 12/12 accepted outputs（13 条 matrix wire，含同 execution key 的一次恢复）；[quality artifact](../evidence/site-builder/m1-g-copy-sonnet-native-quality-2026-08-12.json) 已由 [Git-review acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-quality-git-review-acceptance-2026-08-12.json) 固定 #396 merge。Terra/Sol 没有被重放或比较，MiniMax/Doubao 不进入本轮。当前 promotion approval 与 production route adoption 仍拆分：前者只登记 evidence-bound Sonnet route，后者才可改变 active task route；两者不产生新的模型调用。
>
