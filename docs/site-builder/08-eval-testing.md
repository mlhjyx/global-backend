...
> **2026-08-05 Copy capability pilot 真实准入源码（当前交付分支）**：v3 pilot 继续精确派生 Terra=Responses/medium、Sol=Responses/high、Sonnet=Messages/medium，机械上限仍为 3 executions / 6 physical wires / 每次最多一次 repair。#312 的 loopback runner、原生协议形状和 durable fake ledger 继续只证明测试生命周期；本分支新增不含 client 的纯 admission envelope validator，要求未来 post-merge create-only manifest 固定 source commit、tracked source bundle 与 compiled contracts，并把 purpose-specific 有限凭据的不可逆 digest、精确 alias+protocol+reasoning scope、request-bound bounded settlement observer、单独 authorization/reservation、ledger identity 和全局 3/6 cap 逐摘要绑定。manifest 的 `dispatchAuthorization` 永远是 `NOT_AUTHORIZED`；真实授权只能来自独立短期回执。任何 commit/bundle/scope/quota/expiry/resolver/cap/digest 漂移都拒绝，验证结果也固定为 `SOURCE_CONTRACT_VALIDATION_ONLY`、`dispatchCapable=false`，不能冒充由真实系统产生的可信回执。当前状态为 `REAL_ADMISSION_SOURCE_READY_MANIFEST_REQUIRED`：源码未合并前不能创建其权威 fixed-commit manifest；本分支不读取凭据、不连 new-api/OpenOx、不创建 real adapter/ledger、不调用模型，调用/费用/evidence/晋级/路由变化均为零。Git/credential/settlement/ledger trusted factory 与品牌 receipt、post-wire freeze、repair payload binding 仍是后续源码门，36-execution Copy 质量矩阵未解锁。
