...
> **Copy capability fixed-source gate**：[current manifest v5](../evidence/site-builder/m1-g-copy-real-capability-manifest-v5.json) 已固定 #321 merge `ecdd45b7947b1fec061286d4a68199ab7ad6a49c`，显式覆盖信任公钥、签名 verifier、ledger event/storage、runner，以及与真实 new-api boolean allowlist/token-scoped catalog 一致的 live preflight；69-file source bundle digest=`6bb9626e…`，artifact digest=`b93a0b0b…`。它仍是 `NOT_AUTHORIZED`/zero-call；[v4](../evidence/site-builder/m1-g-copy-real-capability-manifest-v4.json) 及更早版本仅供历史审计。有限凭据、dispatch authorization 与逐 candidate operator authorization 安装前，真实 evidence、晋级、route adoption 和 36/72 Copy 质量矩阵均未发生。
