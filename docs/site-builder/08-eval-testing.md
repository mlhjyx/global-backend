...
> **2026-08-12 Copy Evaluation native Sonnet route adoption（当前单候选收口）**：六个生产形态场景各执行两次；schema、Claim provenance、禁用断言、字符预算与 CTA policy 继续由生产 validator 作硬门。通过硬门的输出再按语言质量、品牌语气、CTA 质量、跨 locale 质量与重复稳定性五维评分：前四维只接受盲评的闭合 finding code，模型评审必须与候选供应商分离；稳定性由两次已验证输出确定性计算。`claude-sonnet-5 × anthropic-messages × medium` 已在现有 New API special 通道的 Sonnet-only 矩阵中取得 12/12 accepted outputs（13 条 matrix wire，含同 execution key 的一次恢复）；[quality acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-quality-git-review-acceptance-2026-08-12.json) 固定 #396，随后 [promotion acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-promotion-git-review-acceptance-2026-08-12.json) 固定 #397。Terra/Sol 没有被重放或比较，MiniMax/Doubao 不进入本轮。当前 PR 只改变 active task route 为 evidence-bound Sonnet/no-fallback，并在 route resolver 中将 Sonnet 维持为 `medium`；DeepSeek Pro→GLM `low` 只保留在显式 rollback。此次 source-level adoption 不产生新的模型调用、部署或 RuntimeEvidence。
>
