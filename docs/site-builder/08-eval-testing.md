...
>
> **2026-08-01 source/manifest split（覆盖下方旧 harness 描述）**：当前机器合同为 `site-builder-model-evaluation-harness/2026-08-01-v17` / `design_spec` suite v14；admission、repair、最终评估共同绑定捕获的 validator，单 wire 与 execution 聚合费用分层核验。本 PR 仅含源码，不含 fixed-commit manifest；合并进入 `origin/main` 后须另开零费用 create-only manifest PR。当前没有真实调用、费用、evidence、promotion 或路由变更。
>
