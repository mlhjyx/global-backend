...
>
> **Copy capability v7 stopped / v8 fixed-source manifest（2026-08-07 当前覆盖）**：固定 v7 按授权只发出 Terra/Responses 首 wire，120 秒 runner timeout 触发 unknown-settlement freeze；new-api 随后在 127.554 秒记录 OpenOx channel #17 Cloudflare 524 并退回临时预扣。Sol/Sonnet 未调用，0 candidate receipt，质量与排名均 inconclusive；见[脱敏 stopped evidence](../evidence/site-builder/m1-g-copy-capability-pilot-v7-stopped-evidence.json)。v8 的零调用合同将结构化 Copy 的 max output 从 4,000 降至 1,200，并把有界 timeout 提升到 240 秒；候选、协议、reasoning、3 executions / 6 wires、一次 repair、子账本隔离及 unknown settlement fail-closed 均不变。[`v8 fixed-source create-only manifest`](../evidence/site-builder/m1-g-copy-real-capability-manifest-v8.json) 已生成并记录 `NOT_AUTHORIZED`、0 wire、0 cost；仍须新的精确授权，且这不解锁 36/72、promotion 或生产路由。

> **Copy versioned evidence acceptance（2026-08-07 当前交付分支）**：#332 已将 v8 manifest 合入 `origin/main@0f6d31a0cb62dd061a22f0c82e40414e831c486d`，在该源码上的 post-merge 零调用 preflight 已通过；它只验证可重建固定输入和 admission，未调用模型。v8 后续仍要求当前执行环境未提供、仓库/常规安全引用中也未发现的对应 Ed25519 私钥或 signer，所以成功的 known-settlement execution 也只能停在 `copy_gateway_settlement_candidate`。当前迁移把 pilot 升为 `site-builder-copy-capability-pilot/2026-08-07-v9`，Runtime candidate 新增完整 `knownSettlementDigest`、脱敏 ordered settlement chain、campaign/ledger authorization 和 execution-time evidence binding；compiled runtime/binding、plan/input/context/prompt、fixture/child slot、model/protocol/reasoning、admission/global/child authorization digest 均同时写入原 `campaign_opened` hash-chain 事实。通用 `git-reviewed-evidence-acceptance/2026-08-07-v1` create-only artifact 绑定 candidate、execution/output、candidate ledger、fixed source、source bundle、manifest、compiled runtime/binding、settlement observer、alias/protocol/reasoning；Copy exact schema 在 artifact 创建和消费时拒绝未知字段与 bearer/API key/raw request ID/prompt/output。机器 verifier 只证明 artifact 单次新增、无后续修改、working/HEAD/`origin/main` 字节一致及 main first-parent PR merge provenance；必需 CI 与独立 reviewer provenance 必须从 main-bound PR 另行核验。真实 runner 继续以 live admission/source/compiled/gateway dispatch；独立 evidence-only runner 则在新进程只从已合并 artifact 与原 ledger/claim/marker 重开，没有 `execute()`、token、network、current plan 或 credential-expiry 门，并一次性写入 `git_evidence_acceptance_consumed` 后生成 `git_reviewed_gateway_settlement_accepted`。Copy Evaluation plan=`site-builder-copy-evaluation-plan/2026-08-07-v5`，其 Git-reviewed quality evidence 仍只是目标合同；当前 scorer 对所有结果无条件加入 `DURABLE_ACCEPTED_ARTIFACT_REPLAY_REQUIRED`，不会接受旧 `real_gateway_settled`、raw candidate、fake/test 或当前进程内结果作为晋级证据。因此 **quality matrix 仍不可执行**：评分入口还要求原执行进程 WeakMap 中的 branded `ModelExecutionResult`，post-merge acceptance 无法在重启后恢复该对象，也没有持久输出正文 artifact 供逐字节重算 output digest 与一次性消费。必须先增加 restart-safe durable accepted execution/output-byte replay binding，再谈 evidenceKind=`quality_matrix` 的 36/72 授权。v9 manifest fixed source 故意留空到本迁移合并后填写，execution status=`REAL_RUNTIME_READY_POST_MERGE_V9_MANIFEST_REQUIRED`；v8 只保留不可变历史。随后仍须独立 v9 create-only manifest 与精确授权；当前没有模型调用、36/72、promotion 或 route adoption。

> **Copy v9 fixed-source manifest（2026-08-07 当前覆盖上方待生成描述）**：[`v9 create-only artifact`](../evidence/site-builder/m1-g-copy-real-capability-manifest-v9.json) 固定 #333 merge `5775945c6e056e99ba9357d8d8794b01fad0c66b` 与 clean prep head `118d657d9ed3f43228a9ebf7d960e7c7fb884b17`，source bundle 为 68 files / `9dc4fed3…`，artifact digest=`f4eaa8e…`。机器合同继续逐字锁定 Terra/Sol/Sonnet、3 executions / 6 wires / 1 repair、zero-call 和 `NOT_AUTHORIZED`，并把 Git-reviewed acceptance source 纳入 bundle、排除已退役 operator-key source。artifact 未读取 credential 或 new-api；3,000,000 quota points 仍须未来 live finite/exact attestation 与独立 authorization 验证，不能从本 artifact 推导已授权或余额可用。v8 只保留 immutable history；36/72、promotion 与 route adoption 仍未解锁。

> **Copy quality restart-safe replay（2026-08-07 当前交付分支，zero-call）**：`site-builder-copy-quality-accepted-output-replay/2026-08-07-v1` 先以 ledger 初始 hash-chain 中的 `real-model-shared-campaign-binding/2026-08-07-v1` 精确冻结 purpose/topology、36/72/1、fixed source、manifest/admission、credential/settlement 与 compiled runtime，再将每个通过生产硬门的输出保存为 candidate receipt 内不超过 64 KiB 的 canonical UTF-8 JSON 精确字节，同时冻结 output digest/length、candidate/fixture/repeat、plan/input/context/prompt 与 known-settlement provenance。`evidenceKind=quality_matrix` 的 create-only artifact 只有在 main-bound Git merge provenance 通过后才可重开原 36/72 共享账本；reopen 使用历史 `execution_completed` snapshot 逐字节重算、与原授权的重叠 provenance 逐项比对，并一次性消费 acceptance/execution/artifact 身份，ledger 只保存 digest/length 而不保存正文。Copy Evaluation v6 的 12 review + 6 stability 聚合只有全量来自 replay-backed receipt 才能移除 `DURABLE_ACCEPTED_ARTIFACT_REPLAY_REQUIRED`；任一 process-local/fake/mixed outcome 都会恢复 blocker，`routeAdoptionAuthorized=false` 不变。历史 quality manifest v1 不包含该语义，只供审计；本分支合并后须生成 fixed-source v2。该实现没有调用模型、访问 new-api 或凭据，也没有 capability evidence、36/72 授权/执行、promotion 或 route adoption；固定 v7 已冻结且不能复用，v9 仍须新的精确 dispatch/token 授权。

> **Copy quality matrix v2 fixed-source manifest（2026-08-07 当前覆盖上方待生成描述，zero-call）**：[`v2 artifact`](../evidence/site-builder/m1-g-copy-quality-matrix-manifest-v2.json) 固定 #335 merge `ad6acad17700e08af9dff96477f5bb24a5a8553b`、71-file source bundle `b02861e6…`、36/72/1、shared campaign ledger、`quality_matrix` Git acceptance 与 canonical output bytes one-shot replay。它保持 `NOT_AUTHORIZED`、`dispatchCapable=false`、0 network/model wire，并显式要求成功 v9 capability evidence、独立 matrix credential/authorization/ledger、逐 wire known settlement 和盲审；因此不是执行回执或晋级证据。
>
...
> **Copy capability fixed-source gate**：[current manifest v8](../evidence/site-builder/m1-g-copy-real-capability-manifest-v8.json) 固定源码 `719aacc2ec328870316b71cb0666d22828b89e74` 与准备 head `5e30427a2a08e95e1f4dfa80f039537bd96f5102`，覆盖候选隔离 admission、三子账本 batch runner、信任公钥、签名 verifier、new-api exact scope、脱敏 API-call error 保真以及 v8 的 1,200 max output / 240 秒观察窗；69-file source bundle digest=`0740bb45…`，artifact digest=`1a08697d…`。manifest 本身仍是 `NOT_AUTHORIZED`/zero-call create-only 输入。先前 v6/v7 campaign 的 OpenOx 失败和 unknown-settlement 冻结证据只作诊断，不能进入评分、晋级或 route adoption，也不能为 v8 复用 authorization。后续调用必须基于 v8 确认上游健康并获得新的 campaign-bound 精确授权。
