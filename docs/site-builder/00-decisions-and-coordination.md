...
| **🔴 R0-3 虚构身份红线** | demo-spec/文案**只用 intake 事实**；对未知企业类型**禁止**默认写 manufacturer/engineering team/QC/export packaging/认证/产能/年限/客户。缺=**中性措辞**（supplier/supply），绝不虚构 | 01 事实红线 + **[ADR-017](../adr/registry.md)**；✅ **CC 已落地**（2026-07-16，本轮）——#113（测试提交流程）已作废关闭+删分支，改由 CC 做。**两条路径全堵**：① 确定性模板去制造业断言（manufacturer→supplier、去 engineering team/Quality Control/Export packaging）；② `sanitizePolish` 扩红线词表、剔除 LLM 润色回灌的虚构角色；③ `activities.ts` 提示词去"manufacturer landing page"+ rules 明禁角色造假；④ `demo-spec.spec.ts` **强制守卫**（非制造业 intake 输出不得含上述任一词，独立声明词表=永久 CI 门）。🔴 **tugjvnh 后续 demo-spec PR 必须保持该守卫绿**——重新写入虚构身份即 CI 红 |
...
| mlhjyx/CC | `fix/site-builder-r0-3-no-fabricated-identity` | ✅ R0-3 去虚构身份（模板中性 + sanitizer + 提示词 + 强制守卫，2026-07-16 本轮 PR） | demo-spec.ts / activities.ts |
| tugjvnh | （待新开 PR） | #113 已作废（仅测试提交流程，关闭+删分支）；B2B 工业模板 + demo-spec 另开新 PR——**须保持 R0-3 守卫绿**（ADR-017，重新写入虚构身份即 CI 红） | demo-spec.ts / Section.astro / themes.ts |
