...
| **模型路由与候选**        | 7 task 的当前事实只认 `task-routes.ts`：仅 `brand_profile` 已是 `gpt-5.6-terra`（Responses）→`claude-sonnet-5`（Messages）的 `promotedRoute`；其余 task 仍为各自 `currentRoute`。非运行时候选唯一看 `site-builder-model-candidate-baseline/2026-08-07-v3` 的[生成文档](model-candidate-baseline.md)，须逐 task 评测与可回滚门后才能切换 | ADR-016/020/021；task-routes.ts；machine baseline                                                                                                                  |
