...
> 文档 ID：`SITE-DEC-001`
> 生命周期：`CURRENT`
> 当前事实来源：当前代码、[状态](../status/current.md)、[路线](../roadmap/release-plan.md) 与 ADR-013~021。
...
| **模型路由与候选**        | 7 task 的当前事实只认 `task-routes.ts`：`brand_profile` 已是 `gpt-5.6-terra`（Responses）→`claude-sonnet-5`（Messages）的 `promotedRoute`；Copy 已在 #398 完成晋级后的 source route adoption（`claude-sonnet-5` / Messages / `medium` / 无 fallback，`promotedRoute`），不代表部署或当前运行证据。非运行时候选唯一看 `site-builder-model-candidate-baseline/2026-08-07-v3` 的[生成文档](model-candidate-baseline.md)，须逐 task 评测与可回滚门后才能切换 | ADR-016/020/021；task-routes.ts；machine baseline                                                                                                                  |
...
**开发与合并权（2026-09-23 更正）**：**Claude Code 是当前开发主体**（2026-09-21 起接替 Codex），负责规划、实现、验证、提 PR、处理审查意见和状态回写；Codex 只在 Claude Code 指派下执行边界明确的任务。旧会话及分支/worktree 只作待审计 provenance，独有提交核验前不得删除。合并遵循 `DEC-AIDEV-004`：开发代理独立审查通过、必需检查全绿并满足该决定其余门后可 squash 合并；不包括 ruleset/仓库设置/权限变更、部署发布与付费调用。碰热点文件 / schema / 合规红线（如 R0-3 去虚构身份）的 PR，**红线未清不予合并**。

## 5. 谁在做什么（滚动状态只查 Git，不固化临时分支）

R1-min（#157）、DI-0（#164）、M1-e-A/B 与 M1-f 已进入 `main`。55 型均为 `m1_e_a_qualified`；Catalog=`m1-e-b/1.0.0` 含六个 approved Family，DesignBrief、受控 adapter/copy-slot/四层 validator、tenant/catalog asset overlay、SiteSpec 1.1、ReleaseManifest v2/v3 与 Temporal 新历史均已接线。P4 `quality_loop` 已 replay-safe 接通，审美模型仍不可用且未晋级；下一阶段为 M1-g 收口。`template-distillation` 未被采纳，不能反向改写批准合同或作为能力完成证据。历史 [R1-min 执行交接](handoffs/r1-min-execution-brief.md) 只保留当时任务边界与 provenance，不再表示当前施工状态。
