...
> ⛔ **历史研究档案，已被 [ADR-019](../adr/registry.md) 取代（2026-07-16）**。本文保留当时调研证据用于解释决策演进，**不再是操作手册或施工依据**。现行唯一允许路线是：Readdy 等 Tier B 来源仅作少量、临时、开发期的**净室视觉参考**，只沉淀跨至少 5 个独立来源/平台原创实验的抽象 `DesignObservation/DesignRule`；运行时零依赖。
>
