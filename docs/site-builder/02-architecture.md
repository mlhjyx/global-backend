...
**2026-07-27 本机网关事实**：历史通用应用令牌 `/v1/models` 当前返回 37 个活跃型号；BGE 不暴露公共名，而只通过专用令牌暴露私有别名 `site-builder-bge-m3-local`。OpenOx 已另建文本 8 型、图片 4 型、视频 5 型的三把模型白名单令牌；图片目录精确包含 `gpt-image-2`、`gpt-image-2-4k`、`gemini-3-pro-image-preview`、`gemini-3.1-flash-image-preview`。这些 `-preview` 型号只能 shadow，不能冒充本节 target portfolio 中尚未由目录提供的稳定名 `gemini-3-pro-image` / `gemini-3.1-flash-image`；`gemini-omni-flash-preview` 仍不可见。分域令牌尚未接入应用消费者，历史 `MODEL_GATEWAY_KEY` 仍为宽权限，故当前不构成端到端凭据隔离。BGE 专用令牌的 `/v1/models` 只返回该一个别名，真实 `/v1/embeddings` 两条输入返回两组 1024 维有限向量。**型号可见/单次连通不等于 task promotion**；本轮也没有图片或视频生成，图片编辑、视觉输入、Structured Output、长任务和 provider 失败语义仍须分别真探。`MODEL_DEFAULT_MODEL=deepseek-v4-flash` 只服务未显式传 model 的 legacy/general 调用，不是 Site Builder 主模型，也不覆盖本节 per-task 路由；在未显式化并评测那些旧调用前不得借 ADR-020 顺手修改它。
