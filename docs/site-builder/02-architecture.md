...
> 开发期设计智能工厂的详设归 [13 号领域模型](13-design-domain-model.md)；[14 号](14-media-foundation-mf0.md)只承载媒体地基。本 02 只承载**生产期平面**的 as-built 架构，两平面通过版本化 `DesignCatalog` 单向对接。v3.2 吸收 v2 合同并提出四层（设计合法复用抽象成内部资产 / 整站视觉语法+页面节奏+家族一致性 / Demo v0 10 秒也像有效海外站 / CC 从 M1-c 起哪些立即改哪些延后）——施工状态以 09、13、14 号为准。
...
- **readdy.ai 结论（ADR-019 现行边界）**：它支持导出代码/Figma（付费档）并有页面级 REST API，但**没有可供第三方产品调用的"素材库"开放接口**，其模板/素材授权也不随导出转移到我们客户的商用站点——直接联动其素材库不可行。默认仅作 `visual_reference_only`：开发期可少量、临时观察并抽象跨来源设计规律，**不导出/复用源码、文案或素材**；只有取得覆盖 AI 建站产品、衍生组件和商业分发的书面授权并登记为 `owned_export_authorized`，才可在授权边界内走一次性导出改造工序。生产素材走下方开放授权生态。
...
| D9 | readdy 定位 | **已由 ADR-019 取代（2026-07-16）**：默认 `visual_reference_only`，仅允许开发期少量、临时、多来源净室观察，持久化抽象 `DesignObservation/DesignRule`；**未经覆盖 AI 建站产品、衍生组件和商业分发的书面授权，不得导出或改造 React/Figma**。书面授权登记为 `owned_export_authorized` 后，才可按授权边界走一次性改造工序；运行时零依赖、禁止逆向，生产素材仍走开放授权生态（§8）。 |
