...
**BGE new-api 配置合同（PR #140 目标；合并前不是 main as-built）**：幂等引导命令 `pnpm --filter @global/api new-api:ensure-embeddings -- --write-env` 创建唯一 OpenAI-compatible channel：base URL=`http://embeddings:11434`（Compose 内网），对应用只声明 `site-builder-bge-m3-local`，并以 model mapping 映射到 Ollama `bge-m3`；同时创建只允许该别名、禁跨 group retry 的专用令牌并真验 1024 维。应用模型别名固定、`EMBEDDINGS_API_KEY` 必填，禁止复用 `MODEL_GATEWAY_KEY` 或用 env 改投公共模型。当前网关 channel/专用令牌已经就绪，但 **current main 的 EmbeddingsClient 仍直连 `:11434` 且不发 Bearer**；只有 #140 合并并执行 env 切换后，应用经 new-api 才成为 as-built。回滚先把应用 `EMBEDDINGS_URL` 指回受控本机上游并恢复上游模型名，维持同一模型/维度/version，不改数据库向量；定位完成后重新运行引导命令恢复统一网关。
