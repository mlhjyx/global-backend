...
- Copy Sonnet recovery v14 当前 create-only 交付：[manifest](site-builder/m1-g-copy-sonnet-recovery-manifest-v14.json)与 [runtime binding](site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v14.json)；两者固定 #359 后的全新 execution/child slot、response compatibility 与 Git acceptance runtime，仍为 `NOT_AUTHORIZED`、0 wire/0 cost，不构成 capability、质量、晋级或生产路由证据。
