...
- Copy Sonnet 2026-08-12 native capability：[evidence](site-builder/m1-g-copy-sonnet-native-capability-2026-08-12.json)。用户授权的 Sonnet Messages 单个 factual fixture 用 2 条 wire（首调 contract reject、一次 repair validated）完成；purpose-specific token 已禁用。该记录待 Git review，且仅证明该 fixture 的 native gateway capability，不构成全量 quality、模型晋级或生产 route adoption 证据。
