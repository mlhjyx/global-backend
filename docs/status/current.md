...
> **2026-08-10 v17 live preflight 停止记录**：#381 已以 merge commit `983a6792e87e75b0cdb53232f3e2999eeb4b77f5` 合入，post-merge Copy focused tests 与 docs governance 均通过。随后已授权的零调用 preflight 在 **v17 token 创建前**经 ToolBroker 读取 OpenOx 公共价格目录时遇到 IPv4 `ETIMEDOUT` / IPv6 `ENETUNREACH`，因此 fail closed；记录的 ToolBroker cost 为 `0`。没有 v17 token、artifact、bearer model request、模型 wire 或模型费用。控制面 readback 已确认 route #20 重新为 disabled，v16 token #24 仍 disabled。此为外部 source 可用性 blocker；不得重试扩容、复用 v16、创建 v18 或推导 capability/quality/promotion/route adoption。仅在网络可用性恢复后，从当前 main 重新进行一次受控 v17 **零调用** preflight；dispatch 仍需单独新授权。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-10 live fetch 已确认 `origin/main@ca16c5336a51f5ada152aff5c39e57ba8ff4589a` 为 #378 merge commit，已包含 #374 运行身份、#375 roles→scopes、#377 Copy fixed-source 影响治理、#376 获客变更与 Copy v16 fixed-source rebase；精确远端 tip 属易漂移外部状态，push/PR/merge 前仍须重新 fetch。#378 合入 v16 manifest/runtime binding、descriptor-anchored source verifier 与 ledger/claim 前重验，但未创建 token、未 dispatch、未提供 capability evidence。获客接管的原始代码审计基线仍为 `4562eab1bae16cdd424ff90a7d3403b0fb30d535`；其后主线变化不改变获客实现边界。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery v16 仍是 `NOT_AUTHORIZED`、零调用 create-only 技术输入，不是 capability evidence。它以 verifier/runner TOCTOU 修复提交 `dcbbff8dfc34e7f2559327ccbdce0d4049387453` 为 manifest fixed source，并从已跟踪 v16 manifest 的干净提交生成 compiled runtime binding；#378 合并后的 ancestry/artifact/compiled-runtime/docs 与机器 eligibility 已重验为 `CURRENT / NOT_AUTHORIZED / BLOCKED`。当前在途分支只新增 purpose-specific finite token 与零模型调用 preflight/attestation 工具链，尚未 live 执行。获客侧既有状态不变；上述内容仍是 source/deterministic evidence，没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | 控制面 `source_thread_id=019fc334-a6b1-79a2-bf18-185620410958` 已授权创建一枚 purpose=`site_builder_copy_sonnet_recovery`、仅 `claude-sonnet-5 × anthropic_messages`、最多 2 条 physical wires、有效期不超过 24 小时的一次性有限 token，并授权执行零模型调用的 route/scope/quota/OpenOx price/request-bound settlement preflight/attestation；该授权明确 **不得 dispatch 模型**。#378 已合入且 post-merge eligibility 为 `CURRENT`；token 仍只能在零调用工具链通过独立 review、Ready PR、required checks、merge commit 与 post-merge 重验后创建。secret 只写当前用户私有临时目录的 `0600` create-only 文件，不进入 Git/stdout；tracked artifact 只保留不可逆摘要与控制面/readback 证明。dispatch、promotion、production route adoption 与 pilot 仍为 `NOT_AUTHORIZED/BLOCKED`。v11–v15 token/authorization/campaign/reservation/wire 永不复用。 |

## 2. 在途工作

Site Builder Copy v16 fixed-source rebase 已由 #378 以 merge commit 合入，并在 clean post-merge checkout 重验 ancestry、manifest/runtime binding、compiled runtime、权威文档与机器 eligibility 为 `CURRENT / NOT_AUTHORIZED / BLOCKED`。当前隔离分支按 RED→GREEN 新增零调用 preflight/attestation：只允许创建单枚 purpose-specific、Sonnet-only、有限 quota、最长 24 小时的 token，只读取 New API token/channel/usage/model/log 控制面，并把 OpenOx 公共价格读取收敛到 required-source-policy `ToolBroker` tool（domain=`openox.tech`、purpose=`site_builder_copy_sonnet_recovery`）；operator 在任何 broker/secret/token 工作前还必须证明 `APP_DATABASE_URL` 是非 superuser、非 BYPASSRLS 的 `app_user`。New API/OpenOx 均手动拒绝 redirect；route 固定 Anthropic channel type `14`、base=`https://openox.tech` 与显式 identity mapping；quota 上界纳入 cache-read/cache-write 最高 input-side rate，duplicate price rows、分页 total 漂移、scope/model inventory 或关键 observation 不完整全部 fail closed。writer 生成 create-only tracked artifact和 Git 外 `0600` secret，并使用私有父目录、descriptor anchoring、inode/mode readback、竞态保护与失败 token disable。当前仅有离线测试/构建/独立 code/security review，尚未访问 live credential/control plane、尚未创建 token、模型 wire 为 0；live DB 缺少对应 source policy 或角色不符时必须停止，不得临时扩权或绕过。在该工具链 Ready PR、required checks、merge commit 与 post-merge 重验完成前不得 live 执行；其后也只执行用户已授权的零调用 preflight/attestation，不得创建模型 client 或发出 wire。
