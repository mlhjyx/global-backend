...
> 最后核验：2026-08-29T23:49:15+08:00（Asia/Shanghai）
...
Phase 0 文档分支的 historical construction base 是 `23d111f7b400403deb7466abf34ab709685b8376`，不是当前 main。当前 source authority 是 `main@c7e39e050b2f30ed9ff155aec139ff206fb850d0`；精确在途候选由 Git/托管 PR head 与冻结 review packet 绑定，不嵌入本 tracked 页，因为一个 commit 无法稳定包含自身最终 SHA。

| Subject | Exact head / state | Evidence class and limitation |
| --- | --- | --- |
| `/global/backend` root `main` | `HEAD=c7e39e050b2f30ed9ff155aec139ff206fb850d0`; local `origin/main=c7e39e050b2f30ed9ff155aec139ff206fb850d0`; tracked/untracked clean at capture; 69 ignored paths were preserved by the controlled sync | Live local Git and `governance-main-worktree-sync` readback after PR #424. This is source provenance only, not runtime/Release proof. |
| Program A | `/global/backend/.codex/worktrees/production-parity-capability-cutover`; `codex/production-parity-capability-cutover@91cae351795cceced59893bcf552c2b502a4ebaa`; clean; `MERGE_HEAD=NONE`; `59 ahead / 14 behind` local `origin/main@c7e39e050b2f30ed9ff155aec139ff206fb850d0` | Live local Git readback. This is the closed audit packet and remains `NON_DEPLOYABLE / PROVENANCE_ONLY`, not a current-main acceptance claim; any later head/index/working-tree/merge movement reopens the delta audit. |
| Program B successor candidate | `/global/backend/.codex/worktrees/discovery-query-materialization-successor`; branch `codex/discovery-query-materialization-successor`; exact base `c7e39e050b2f30ed9ff155aec139ff206fb850d0`; candidate exact head is bound by PR #425/Git rather than self-embedded here | Live local/hosted Git readback. The card is `ZERO_PRODUCT_CODE / AWAITING_CURRENT_MAIN_READBACK`; it is not G0 PASS or implementation acceptance. |
| Closeout plan | `/global/backend/.codex/worktrees/root-worktree-remote-closeout-plan`; `codex/root-worktree-remote-closeout-plan@3c9156d05ba6509c58bd4b806349fb5f07aef9a6`; `36 ahead / 59 behind` local `origin/main` | Live local Git readback; provenance only. |
...
| --- | --- | --- |
| `global-api.service` | `ActiveState=activating`, `SubState=auto-restart`, `NRestarts=6445`, `ExecMainStatus=0` | Degraded development-runtime observation, not a healthy API proof. |
| `global-worker.service` | `ActiveState=activating`, `SubState=auto-restart`, `NRestarts=6445`, `ExecMainStatus=0` | Degraded development-runtime observation, not a consuming Worker proof. |
