...
| 产品阶段                | Site Builder Copy Sonnet recovery v15 仍是 #372 合入的 `NOT_AUTHORIZED`、零调用 create-only 技术输入，不是 capability evidence。获客侧按“治理真值 → 可证明运行身份 → roles/scopes 与合规 → durable ops → 质量标签/identity → backend-only controlled pilot”顺序收口：API build attestation、controlled mode 安全收敛与 layered health 已由 #374 以 merge commit `bd36d62bdb78837a10b7ce56a5e745a2590fc43a` 进入 main；当前变更集继续实现服务端 roles→scopes、JWKS claim 合同、控制器最小权限和 OpenAPI scope 元数据。两项都仍是 source/deterministic evidence；没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。                                                                                                                                                                                                                                                                      |
...
其中运行身份 source slice 已由 #374 合入；当前 roles→scopes slice 以九个闭合 scope、服务端 role map、未知 role 零权限、控制器双 guard、PII/合规复合门和 `x-required-scopes` OpenAPI 元数据为实现边界。DevTokenVerifier 只允许显式 development、显式 `AUTH_ALLOW_DEV_TOKENS=true` 与 loopback bind；pilot/production 仍必须使用 JWKS、issuer、audience 与显式 role map。该 source 状态不能替代 SaaS 真 token、live startup admission、跨 workspace 负例或部署回读。
...
3. **集成门**：运行身份 source slice 已合入；roles→scopes slice 仍须完成全量验证、独立 review、exact-head hosted CI 与主线合并，并在批准的 release checkout/目标环境上用真实 JWKS token、受限 role map 和跨 workspace 负例形成运行证据。
