...
> 2026-07-10 立；**2026-07-17 R1-safety、R2-A1–A4 与 MF0-A 更新**。这是当前状态权威；历史见 [../roadmap/changelog.md](../roadmap/changelog.md)。
...
- **R2-A1/A4 Asset cleanup 已闭环（2026-07-17）**：`pending_upload/failed_retryable/expired committing → committing` 由 attempt+UUID token+lease CAS 认领，完成/失败回写全部 fenced；canonical copy 按 content hash 幂等，P2002 明确转 `duplicate`，瞬时存储失败保留 staging，DELETE 改 tombstone。状态转换与 `AssetObjectCleanupRequested` 同事务落 Outbox；staging 命令由 Temporal 在原 presigned PUT 到期后追加代码固定、客户端不可缩短的 15 分钟受支持在途上传宽限，回查 Outbox+Asset 双 provenance 后首删+HEAD，再 durable settle 5 分钟并二次删除+HEAD，覆盖到期前已获授权且在窗口内完成的 PUT 复活竞态；不再把“URL 到期”单独当成无在途请求的证明。失败有有限重试、结构化告警与 guarded redrive。canonical 命令仍 parked，MF-0 引用扫描器前不会自动删除。
...
- **MF0-A 媒体数据地基已完成（2026-07-17）**：新增 `AssetVariant` additive 表，直接携带 workspace/site/Asset scope；父 Asset 与 source Variant 复合 FK 防跨租户/跨站/跨 Asset provenance，source 必须 ready+checksummed，单输出 `unique(assetId,recipeHash)`、MIME→规范扩展名精确绑定的 Variant 专属对象键、checksum/正尺寸/状态 CHECK、行身份/来源/ready 物化不可改写、显式 `app_user` CRUD 与 RLS/FORCE RLS 已落；升级预检在锁内显式关闭行过滤，能全表扫描或因权限不足直接失败，遇历史坏来源/key 一律 fail-closed。`@global/contracts` 已定义 recipe/响应式兼容 manifest，纯 projector 稳定保留各 codec 的多宽度 ready Variant；尚无 Sharp writer，也未 backfill 历史 `derivedKeys`。Ubuntu 开发库已验证 44 条 migration 增量与空库 deploy、schema diff=0、app_user A/B/unset 隔离、约束负例及并发同 recipe 恰一胜；verifier 以数据库 advisory lock 串行，并在开跑时定向清理带专用 marker 的中断残留；不代表生产部署。**MF0-B 的 SiteSpec+Profile 扫描、409、并发写侧门与 canonical/Variant 回收仍未完成，canonical 继续 parked。**
...
- **当前关键路径**：**R1-safety ①+②、R2-A1–A4 与 MF0-A 已完成**；下一项 **MF0-B（扫描/409/并发/回收）**，其后才是 **M1-c 纯 Sharp**。R1-min 其余原子预览与 unknown component fail-closed 可并行；MF0-B 前 canonical 不会自动删。
