...
> **2026-08-10 Copy Sonnet recovery 当前状态**：`origin/main@0a061ee1c5f39c5f604fc2e17249f57adc9f27a1` 已包含 #384 的 v17 endpoint canonicalization 修复。v16 token #24 与 v17 token #25 均是不可重用的 disabled 失败审计记录，route #20 为 disabled。#381/#382 的历史停止证据保持冻结，不会因后续授权改写。产品负责人现已明确授权在 v18 合同经 PR/CI/review/merge 后，从最新 main 执行一次受控 **零调用** v18 preflight；模型 dispatch 仍需新的明确授权。
...
| Copy 技术/运行授权      | 控制面已授权创建一枚 purpose=`site_builder_copy_sonnet_recovery`、仅 `claude-sonnet-5 × anthropic_messages`、最多 2 条 physical wires、有效期不超过 24 小时的一次性有限 **v18** token，并授权一次零模型调用 route/scope/quota/OpenOx price/request-bound settlement preflight；该授权明确 **不得 dispatch 模型**。v16 #24 与 v17 #25 必须保持 disabled 审计记录且绝不复用。secret 只可写用户私有 `0600` 文件，不进入 Git/stdout；dispatch、promotion、production route adoption 与 pilot 仍为 `NOT_AUTHORIZED/BLOCKED`。 |
