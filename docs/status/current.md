...
> **2026-08-11 Copy Sonnet recovery 当前状态**：`origin/main@274f39aadba6b0fa4ffc3f1981c7c6cf03785536` 已合入 #391 的 v20 successor 合同与 #392 的 v20 stopped evidence。已获授权的单次 v20 **零调用** preflight 在 clean main 实际执行：ToolBroker 成功读取 OpenOx public pricing source（0 model cost、2975ms），随后 token #27 的 bearer `/v1/models` inventory readback 收到安全记录的 HTTP 403，正式 writer 在读取响应体前 fail closed 并 cleanup。脱敏事实见 [`v20 stopped evidence`](../evidence/site-builder/m1-g-copy-sonnet-recovery-v20-zero-call-preflight-stopped-evidence.json)：route #20、v16 #24、v17 #25、v19 #26 与 v20 #27 均为 disabled；不存在成功 artifact、模型 dispatch request、模型 wire 或模型费用。v20 授权已消费，不得重试或创建 successor；模型 dispatch 仍需新的明确授权。其后一次 operator-side 诊断输出被判定为可能含上游 credential material，必须按 [`credential security hold`](../evidence/site-builder/m1-g-copy-sonnet-recovery-v20-credential-security-hold.json) 先轮换潜在共享的上游 credential 并完成无泄露切换；在此之前禁止所有控制面探测、token、route 变更和模型调用。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-11 live fetch 已确认 `origin/main@274f39aadba6b0fa4ffc3f1981c7c6cf03785536` 为 #392 merge commit，first-parent 已含 #391 的 v20 successor 合同、#392 的 stopped evidence、#385/#386 的 v18 stop chain、#387 v19 successor 合同、#388 v19 stopped evidence 与 #389 endpoint-safe preflight failure diagnosis。#378 的 v16 fixed-source rebase 是其历史前置，不能再作为 current main 或“未执行 preflight”的操作依据。精确远端 tip 属易漂移外部状态，push/PR/merge/preflight 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery 保持 `NOT_AUTHORIZED / BLOCKED`，不是 capability evidence。v16 #24、v17 #25、v19 #26 与 v20 #27 是 disabled 的不可复用失败审计记录；v18 没有 token 且其 stopped evidence 不可改写。v20 已在 post-create bearer model-inventory readback 403 fail closed，成功 artifact 不存在；随后 operator-side 诊断输出可能暴露上游 credential material，故在潜在共享 credential 完成轮换与无泄露切换前，全部控制面及模型运行操作均为 security hold。没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | v20 一次性有限 token/preflight 授权已消费：ToolBroker price source 成功后 token #27 被创建，但 post-create bearer `/v1/models` inventory readback 403 fail closed，cleanup 已禁用 #27；结果仅为 stopped evidence。operator-side diagnostic exposure 使其后的操作进入 `PENDING_UPSTREAM_CREDENTIAL_ROTATION` security hold；任何 retry、successor token、控制面探测与模型 dispatch 均需要先完成无泄露凭据轮换/切换，并分别取得新的明确授权。v16 #24、v17 #25、v19 #26 与 v20 #27 必须保持 disabled 审计记录且绝不复用；v18 stopped authorization 不可重放。 |

## 2. 在途工作

Site Builder Copy 的 v16 fixed-source rebase 已由 #378 合入，#381 已将 v17 zero-call successor preflight 合入，#384 修复了 New API canonical usage endpoint，#385/#386 完整留存 v18 的 source-outage 停止事实，#387 提供独立 v19 namespace，#388 合入 v19 stopped evidence，#389 合入 endpoint-safe preflight failure diagnosis，#391 合入 v20 合同，#392 合入 v20 stopped evidence。v20 的授权已实际执行并冻结：ToolBroker 价格源成功，但 token #27 创建后的 bearer `/v1/models` inventory readback 403，cleanup 已禁用 #27 且 route #20 已关闭；不得重试、复用或创建 successor。其后 operator-side diagnostic output 可能含上游 credential material，潜在共享 credential 的轮换与无泄露切换已成为恢复 exact bearer inventory readback 前的硬门；在此之前禁止 token、route/control-plane 探测及任何模型调用。完成安全处置后，仍须先因果诊断并恢复该 exact bearer inventory readback，再取得新的明确零调用授权；模型 dispatch 继续要求更严格的单独授权。不得把停止记录、preflight、控制面 attestation 或代码绿灯当作 capability、质量、promotion 或 production route adoption。
