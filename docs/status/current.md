...
| Source-only 质量/安全门 | 2026-08-08 基于含 `origin/main@f48c5d2` 的干净集成树重新验收中；上一轮 367 个测试文件、5,419 个测试全通过，全范围 V8 结果为 statements 85.25%、branches 80.12%、functions 86.90%、lines 86.69%，关键 cohort 分支门均达到 80%。新增主线生产代码必须重新计算后才能维持 `TARGET_MET`。PostgreSQL/Temporal 集成、恢复演练、GitHub ruleset 与镜像外部 provenance 仍是 `BLOCKED/NOT_RUN/UNVERIFIED`。 |
