...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | `origin/main@a3c5c323e93ca398c12c96f89cf2967218862070`；2026-08-10 live fetch 核验。#360 已以 merge commit 合入 v14 create-only manifest/runtime binding，#361 又合入 Anthropic Messages 动态 object schema 语义与 dispatch 前 64 KiB UTF-8 schema 上限；#362 随后合入独立的供应链 canary/audit ratchet，未改 Copy/runtime 源码。v15 manifest 的固定源码仍严格是 #361 merge `fcb61e3060dd3289fec93bca11d02584f8080791`，不能随无关主线提交漂移。获客接管的原始代码审计基线仍为 `4562eab1bae16cdd424ff90a7d3403b0fb30d535`；其后主线变化不改变获客实现边界。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 产品阶段                | Site Builder Copy Sonnet recovery v15 已完成零调用 create-only 技术输入：[`v15 manifest`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v15.json) 固定 #361 merge `fcb61e3060dd3289fec93bca11d02584f8080791`，只允许 `claude-sonnet-5 / anthropic_messages / medium` 的 1 execution / 最多 2 wires / 最多 1 repair；[`v15 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v15.json) 从已跟踪 manifest 的干净提交 `64dcad8bb93dc96ad32f39aef2adc31dcc705f4c` 构建，固定 82-file source bundle、53-file compiled tree，以及 #361 后的 Anthropic adapter、adapter-result、types 与完整 runner/verifier/gateway/recovery closure。两者均为 `NOT_AUTHORIZED`、`dispatchCapable=false`、零真实调用和零费用。v14 两份 artifact 继续作为不可变历史独立验真，不能交给 v15 validator、不能成为 v15 live source/admission，也不能复用其 authorization/wire。Capability、真实 36/72 quality、promotion 与 production route adoption 仍均未完成。获客侧仍处于受控 pilot 前准备，尚未进入真实 pilot。 |
...
| Copy 技术/运行授权      | 控制面已收到 `source_thread_id=019fc334-a6b1-79a2-bf18-185620410958` 下仅限当前 v15 技术 PR 的 push、Ready PR 与满足 exact-head 检查/审查后的 merge-commit 合并授权声明；仓库没有可信外部 readback，因此本页和 PR 正文对该声明都只能记为 `EXTERNAL_UNVERIFIED`，不得由未来会话、其他 PR 或新 head 推断或复用。实际合并前仍须实时回读本次任务授权 provenance，并在 PR 上分别取得机器检查、独立 review、决策卡 freshness 与 merge-method provenance。v15 真实 dispatch、promotion 与 production route adoption 均为 `NOT_AUTHORIZED`；技术合并不得推导任何运行或发布授权。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

## 2. 在途工作

Site Builder Copy 当前在途工作是 v15 create-only PR 收口：RED → GREEN → clean manifest → clean tracked-manifest runtime binding 已形成可审计提交链；当前分支不包含真实模型调用、凭据读取/变更、36/72 质量矩阵、晋级或生产路由切换。下一门是当前 head 的独立 code/security review、CI 与决策卡；只有实时回读本次外部任务授权、绑定 exact PR/head 并关闭这些门后，才可按该次授权以 merge commit 合并并做 post-merge ancestry/artifact/compiled-runtime/docs 重验。只有这些技术门完成后，才可另行准备全新的 purpose-specific Sonnet-only finite credential、known settlement/ledger 与 v15 exact dispatch 决策卡；没有新的明确 v15 dispatch 授权不得创建 client 或发出任何 wire。Terra/Sol v11 成功 wire，以及 v12/v13/v14 authorization/wire 均不得重放，MiniMax/Doubao 保持 retired。
