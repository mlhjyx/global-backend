...
- **MF0-A 媒体数据地基已完成（2026-07-17）**：新增 `AssetVariant` additive 表，直接携带 workspace/site/Asset scope；父 Asset 与 source Variant 复合 FK 防跨租户/跨站/跨 Asset provenance，单输出 `unique(assetId,recipeHash)`、Variant 专属对象键、checksum/正尺寸/状态 CHECK、来源与 ready 物化不可改写、显式 `app_user` CRUD 与 RLS/FORCE RLS 已落。`@global/contracts` 已定义 recipe/响应式兼容 manifest，纯 projector 稳定保留各 codec 的多宽度 ready Variant；尚无 Sharp writer，也未 backfill 历史 `derivedKeys`。Ubuntu 开发库已验证 40 条 migration 增量与空库 deploy、schema diff=0、app_user A/B/unset 隔离、约束负例及并发同 recipe 恰一胜；不代表生产部署。**MF0-B 的 SiteSpec+Profile 扫描、409、并发写侧门与 canonical/Variant 回收仍未完成，canonical 继续 parked。**
