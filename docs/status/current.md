...
> **2026-08-11 Copy Sonnet recovery 当前状态**：`origin/main@cee647bfeb543014b93784caa49bb27a95077532` 已包含 #385 的 v18 successor preflight 合同。获授权的单次 v18 **零调用** preflight 已从该 merge commit 执行，但 ToolBroker 在读取 OpenOx public pricing source 时遇到 IPv4 `ETIMEDOUT` / IPv6 `ENETUNREACH`，故在 token 创建前 fail closed。冻结、脱敏事实见 [`v18 stopped evidence`](../evidence/site-builder/m1-g-copy-sonnet-recovery-v18-zero-call-preflight-stopped-evidence.json)：不存在 v18 token 或成功 artifact，route #20、v16 #24 与 v17 #25 均为 disabled，且没有 bearer model request、模型 wire 或模型费用。此单次授权已消费；不得重试、创建 successor 或推导 capability/quality/promotion/route adoption。模型 dispatch 仍需新的明确授权。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-11 live fetch 已确认 `origin/main@cee647bfeb543014b93784caa49bb27a95077532` 为 #385 merge commit，first-parent 已含 #384 endpoint canonicalization 与 #385 v18 successor preflight 合同。#378 的 v16 fixed-source rebase 是其历史前置，不能再作为 current main 或“未执行 preflight”的操作依据。精确远端 tip 属易漂移外部状态，push/PR/merge 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery 保持 `NOT_AUTHORIZED / BLOCKED`，不是 capability evidence。v16 compiled runtime binding 仍为 current fixed input；v16 #24、v17 #25 是 disabled 的不可复用失败审计记录。#385 后唯一获授权的 v18 zero-call preflight 已在 public pricing source 不可达时 token 创建前停止，冻结事实见上方 stopped evidence；没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | v18 的一次性有限 token/preflight 授权已在 token 创建前因 OpenOx price source 不可达而消费，结果仅为 stopped evidence；没有 token、secret 或成功 artifact 可复用。v16 #24 与 v17 #25 必须保持 disabled 审计记录且绝不复用。新的 zero-call retry、任何 successor token 与模型 dispatch 均需要新的明确授权；dispatch、promotion、production route adoption 与 pilot 仍为 `NOT_AUTHORIZED/BLOCKED`。 |

## 2. 在途工作

Site Builder Copy 的 v16 fixed-source rebase 已由 #378 合入，#381 已将 v17 zero-call successor preflight 合入，#384 修复了 New API canonical usage endpoint，#385 则将 v18 successor 合同合入并完成 post-merge 技术重验。v18 的唯一获授权 live preflight 已在 `ToolBroker` 读取 OpenOx 公共价格时因 IPv4 timeout / IPv6 unreachable 停止，早于任何 token、secret、成功 artifact 或 model request；冻结 readback 见上方 v18 stopped evidence。不得把这一外部 source 停止当作控制面、capability 或质量通过，也不得重试、复用 v16/v17、创建 successor、创建模型 client 或发出 wire。只有 OpenOx public pricing source 连通性独立恢复、重新完成 exact-head 技术核验并获得新的明确零调用授权后，才能进行新的 preflight；模型 dispatch 继续要求更严格的单独授权。
