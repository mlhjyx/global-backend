...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-11 live fetch 已确认 `origin/main@673772b529b0bc719acefddaabad64b7326f4904` 为 #387 merge commit，first-parent 已含 #385/#386 的 v18 stop chain 与 #387 v19 successor 合同。#378 的 v16 fixed-source rebase 是其历史前置，不能再作为 current main 或“未执行 preflight”的操作依据。精确远端 tip 属易漂移外部状态，push/PR/merge 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
...
| Copy 技术/运行授权      | v19 一次性有限 token/preflight 授权已消费：ToolBroker price source 成功后 token #26 被创建，但 post-create control-plane readback fail closed，cleanup 已禁用 #26；结果仅为 stopped evidence。任何 retry、successor token 与模型 dispatch 均需要新的明确授权。v16 #24、v17 #25 与 v19 #26 必须保持 disabled 审计记录且绝不复用；v18 stopped authorization 不可重放。 |

## 2. 在途工作

Site Builder Copy 的 v16 fixed-source rebase 已由 #378 合入，#381 已将 v17 zero-call successor preflight 合入，#384 修复了 New API canonical usage endpoint，#385/#386 完整留存 v18 的 source-outage 停止事实，#387 则提供独立 v19 namespace。v19 的新授权已实际执行：ToolBroker 价格源成功，但 token #26 创建后的 New API control-plane readback 不可用，故 cleanup 禁用 #26 并停止；不得重试、复用或创建 successor。任何后续恢复必须先因果诊断 New API control-plane 可用性，完成新的 exact-head 核验并取得新的明确零调用授权；模型 dispatch 继续要求更严格的单独授权。不得把停止记录、preflight、控制面 attestation 或代码绿灯当作 capability、质量、promotion 或 production route adoption。
