...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-11 live fetch 已确认 `origin/main@eb24889d38d9eadc9794a29feb64bc075dca6b37` 为 #389 merge commit，first-parent 已含 #385/#386 的 v18 stop chain、#387 v19 successor 合同、#388 v19 stopped evidence 与 #389 endpoint-safe preflight failure diagnosis。#378 的 v16 fixed-source rebase 是其历史前置，不能再作为 current main 或“未执行 preflight”的操作依据。精确远端 tip 属易漂移外部状态，push/PR/merge/preflight 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery 保持 `NOT_AUTHORIZED / BLOCKED`，不是 capability evidence。v16 #24、v17 #25 与 v19 #26 是 disabled 的不可复用失败审计记录；v18 没有 token 且其 stopped evidence 不可改写。v19 已在 post-create control-plane readback fail closed，成功 artifact 不存在；没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | v19 一次性有限 token/preflight 授权已消费：ToolBroker price source 成功后 token #26 被创建，但 post-create control-plane readback fail closed，cleanup 已禁用 #26；结果仅为 stopped evidence。用户已明确授权一枚新的 v20 successor 有限 token用于一次零调用 preflight，但该授权只能在 v20 合同经 PR/CI/review/merge 后由 clean main 消费；模型 dispatch 仍需要新的明确授权。v16 #24、v17 #25 与 v19 #26 必须保持 disabled 审计记录且绝不复用；v18 stopped authorization 不可重放。 |

## 2. 在途工作

Site Builder Copy 的 v16 fixed-source rebase 已由 #378 合入，#381 已将 v17 zero-call successor preflight 合入，#384 修复了 New API canonical usage endpoint，#385/#386 完整留存 v18 的 source-outage 停止事实，#387 提供独立 v19 namespace，#388 合入 v19 stopped evidence，#389 合入 endpoint-safe preflight failure diagnosis。v19 的授权已实际执行并冻结：ToolBroker 价格源成功，但 token #26 创建后的 New API control-plane readback 不可用，cleanup 已禁用 #26；不得重试或复用。v20 zero-call successor 授权已经由用户给出，但必须先让 v20 create-only 合同完成 PR/CI/review/merge，再从 clean main 创建单枚新 token 并执行 route/scope/quota/OpenOx price/request-bound settlement preflight；模型 dispatch 继续要求更严格的单独授权。不得把停止记录、preflight、控制面 attestation 或代码绿灯当作 capability、质量、promotion 或 production route adoption。
