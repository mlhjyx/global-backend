...
> **2026-08-12 Copy Sonnet native quality（已 Git review）**：用户已单独授权剩余质量、晋级与路由三门。以 `claude-sonnet-5 × Anthropic Messages × special` 对六个 Copy production fixtures 各运行两次：12/12 通过当前生产 validator 与事实槽位硬门；`copy-cta-budget:r1` 的首个 structured-output 解析失败后，在同一执行键的第 2 条 wire 通过。本质量矩阵共 13 条物理 wire，形成 12 个 accepted outputs；另有 1 条更早的本地 serializer 诊断 wire 独立保留审计，不计入该矩阵或评分。独立盲审的语言、品牌、CTA、跨 locale 均值分别为 4、4、3.8、4（唯一 minor finding=`action_label_weak`）。所有 purpose-specific token 均已禁用；原始 prompt/output/token/request ID 均未提交。质量 artifact 已由 [Git-review acceptance](../evidence/site-builder/m1-g-copy-sonnet-native-quality-git-review-acceptance-2026-08-12.json) 绑定 #396 merge commit。当前 PR 仅把这项 Sonnet-only quality 结论登记为可供采用的 model promotion；active Copy 路由仍是 DeepSeek Pro→GLM，production route adoption 必须在下一独立 PR/CI/review/merge 中变更。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-12 live fetch 已确认 `origin/main@d49f4642868243b2bfb912d2b830ef866d42b13b`；它是 #396 的 merge commit，并保留 quality evidence head 的 ancestry。本分支从该 clean main 施工；push/PR/merge 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet 为 `NATIVE_CAPABILITY_PASS / GIT_REVIEWED` 与 `NATIVE_QUALITY_PASS / GIT_REVIEWED`：Sonnet-only 六 fixtures × 两次的 12/12 accepted outputs 均通过生产硬门；本矩阵为 13 条物理 wire（包含一次失败后的有界补发），另有 1 条先前诊断 wire 被排除。purpose-specific token 均已禁用。当前进入独立 `MODEL_PROMOTION_APPROVAL / GIT_REVIEW_PENDING`，active route 仍未改变；尚无生产路由采用、部署、fresh RuntimeEvidence 或 Release Bundle。v16 #24、v17 #25、v19 #26、v20 #27 与先前失败 token 均为不可复用历史审计。 |
...
Site Builder Copy 的 v16 fixed-source rebase、v17/v18/v19/v20 stopped records及其历史 token 均已保留。2026-08-12 的最小 native capability run 与其后的 Sonnet-only native quality 决策集均已完成 Git review/CI/merge；当前 PR 将质量 acceptance 登记为 Sonnet-only promotion approval，但不改变实际 Copy route。历史 credential security hold 仍须独立、安全地处置，不能通过改写或删除历史 artifact 回避。
