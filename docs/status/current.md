...
| Source-only 质量/安全门 | 2026-08-09 基于含 `origin/main@f48c5d2` 的干净集成树，368 个测试文件、5,426 个测试全通过；全范围 V8 为 statements 85.12%（23,829/27,994）、branches 80.01%（18,539/23,168）、functions 86.85%（4,711/5,424）、lines 86.53%（21,959/25,376），关键 cohort 分支门均达到 80%，状态为 `TARGET_MET`。branch 余量仍薄，后续必须增测而不能降门或扩大排除。PostgreSQL/Temporal 集成、恢复演练、GitHub ruleset 与镜像外部 provenance 仍是 `BLOCKED/NOT_RUN/UNVERIFIED`。 |
...
3. **集成门**：获客恢复切片已完成本地 source-only 集成与全量测试；Draft PR 的瞬时 CI 状态只认 GitHub 对各 exact head 的受信 check run，不在本页复制。独立 review、逐 PR 用户合并授权，以及最终合入提交和目标环境上的新运行证据仍是分离门。
