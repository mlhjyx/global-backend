...
| 平台基础设施 | #455、#457、#459、#460、#462 已合入；当前主线合并提交为 `0f775f7511074ffae04bfca1d9b7cb822ecb6878`，其中 #460 为 4D 持久 fence 基础、#462 为 managed activity 物理 wire 接线 | 源码接纳与平台权限、精确制品、GrowthOS/Builder/New API 的后续运行采用分开；不把已合入源码继续列为候选 |
...
本次受控切换已将 API 和 Worker 都切到同一 exact OCI：`ghcr.io/mlhjyx/global-backend@sha256:b35b1ad5b8d9f2c0c67459afff3e9cba2d008a4f5faae2735c2aa8f4daa72fcb`，绑定 build SHA `0f775f7511074ffae04bfca1d9b7cb822ecb6878`；两者容器 image ID、artifact identity 与 migration revision 均一致。此前缺失的 3 个迁移已应用，数据库现为 130/130 up-to-date；切换前已生成并核验 PostgreSQL custom-format backup。当前未执行真实模型调用。
...
| Readiness           | `/api/v1/health/build`=200 且 identity 已切换；`/api/v1/health/ready`=503，原因含 GrowthOS platform quote unavailable、settlement readback credential 未配置、matching Worker/Relay lease 未 ready | 这是 fail-closed 运行事实；不得把 exact image 误报为可接单或可付费调用 |
