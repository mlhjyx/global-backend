...
| Source-only 质量/安全门 | 当前 main 的仓内 CI 包含 build/typecheck/test、OpenAPI drift/lint/breaking、Gitleaks，以及 #362 新增的 production dependency audit ratchet、Dependency Review 与 CodeQL canary；后两者仍是 non-required canary，container/Compose/IaC、PostgreSQL/Temporal 集成与恢复演练也尚未形成完整 required gate。2026-08-09 main 基线 lock（SHA-256 `d98a61553ffa6ea3bca177f47c7c2a82362f774697ffd4c89fa299465072e868`）的 production audit 为 36 项（18 high、14 moderate、4 low、0 critical）。本地 `codex/deps-security-remediation` 候选从 `origin/main@412716a2…` 以 TDD 将官方 production-only audit 收敛为 7 项（0 high、6 moderate、1 low、0 critical），实现 checkpoint 为 `7bbc8d80`，并把 Astro renderer promotion 绑定到显式受控输出根；但尚未 push/PR/hosted CI/合并，全局 coverage 与 Copy fixed-source 门仍 HOLD。完整边界见 [dependency security TDD 记录](../implementation-records/dependency-security-remediation-tdd.md)。 |
...
6. **覆盖率门**：本地全量 API 为 4664 PASS / 2 skipped，但 statements 70.75%、branches 64.65%、functions 74.22%、lines 72.49%，未达到项目 80% 门；本候选继续 HOLD。
