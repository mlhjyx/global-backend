...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 当前远端主线            | 本轮只读固定并合入的主线快照为 `origin/main@4c1cfd5faf8d3cff4ce03b98e875e5a03dcfdee5`。独占 worktree 以 PR #400 当前远端 head `c7bd71c5d8869a3312892f883104042800f50b0b` 为第一父、该 main 快照为第二父形成实现 merge checkpoint `825368d44e26b4b23688d50c7f128f54c37cb71f`，再由 docs-only 后继收敛当前真值。该 successor 尚未 push，也尚未更新 PR #400；hosted CI 未运行。任何 push/PR/merge 前都必须重新只读核对 live `origin/main` 与 PR head，发生漂移立即停止并重新同步授权。`/global/backend` 根工作区存在用户现场且不是施工面，不得复位或清理。 |
...
| Source-only 质量/安全门 | 当前 main 的仓内 CI 包含 build/typecheck/test、OpenAPI drift/lint/breaking、Gitleaks、production dependency audit ratchet、Dependency Review 与 CodeQL canary；后两者仍是 non-required canary，container/Compose/IaC、PostgreSQL/Temporal 集成与恢复演练也尚未形成完整 required gate。依赖安全收口已进入 `825368d…` successor：官方 production-only audit 对 839 个生产依赖返回 0 critical/high/moderate/low、0 advisories，`extract-zip` / `GHSA-jmr9-qjv8-65gv` 不存在，仓库 ratchet 为 `PASS_CLEAR`；没有 dismiss 告警或扩充 baseline。完整 API `src/**/*.ts` inventory 为 370 files / 5401 PASS / 2 skipped，statements 85.73%（23147/26999）、branches 80.11%（18086/22574）、functions 88.04%（4583/5205）、lines 87.03%（21346/24527），且 policy 禁止追加源码 exclude 或 coverage-ignore pragma。本轮触及的外部 adapter/provider 响应、ToolBroker trace、HTTP 500 与 workflow/patent failure evidence 只保留闭合错误码或幂等 SHA-256 diagnostic token；所有 5xx `HttpException` 与未知异常都返回闭合 `INTERNAL` 响应。该证明不覆盖仓库所有历史日志面、Temporal activity failure event 或 terminal rethrow history。successor 尚未 push/更新 PR #400/运行 hosted CI/合并，Copy fixed-source 门仍 HOLD。完整边界见 [dependency security TDD 记录](../implementation-records/dependency-security-remediation-tdd.md)。 |
...
6. **覆盖率门**：successor 实现 checkpoint `825368d…` 的本地全量 API 为 5401 PASS / 2 skipped；完整 `src/**/*.ts` inventory 为 statements 85.73%、branches 80.11%、functions 88.04%、lines 87.03%，四项均已达到项目 80% 门；精确 include/exclude 与 coverage-ignore 扫描阻止后续静默缩小源码分母。该门已在本地 source/test 层关闭，但 hosted required CI 尚未运行，不能据此推导合并或试点授权。
