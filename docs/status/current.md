...
| 产品阶段                | Site Builder Copy Sonnet recovery v15 零调用 create-only 技术输入已由 #372 以 merge commit `ef603762359fa20c2fbbef93a62e523ed5b312c7` 合入：[`v15 manifest`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v15.json) 固定 #361 merge `fcb61e3060dd3289fec93bca11d02584f8080791`，只允许 `claude-sonnet-5 / anthropic_messages / medium` 的 1 execution / 最多 2 wires / 最多 1 repair；[`v15 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v15.json) 从已跟踪 manifest 的干净提交 `64dcad8bb93dc96ad32f39aef2adc31dcc705f4c` 构建，固定 82-file source bundle、53-file compiled tree，以及 #361 后的 Anthropic adapter、adapter-result、types 与完整 runner/verifier/gateway/recovery closure。两者均为 `NOT_AUTHORIZED`、`dispatchCapable=false`、零真实调用和零费用。获客侧按“治理真值 → 可证明运行身份 → roles/scopes 与合规 → durable ops → 质量标签/identity → backend-only controlled pilot”顺序收口；独立本地分支已补 API build attestation、controlled mode 安全收敛与 layered health，仍须完成 latest-main 合成树验证、exact-head CI/review 与合并。它尚未部署、没有 fresh RuntimeEvidence，也未进入真实 pilot。 |
