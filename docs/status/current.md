...
> **2026-08-11 Copy Sonnet recovery 当前状态**：`origin/main@7b6930524e03bbc2584a22cc3a32f2984ffc903e` 已包含 #386 的 v18 stopped evidence。获授权的单次 v18 **零调用** preflight 已在 ToolBroker 读取 OpenOx public pricing source 时遇到 IPv4 `ETIMEDOUT` / IPv6 `ENETUNREACH`，故在 token 创建前 fail closed；冻结、脱敏事实见 [`v18 stopped evidence`](../evidence/site-builder/m1-g-copy-sonnet-recovery-v18-zero-call-preflight-stopped-evidence.json)。不存在 v18 token 或成功 artifact，route #20、v16 #24 与 v17 #25 均为 disabled，且没有 bearer model request、模型 wire 或模型费用。产品负责人现已为 source 恢复后的 **单次 v19** zero-call successor preflight 授权；它仍须先经独立 v19 合同的 PR/CI/review/merge/post-merge 核验，且不得 dispatch。v16 #24、v17 #25 保持 disabled 审计记录，v18 停止授权绝不重放。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-11 live fetch 已确认 `origin/main@7b6930524e03bbc2584a22cc3a32f2984ffc903e` 为 #386 merge commit，first-parent 已含 #385 v18 successor 合同与 #386 stopped evidence。#378 的 v16 fixed-source rebase 是其历史前置，不能再作为 current main 或“未执行 preflight”的操作依据。精确远端 tip 属易漂移外部状态，push/PR/merge 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery 保持 `NOT_AUTHORIZED / BLOCKED`，不是 capability evidence。v16 compiled runtime binding 仍为 current fixed input；v16 #24、v17 #25 是 disabled 的不可复用失败审计记录，v18 没有 token 且其 stopped evidence 不可改写。新的 v19 zero-call successor 合同正在独立 PR 前置中；没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | 已获得一次新的 v19 successor 有限 token/preflight 授权：purpose=`site_builder_copy_sonnet_recovery`、仅 `claude-sonnet-5 × anthropic_messages`、最多 2 条 physical wires、≤24 小时，只允许 route/scope/quota/OpenOx price/request-bound settlement 的零模型调用 attestation；**不得 dispatch 模型**。它必须等待 v19 PR/CI/review/merge/post-merge 核验，并在 ToolBroker price source 可达后才可创建 token。v16 #24 与 v17 #25 必须保持 disabled 审计记录且绝不复用；v18 stopped authorization 不可重放。 |

## 2. 在途工作

Site Builder Copy 的 v16 fixed-source rebase 已由 #378 合入，#381 已将 v17 zero-call successor preflight 合入，#384 修复了 New API canonical usage endpoint，#385/#386 则完整留存 v18 的 source-outage 停止事实。v19 作为单独 namespace 的 successor 合同，必须先经 PR/CI/review/merge/post-merge 核验；之后才可在 `ToolBroker` 确认 OpenOx public pricing source 可达时，按新的单次授权创建有限 token 并运行零调用 attestation。任何价格 source 失败、控制面/范围/结算形状漂移都必须在 token 或模型调用前 fail closed；不得把 v18 停止、v19 preflight、控制面 attestation 或代码绿灯当作 capability、质量、promotion 或 production route adoption。模型 dispatch 继续要求更严格的单独授权。
