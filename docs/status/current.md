...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 当前远端主线            | `origin/main@a3c5c323e93ca398c12c96f89cf2967218862070`；2026-08-10 live fetch 核验。该 merge commit 合入 #362 的 supply-chain canary/audit ratchet，并在祖先链中保留 #360 的 Copy Sonnet recovery v14 与 #358 的 CI topology 治理收口。获客接管的原始代码审计基线仍为 `4562eab1bae16cdd424ff90a7d3403b0fb30d535`；其后主线变化不改变获客实现边界。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。                                                                                                                                                                                                   |
| 产品阶段                | Site Builder Copy Sonnet recovery v14 已作为零调用合同随 #360 进入主线：[`v14 manifest`](../evidence/site-builder/m1-g-copy-sonnet-recovery-manifest-v14.json) 与 [`v14 runtime binding`](../evidence/site-builder/m1-g-copy-sonnet-recovery-runtime-binding-v14.json) 仍为 `NOT_AUTHORIZED`、`dispatchCapable=false`，不授权真实模型调用、费用、路由或 promotion。获客侧正在按“治理真值 → 可证明运行身份 → roles/scopes 与合规 → durable ops → 质量标签/identity → backend-only controlled pilot”顺序收口；独立本地分支已补 API build attestation、controlled mode 安全收敛与 layered health，仍须完成 exact-head CI/review/合并。它尚未部署、没有 fresh RuntimeEvidence，也未进入真实 pilot。 |
| 当前交付状态            | Buyer/Intent 的机器追踪链均为 `INTERNAL_ONLY`。当前不存在可用于晋级的 RuntimeEvidence，也不存在真实 Release Bundle。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Source-only 质量/安全门 | 当前 main 的仓内 required CI 包含 build/typecheck/test、OpenAPI drift/lint/breaking、governance、decision card 与 Gitleaks；#362 另加入 non-required Dependency Review、production audit ratchet 与 CodeQL canary，主线 canary 已绿，但 live ruleset 未把新 context 升为 required。production audit 基线仍记录 36 项遗留债务（18 high、14 moderate、4 low、0 critical）和 183 条 exposure；ratchet 绿色只证明没有相对受信 base 扩张，绝不等于漏洞清零。container/Compose/IaC、PostgreSQL/Temporal 集成和恢复演练仍未形成完整 required gate。                                                                                   |
...
Site Builder Copy v14 已随 #360 进入主线，但只保留 create-only manifest/runtime binding；它排除 Terra/Sol v11 成功 wire以及停止的 v12/v13 Sonnet wire/authorization，不包含新的真实模型调用、凭据操作、质量矩阵、晋级或生产路由切换。任何后续 Sonnet-only dispatch 仍须重新形成有限凭据、settlement/ledger、exact authorization 与用户费用授权。
