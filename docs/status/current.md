...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-10 live fetch 已确认 `origin/main@39a35f0789c655c86da41f9a4ee6f067ebcd782b` 已包含 #374 运行身份、#375 roles→scopes、#377 Copy fixed-source 影响治理与 #376 获客变更；精确远端 tip 属易漂移外部状态，push/PR/merge 前仍须重新 fetch。#377 只新增 v15 source eligibility 与 `STALE_HOLD` 治理，未重建固定源、未授权 dispatch。当前 Copy v16 fixed-source rebase 仍是本分支候选，不能写成已进入 main。获客接管的原始代码审计基线仍为 `4562eab1bae16cdd424ff90a7d3403b0fb30d535`；其后主线变化不改变获客实现边界。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery v16 候选仍是 `NOT_AUTHORIZED`、零调用 create-only 技术输入，不是 capability evidence。它以 verifier/runner TOCTOU 修复提交 `dcbbff8dfc34e7f2559327ccbdce0d4049387453` 为 manifest fixed source，并从已跟踪 v16 manifest 的干净提交生成 compiled runtime binding；本分支机器 receipt 已回到 `CURRENT`，但尚未经过 hosted CI、PR 合并与 post-merge 重验。获客侧既有状态不变；上述内容仍是 source/deterministic evidence，没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | 控制面 `source_thread_id=019fc334-a6b1-79a2-bf18-185620410958` 已授权创建一枚 purpose=`site_builder_copy_sonnet_recovery`、仅 `claude-sonnet-5 × anthropic_messages`、最多 2 条 physical wires、有效期不超过 24 小时的一次性有限 token，并授权执行零模型调用的 route/scope/quota/OpenOx price/request-bound settlement preflight/attestation；该授权明确 **不得 dispatch 模型**。token 只可在 v16 合入并通过 post-merge `CURRENT` 重验后创建，secret 不进入 Git/stdout，artifact 只保留不可逆摘要。v16 source eligibility 由[机器 receipt](../evidence/site-builder/copy-runtime-eligibility.json)逐文件重算；本分支为 `CURRENT`，但 dispatch、promotion、production route adoption 与 pilot 仍为 `NOT_AUTHORIZED/BLOCKED`。v11–v15 token/authorization/campaign/reservation/wire 永不复用。 |

## 2. 在途工作

Site Builder Copy v16 fixed-source rebase 正在本隔离分支收口：已按 RED→GREEN 关闭中间目录 descriptor anchoring 与 ledger/claim 前 source revalidation 缺口；v16 manifest/binding 分别从干净 GREEN commit 与干净 tracked-manifest commit 生成，binding 精确包含 Messages adapter、adapter-result、types 与完整 runner/verifier tree；机器 eligibility 当前为 `CURRENT`。在 Ready PR、required checks、独立 review、merge commit 与 post-merge 重验完成前，不得创建专用 token；其后只执行用户已授权的零调用 preflight/attestation，仍不得创建模型 client 或发出 wire。
