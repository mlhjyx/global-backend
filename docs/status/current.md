...
- **R2-A2 KB 已闭环（2026-07-17）**：canonical doc 的 `queued(due)/expired processing → processing` 复用 Asset attempt+UUID token+lease，所有续租、失败与完成回写 fenced；`KbDocument.assetId` 历史重复按健康度/向量版本/更新时间确定性收敛后加 nullable unique，document→Asset 与 chunk→document 用复合 workspace/site FK 守 provenance。文档/chunks 与 Asset ready 同一事务提交，旧 worker 接管后不能 zombie write；MinIO/Docling/BGE 以 typed code 分流，瞬时错误回 `queued+retryAt`，真损坏文档才 `failed_terminal`。单素材 workflow 不再无界扫站，5 分钟 recovery Schedule、有界扫描/结构化告警和人工 redrive 已落；完整跨系统 cleanup/公共错误码验收仍属 R2-A4。开发环境真 PostgreSQL/app_user RLS/MinIO/Docling/BGE、脏数据升级和空库 36 migration 已验证。
