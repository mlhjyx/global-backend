...
| --- | --- | --- |
| G0 — Truth & Ownership | `HOLD_OWNERSHIP` | Writer inactivity、clean/no-`MERGE_HEAD` `91cae351` packet、delta classification 与 binding-ledger/provenance correction 已完成。剩余顺序是：(1) 将 ADR-025/`DEC-GPP-001` 合入并逐字回读 current main；(2) 由该 main 将 mega-branch 非破坏性登记为 `NON_DEPLOYABLE / PROVENANCE_ONLY`；(3) 从该 main 指派一个唯一 Program B successor/card/handoff 与 writer；(4) 独立 G0 readback。不得删除/revert 历史或从隔离提交 cherry-pick。Program B implementation/TDD 属于 G2，DB/RLS/replay 和集成属于 G3，均不是 G0 条件。 |
