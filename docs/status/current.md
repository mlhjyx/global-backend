...
| Source-only 质量/安全门 | 2026-08-09 基于含 `origin/main@f48c5d2` 的干净集成树，368 个测试文件、5,425 个测试全通过；全范围 V8 为 statements 85.08%（23,819/27,994）、branches 80.00%（18,536/23,168）、functions 86.85%（4,711/5,424）、lines 86.49%（21,949/25,376），关键 cohort 分支门均达到 80%，状态为 `TARGET_MET`。branch 余量很薄，后续必须增测而不能降门或扩大排除。PostgreSQL/Temporal 集成、恢复演练、GitHub ruleset 与镜像外部 provenance 仍是 `BLOCKED/NOT_RUN/UNVERIFIED`。 |
