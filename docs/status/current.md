...
> **2026-08-20 开发即生产 / 单一产品运行路径（分支 as-built，未合并）**：`codex/production-parity` worktree（`.codex/worktrees/production-parity`，基线 `a5948b85`）已完成本计划的仓内实现与本地非发布制品验证，但尚未完成合并、目标环境部署和真实产品 Grant 构建。24 小时 model settlement attestation 已从正常产品路径删除；正常产品调用统一为 SaaS 签名 Budget Grant（`X-Site-Build-Budget-Grant`，`site-builder-budget-grant/v1`）+ PostgreSQL 持久 reserve/settle + append-only reconciliation。Stub/Sandbox/DevTokenVerifier、项目测试 fixture、gallery 与 evaluation runner 已退出产品 composition root，并进入 `packages/test-support`、`apps/site-renderer-visual-harness`、`apps/site-builder-eval-runner` 等独立测试入口；第三方依赖仅保留一处经真实启动链证明必需的 `@nestjs/swagger/dist/fixtures` 运行模板。API/Worker 共用单一 OCI contract、release identity admission、durable lease/heartbeat 与统一 readiness；缺依赖时 API 只提供诊断且拒绝新任务，Worker 保持不就绪且不轮询。Temporal 新 history 只传对象存储 artifact reference/digest，环境分叉由 19 项机器政策精确登记。最终本地脏树候选 `global-backend@sha256:c32e99d06fcd25b95b6389fcfbd21a68223ede5f009bfbcfba03f1e459823dae` 以 UID 10001 非 root 运行，artifact digest=`sha256:3891d9d7bbde14f15f873b2fc6989d97830a3ebc744a88c2e150cedc198b1b26`，source tree digest=`sha256:064bb5ee8e9d03d856b13675c81f3bf78fdf8a39e3a4aacbf892c139070ed3ec`；全镜像验证、Chromium、`prlimit`、API 依赖闭包均通过。无依赖 API smoke 实际返回 ready 503（Renderer/Browser ready，其余 capability fail-closed），Worker 日志明确 `Temporal polling remains disabled`。源码门为 API 305 files / 4529 tests、governance 113/113、runtime contracts 15/15、`docs:verify` 通过；lint 为 0 error/12 既有 warning。该 digest 绑定未提交 source tree，只可作本地诊断证据，绝不可晋级。**未做**：fresh retained PostgreSQL/Redis/Temporal 集成重跑、clean-commit OCI、hosted CI、push/PR、部署、服务重启、真实模型调用与 RuntimeEvidence；小额真实 Grant 同路径构建仍需届时单独授权测试额度。

> **2026-08-20 精确提交边界（更新）**：上述实现现已形成未 push 的本地提交 `f5c0fc9ccc39f2af65a86bacd04d0ba822cfcdd4`（`feat(runtime): enforce production parity single path`），提交前秘密门没有被绕过；它先拒绝了一个测试中的通用 credential 命名，改为明确的非生产 HMAC 测试材料并重跑 Outbox test 后才通过。CI 增加 final OCI Worker fail-closed smoke：断网、只读条件下必须出现 `Temporal polling remains disabled`，且不得出现 Worker polling 日志。对该 clean commit 执行 `docker build --network=none` 时，Debian runtime-base 的 `apt-get` 因网络隔离无法解析 `deb.debian.org` 而安全失败；随后在用户明确允许的一次本地联网构建中，APT 索引下载超过五分钟仍无连接或阶段推进，已精确停止该单一构建进程组，未产出镜像标签。故**当前仍无 clean-commit OCI 的本地重建证据**；这不是可用 RuntimeEvidence，也不得以先前脏树候选替代。下一步只能在受控 CI 的正常构建或可观测网络恢复后，以该 exact commit（或其经审查的后继提交）重新构建，并完成镜像/部署 readback。

> **2026-08-20 Hosted CI / review-followup readback（最新）**：分支 `codex/production-parity@b236cdd9aae85261275639ae20101fea915c3237` 已推送为 [PR #413](https://github.com/mlhjyx/global-backend/pull/413)，用户已批准 v1 Budget Grant / decimal-string breaking contract 并添加 `breaking-change-approved`。受控 CI [run 32362534018](https://github.com/mlhjyx/global-backend/actions/runs/32362534018) 对 PR merge candidate `2a41da264e29e1e48d940a6a1d180d6a6f6b84b4` 通过 required checks：Security、Supply Chain、Governance、CodeQL、OpenAPI export/drift/Spectral、真实 PostgreSQL migration + runtime lease principal 正负权限、全 API Vitest、renderer fixture / three-viewport visual / multilingual smoke。CI 从 clean candidate 实际构建最终 OCI，local CI image config id 为 `sha256:997f807f6783e8963b2113eb810d5096a5fdc4e8b17c3f9a115a7570b63d19f8`（不是 registry digest，不能部署或晋级）；artifact/image verifier、non-root、SBOM、Chromium、`prlimit` 与断网只读 Worker smoke 均通过，Worker 明确记录 `Temporal polling remains disabled`。该证据证明 clean CI 制品合同，不是 retained 环境 RuntimeEvidence、部署 readback、Release Bundle、Pilot/GA 或真实产品 Grant 构建；合并、部署、迁移、重启和真实模型调用仍未执行。
