...
> **2026-08-10 Copy Sonnet recovery 当前状态**：`origin/main@a14f2f25fc15bb382860574c67764798a2364d1a` 已包含 #381 的 v17 successor preflight 和 #382 的停止记录。v16 token #24 是不可重用的 disabled 失败审计记录；v17 token 未创建，route #20 为 disabled。#381 post-merge 的唯一 live v17 零调用 preflight 在 token 创建前因 OpenOx 公共价格 source 网络不可达而 fail closed；冻结、脱敏事实见 [`v17 stopped evidence`](../evidence/site-builder/m1-g-copy-sonnet-recovery-v17-zero-call-preflight-stopped-evidence.json)。它记录 0 token、0 artifact、0 bearer model request、0 model wire、0 model cost，且不构成 capability、质量、promotion 或 production route adoption evidence。当前唯一允许的恢复路径是在 source 连通性独立恢复后，从最新 main 再执行一次受控 **零调用** v17 preflight；模型 dispatch 仍需新的明确授权。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-10 live fetch 已确认 `origin/main@a14f2f25fc15bb382860574c67764798a2364d1a` 为 #382 merge commit，first-parent 已含 #381 v17 successor preflight。#378 的 v16 fixed-source rebase 是其历史前置，不能再作为 current main 或“未执行 preflight”的操作依据。精确远端 tip 属易漂移外部状态，push/PR/merge 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery 保持 `NOT_AUTHORIZED / BLOCKED`，不是 capability evidence。v16 compiled runtime binding 仍为 current fixed input；#381 后的 v17 只改变零调用 successor credential preflight 的隔离与审计规则。该 preflight 已 live 尝试一次并在 public pricing source 不可达时 token 创建前停止；唯一冻结事实见上方 stopped evidence。没有部署、fresh RuntimeEvidence、Release Bundle 或真实 pilot。 |
...
| Copy 技术/运行授权      | 控制面已授权创建一枚 purpose=`site_builder_copy_sonnet_recovery`、仅 `claude-sonnet-5 × anthropic_messages`、最多 2 条 physical wires、有效期不超过 24 小时的一次性有限 **v17** token，并授权零模型调用 route/scope/quota/OpenOx price/request-bound settlement preflight；该授权明确 **不得 dispatch 模型**。#381 已满足 PR/CI/review/merge/post-merge 技术前置，但实际 preflight 因 source 网络失败而未创建 token。secret 只可写用户私有 `0600` 文件，不进入 Git/stdout；dispatch、promotion、production route adoption 与 pilot 仍为 `NOT_AUTHORIZED/BLOCKED`。v11–v16 token/authorization/campaign/reservation/wire 永不复用。 |

## 2. 在途工作

Site Builder Copy 的 v16 fixed-source rebase 已由 #378 合入，#381 已将 v17 zero-call successor preflight 合入并通过 post-merge 技术重验。preflight 只允许单枚 purpose-specific、Sonnet-only、有限 quota、最长 24 小时的 token，只读取 New API token/channel/usage/model/log 控制面，并把 OpenOx 公共价格读取收敛到 required-source-policy `ToolBroker` tool（domain=`openox.tech`、purpose=`site_builder_copy_sonnet_recovery`）；operator 在任何 broker/secret/token 工作前还必须证明 `APP_DATABASE_URL` 是非 superuser、非 BYPASSRLS 的 `app_user`。New API/OpenOx 均手动拒绝 redirect；route 固定 Anthropic channel type `14`、base=`https://openox.tech` 与显式 identity mapping；quota 上界纳入 cache-read/cache-write 最高 input-side rate，duplicate price rows、分页 total 漂移、scope/model inventory 或关键 observation 不完整全部 fail closed。#381 后的唯一 live preflight 已在 OpenOx 价格读取阶段停止，冻结事实见上方 stopped evidence；它没有创建 token 或发出 model wire。当前不应新开 v18 或重试扩容；只有 source 连通性恢复并独立验证后，才可从最新 main 再进行一次已授权的零调用 preflight，且不得创建模型 client 或发出 wire。
