...
> **2026-08-12 Copy Sonnet native quality（待 Git review）**：用户已单独授权剩余质量、晋级与路由三门。以 `claude-sonnet-5 × Anthropic Messages × special` 对六个 Copy production fixtures 各运行两次：12/12 通过当前生产 validator 与事实槽位硬门；`copy-cta-budget:r1` 的首个 structured-output 解析失败后，在同一执行键的第 2 条 wire 通过。本质量矩阵共 13 条物理 wire，形成 12 个 accepted outputs；另有 1 条更早的本地 serializer 诊断 wire 独立保留审计，不计入该矩阵或评分。独立盲审的语言、品牌、CTA、跨 locale 均值分别为 4、4、3.8、4（唯一 minor finding=`action_label_weak`）。所有 purpose-specific token 均已禁用；原始 prompt/output/token/request ID 均未提交。见 [native quality evidence](../evidence/site-builder/m1-g-copy-sonnet-native-quality-2026-08-12.json)。它是 Sonnet-only qualification，尚待本 PR Git review；模型 promotion 与 production route adoption 仍须各自独立 PR/CI/review/merge，不能由这条证据自动改变路由。

## 1. 当前基线

| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前远端主线            | 2026-08-12 live fetch 已确认 `origin/main@642f3b9185b3e667c6b73622bcd11448f8be7a62`。本分支从该 clean main 施工；push/PR/merge 前仍须重新 fetch。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet 为 `NATIVE_CAPABILITY_PASS / GIT_REVIEWED`，并新增 `NATIVE_QUALITY_PASS / GIT_REVIEW_PENDING`：Sonnet-only 六 fixtures × 两次的 12/12 accepted outputs 均通过生产硬门；本矩阵为 13 条物理 wire（包含一次失败后的有界补发），另有 1 条先前诊断 wire 被排除。purpose-specific token 均已禁用。尚无模型晋级、生产路由采用、部署、fresh RuntimeEvidence 或 Release Bundle。v16 #24、v17 #25、v19 #26、v20 #27 与先前失败 token 均为不可复用历史审计。 |
...
Site Builder Copy 的 v16 fixed-source rebase、v17/v18/v19/v20 stopped records及其历史 token 均已保留。2026-08-12 的最小 native capability run 已完成 Git review/CI/merge；其后的 Sonnet-only native quality 决策集正在本 PR 进入 Git review，不能将这次质量通过提前写成模型晋级或生产路由已经发生。历史 credential security hold 仍须独立、安全地处置，不能通过改写或删除历史 artifact 回避。
