...
| 项目                    | 当前事实                                                                                                                                                                                                                                                                                                                                         |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 当前远端主线            | `origin/main@2a54024512c0c7f5e373cd31c4e4e208e2fa1106`；2026-08-09 live fetch 核验。获客接管的原始代码审计基线仍为 `4562eab1bae16cdd424ff90a7d3403b0fb30d535`；其后主线新增 Site Builder Copy transport、manifest、capability evidence、Sonnet recovery runtime binding 与获客治理合同，不改变获客实现边界。`/global/backend` 根工作区存在用户现场且不是远端主线真值，不得复位或清理。 |
| 产品阶段                | Site Builder Copy Sonnet recovery v13 create-only 交付正在 #356 审查：它固定 #355 merge `a29b222a45ae5fdb4868d5235cc94aeab1574ecd`，只允许 `claude-sonnet-5 / anthropic_messages / medium` 的 1 execution / 最多 2 wires / 最多 1 repair；artifact 仍为 `NOT_AUTHORIZED`、`dispatchCapable=false`、零真实调用和零费用，且尚未进入 main。v12 已降为历史输入。下一门依次是 #356 单独合并授权、post-merge source/compiled revalidation、有限 Sonnet-only 凭据、settlement/ledger 与 exact dispatch authorization。获客侧处于“受控 pilot 前的治理、授权、运行身份与可观测性补齐”，尚未进入真实 pilot。 |
...
Site Builder Copy 当前只有 #356 的 v13 create-only manifest/runtime binding 在途；Terra/Sol v11 成功 wire 与停止的 v12 Sonnet wire/authorization 均不得复用。该 PR 不包含真实模型调用、36/72 质量矩阵、晋级或生产路由切换。
