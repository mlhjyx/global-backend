...
> 2026-07-10 立；**2026-07-17 R1-safety、R2-A1–A4、MF0-A/B 与 M1-c 更新**。这是当前状态权威；历史见 [../roadmap/changelog.md](../roadmap/changelog.md)。
...
- **M1-c 纯 Sharp 图片管线已完成（2026-07-17）**：API 直接精确锁定 Sharp 0.35.0，`pipelineVersion` 同时固定 libvips；≤20 MiB/40 MP/4 channels/单页严格解码，自动方向、sRGB、默认剥 EXIF/GPS/XMP，模糊/曝光/噪点只产生版本化 warning。按 kind→role 与显式 focal point 做不放大裁切，输出 320/640/960/1440/1920 中源图可承受的 AVIF+WebP+JPEG/PNG fallback；logo/cert 三编码均无损，Sharp 在并发=1、cache=false 的可杀子进程内运行。首个 writer 以父 Asset 行锁 + canonical key advisory lock 串行、逐对象 hash/MIME/bytes 回读；完整 ready-set 重放不启动 Sharp。PUT 响应丢失可收敛，DB 回滚只删除无 owner 对象，删除不可用时建 durable failed Variant 供 cleanup/重试；ready `AssetVariant` 与 `derivedKeys` 同事务，≤120 行预算不会撞 MF0-B 128 清理上限。refurbish P2 已正式消费并逐图降级、取消穿透；`hasPerson` 只保留已有真值，`aiEdited=false`。真 Ubuntu 开发 PG/app_user RLS/MinIO 验证同 key 双 producer、30 变体、零编码重放、响应丢失、回滚/删除故障补偿、容量门、坏图隔离与残留清零；不代表生产部署。MediaJob、AssetUsage、rembg、生成式图片、视频、公开 process/select API 与 Renderer `<picture>` 均未加入。
...
- **当前关键路径**：**R1-safety ①+②、R2-A1–A4、MF0-A/B 与 M1-c 已完成**；下一能力项是 **M1-d 多语种事实受限文案**。R1-min 其余原子预览与 unknown component fail-closed 在 M1-e 可见预览前完成，可作为下一独立安全交付并行推进。
