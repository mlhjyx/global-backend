...
- **⬜ 下一步**：P4 `attributes.fda.*` 富集（分类事实/清关历史）· P5 monitoring sweep（定时扫新注册/清关 `created_date`/`decision_date` 增量 + 510k intent 投影上 Temporal Schedule）· FDA 分类扩 `foiclass.zip` 全表种子（resolver/宽网机制已就位，扩种子即生效）。
