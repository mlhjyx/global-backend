...
- **✅ 名录/列表发现**（**已端到端实测**）：`DirectoryDiscoveryProvider` + `discovery.extract_list`（一页多公司）。一次真实运行从 3 个静态目录（metalstamper/mrforum/thefabricator）抽出 **151 家真实公司**（带官网+地址），并正确拒绝非名录页（单会员/单供应商详情页判 not-a-directory）。剩余短板：(1) 大展会参展商 **JS-SPA** 页 crawl4ai 静态抓取只拿壳；(2) 地域精度——查 Germany 会召回美国目录，需下游 fit/地域过滤收敛。
