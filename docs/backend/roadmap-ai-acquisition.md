...
- **⬜ 下一步**：P4 招标 SAM.gov Sources Sought（早数月意图）；招标 P3 投影上 Temporal Schedule（§8.8 门与幂等已就位）。

### openFDA 认证注册库 provider（P1 器械注册发现，2026-07-09）
...
- **⬜ 下一步**：P2 ICP→FDA 产品码映射（foiclass.zip 全表种子 + ISIC↔panel crosswalk + `resolveIcpToFda`，LLM enum 限 panel 子树）· P3 510(k)→`attributes.intent.events[{type:'FDA_CLEARANCE'}]` 动 Intent 维 · P4 `attributes.fda.*` 富集 · P5 monitoring sweep。
