...
- **⬜ 下一步**：P3 510(k)→`attributes.intent.events[{type:'FDA_CLEARANCE', at:decision_date}]` 动 Intent 维（镜像 TED intent 投影）· P4 `attributes.fda.*` 富集 · P5 monitoring sweep · FDA 分类扩 `foiclass.zip` 全表种子（resolver/宽网机制已就位，扩种子即生效）。
