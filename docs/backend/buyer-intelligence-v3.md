...
- **评分升级**：六维**加法 → 乘法门**——`priority = Fit^a × (1+Intent) × Reachability × (1+DemandProof)`，让**可达性=0 或需求证据=0 的线索不被高 Fit 冲淡误排前**。
...
**P0 —— 零/极少新基建，最高 ROI（全靠复用）**
1. **数字足迹富集** `digital-footprint.provider.ts`：从已抓 HTML 解析技术栈/广告像素/hreflang + MX/crt.sh/RDAP + Meta Ad Library。→ 喂 Intent/Reachability。
2. **结构化收割**（JSON-LD/sitemap/RSS via extruct）：免费补公司事实 + JobPosting 招聘信号。
3. **自建邮箱验证管线**（MX+SMTP RCPT）：把已抽的决策人邮箱 `UNVERIFIED→VALID`。
4. **网站变更 = intent 引擎**：复用 `source_entity_change` diff 盯目标公司产品/招聘/求供应商页。
