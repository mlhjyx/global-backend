...
| **P0** | **Wikidata SPARQL** | WDQS 直连，按行业 P452 + 国家 P17 枚举，**零爬取**直接返回名+官网 P856+员工数 P1128+坐标 | 名/官网/员工数/行业/母公司/LEI/坐标 | 🟢 CC0 | `WikidataDiscoveryProvider` + `wikidata.sparql` Tool（**已实现+实测**） |
...
| **P0** | **OpenStreetMap** | Overpass API 按工业标签(landuse=industrial/craft=metal_*/man_made=works)+地区，或 SearXNG osm 引擎 | 名/精确坐标/地址/OSM标签/有时官网 | 🟢 ODbL | `OsmDiscoveryProvider` + `osm.overpass` Tool（**已实现+实测**） |
| **P0** | 行业协会会员名录（VDMA ~3500 家/AMT） | SearXNG general/files 定位会员目录/PDF → Crawl4AI 分页 → Gemini **列表抽取**（一页多公司） | 名/官网/地址/sector/机械 nomenclature | 🟢 公开会员册 | `DirectoryDiscoveryProvider`（industry_data，**已实现**；继续扩源） |
| **P0** | 展会参展商名录（Hannover Messe/EuroBLECH/广交会） | 托管搜索/API 模板优先，余者名录抽取 | 名/官网/展位/产品分类/国家/参展年份(活跃度) | 🟢🟡 逐源审查 | `TradeFairDiscoveryProvider`（industry_data，**已实现**；继续扩展会） |
...
| **P1** | EU TED 招投标 | 官方开放 API，按 CPV 品类反查需求方/中标方 | 采购方/CPV/标的/金额/中标方 | 🟢 CC BY 4.0，须署名 | `TedDiscoveryProvider` + `TENDER_PUBLISHED` ingest/projection（**已实现**） |
| **P1** | SAM.gov Sources Sought | keyless Contract Opportunities CSV，按 NAICS 过滤 | 美国联邦机构的招标前市场调研需求 | 🟢 美国政府作品；联系官字段剔除 | ingest-once + `US_FED_SOURCES_SOUGHT` projection（**已实现，provider 默认 DISABLED**） |
...
| **P1（上下文）** | 海关贸易 **国家级**（UN Comtrade/Eurostat Comext） | 官方免费 REST/CSV | 国家×HS 进出口额，**无公司名** | 🟢 官方免费 | 目标 `comtrade` adapter（trade_data）；**当前未实现**，只规划市场/HS 优先级而非买家名单 |
...
**第一版目标（尚未实现）**：`trade_data` 类拟只落**国家级上下文**（Comtrade/Eurostat），明确标注无公司名、不进公司发现主流程；**公司级不接付费 reseller、不自建 manifest 抓取**。当前只有 `SourceClass`/sandbox 契约插槽，没有生产 `data_provider` seed、ProviderAdapter、ToolBroker 工具或 Temporal 消费链。对 TRUMPF（卖设备给私营金属加工厂）场景，贸易数据本就匹配偏弱（提单多是成品/零件流而非设备采购），故整体列为**上下文/信号层**而非主发现源。

> **⚠️ 更新（2026-07-07，见 [buyer-intelligence-v3.md §A1/§10.1](../research/buyer-intelligence-v3.md)）**：上面「公司级只能付费」的结论**部分放宽**——对**中国工厂找海外买家**的主用户场景，美国海运提单**法定公开**（19 USC §1431），存在免费公司级通道：**ImportYeti 免费按公司名搜**（~50 票/公司顶，无 HS 反查/无 CSV/无免费 API）+ **Data Liberation Project 的 FOIA 提单开放数据集**（真免费可再分发，做离线基线）+ 逆向 ImportYeti 内部 JSON API 做 HS 反查（crawl4ai capture）【**已撤回：违反 ToS，不得实施**；2026-09-23 更正，见 [展会情报实施记录的 Algolia 结论](../implementation-records/trade-fair-intelligence.md)】。付费商卖的是「检索+聚合」不是数据授权。美国以外交易级仍基本付费。consignee 含自然人时 🔴 隔离。

## 与现有管线的集成点

1. **路由 fan-out（已实现）**（`discovery.activities.ts executeQuery`）：`routeCompanyDiscovery` 返回该 source_class 下全部 ENABLED 适配器并由 `Promise.allSettled` 并行召回；只有 `filters.source_hint` 存在时才收窄具体子源。所有源产出统一进 `raw_source_record` → `canonicalizeRun` 去重归并。
