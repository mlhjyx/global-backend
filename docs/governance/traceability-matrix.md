...
|---|---|---|---|---|---|---|---|
| 公共 Shell / 今日 | `CAP-SHELL-001`、`CAP-ID-001`、`CAP-ONB-001`、`CAP-TODAY-001` | `JRN-FE-001/007/008`；`JOB-FE-001/005` | `PAGE-FE-001..010` | `OBJ-FE-001/002/025/026` | `OWN-SAAS-PLATFORM` | `PRODUCT=APPROVED_IA / SOURCE=LOCAL_SOURCE_AUTHORITY_FOUND / REMOTE_CI_RELEASE=UNVERIFIED / RUNTIME_UAT=UNVERIFIED`；capability manifest 未实现 | [页面目录](../frontend/04-page-and-capability-catalog.md) |
...
| 独立站管理 | `CAP-SITE-001..005` + Site child IDs | `JRN-FE-001..003/006/007`；`JOB-FE-003..005` | `PAGE-FE-030..057` | `OBJ-FE-003..008/012..017/027` | 企业事实、Site、SaaS 接缝分层 | `PRODUCT/UX=SPEC_READY_WITH_BLOCKERS / GROWTHOS_SOURCE=LOCAL_SOURCE_AUTHORITY_FOUND / BACKEND_SOURCE=PARTIAL_AS_BUILT / TEST=NOT_RUN_NOW / REMOTE_CI_RUNTIME_RELEASE_UAT=UNVERIFIED` | [全局规则](../frontend/README.md) + [Phase 5 Site Pack](../frontend/modules/independent-site-management/README.md) |
| 增长执行 | `CAP-CAMP-001`、`CAP-CONTENT-001`、`CAP-PUBLISH-001` | `JRN-FE-006/008`；`JOB-FE-008/009` | `PAGE-FE-070..079` | `OBJ-FE-018/019/024/025` | `OWN-SAAS-PLATFORM` | `PRODUCT=NOT_OFFERED_CURRENT_MVP / SOURCE=LOCAL_ROUTE_PAGE_OBSERVATION_ONLY / API=NONE / TEST_RUNTIME_RELEASE_UAT=NOT_RUN_OR_UNVERIFIED` | [页面目录](../frontend/04-page-and-capability-catalog.md) |
| 互动与商机 | `CAP-ENGAGE-001`、`CAP-OPP-001` | `JRN-FE-005/008`；`JOB-FE-010/011` | `PAGE-FE-080..083` | `OBJ-FE-020..022/027/028..030` | `OWN-SAAS-PLATFORM` / Program C | `PRODUCT=APPROVED / CONVERSATION_SOURCE=LOCAL_SOURCE_AUTHORITY_FOUND / RECEIPT_SNAPSHOT_OPPORTUNITY_ACCEPTANCE_OUTCOME_SOURCE=NOT_IMPLEMENTED / TEST=NOT_RUN / RUNTIME_RELEASE_UAT=UNVERIFIED` | [页面目录](../frontend/04-page-and-capability-catalog.md) |
| 洞察 | `CAP-INSIGHT-001` | `JRN-FE-008`；`JOB-FE-012` | `PAGE-FE-084..086` | `OBJ-FE-023` + read models | `OWN-SAAS-PLATFORM` | `PRODUCT=NOT_OFFERED_CURRENT_MVP / SOURCE=LOCAL_ROUTE_PAGE_OBSERVATION_ONLY`；Site cost 子集存在；平台 API/test/runtime/release/UAT none/unverified | [页面目录](../frontend/04-page-and-capability-catalog.md) |
| 团队/集成/设置/运营 | `CAP-INTEG-001`、`CAP-TEAM-001`、`CAP-SET-001`、`CAP-ADMIN-001` | 多旅程横切 | `PAGE-FE-090..096` | `OBJ-FE-001/002/024..026` | `OWN-SAAS-PLATFORM` | `SOURCE=LOCAL_SOURCE_AUTHORITY_FOUND / REMOTE_CI_RELEASE=UNVERIFIED / RUNTIME_UAT=UNVERIFIED`；customer Billing/Credits `DEFERRED/NOT_IMPLEMENTED` | [页面目录](../frontend/04-page-and-capability-catalog.md) |
...
| `TRC-FE-STATE-001` | Site/Build/Release/Public service | Object lifecycle + OpenAPI/code | [当前状态规范](../frontend/07-state-error-degradation-and-recovery.md) + Site 四层映射是 `NORMATIVE_SPEC_ONLY`；正式设计、组件实现、E2E、runtime/release/UAT `NOT_IMPLEMENTED/NOT_RUN` | `OWN-DESIGN` / `BLK-FE-001/002/006` |
...
| `TRC-FE-QA-001` | 所有 UI scenarios | tests + scenario catalog | Design system/a11y/visual/performance 是 `NORMATIVE_SPEC_ONLY`；component/token implementation、executable fixtures、frontend E2E、a11y/visual/perf/security tests、release evidence、UAT 均 `NOT_IMPLEMENTED/NOT_RUN` | `OWN-QA-EVIDENCE` / `BLK-FE-001/002/006` |
