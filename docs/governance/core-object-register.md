...
本表是跨产品域 Object ID、System of Record、社会属性和业务生命周期的唯一登记。Phase 2 [对象生命周期与 SoR](../roadmap/saas-frontend-phase-2/object-lifecycle-and-sor-register.md)保留论证 provenance；Prisma/OpenAPI/事件仍是机器实现真值。
...
这些 blocker 不由 Phase 3 补猜；它们进入 [冲突登记](conflict-register.md)并在对应 Gate 关闭。
