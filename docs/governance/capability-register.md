...
- `正式能力包` 为未来规范归属；路径不存在时明确写 `FUTURE`，不把 Phase 2 评审包冒充 Dev-Ready 文档。
...
| `CAP-SITE-001` | 在统一 SaaS 中持续管理海外独立站 / 独立站管理 | `APPROVED` 一级区域 | `FLOW_ONLY/PROTOTYPE / MOCK_PROTOTYPE` | 本仓子域部分 `VERIFIED` | 后端 `CONTRACT+REAL_SERVICE`；用户 `INTERNAL_ONLY` | `OWN-PRODUCT` | `FUTURE_PHASE_5_SITE_PACK` |
| `CAP-SITE-002` | 完成 intake、资料、素材、知识与事实准备 / 独立站管理 | `APPROVED` 首个纵切 | `FLOW_ONLY/PROTOTYPE / MOCK_PROTOTYPE` | intake/profile/asset/KB `VERIFIED`；Claim public contract 缺 | `CONTRACT+REAL_SERVICE / INTERNAL_ONLY` | `OWN-PRODUCT` | `FUTURE_PHASE_5_SITE_PACK` |
