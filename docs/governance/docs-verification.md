...
2026-09-21 起不再对 PR 正文做机器检查：原 `nontechnical decision card freshness` context、其 `pull_request_target` 工作流与 `pr-decision-card-status/v4` 解析器已移除（历史合同见 git 历史与[真实验证记录](../ai-development/decision-card-live-validation.md)）。PR 模板保留「给产品负责人的说明」区段，它是作者自述，不被任何 CI 解析，也不产生合并授权或外部证据认证。

真正的 CI、独立审查、未解决讨论和用户授权由执行合并的开发代理在操作前对当前 exact head 分别回读，不依赖 PR 正文或旧事件快照。

该职责拆分不取消任何构建、安全、RLS、迁移或其余 required context，也不改变 RuntimeEvidence、Release Bundle、Pilot/GA 的外部证明规则。源码合并不等于部署或晋级。
