...
字段使用精确的单行 `- ` 项目，其他顶层列表符和续行不能隐藏重复、矛盾或超长字段。
14 个保留字段只允许出现在唯一 canonical 卡片区段内，卡片外的同名顶层字段也失败；
不依赖完整模拟 GitHub 的 character reference 或 inline HTML 呈现来识别第二卡。
卡片标题使用精确二级 ATX 标题；含保留标题文本的 Setext 或其他 ATX 替代标题被保守拒绝，
包括强调、链接或 inline code 修饰，不能隐藏第二张卡片。
标题检测使用独立的去注释/不可见格式字符视图作保守拒绝；该视图不能用于接受 canonical 标题，
也不能用于字段、枚举或提交 SHA 的解析。字段视图始终保留注释边界，防止字符拼接。
说明和代码示例放在卡片字段之前或另一个二级区段；卡片外 fenced code 中的注释符只是示例文本。
