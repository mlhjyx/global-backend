...
> 内容 Owner：`OWN-DESIGN`
> 关联：`CAP-SHELL-001`、`CAP-TODAY-001`、`SCN-FE-SHELL-001..004`、`DEC-FE-P9-019/020`

## 1. Shell 组成

```mermaid
flowchart TB
  TOP["Top bar: Workspace | Search | Quick Create | Work Center | Notifications | Help | User"]
  NAV["Primary nav: 今日 | 企业资料 | 客户开发 | 独立站 | 增长执行 | 互动与商机 | 洞察 | 管理与设置"]
  ...
  ADMIN["Management: Workspace | Members | Delegation | Integrations | AI policy | Security | Billing | Data exit"]
  TOP --> CTX
  NAV --> MAIN
  CTX --> MAIN
  MAIN --> SIDE
...
桌面 Shell 有 7 个全局视觉控件；这不删除既有 8 个 `SHELL-FE-*` 能力 ID。Runs、Approvals、Incidents 在视觉上聚合为 Work Center，但仍保持不同对象、状态、权限和深链。
...
| `SHELL-FE-003` | Work Center / Runs | Build/导入/研究/发布的状态、影响、取消、结果和恢复 | 只显示 spinner；将 ACK 不明写成失败/成功 |
| `SHELL-FE-004` | Work Center / Approvals | 待办、Evidence、范围、提交者、差异、截止、决定深链 | 在聚合卡里绕过域对象和审计决定 |
| `SHELL-FE-005` | Work Center / Incidents | 业务影响、已保留结果、下一步、correlation ID、运营入口 | 原始堆栈/provider 错误作为主文案 |
...
Quick Create 是 allowed-actions 驱动的组合控件，不新建独立业务 SoR；没有可创建对象时不显示。它不能绕过 Intake、Approval、Authorization 或 Provider scope。
...
1. 完整管理员视图固定 8 个一级/38 个二级；普通成员按服务端 capability/entitlement/allowed actions 看到其可发现子集。当前一级、二级、对象 Tab、对象上下文和来源筛选分别表示，避免面包屑把聚合页当对象父级。
...
| 模式 | 优先任务 | 导航策略 |
|---|---|---|
| 宽屏桌面 | 密集列表、比较、编辑、Build/证据/多面板 | 8 个一级常驻；只展开当前一级的 3–8 个二级；对象 Tab 位于对象头下方 |
| 窄屏/平板 | 审批、状态、摘要、轻编辑 | 一级/二级进入可访问抽屉；上下文和动作按优先级收敛 |
| 移动 | 通知、审批、异常、任务跟踪、摘要 | 底栏保留 Today、当前高频域、两个快捷入口和“更多”；“更多”可达全部 8 个一级、帮助、个人和退出；重编辑提供安全桌面接力 |
