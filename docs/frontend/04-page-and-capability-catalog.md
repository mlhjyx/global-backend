...
本索引固定“项目不能漏掉哪些前端表面”。`建议阶段`不是实现状态；Capability、多轴状态和缺口以 [Capability Registry](../governance/capability-register.md)为准，对象/合同仍回到各自事实源。
...
- 模块页面目录只记录差异；完整 page family 的当前归属由本表维护，审批论证保留在 Git/PR。
