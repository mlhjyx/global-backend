...
每个 Bundle 文件先使用以下六个精确元数据键；值必须是真实值，不能保留 `TBD/TODO/待定` 或占位符：

```markdown
> Release ID：REL-20260723-001
> 状态：READY_FOR_RELEASE_REVIEW
> 环境：production
> Release Owner：OWN-QA-EVIDENCE / 实际责任人
> 实现提交：完整 Git SHA
> 发布时间：ISO-8601 时间
```

随后必须使用 `## Identity`、`## Scope`、`## Promise`、`## Source`、`## Evidence`、`## Operations`、`## Data`、`## Rollback and exit`、`## Guides`、`## Approval`、`## Learning` 这 11 个精确二级标题。标题名称属于机器合同，不翻译、不缩写。
...
| Rollback and exit | 可逆范围、数据兼容、旧版本、第三方退出和恢复验证 |
