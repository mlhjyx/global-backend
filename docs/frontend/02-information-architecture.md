...
本规范固定该层级与边界。`PAGE-FE-030..057` 的页面级流程和状态由[独立站管理 Capability Pack](modules/independent-site-management/README.md)承接；未建子区域不得以空白 Tab 冒充可用能力。
...
Site 书面流程已经批准；正式设计/实装前仍应使用 `RES-FE-003..007` 和至少以下任务走查：首次找 Site、从 Today 恢复失败 Build、从 Approval 进入 Claim、跨 Workspace 切换、从 Company 追到 Site 引用、从通知返回原筛选。若真实用户研究推翻入口名称或优先级，先登记 Decision；不能仅改侧栏代码。
