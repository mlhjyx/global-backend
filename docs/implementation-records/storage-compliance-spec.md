...
- **2026-08-10 治理续篇（当前 source 变更集）**：`suppression_record` 新增 fail-closed 的 `PREFERENCE/LEGAL` 保护分类，既有未知原因按 LEGAL 回填，记录只能单向提升且 app role 被撤销 DELETE；类型化 canonicalizer 在任何 DB 写入前拒绝空白、非法邮箱/域名、未知类型与超长值，并让 email/domain/company-name 的写入键与消费匹配键同构。为兼容旧 API 留下的非规范值，所有 suppression 读取边界也使用同一 canonicalizer；原事实不改写、非法旧值继续可见并留待人工治理。新增 domain/company-name suppression 会分块扫描现有公司、按 canonical key 更新命中 id，联系人发现、邮箱猜测、既有 contact point 验证和 Lead accept 终极闸也直接复核 suppression，不只信任派生 status；BLOCKED 邮箱验证在 provider 路由/SMTP 前终止。release/identity-correction 只写带幂等 request、actor、闭合 reason 和时间的 `suppression_decision`，RLS + 复合 FK + app role `SELECT/INSERT` 保证租户隔离与 append-only；DB CHECK 还固定 protection/reason 与 decision/reason 的合法组合。兼容 DELETE 只为偏好类提交 request、恒返回 `deleted=false`；法定记录写拒绝 decision 后 409，且 OpenAPI 显式列出与实际 filter 一致的闭合 409 error code。当前没有“批准并真正解除”的普通 API。两个含审计/PII 的 GET 均采用 cursor page envelope、默认 50、响应硬上限 100。`LeadService.decide(accept)` 的首次请求和已 QUALIFIED 重试都会复核 suppression；DENY 的公司状态派生与 `policy_decision_log` 在 sanctions 等后续异常门之前提交，事务完成后才向调用方返回 409。DENY 不执行 Lead CAS、不写 LeadDecision/LeadQualified/Outbox；通过复核的 accept 重试也不重复写 decision/outbox。若终极闸命中历史 suppression，该同一事务会把公司状态物化为 `SUPPRESSED`，这是禁联事实的安全派生状态，不是 Lead 交付副作用。隔离空库验证覆盖 82 migrations、app role 权限、LEGAL 防降级和跨 workspace 负例；这不是部署或真实数据证据。
