...
- **2026-08-10 治理续篇（当前 source 变更集）**：`suppression_record` 新增 fail-closed 的 `PREFERENCE/LEGAL` 保护分类，既有未知原因按 LEGAL 回填，记录只能单向提升且 app role 被撤销 DELETE；类型化 canonicalizer 拒绝空白、非法邮箱/域名、未知类型与超长值，并让 email/domain/company-name 的写入键与消费键同构。旧 API 留下的非规范值在读取边界同样 canonicalize；原事实不改写、非法旧值继续可见并留待人工治理。新增 domain/company-name suppression 会分块更新命中公司；fit/enrich/signal/watch/contact/guess 六个 backlog 阶段在外部处理前执行同一原始匹配闸并修复 stale status。suppression 创建、联系人发现/邮箱猜测的写事务、邮箱验证回写与 Lead accept 通过同一 workspace 级 PostgreSQL transaction advisory lock 排序；长网络后重读当前 suppression，新禁联已先提交时候选/PII 不落库，SMTP 结果降为 BLOCKED。`LeadService.decide(accept)` 过滤命中 email suppression 的 contact refs，剩余零可达时不执行 Lead CAS、LeadDecision 或 Outbox。`suppression_decision` 分开保存 requested command 与 outcome，DB CHECK 固定合法组合，同 requestId 不同原始 reason 不再被结果折叠。兼容 DELETE 恒为 `deleted=false`，普通 API 不执行真实 release；OpenAPI 声明新操作的 UUID path 和实际可达 400/404/409 统一 error envelope。含审计/PII 的 GET 均采用 cursor page envelope、默认 50、响应硬上限 100。DataRights DENY 的公司状态派生与 `policy_decision_log` 在 sanctions 前提交，事务完成后才返回 409。隔离无卷 PostgreSQL 16 已在最终候选源码上通过 82/82 migration、app role RLS/REVOKE、requested/outcome CHECK、LEGAL 防降级与同 workspace advisory-lock 并发排序；临时容器已经停止并自动删除。该证据不是部署或真实数据证据；本迁移没有 destructive down 脚本，生产回退仍必须依赖发布前备份/恢复演练或经过审查的 forward fix。
