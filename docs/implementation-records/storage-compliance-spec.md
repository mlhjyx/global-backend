...
- **2026-08-10 治理续篇（当前 source 变更集）**：`suppression_record` 新增 fail-closed 的 `PREFERENCE/LEGAL` 保护分类，既有未知原因按 LEGAL 回填，记录只能单向提升且 app role 被撤销 DELETE；release/identity-correction 只写带幂等 request、actor、闭合 reason 和时间的 `suppression_decision`，RLS + 复合 FK + app role `SELECT/INSERT` 保证租户隔离与 append-only。兼容 DELETE 只为偏好类提交 request、恒返回 `deleted=false`；法定记录写拒绝 decision 后 409。当前没有“批准并真正解除”的普通 API。`LeadService.decide(accept)` 的 DENY 改为在业务状态写之前记录 `policy_decision_log`，事务提交后才向调用方返回 409，因而 ALLOW/DENY 都有日志且 DENY 不产生 LeadDecision/LeadQualified。隔离空库验证覆盖 82 migrations、app role 权限、LEGAL 防降级和跨 workspace 负例；这不是部署或真实数据证据。
