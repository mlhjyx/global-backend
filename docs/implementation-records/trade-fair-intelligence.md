> 【Implementation Record 2026-07-10】原设计的 5+1 展会专表方案**已被通用采集监控层（monitored_source 等 4 表）替代/吸收**；合规分级结论（§0 红线）仍有效且被 ADR-010 引用。Algolia 路线暂停加深待 DPIA（决策见 product-scope §9）。

# 展会情报子系统 · 设计（实时监控 + 历史届 + 持续抓取）

> 生命周期：`CLOSED`
> 生命周期依据：展会情报子系统设计；已实现（apps/api/src/discovery/providers/trade-fair.provider.ts 等）
