...
- `AUTH_JWKS_URI` 与 `AUTH_ISSUER` 必须成对配置；controlled runtime 还由 runtime admission 要求 audience 与 canonical HTTPS identity configuration。
...
| 验证 | 结果 | 证明边界 |
| --- | --- | --- |
| authz 聚焦行为合同 | 8 files / 45 tests PASS | role policy、双 guard、Dev/JWKS verifier、controller/OpenAPI 合同 |
| authz 关键模块 coverage | 6 files / 29 tests PASS；statements 95.83%、branches 89%、functions 95%、lines 96.32% | 仅本切片关键 auth 模块，不冒充 `apps/api/src` 全局覆盖率 |
| 全 API Vitest | 293 files PASS；4465 passed / 2 skipped | 现有 API 单元/组件回归未退化；不等同 PostgreSQL/Temporal/live JWKS E2E |
...
| code-intelligence scan/check | 967 files、8690 nodes、19973 edges、0 error、6 warnings；deterministic check PASS | 本轮 dirty 文档树的静态关系辅助；最终 clean exact-head 仍需重扫 |
