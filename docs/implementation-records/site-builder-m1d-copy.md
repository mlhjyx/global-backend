...
槽位类型包含 plain/rich/SEO/CTA/form/alt/legal；预算按 Unicode grapheme 硬判，超限拒绝而非截断。restricted rich text 只允许 doc/paragraph/text/strong/em/list/link，禁 raw HTML；外链须命中精确 HTTPS host allowlist。factual slot 必须引用 snapshot Claim，未知 ref 拒绝；数值、单位与认证 token 不得被翻译或语气步骤静默换算/归一。
...
已知门：带已批准 Claim 的 de-DE 模型路径尚无独立 task-shaped 质量晋级证据，因此仍走现役 copy currentRoute，并继续受 deterministic bundle gate 与 R4-B hard cap 约束；不得据此宣称 MODEL-1 晋级或生产流量验证。
