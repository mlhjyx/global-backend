...
test('runtime artifact scan rejects product test and synthetic-provider bytes', async (t) => {
  ...
  await writeFile(join(root, 'site-builder', 'eval', 'campaign.js'), 'evaluation');
  ...
  await assert.rejects(() => assertRuntimeArtifactClean(root), /forbidden runtime artifacts/i);
});
...
test('renderer runtime bundle is created from a narrow product allowlist', async (t) => {
  ...
  await mkdir(join(source, 'product-assets', 'design-demo-visuals'), { recursive: true });
  ...
  await writeFile(
    join(source, 'product-assets', 'component-catalog-v1', 'hero-banner-spec.json'),
    '{"schemaVersion":"1.1.0"}',
  );
  ...
  const files = (await readdir(target, { recursive: true, withFileTypes: true }))
    ...
    .map((entry) => `${entry.parentPath.slice(target.length + 1)}/${entry.name}`.replace(/^\//, ''))
    .sort();
...
test('managed Worker loads qualified component templates from the product catalog', async () => {
  ...
  assert.doesNotMatch(loader, /["']fixtures["'],\s*["']component-qualification/);
});
...
test('single OCI Dockerfile uses one non-root runtime with api and worker entrypoints', async () => {
  ...
  assert.match(dockerfile, /ENTRYPOINT \["node",\s*"\/app\/runtime-entrypoint\.mjs"\]/);
  ...
  assert.match(dockerfile, /node_modules\/\.pnpm\/node_modules\/@global\/site-renderer/);
  ...
  assert.doesNotMatch(dockerfile, /\/workspace\/apps\/site-renderer \.\/apps\/site-renderer/);
...
test('CI starts the final Worker artifact fail-closed before it can poll Temporal', async () => {
  ...
  assert.match(workflow, /RUNTIME_IMAGE_REFERENCE="global-backend-ci@\$\{LOCAL_IMAGE_DIGEST\}"/);
...
test('production dependency pruning removes tests but preserves runtime modules', async (t) => {
  ...
  await writeFile(join(swaggerFixtures, 'document.base.js'), 'required runtime module');

  assert.equal(await pruneRuntimeDependencyTests(modules), 2);
  await assert.doesNotReject(() => readFile(join(modules, 'package', 'runtime.js')));
  await assert.doesNotReject(() => readFile(join(swaggerFixtures, 'document.base.js')));
  await assert.rejects(() => readFile(join(modules, 'package', 'test', 'case.js')), {
    code: 'ENOENT',
  });
  await assert.rejects(() => readFile(join(modules, 'package', 'src', 'runtime.spec.js')), {
    code: 'ENOENT',
  });
});
...
test('final image verification binds every product component and permits only contained dependency links', async (t) => {
  ...
  const manifestDigest = `sha256:${(await import('node:crypto')).createHash('sha256').update(await readFile(manifestPath)).digest('hex')}`;
  const sbomDigest = `sha256:${(await import('node:crypto')).createHash('sha256').update(await readFile(sbomPath)).digest('hex')}`;
  await writeFile(
    join(root, 'apps/api/dist/build-attestation.json'),
    JSON.stringify({
      artifact_manifest_digest: manifestDigest,
      sbom_digest: sbomDigest,
      renderer_digest: manifest.components.find(({ name }) => name === 'renderer').digest,
    }),
  );
  await mkdir(
  ...
  await mkdir(join(root, 'apps/site-renderer/node_modules'), { recursive: true });
  await writeFile(
    join(root, 'apps/api/node_modules/.pnpm/zod@3.23.8/node_modules/zod/package.json'),
  ...
  for (const deployment of ['apps/api', 'apps/site-renderer']) {
    ...
    await mkdir(join(root, deployment, 'node_modules/@global'), { recursive: true });
  ...
  await assert.rejects(() => assertRuntimeImageValid(root), /renderer component digest mismatch/);
});
...
test('final image verification rejects dependency links escaping their deployment and dev-only packages', async (t) => {
  ...
  await assert.rejects(() => assertRuntimeImageValid(root), /dependency symlink escapes deployment root/);

  await rm(join(root, 'apps/api/node_modules/escape'));
  await mkdir(join(root, 'apps/api/node_modules/.pnpm/vitest@4.1.10'), { recursive: true });
  await assert.rejects(() => assertRuntimeImageValid(root), /DEV_DEPENDENCY_PRESENT/);
});
...
test('Docker build context excludes local credentials while preserving env examples', async () => {
  ...
  for (const pattern of [
    ...
    assert.ok(dockerignore.split(/\r?\n/u).includes(pattern), `missing ${pattern}`);
  }
...
test('artifact manifest binds API, contracts, pruned renderer, SBOM and source tree', async (t) => {
  ...
  assert.ok(
    manifest.source_roots[0].files.every((file) => !file.path.endsWith('.spec.ts')),
  );
