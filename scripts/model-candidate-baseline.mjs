...
export function renderModelCandidateBaselineDocument(baseline) {
  ...
  const lines = [
    ...
    '## 边界',
    '',
    `本基线 ${code(baseline.candidateBaselineId)} 只登记非运行时候选与后续评测顺序。它不修改 ${code('site-builder-model-policy/v3')}、${code('currentRoute')}、${code('promotedRoute')}、rollback、环境变量、Temporal、P4、DesignEvaluation、ReleaseManifest、MediaGateway 或公共 API，也不证明模型质量、生产可用性或部署状态。`,
    '',
