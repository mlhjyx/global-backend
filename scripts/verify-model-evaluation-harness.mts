...
export function renderModelEvaluationHarnessBaseline(): string {
  ...
  return [
    ...
    "- wire/settlement dependencies 在 branding 时固定：transport 只能由 factory 使用 attested credential handle 的 bearer secret、HTTPS base URL 与 fetch 构造，不再接受独立预配置 client 加身份元数据；secret 在闭包中捕获，外部修改 handle 不能切回历史宽权限 token。cost settlement 只认显式注入、带 canonical resolverId 的 resolver；factory 冻结真实 resolver receiver，并捕获/bind validated resolverId 与 resolve 函数引用，既保留 class/private-state receiver 语义，也禁止创建后替换公开身份/方法。settled basis 以 `<basis>@<resolverId>` 绑定可审计 resolver/价格快照身份，且每个 settled resolution 必须回传与 resolver context 完全相同的 `executionId`，缺失或错绑即 invalid/unknown。wire request 与 resolver context 共用 harness 生成的稳定 `executionId`，使 verified billing row 精确绑定 campaign/probe 或 fixture/attempt；resolver 只收到独立深冻结 usage/cost observation，不能改写返回 evidence。token 与聚合值必须是 safe integer。`provider_reported` 必须闭合全部 call cost observation；`frozen_pricing_snapshot` 必须有完整 usage 与 callCount，partial repair usage 不可结算；`verified_billing_export` 表示独立核验且覆盖该 executionId 整个 attempt 的账单依据，可不依赖 usage 完整性。没有上述依据即 `unknown`，绝不记 0。`provider_attested_not_incurred` 只允许首个 dispatch 失败、callCount=1 且唯一 provider cost observation 为 null；任何已有成功/成本观察后的 repair failure 都强制为 unknown。repair 失败时 resolver 仍收到既有 token 合计、完整 callCount 与 `complete=false`，不能把未知部分当 0；成功 repair 的 usage 与 callCount 必须完整合并。",
    "",
    ...
    "- attestation branding 只证明输入满足机器合同，不冒充 new-api 管理面真值。后续真实 evidence runner 必须从固定提交读取并保存脱敏凭据管理快照及其 SHA-256、价格快照和 resolver 实现，先核验真实 token quota/scope 再构造 attestation；还必须为 executor 提供 authorization 已绑定 ledgerId + absolute-directory SHA-256 的 append-only、fsync 文件账本，换目录不能重新开户。authorizationId 以 exclusive-create claim，claim/reserve/settle/freeze 全部持久记录；进程崩溃后该 authorization 永久 fail-closed，不能重领整份 campaign 额度。本 PR 不提供绕过这些步骤的默认值、环境变量读取或 live introspection。",
    "- harness 在任何 budget reserve/client dispatch 前重新核对 target + canonical legacy comparator 完整 scope 与 canonical plan，并确认完整矩阵所需的 execution/wire-call headroom；executor 在每个请求和每次实际 wire call 前重新核对 alias、协议、实际 prompt bytes、output 上限、冻结/abort 状态与冻结价格的最坏费用，repair 扩展 prompt 不能沿用首调字节数，并保守占用 repair 最坏调用数。每次响应还必须核对 provider-reported output usage 没有超过 request/attestation 上限，越界输出不进入 artifact/evidence。executor 的 authorization 级 execution/wire/费用账本不因换一个 budget guard 而重置；harness 到达 hard stop 时立即通过 trusted executor identity 冻结同一 authorization，即使底层迟迟不响应 abort、随后返回 schema-invalid 结果，也不能以 repair 或新 budget 继续 dispatch。`executionId` 是唯一 request billing identity。缺 request 级可核验结算、冻结价格金额漂移或出现 unknown 时继续沿用 `freeze_campaign`，不得记零或继续下一调用。媒体 generic channel test 固定 forbidden。",
