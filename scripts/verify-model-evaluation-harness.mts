...
export function renderModelEvaluationHarnessBaseline(): string {
  ...
  return [
    ...
    `- source bundle contract：${code(brandSuite.sourceBundleContractId)}；固定 ${brandSuite.sourceBundleFiles.length} 份 Git 跟踪的源码/合同文件（不引用 ignored build output），路径条目深度冻结且禁止绝对/逃逸路径，同一比较组必须固定为一个 source bundle SHA-256，且每次调用完成后必须重新指纹`,
    ...
    "### design_spec suite",
    "",
    `- suite：${code(designSuite.suiteId)}；adapter ${code(designSuite.adapterId)}；fixture set ${code(designSuite.fixtureSetId)} / schema ${code(designSuite.fixtureSchemaVersion)}；${designSuite.fixtureIds.length} fixtures × ${designSuite.repeats} repeats；task system prompt SHA-256 ${code(designSuite.systemPromptSha256)} 纳入 task-contract fingerprint，executor 只发送模块加载时捕获的同一字符串，运行前导出对象漂移会在 budget reserve/client 前拒绝。`,
    ...
    `- create-only runner 仅允许 ${code("docs/evidence/site-builder/")} 新路径，并只接受本进程真实 builder 私有品牌的 build attestation；在导入 suite 前删除 ignored 的 contracts build output，按 fixed commit 本地重建 ${code("@global/contracts")}，同时冻结全部 tracked contracts source tree 与实际加载的 ${code("dist/**/*.js")} tree。artifact-tree canonical serializer 不调用实时 Array ${code("map")} / ${code("join")} / ${code("sort")}，运行中原型篡改不能同时伪造 expected/observed digest。其 ${designSuite.compiledContractsRuntimeBinding?.compiledArtifactCount ?? 0} 个 artifact / ${code(designSuite.compiledContractsRuntimeBinding?.compiledArtifactTreeSha256 ?? "unbound")} 摘要进入 suite、case、probe、run 与候选汇总；harness 加载时以及首调/repair 每个物理 wire call 紧邻调用前后均重新指纹。调用前漂移按 ${code("rejected_before_dispatch")} 拒绝；即使首个物理 wire 尚未发生，reservation 后发现 runtime drift 也先持久冻结 authorization；首调后漂移会在 repair 前完成已有调用的结算并冻结 authorization，不会发送第二次付费调用；调用中漂移使返回结果成为 ${code("provenance_invalid")}。`,
    "",
    "## 闭合结果与排序",
    "",
