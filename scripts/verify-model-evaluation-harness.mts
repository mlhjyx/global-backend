...
export function renderModelEvaluationHarnessBaseline(): string {
  ...
  return [
    ...
    `- source bundle contract：${code(brandSuite.sourceBundleContractId)}；固定 ${brandSuite.sourceBundleFiles.length} 份仓库内源码/合同文件，路径条目深度冻结且禁止绝对/逃逸路径，同一比较组必须固定为一个 source bundle SHA-256`,
