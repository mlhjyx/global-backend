...
export function renderModelEvaluationHarnessBaseline(): string {
  ...
  return [
    ...
    `- create-only runner 仅允许 ${code("docs/evidence/site-builder/")} 新路径，并只接受本进程真实 builder 私有品牌的 build attestation；在导入 suite 前删除 ignored 的 contracts build output，按 fixed commit 本地重建 ${code("@global/contracts")}，同时冻结全部 tracked contracts source tree 与实际加载的 ${code("dist/**/*.js")} tree。其 ${designSuite.compiledContractsRuntimeBinding?.compiledArtifactCount ?? 0} 个 artifact / ${code(designSuite.compiledContractsRuntimeBinding?.compiledArtifactTreeSha256 ?? "unbound")} 摘要进入 suite、case、probe、run 与候选汇总；harness 加载时以及首调/repair 每个物理 wire call 紧邻调用前后均重新指纹。调用前漂移按 ${code("rejected_before_dispatch")} 拒绝；首调后漂移会在 repair 前完成已有调用的结算并冻结 authorization，不会发送第二次付费调用；调用中漂移使返回结果成为 ${code("provenance_invalid")}。`,
    "",
    "## 闭合结果与排序",
    "",
