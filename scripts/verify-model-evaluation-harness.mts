...
export function renderModelEvaluationHarnessBaseline(): string {
  ...
  return [
    ...
    "- `openai-responses` 与 `anthropic-messages` 仅接受 candidate baseline 中 task pool 的精确 runnable alias+protocol；`openai-chat-completions` 只有隔离的 legacy comparator 入口，且 alias 必须属于该 task 冻结的 legacy current/rollback route，其他 task 的 legacy-only alias 不能混入 comparator，更不能进入 target dispatch。",
    ...
    "- cost settlement 只认显式注入、带 canonical resolverId 的 resolver；factory 在 branding 时一次性捕获并冻结 validated resolverId 与 resolve 函数引用，创建后替换属性/getter 不能漂移算法或身份。settled basis 以 `<basis>@<resolverId>` 绑定可审计 resolver/价格快照身份，缺该身份即 invalid/unknown。resolver 只收到独立深冻结 usage/cost observation，不能改写返回 evidence；token 与聚合值必须是 safe integer。`provider_reported` 必须闭合全部 call cost observation；`frozen_pricing_snapshot` 必须有完整 usage 与 callCount，partial repair usage 不可结算；`verified_billing_export` 表示独立核验且覆盖整个 attempt 的账单依据，可不依赖 usage 完整性。没有上述依据即 `unknown`，绝不记 0。`provider_attested_not_incurred` 只允许首个 dispatch 失败、callCount=1 且唯一 provider cost observation 为 null；任何已有成功/成本观察后的 repair failure 都强制为 unknown。repair 失败时 resolver 仍收到既有 token 合计、完整 callCount 与 `complete=false`，不能把未知部分当 0；成功 repair 的 usage 与 callCount 必须完整合并。",
    "",
