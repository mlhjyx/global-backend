...
function company(over: Record<string, unknown> = {}) {
  return {
    ...
    dedupeKey: 'd:pump.example', industry: 'pumps', employeeCount: 100, status: 'ENRICHED', attributes: {}, contacts: [], ...over,
  };
}
...
describe('LeadService decision guards and handoff', () => {
  ...
  it('rechecks accept gates, short-circuits an idempotent target state, and rejects a lost CAS', async () => {
    const accepted = harness();
    accepted.tx.lead.findUnique.mockResolvedValue(lead({ status: 'QUALIFIED' }));
    await expect(accepted.service.decide(ctx as never, 'lead-1', 'accept')).resolves.toMatchObject({ status: 'QUALIFIED' });
    expect(accepted.tx.suppressionRecord.findMany).toHaveBeenCalledTimes(1);
    expect(accepted.tx.lead.updateMany).not.toHaveBeenCalled();

    const rejected = harness();
    rejected.tx.lead.findUnique.mockResolvedValue(lead({ status: 'REJECTED' }));
    await expect(rejected.service.decide(ctx as never, 'lead-1', 'reject')).resolves.toMatchObject({ status: 'REJECTED' });
    expect(rejected.tx.lead.updateMany).not.toHaveBeenCalled();
  ...
  it('creates a minimized LeadQualified snapshot after allowed rights and clear screening', async () => {
    ...
    const named = company({
      contacts: [{
        id: 'contact-1', fullName: 'Jane Buyer', title: 'Procurement', seniority: 'director', department: 'procurement',
