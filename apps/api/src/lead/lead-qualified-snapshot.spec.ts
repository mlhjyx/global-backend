...
function contractSchemaPath(): string {
  ...
  throw new Error(
    "lead-qualified.v1.schema.json not found — contract file missing",
  );
}
...
describe("buildLeadQualifiedSnapshot — 快照 v1（Consumer Test：契约 schema 校验）", () => {
  it("完整 lead+company+contacts → ajv 校验通过，字段映射正确", () => {
    ...
    expect(snap.evidence_refs).toEqual({
  ...
  it("未评分 lead（scores=null/totalScore=null）→ 各维 null、total null，仍过 schema", () => {
    ...
    expect(snap.evidence_refs).toEqual({
      score_detail_available: false,
      fit_reasons_available: false,
    });
  });
  ...
  it("schema 拒绝缺 required 字段与多余字段（additionalProperties:false）", () => {
    const { validate } = loadValidator();
    const good = buildLeadQualifiedSnapshot({
      lead: makeLead(),
      company: makeCompany(),
      icpVersion: 3,
    });
    ...
    const leakedContact = JSON.parse(JSON.stringify(good)) as {
...
function makeDecideTx(
  ...
  return {
    ...
    lead: {
      ...
      updateMany: vi.fn(
        async ({
          ...
          if (where.version !== undefined && lead.version !== where.version)
            return { count: 0 };
    ...
    identityLink: {
      findMany: vi.fn(
        async (): Promise<Array<{ rawRecord: { providerKey: string } }>> => [
          { rawRecord: { providerKey: "wikidata" } },
        ],
      ),
    },
    suppressionRecord: {
      findMany: vi.fn(
        async (): Promise<Array<{ type: string; value: string }>> => [],
      ),
    },
    ...
    fieldEvidence: {
      groupBy: async () => {
        ...
        for (const r of evidenceRows) {
          ...
          if (cur === undefined || toMs(r.fetchedAt) < toMs(cur))
            minByClass.set(r.dataClass, r.fetchedAt);
        }
        return [...minByClass].map(([dataClass, fetchedAt]) => ({
          dataClass,
          _min: { fetchedAt },
        }));
      },
...
function makeDecideService(
  tx: unknown,
  rights: { effect: string; allowed: boolean } = {
    effect: "ALLOW",
    allowed: true,
  },
  ...
  const prisma = {
    withWorkspace: async (_ws: string, fn: (t: unknown) => Promise<unknown>) =>
      fn(tx),
  };
...
describe("LeadService.decide(accept) — 同事务取数、payload=快照（对旧代码 RED）", () => {
  it("治理隔离隐藏 USAspending 历史来源后零 ACTIVE 来源也 fail-closed，且不写 LeadQualified", async () => {
    const outboxCreate = vi.fn();
    const decisionCreate = vi.fn();
    const tx = makeDecideTx(
      makeLead(),
      makeCompany(),
      outboxCreate,
      decisionCreate,
    );
    tx.identityLink.findMany = vi.fn(async () => []);

    await expect(
      makeDecideService(tx).decide(decideCtx, LEAD_ID, "accept"),
    ).rejects.toMatchObject({
      response: { error: { code: "HISTORICAL_RESEARCH_ONLY" } },
    });
  ...
  it("根企业及活动别名只有 USAspending ACTIVE Raw 来源时拒绝交棒且不写业务事实", async () => {
    const aliasId = "66666666-6666-6666-6666-666666666666";
    const outboxCreate = vi.fn();
    const decisionCreate = vi.fn();
    const tx = makeDecideTx(
      makeLead(),
        ...
        findMany: vi.fn(async () => [
          {
            ...
            sourceCompanyId: aliasId,
          },
        ]),
      },
    });
    tx.identityLink.findMany = vi.fn(async () => [
      { rawRecord: { providerKey: "usaspending_awards" } },
      { rawRecord: { providerKey: "usaspending_awards" } },
    ]);
    ...
    expect(tx.identityLink.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          canonicalId: { in: [COMPANY_ID, aliasId] },
  ...
  it("USAspending 之外存在独立 ACTIVE Raw 来源时继续原有人工交棒链", async () => {
    const outboxCreate = vi.fn(
  ...
  it("商业交棒先取得 suppression + identity 锁，再读取 Lead 与身份图", async () => {
    const order: string[] = [];
    const tx = makeDecideTx(makeLead(), makeCompany(), vi.fn(), vi.fn());
    tx.$queryRaw = vi.fn(async () => {
      order.push("lock");
      return [] as unknown[];
    });
    tx.lead.findUnique = async () => {
      order.push("lead");
      return { ...makeLead() };
    };
    Object.assign(tx, {
      organizationCanonicalMapping: {
        findFirst: vi.fn(async () => {
          order.push("mapping-source");
          return null;
        }),
        findMany: vi.fn(async () => {
          order.push("mapping-aliases");
          return [];
        }),
      },
    });
    ...
    expect(tx.$queryRaw).toHaveBeenCalledTimes(3); // suppression、identity、company row
  });
  ...
  it("accept → outboxEvent.create payload 为 v1 快照且 schemaVersion=1", async () => {
    ...
    expect(outboxCreate).toHaveBeenCalledTimes(1);
    const created = outboxCreate.mock.calls[0][0].data as Record<
      string,
      unknown
    >;
  ...
  it("#72 P2：accept 在交棒事务内写存储权利审计（logDecision 收同一 tx + subject/actor，且在 outbox 写之前）", async () => {
    const outboxCreate = vi.fn(
      async ({ data }: { data: Record<string, unknown> }) => data,
    );
    ...
    expect(logDecision.mock.calls[0][0]).toBe(tx); // 🔴 同一交棒事务 tx（审计与交棒原子）
    expect(logDecision.mock.calls[0][1]).toBe(WS); // workspaceId
    expect(logDecision.mock.calls[0][4]).toMatchObject({
      subjectType: "lead",
      subjectId: LEAD_ID,
      actorId: "user-1",
    });
    // 审计先于 LeadQualified 交棒写（Codex："log ... before creating the outbox event"）。
    expect(logDecision.mock.invocationCallOrder[0]).toBeLessThan(
      outboxCreate.mock.invocationCallOrder[0],
    );
  });
  ...
  it("DENY 路径提交审计后再拒绝，且不修改 Lead、不建人工裁决、不发 handoff", async () => {
    ...
    await expect(
      svc.decide(decideCtx, LEAD_ID, "accept"),
    ).rejects.toMatchObject({
    ...
    expect(logDecision.mock.calls[0][3]).toMatchObject({
      effect: "DENY",
      allowed: false,
    });
    expect(logDecision.mock.calls[0][4]).toMatchObject({
  ...
  it("email suppression 命中的联系人不进 LeadQualified，其他可达联系人仍可交付", async () => {
    ...
    tx.suppressionRecord.findMany = vi.fn(async () => [
      { type: "email", value: " MAX@ACME-PUMPEN.DE " },
    ]);

    await makeDecideService(tx).decide(decideCtx, LEAD_ID, "accept");

    const payload = outboxCreate.mock.calls[0][0].data.payload as {
      contact_refs: { contact_id: string }[];
    };
    ...
    expect(
      payload.contact_refs.some((ref) => ref.contact_id === CONTACT_ID),
    ).toBe(false);
  });
      ...
      contacts: [
        {
          ...
          contactPoints: [
            { status: "VALID", type: "email", value: "max@acme-pumpen.de" },
          ],
        },
      ],
    });
      ...
      contacts: [
        {
          ...makeCompany().contacts[0],
          contactPoints: [
            { status: "VALID", type: "email", value: "buyer@agency.example" },
          ],
        },
      ],
    });
  ...
  it("contact_key 命中的无邮箱具名人不进入 LeadQualified", async () => {
    ...
    const key = blindContactKey(
      contactSuppressionKeys("Max Mustermann", "d:acme-pumpen.de")[0],
    );
    const tx = makeDecideTx(makeLead(), company, outboxCreate, decisionCreate);
    tx.suppressionRecord.findMany = vi.fn(async () => [
      { type: "contact_key", value: key },
    ]);
  ...
  it("H：含具名人 refs 的快照 → 事件 privacyClassification=RESTRICTED；无联系人 → CONFIDENTIAL", async () => {
    // 有联系人分支
    const withContacts = vi.fn(
      async ({ data }: { data: Record<string, unknown> }) => data,
    );
    await makeDecideService(
      makeDecideTx(makeLead(), makeCompany(), withContacts, vi.fn()),
    ).decide(decideCtx, LEAD_ID, "accept");
    expect(
      (withContacts.mock.calls[0][0].data as Record<string, unknown>)
        .privacyClassification,
    ).toBe("RESTRICTED");

    // 无联系人分支
    const noContacts = vi.fn(
      async ({ data }: { data: Record<string, unknown> }) => data,
    );
    await makeDecideService(
      makeDecideTx(
    ...
    expect(
      (noContacts.mock.calls[0][0].data as Record<string, unknown>)
        .privacyClassification,
    ).toBe("CONFIDENTIAL");
  });
});
...
describe("LeadService.decide — 幂等短路 + 并发 CAS（C）", () => {
  it("C 回归：同状态重复 accept → 只 1 条 outbox 事件 + 1 条 decision（第二次直接返回现状）", async () => {
    ...
    const svc = makeDecideService(
      makeDecideTx(lead, makeCompany(), outboxCreate, decisionCreate),
    );

    await svc.decide(decideCtx, LEAD_ID, "accept");
    const second = (await svc.decide(decideCtx, LEAD_ID, "accept")) as Record<
      string,
      unknown
    >;
  ...
  it("QUALIFIED→reject 的人工改判仍允许（不落幂等短路）", async () => {
    ...
    const r = (await svc.decide(decideCtx, LEAD_ID, "reject")) as Record<
      string,
      unknown
    >;
...
describe("classifyLeadQualified — 分级按内容定（H，与快照同源）", () => {
  it("有 contact_refs → RESTRICTED；空 → CONFIDENTIAL", () => {
    const withRefs = buildLeadQualifiedSnapshot({
      lead: makeLead(),
      company: makeCompany(),
      icpVersion: 1,
    });
...
describe("valid_until 鲜度模型 v2（computeValidUntil + decide 接线）", () => {
  ...
  it("单条 green evidence → fetchedAt + 180d", () => {
    ...
    expect(computeValidUntil([{ dataClass: GREEN, fetchedAt }], {})).toBe(
      iso(Date.parse(fetchedAt) + 180 * day),
    );
  });

  it("取 min：同刻抓取的 red(90d) 比 green(180d) 先失效 → valid_until = red 到期", () => {
    const fetchedAt = "2026-01-01T00:00:00.000Z";
    const v = computeValidUntil(
      [
        { dataClass: GREEN, fetchedAt },
        { dataClass: RED, fetchedAt },
      ],
      {},
    );
  ...
  it("未知分级 → 保守取默认 90d", () => {
    const fetchedAt = "2026-01-01T00:00:00.000Z";
    expect(computeValidUntil([{ dataClass: "unknown", fetchedAt }], {})).toBe(
      iso(Date.parse(fetchedAt) + 90 * day),
    );
  });
  ...
  it("intent 取最新事件 at + 时机窗(90d)，比 evidence 更早时它决定 valid_until", () => {
    ...
    const v = computeValidUntil(
      [{ dataClass: GREEN, fetchedAt: evFetched }],
      attrs,
    );
  ...
  it("intent 无逐事件但有 last_change_at → 用它 + 时机窗", () => {
    ...
    expect(computeValidUntil([], { intent: { last_change_at: at } })).toBe(
      iso(Date.parse(at) + 90 * day),
    );
  });
  ...
  it("不可解析 fetchedAt 跳过（不注入 NaN）；仅剩 intent 决定", () => {
    const at = "2026-03-01T00:00:00.000Z";
    const v = computeValidUntil(
      [{ dataClass: GREEN, fetchedAt: "not-a-date" }],
      { intent: { events: [{ at }] } },
    );
  ...
  it("全部不可解析且无 intent → null", () => {
    expect(
      computeValidUntil([{ dataClass: GREEN, fetchedAt: "x" }], {}),
    ).toBeNull();
  });

  it("Date 对象 fetchedAt 也支持（Prisma 返回 Date）", () => {
    const d = new Date("2026-01-01T00:00:00.000Z");
    expect(computeValidUntil([{ dataClass: GREEN, fetchedAt: d }], {})).toBe(
      iso(d.getTime() + 180 * day),
    );
  });
  ...
  it("decide 接线：fieldEvidence.groupBy 返回每分级最早证据 → payload.valid_until 落值", async () => {
    ...
    await svc.decide(decideCtx, LEAD_ID, "accept");
    const payload = (
      outboxCreate.mock.calls[0][0].data as Record<string, unknown>
