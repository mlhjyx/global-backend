...
import type {
  DataRightsContext,
  DataRightsDecision,
} from "../compliance/data-rights.types";
import type { PolicyDecisionMeta } from "../compliance/data-rights.service";
import {
...
export class LeadService {
  ...
  async decide(
    ...
    try {
      return await this.prisma.withWorkspace(ctx.workspaceId, async (tx) => {
        ...
        if (action === "accept") {
          ...
          const evidenceByClass = await tx.fieldEvidence.groupBy({
            ...
            where: {
              entityId: {
                in: [company.id, ...company.contacts.map((c) => c.id)],
              },
            },
            _min: { fetchedAt: true },
          });
          ...
          if (!rights.allowed) {
            throw new StorageRightsDeniedAudit(rightsCtx, rights, {
              subjectType: "lead",
              subjectId: leadId,
              actorId: ctx.userId,
            });
      ...
      if (error instanceof StorageRightsDeniedAudit) {
        // The rejected handoff transaction has rolled back at this point. Write
        // the denial in a fresh tenant transaction; if audit persistence fails,
        // propagate that failure rather than returning an unaudited decision.
        await this.dataRights.logDecisionForWorkspace(
          ctx.workspaceId,
          error.context,
          error.decision,
          error.meta,
        ...
        throw new ConflictException({
          error: {
            ...
            message: `storage rights ${error.decision.effect} — handoff blocked pending approval/lawful basis`,
          },
        });
      }
      throw error;
...
class StorageRightsDeniedAudit extends Error {
  constructor(
    readonly context: DataRightsContext,
    readonly decision: DataRightsDecision,
    readonly meta: PolicyDecisionMeta,
  ) {
    super("STORAGE_RIGHTS_NOT_GRANTED");
    this.name = "StorageRightsDeniedAudit";
  }
}
