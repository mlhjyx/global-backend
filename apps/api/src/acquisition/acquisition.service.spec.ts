...
function makeHarness(args?: {
  ...
  const prisma = {
    monitoredSource: {
      ...
      update: monitoredSourceUpdate,
    },
    sourceFetch: {
      create: sourceFetchCreate,
    ...
    sourceEntityChange: {
      createMany: sourceEntityChangeCreateMany,
    },
...
describe('AcquisitionService', () => {
  ...
  it('persists and returns only a digest when a source throws arbitrary text', async () => {
    const h = makeHarness({
      fetchError: new Error('provider echoed Jane Doe and private body text'),
    });
  ...
  it('does not accumulate misses when the adapter reaches the fetch limit', async () => {
    ...
    expect(h.adapterFetch).toHaveBeenCalledWith(
      expect.objectContaining({ sourceKey: 'directory:test' }),
      1,
    );
    expect(h.sourceEntityUpdate).not.toHaveBeenCalledWith(
