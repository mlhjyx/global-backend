...
function harness(args?: {
  ...
  const tx = {
    suppressionRecord,
    canonicalCompany: { create: canonicalCreate, update: canonicalUpdate },
    identityLink: {
...
describe('TenantProjectionService', () => {
  ...
  it('creates a suppressed company while withholding a personal mailbox and omitting null evidence', async () => {
    const row = entity({
      cleaned: {
    ...
    expect(h.canonicalCreate).toHaveBeenCalledWith({
    ...
    const attributes = h.canonicalCreate.mock.calls[0][0].data.attributes as Record<string, unknown>;
    expect(attributes).not.toHaveProperty('contact_email');
    expect(h.identityLinkCreate).toHaveBeenCalledTimes(1);
    expect(identityMocks.appendCompanyIdentityDecisionEvidence).toHaveBeenCalledTimes(1);
    expect(h.fieldEvidenceCreate).toHaveBeenCalledTimes(4);
  });
  ...
  it('does not duplicate immutable provenance when the source identity link already exists', async () => {
    ...
    expect(h.canonicalCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({ domain: null, country: null, status: 'SUPPRESSED' }),
    });
  });
});
