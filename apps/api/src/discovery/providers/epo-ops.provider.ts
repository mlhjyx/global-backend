...
import { EPO_OPS_LICENSE, EpoApplicant } from '../../adapters/epo-ops';
import {
...
const ESPACENET_BASE = 'https://worldwide.espacenet.com/'; // 公开专利门户（证据留痕）
// 🔴 公司对齐门（比公司发现门 0.72 更严）：贴错公司 = 把 A 公司发明人挂到 B 公司，危害大（同 CH）。
const ALIGN_MIN_SCORE = 0.9;
const ALIGN_MIN_MARGIN = 0.1;
// 过滤旋钮（用户拍板）：近 5 年 + 每公司上限 25 位 distinct 发明人（防大公司爆量 + 数据最小化）。
 ...
 * 无 externalIds：EPO 无消歧到人的稳定 id，硬凑「公开号+名字」键会令同一人跨专利被误拆（`hasExternalIdConflict`
 * 触发）——故不产 Tier 0 键，合并走待办 2 的归一名（Tier 2/3）。
 */
export function toContactRecord(rawName: string, applicantName: string): ProviderContactRecord {
  ...
  return {
    externalId: `epo_ops:${applicantName.toLowerCase().replace(/\s+/g, '-')}:${idKey}`, // 记录自身键（persist 不据此去重，仅留痕）
    fullName,
    title: 'Inventor',
    ...
    sourcePage: ESPACENET_BASE,
    license: EPO_OPS_LICENSE, // CC BY 4.0 署名义务（写入 field_evidence.license，非硬编码 licensed）
    // 🔴 无 externalIds（见上）——不走 Tier 0，走 Tier 2/3 归一名并。
  };
}

/**
 * EPO OPS 发明人发现 Provider（待办 3 第二个身份源；`ContactDiscoveryAdapter`）。
 * 官方 OPS API（非爬）→ 近期专利具名 inventor = 技术买家 → 高置信对齐公司 → 归一名并入决策人图谱。
 *
 * 🔴 三道护栏绝不挂错公司：applicant 名高置信对齐（score≥0.9 且 margin≥0.1）+ 国别门 + 歧义即弃返空。
 * 🔴 数据最小化：只取 inventor name（residence/地址/国籍在 adapter 层就不映射）。
 * 🔴 用途门：epo_ops.search = required 工具，直连前过 §8.8 fail-closed（无 broker = 不出网）。
 * fail-safe：任何失败（无 creds/网络/闸门拒绝/token 取不到）返空、不抛穿（单源不阻断其余）。
 */
export class EpoOpsInventorProvider implements ContactDiscoveryAdapter {
  readonly key = 'epo_ops';

  constructor(private readonly deps?: { broker?: ExecutionBroker; now?: () => number }) {}

  private log(msg: string): void {
    console.warn(`[epo_ops] ${msg}`);
  }
  ...
  async discoverContacts(
    ...
    try {
      const res = await this.deps.broker.invoke<EpoOpsInput, EpoOpsOutput>(
        'epo_ops.search',
      ...
      const applicantMap = new Map<string, EpoApplicant>();
      ...
      // 🔴 EPO biblio 的 applicants[]/inventors[] 无「谁属谁」映射：合著专利（Siemens+Bosch）无法判定某发明人
