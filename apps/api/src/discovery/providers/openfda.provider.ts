...
const FDA_API_DOMAIN = 'api.fda.gov'; // §8.8 source_policy 门锚点
...
export class OpenFdaDiscoveryProvider implements CompanyDiscoveryAdapter {
  ...
  async discoverCompanies(query: CompanyDiscoveryQuery, _opts?: DiscoveryOptions): Promise<DiscoveryResult> {
    const filters = query.filters ?? {};
    const productCodes = readProductCodes(filters);
    if (!productCodes.length) return { records: [], costCents: 0 }; // 无产品码 → 不启动（绝不裸拉全库）

    // §8.8 合规门：registrationlisting 记录可能含具名 us_agent → 直连 HTTP 前必校验 source_policy（用途 + SUSPENDED）。
  ...
  /**
   * §8.8 用途门（镜像 ted.provider.purposeAllowed）：reader 在场时校验 source_policy(api.fda.gov)——
   * SUSPENDED / 策略缺失 / 用途不含 discovery / reader 抛错 一律 fail-closed（不发请求，含个人数据源）。
   * 无 reader（直连探针/单测）→ fail-open（生产 registry 注入 reader）。
   */
  private async purposeAllowed(): Promise<boolean> {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(`[openfda] source_policy 读取失败，fail-closed: ${String(err).slice(0, 120)}`);
      return false;
    }
    if (!policy || policy.suspended) {
      // eslint-disable-next-line no-console
      console.warn(`[openfda] source_policy 未批准/缺失/SUSPENDED，跳过直连（${FDA_API_DOMAIN}）`);
      return false;
    }
    if (policy.allowedPurpose && !policy.allowedPurpose.includes(PURPOSE_DISCOVERY)) {
      // eslint-disable-next-line no-console
      console.warn('[openfda] source_policy 不含 discovery 用途，跳过直连');
      return false;
    }
    return true;
  }
