...
describe('OSM discovery provider', () => {
  ...
  it('uses built-in vocab and degrades broker failures to an empty result', async () => {
    ...
    const invoke = vi.fn(async () => {
      throw new Error('policy denied for jane@example.com token=osm-secret');
    });
    ...
    expect(JSON.stringify(warn.mock.calls)).toMatch(/ERROR_TEXT_SHA256:[0-9a-f]{64}/);
    expect(JSON.stringify(warn.mock.calls)).not.toContain('jane@example.com');
    expect(JSON.stringify(warn.mock.calls)).not.toContain('osm-secret');
    warn.mockRestore();
  });
});
