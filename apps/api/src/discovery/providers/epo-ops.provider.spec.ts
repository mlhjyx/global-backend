...
import {
  EpoOpsInventorProvider,
  countryConflicts,
  toReadableName,
  toContactRecord,
  currentYear,
} from './epo-ops.provider';
import type { EpoApplicant, EpoPatentRecord } from '../../adapters/epo-ops';
import type { EpoOpsInput, EpoOpsOutput } from '../../tools/source-tools';
...
function fakeBroker(opts: { patents?: () => EpoPatentRecord[]; throwErr?: boolean }): ExecutionBroker & {
  invokeMock: ReturnType<typeof vi.fn>;
} {
  const invokeMock = vi.fn(async (_toolId: string, _input: EpoOpsInput): Promise<ToolResult<EpoOpsOutput>> => {
...
const appl = (name: string, country?: string): EpoApplicant => ({ name, country });
const patent = (applicants: EpoApplicant[], inventors: string[], publicationYear = 2024): EpoPatentRecord => ({
  applicants,
  inventors: inventors.map((name) => ({ name })),
  publicationYear,
});

describe('EPO · countryConflicts（国别门）', () => {
...
describe('EPO · toContactRecord', () => {
  it('发明人 → technical_buyer + personalData + CC BY + 🔴 无 externalIds', () => {
    ...
    expect(rec.externalIds).toBeUndefined(); // 🔴 EPO 不产 Tier 0 键
  });
});

describe('EPO · currentYear', () => {
  it('注入时钟 → UTC 年', () => {
    expect(currentYear(FIXED_NOW)).toBe(2026);
  });
});

describe('EPO · discoverContacts', () => {
  const company = { name: 'Siemens', domain: 'siemens.com', country: 'DE' };

  it('name 空 → 空（不调 broker）', async () => {
    const broker = fakeBroker({ patents: () => [] });
    const res = await new EpoOpsInventorProvider({ broker, now: FIXED_NOW }).discoverContacts({ name: '  ' }, CTX);
    expect(res.contacts).toEqual([]);
    expect(broker.invokeMock).not.toHaveBeenCalled();
  });

  it('无 broker → 空（fail-closed，不出网）', async () => {
    const res = await new EpoOpsInventorProvider({}).discoverContacts(company, CTX);
    expect(res.contacts).toEqual([]);
  });

  it('对齐命中 → 发明人（technical_buyer，无 externalIds）；purpose+年区间贯穿', async () => {
    const broker = fakeBroker({
      patents: () => [patent([appl('Siemens AG', 'DE')], ['Müller, Hans', 'Schmidt, Anna'])],
    });
    const res = await new EpoOpsInventorProvider({ broker, now: FIXED_NOW }).discoverContacts(company, CTX);
    ...
    // 工具调用：id/输入/purpose/年区间
    const [toolId, input, toolCtx] = broker.invokeMock.mock.calls[0] as [string, EpoOpsInput, ToolContext];
    expect(toolId).toBe('epo_ops.search');
  ...
  it('name 变体同一公司（"Acme"/"Acme Inc" 归一同名）→ 去重后对齐、不误弃', async () => {
    ...
    const res = await new EpoOpsInventorProvider({ broker, now: FIXED_NOW }).discoverContacts(
