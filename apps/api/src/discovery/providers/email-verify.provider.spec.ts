...
describe('自建邮箱验证 · 纯逻辑（诚实上限）', () => {
  ...
  const base = { mxPresent: true, provider: 'other_or_self_hosted', enumResistant: false, smtpReachable: true, mailFromOk: true,
  } as const;
  ...
  it('裁决：唯一 VALID = 可达+MAIL FROM过+接受+catch-all已证伪+非反枚举', () => {
    expect(decideEmailVerdict({ ...base, rcptCode: 250, catchAllStatus: 'not_catch_all',
      }).status,
    ).toBe('VALID');
  });
  ...
  it('裁决：RCPT 550 明确拒收(MAIL FROM 过) = INVALID', () => {
    expect(decideEmailVerdict({ ...base, rcptCode: 550, catchAllStatus: 'not_catch_all',
      }).status,
    ).toBe('INVALID');
  });

  it('裁决：无 MX = INVALID', () => {
    expect(decideEmailVerdict({ ...base, mxPresent: false, rcptCode: null, catchAllStatus: 'inconclusive',
      }).status,
    ).toBe('INVALID');
  });
});
...
describe('自建邮箱验证 · ToolBroker 闸门 + source_policy', () => {
  ...
  function fakeBroker(opts: { suspend?: string[]; denyPurpose?: string[]; probe?: SmtpProbeOutput; durableReceipt?: DurableExecutionReceipt; throwName?: string; throwCode?: string; policyThrows?: boolean;
    ...
    const broker = { invoke, checkSourcePolicy,
  ...
  it('suppression action denial before MX returns BLOCKED without DNS or SMTP', async () => {
    const { broker, invoke } = fakeBroker();
    const authorizeExternalAction = vi.fn(async () => false);

    const result = await new SelfHostedEmailVerifier(broker).verifyEmail('user@acme.de', {
      workspaceId: 'w',
    ...
    expect(authorizeExternalAction).toHaveBeenCalledOnce();
    expect(mockedMx).not.toHaveBeenCalled();
    expect(invoke).not.toHaveBeenCalled();
  });
