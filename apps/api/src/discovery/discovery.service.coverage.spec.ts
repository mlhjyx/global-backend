...
function harness() {
  const canonicalCompany = company({ attributes: {} });
  ...
  const tx = {
    ...
    canonicalCompany: {
      ...
      findUnique: vi.fn(async () => canonicalCompany),
      update: vi.fn(async ({ data }: any) => ({ ...canonicalCompany, ...data })),
    ...
    suppressionRecord: {
      ...
      upsert: vi.fn(async ({ create }: any) => ({ id: 'suppression-1', ...create })),
    ...
    canonicalContact: {
      ...
      findUnique: vi.fn(async () => canonicalContact),
    },
...
describe('DiscoveryService', () => {
  ...
  it('fans out contact providers, isolates failures, persists sequentially, and charges only positive cost', async () => {
    ...
    vi.mocked(persistDiscoveredContacts)
      .mockResolvedValueOnce({ inserted: 1, updated: 0, skippedSuppressed: 2, skippedInvalid: 0 } as any)
      .mockResolvedValueOnce({ inserted: 0, updated: 0, skippedSuppressed: 0, skippedInvalid: 0 } as any);
  ...
  it('adds normalized suppressions, verifies writes, and immediately suppresses domains', async () => {
    ...
    expect(tx.suppressionRecord.upsert).toHaveBeenCalledWith({
      where: {
      ...
      update: {},
      create: expect.objectContaining({ value: 'pump.example', reason: 'manual' }),
    });
    expect(tx.canonicalCompany.findMany).toHaveBeenCalled();

    await service.addSuppression(ctx, { type: 'email', value: 'A@PUMP.EXAMPLE', reason: 'unsubscribe' });
    expect(tx.suppressionRecord.upsert).toHaveBeenLastCalledWith(
      expect.objectContaining({
        update: { protectionClass: 'LEGAL' },
        create: expect.objectContaining({ value: 'a@pump.example', protectionClass: 'LEGAL' }),
      }),
    );

    await expect(service.addSuppression(ctx, { type: 'unknown', value: 'x' })).rejects.toBeInstanceOf(BadRequestException);
  });

  it('lists providers/suppressions and keeps deprecated removal fail-closed', async () => {
    const { service, tx } = harness();
    await expect(service.listSuppressions(ctx)).resolves.toEqual({
      rows: [{ type: 'email', value: 'blocked@pump.example' }],
      nextCursor: null,
      hasMore: false,
    });
    ...
    tx.suppressionRecord.findUnique.mockResolvedValueOnce({ id: 'suppression-1', protectionClass: 'LEGAL' });
    tx.suppressionDecision.findUnique
      .mockResolvedValueOnce(null)
