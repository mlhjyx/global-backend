...
const LINEAGE='eb5f6f09da3e68694b43070eabf2f76340d2c84c8ff6712486495aa64d1630c0';
...
function command(extra={}) { return {schemaVersion:'discovery-query-lineage-command/v1',contractSha256:LINEAGE,
  ...
  queryReceiptContractSha256:LINEAGE,rawRelationContractSha256:RAW,attempts:[],items:[],
...
describe('Discovery query lineage SQL repository',()=>{
  ...
  it('accepts a dense one-item command and exact authorization projection',async()=>{
    ...
    await expect(m.appendQueryLineageV1(db.value,itemCommand())).resolves.toEqual({
      status:'APPLIED',attemptCount:1,itemCount:1,queryKey:QUERY});
    await expect(m.appendQueryLineageV1(tx([]).value,command({authorization:{
  ...
  it('attests with identity only and preserves NOT_FOUND and REPLAYED',async()=>{
    const m=await load();for(const row of [{status:'NOT_FOUND',query_receipt:null,attempt_count:0,item_count:0,replay:false},
      {status:'REPLAYED',query_receipt:command().queryReceipt,attempt_count:1,item_count:1,replay:true}]){
  ...
  it('rejects multi-row and malformed database results at the repository boundary',async()=>{
    ...
    for(const rows of [[],[applied,applied],[{...applied,status:'REPLAYED'}],
      ...
      await expect(m.appendQueryLineageV1(tx(rows).value,command())).rejects.toThrow(
        'DOMAIN_ACK_DISCOVERY_QUERY_LINEAGE_UNAVAILABLE');
    for(const rows of [[{status:'NOT_FOUND',query_receipt:{},attempt_count:0,item_count:0,replay:false}],
      [{status:'REPLAYED',query_receipt:{queryKey:QUERY},attempt_count:1,item_count:1,replay:true}],
      [{status:'NOT_FOUND',query_receipt:null,attempt_count:1,item_count:0,replay:false}]])
      await expect(m.attestQueryLineageV1(tx(rows).value,key())).rejects.toThrow(
        'DOMAIN_ACK_DISCOVERY_QUERY_LINEAGE_UNAVAILABLE');
  });
});
