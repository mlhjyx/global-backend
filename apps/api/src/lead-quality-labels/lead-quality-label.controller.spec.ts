...
describe('LeadQualityLabelsController', () => {
  it('passes only RequestContext plus the validated DTO to the service and returns the common envelope', async () => {
    ...
    const controller = new LeadQualityLabelsController({ create } as unknown as LeadQualityLabelsService);
    ...
    expect(dto).not.toHaveProperty('actor_id');
  });
});
...
describe('lead-quality-label OpenAPI contract', () => {
  const openapi = JSON.parse(
    ...
    components: { schemas: Record<string, { required?: string[]; properties?: Record<string, { enum?: string[] }> }> };
  };
