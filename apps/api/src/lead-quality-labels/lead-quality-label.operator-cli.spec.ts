...
describe("FileLeadQualityLabelOperatorStateStore", () => {
  ...
  it.each([
    [[], /command is required/],
    [["pull", "position"], /unexpected positional/],
    [["pull", "--execute", "--execute"], /specified once/],
    [["pull", "--limit"], /requires a value/],
    [["pull", "--limit", "1", "--limit", "2"], /specified once/],
    [["unknown"], /unknown command/],
    [["defer", "--event-id", EVENT_ID, "--extra", "x"], /not valid/],
    [["defer", "--event-id", "bad"], /UUID v4/],
    [["qgo", "--input", "label.json"], /--event-envelope is required/],
  ] as const)("rejects malformed command arguments %#", async (args, message) => {
    const h = deps(vi.fn() as unknown as typeof fetch);
    await expect(runLeadQualityLabelOperator(args, h.value)).rejects.toThrow(message);
  });

  it.each([
    [["pull", "--execute"], /not valid for pull/],
    [["pull", "--limit", "0"], /integer from 1 to 200/],
    [["pull", "--limit", "201"], /integer from 1 to 200/],
    [["pull", "--limit", "1.5"], /integer from 1 to 200/],
    [["pull", "--cursor", "next"], /cursor must be numeric/],
  ] as const)("rejects unsafe pull pagination %#", async (args, message) => {
    const h = deps(vi.fn() as unknown as typeof fetch);
    await expect(runLeadQualityLabelOperator(args, h.value)).rejects.toThrow(message);
  });

  it.each([
    [undefined, /BASE_URL is required/],
    ["not-a-url", /absolute URL/],
    ["https://user:pass@example.test", /userinfo/],
    ["https://example.test?x=1", /query or fragment/],
    ["http://example.test", /must use HTTPS/],
    ["https://example.test/api", /origin without a path/],
  ] as const)("rejects unsafe API origins %#", async (baseUrl, message) => {
    const h = deps(vi.fn() as unknown as typeof fetch);
    h.value.env.GLOBAL_API_BASE_URL = baseUrl;
    await expect(runLeadQualityLabelOperator(["pull"], h.value)).rejects.toThrow(message);
  });

  it("rejects missing or control-character bearer credentials before network", async () => {
    for (const token of [undefined, "bad token", "bad\nheader"]) {
      const fetchImpl = vi.fn();
      const h = deps(fetchImpl as unknown as typeof fetch);
      h.value.env.GLOBAL_API_BEARER_TOKEN = token;
      await expect(runLeadQualityLabelOperator(["pull"], h.value)).rejects.toThrow(/BEARER_TOKEN/);
      expect(fetchImpl).not.toHaveBeenCalled();
    }
  });
});
