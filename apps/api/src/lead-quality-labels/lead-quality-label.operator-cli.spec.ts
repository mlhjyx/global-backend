...
describe('lead-quality-label operator CLI', () => {
  ...
  it('pulls only LeadQualified, validates envelope/payload, and reports durable dedupe state without raw data', async () => {
    ...
    await expect(runLeadQualityLabelOperator(['pull'], h.value)).resolves.toBe(0);
  ...
  it('qgo POST success/replay durably records LABEL_POSTED before ACK, then records ACKED', async () => {
    ...
    await expect(
      runLeadQualityLabelOperator(['qgo', '--input', 'label.json', '--execute'], h.value),
    ).resolves.toBe(0);

    expect(fetchImpl).toHaveBeenCalledTimes(2);
    expect(String(fetchImpl.mock.calls[0]?.[0])).toContain('/api/v1/lead-quality-labels');
    expect(String(fetchImpl.mock.calls[1]?.[0])).toContain('/api/v1/events/ack');
    expect(h.stateStore.writes.map((entry) => entry.status)).toEqual(['LABEL_POSTED', 'ACKED']);
  ...
  it('reject action requires a rejection input/reason and follows the same POST-then-ACK state machine', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ data: { id: '55555555-5555-4555-8555-555555555555', replayed: true } }),
          { status: 200, headers: { 'content-type': 'application/json' } },
        ),
      )
      .mockResolvedValueOnce(
    ...
    const h = deps(fetchImpl as unknown as typeof fetch, new MemoryStateStore(), rejectInput);

    await expect(
      runLeadQualityLabelOperator(['reject', '--input', 'label.json', '--execute'], h.value),
    ).resolves.toBe(0);
    expect(h.stateStore.writes.at(-1)?.status).toBe('ACKED');

    const wrong = deps(vi.fn() as unknown as typeof fetch, new MemoryStateStore(), qgoInput);
    await expect(
      runLeadQualityLabelOperator(['reject', '--input', 'label.json', '--execute'], wrong.value),
  ...
  it('does not ACK or advance state when POST fails; defer also performs no network/state change', async () => {
    ...
    fetchImpl.mockClear();
    await expect(
      runLeadQualityLabelOperator(['defer', '--event-id', EVENT_ID, '--execute'], h.value),
    ).resolves.toBe(0);
    expect(fetchImpl).not.toHaveBeenCalled();
    expect(h.stateStore.writes).toHaveLength(0);
  });
  ...
  it('keeps LABEL_POSTED after ACK failure and retry-ack is gated to that durable state', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ data: { id: '55555555-5555-4555-8555-555555555555', replayed: false } }),
          { status: 201, headers: { 'content-type': 'application/json' } },
        ),
      )
    ...
    await expect(
      runLeadQualityLabelOperator(['retry-ack', '--event-id', EVENT_ID, '--execute'], h.value),
    ).resolves.toBe(0);
    expect(h.stateStore.records.get(EVENT_ID)?.status).toBe('ACKED');

    const pending = deps(vi.fn() as unknown as typeof fetch);
    await expect(
      runLeadQualityLabelOperator(['retry-ack', '--event-id', EVENT_ID, '--execute'], pending.value),
  ...
  it('rejects unsafe base URLs and oversized responses without echoing token/response body', async () => {
    ...
    const fetchImpl = vi.fn(async () => new Response(bodyMarker.repeat(400_000), { status: 200 }));
    const output = io();
    await expect(
      runLeadQualityLabelOperator(['pull'], {
        env: { GLOBAL_API_BASE_URL: 'http://api.example.test', GLOBAL_API_BEARER_TOKEN: FAKE_BEARER },
    ...
    await expect(
      runLeadQualityLabelOperator(['pull'], {
        env: { GLOBAL_API_BASE_URL: 'https://user@api.example.test', GLOBAL_API_BEARER_TOKEN: FAKE_BEARER },
        fetchImpl: fetchImpl as unknown as typeof fetch,
        stateStore: new MemoryStateStore(),
        readFileText: () => '',
        write: output.write,
  ...
  it('fails closed on an invalid LeadQualified payload without ACK', async () => {
    ...
    await expect(runLeadQualityLabelOperator(['pull'], h.value)).rejects.toThrow(/schema/i);
    expect(fetchImpl).toHaveBeenCalledOnce();
    expect(h.stateStore.writes).toHaveLength(0);
  });
});
...
describe('FileLeadQualityLabelOperatorStateStore', () => {
  it('persists only minimal state via a 0600 file in a 0700 directory', () => {
    ...
    expect(JSON.parse(persisted)).toEqual({
      version: 1,
      events: { [EVENT_ID]: expect.objectContaining({ status: 'LABEL_POSTED' }) },
    });
  });
});
