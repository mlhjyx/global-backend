...
describe("ArtifactSubjectBindingContract", () => {
  it("parks platform sanctions and first-discovery HTTP/Crawl without a canonical subject", async () => {
    ...
    for (const resultSchema of WORKSPACE_SCHEMAS) {
      await expect(
        contract.resolve(tx, workspaceInput(resultSchema)),
      ).resolves.toEqual({
        status: "SUBJECT_BINDING_HOLD",
  ...
  it.each([
    ...
    "binds %s only after the repository verifies an exact current-workspace %s",
    async (resultSchema, subjectType, subjectId) => {
      const subjectRef = Object.freeze({ subjectType, subjectId });
      const subjects = repository({ workspaceId: WORKSPACE_ID, ...subjectRef });
      const contract = new ArtifactSubjectBindingContract(subjects);

      const decision = await contract.resolve(
        tx,
        workspaceInput(resultSchema, subjectRef),
      );
      ...
      expect(
        Object.isFrozen(
          (
            decision as Extract<
              ArtifactSubjectBindingDecision,
              { status: "BOUND" }
            >
          ).subjectRef,
        ),
  ...
  it.each([
    ...
    "denies a syntactically valid claim when the repository reports %s",
    async (_case, resolved) => {
  ...
  it("denies proxies, accessors, non-enumerable and symbol fields without reading them", async () => {
    const subjects = repository(null);
    const contract = new ArtifactSubjectBindingContract(subjects);
    const getter = vi.fn(() => "http-get/v1");
    const accessor = {
      scopeKind: "workspace",
      workspaceId: WORKSPACE_ID,
    };
    Object.defineProperty(accessor, "resultSchema", {
      enumerable: true,
      get: getter,
    });
    const nonEnumerable = workspaceInput("http-get/v1");
    Object.defineProperty(nonEnumerable, "hidden", {
      enumerable: false,
      value: COMPANY_ID,
    });
    const symbolField = workspaceInput("http-get/v1") as Record<
      string | symbol,
      unknown
    >;
    symbolField[Symbol("subject")] = COMPANY_ID;
    const proxy = new Proxy(workspaceInput("http-get/v1"), {
      ownKeys: () => {
        throw new Error("must not escape closed input handling");
      },
    });

    for (const input of [accessor, nonEnumerable, symbolField, proxy]) {
      await expect(contract.resolve(tx, input)).resolves.toEqual({
        status: "DENIED",
        reason: "SUBJECT_BINDING_INVALID",
      });
