...
function ownDataRecord(
  ...
  try {
    ...
    if (
      ...
      error.message === "MODEL_RESULT_PROJECTION_INVALID"
    )
      throw error;
    projectionInvalid();
  }
...
function readModelResult(
  ...
  const result = ownDataRecord(raw, MODEL_RESULT_KEYS, [
    "data",
    "provider",
    "model",
  ]);
  return {
    ...
    ...(hasField(result, "reportedModel")
      ? { reportedModel: field(result, "reportedModel") }
      : {}),
    ...(hasField(result, "modelResolutionSource")
      ? { modelResolutionSource: field(result, "modelResolutionSource") }
      : {}),
    ...(hasField(result, "usage")
      ? { usage: readModelUsage(field(result, "usage")) }
      : {}),
    ...(hasField(result, "callCount")
      ? { callCount: field(result, "callCount") }
      : {}),
  };
}
...
function readModelUsage(value: unknown): UnknownRecord {
  return ownDataRecord(
    value,
    ["inputTokens", "outputTokens", "costUsd", "gatewaySettlements"],
    [],
  );
}
...
const numberSchema = (
  ...
  maximum: number,
  integer = false,
): JsonSchema => ({ type: integer ? "integer" : "number", minimum, maximum });
...
const FACT_SCALAR_SCHEMA: JsonSchema = {
  oneOf: [
    stringSchema(2000),
    numberSchema(-1_000_000_000_000_000, 1_000_000_000_000_000),
    { type: "boolean" },
    { type: "null" },
  ],
};
...
const RULE_SCALAR_SCHEMA: JsonSchema = {
  oneOf: [
    stringSchema(1000),
    numberSchema(-1_000_000_000_000_000, 1_000_000_000_000_000),
    { type: "boolean" },
...
const factEntriesSchema = (
  keys: readonly string[],
  value: JsonSchema,
): JsonSchema =>
  arraySchema(
    32,
    objectSchema({ key: stringSchema(120, { enum: [...keys] }), value }, [
      "key",
      "value",
    ]),
  );
const readbackProbeSchema = objectSchema(
  {
    sequence: numberSchema(1, 2, true),
    phase: stringSchema(64, {
      enum: [
    ...
    httpStatusClass: {
      oneOf: [
        { type: "integer", minimum: 2, maximum: 5, enum: [2, 4, 5] },
        { type: "null" },
      ],
    },
  },
  ["sequence", "phase", "httpStatusClass"],
);
const transportObservationSchema = objectSchema(
  {
    schemaVersion: stringSchema(64, {
      const: "site-build-provider-transport-observation/v1",
    }),
    physicalWireAttempt: numberSchema(1, 2, true),
    finalPhase: stringSchema(64, {
      enum: [
        "gateway_unavailable",
    ...
    gatewayIdState: stringSchema(32, {
      enum: ["observed", "missing", "not_observable"],
    }),
    upstreamIdState: stringSchema(32, {
      enum: ["observed", "absent", "not_exposed", "unknown"],
    }),
    payloadState: stringSchema(32, {
      enum: ["not_read", "available", "unavailable"],
    }),
    readbackProbes: arraySchema(2, readbackProbeSchema),
  },
  [
    "schemaVersion",
    "physicalWireAttempt",
    ...
    objectSchema(
      {
        ...
        upstreamIdState: stringSchema(16, { enum: ["observed", "absent"] }),
        transportObservation: transportObservationSchema,
      },
      [
        "status",
        "physicalWireAttempt",
  ...
  return objectSchema(
    {
      ...
      usage: optionalObjectSchema({
        ...
        gatewaySettlements: arraySchema(2, gatewaySettlementSchema),
      }),
...
function restoreModelResult(
  ...
  return {
    ...
    ...(hasField(projected, "modelResolutionSource")
      ? {
          modelResolutionSource:
            projected.modelResolutionSource as ModelResult<unknown>["modelResolutionSource"],
        }
      : {}),
...
const claimsDefinition: TypedProjectionDefinition<unknown, unknown> = {
    ...
    return projectModelResultEnvelope(result, {
      claims: denseArray(field(result.data, "claims")).map((claim) => {
        ...
        return {
          ...
          ...(hasField(source, "evidence")
            ? { evidence: field(source, "evidence") }
            : {}),
  ...
  restore(projected) {
    const result = projected as {
      data: { claims: readonly UnknownRecord[] };
      provider: string;
      model: string;
    };
...
const profileDefinition: TypedProjectionDefinition<unknown, unknown> = {
  ...
  restore(projected) {
    const result = projected as {
      data: { industry: string; summary: string };
      provider: string;
      model: string;
    };
    return restoreModelResult(result, {
...
const offeringsDefinition: TypedProjectionDefinition<unknown, unknown> = {
  ...
  project(raw) {
    ...
    return projectModelResultEnvelope(result, {
      offerings: denseArray(field(result.data, "offerings")).map((offering) => {
        const source = ownDataRecord(
          offering,
          ["name", "description", "attributes", "evidence", "confidence"],
          ["name", "confidence"],
        );
        return {
          name: field(source, "name"),
          ...(hasField(source, "description")
            ? { description: field(source, "description") }
            : {}),
          ...(hasField(source, "attributes")
            ? {
                factEntries: projectFactEntries(
                  field(source, "attributes"),
                  OFFERING_ATTRIBUTE_KEYS,
                ),
              }
            : {}),
          ...(hasField(source, "evidence")
  ...
  restore(projected) {
    const result = projected as {
      data: { offerings: readonly UnknownRecord[] };
      provider: string;
      model: string;
    };
    return restoreModelResult(result, {
...
const icpDesignDataSchema = objectSchema(
  {
    ...
    companyAttributeEntries: factEntriesSchema(
      ICP_ATTRIBUTE_KEYS,
      FACT_VALUE_SCHEMA,
    ),
    ...
    qualificationRules: arraySchema(
      64,
      objectSchema(
        {
          kind: stringSchema(20, {
...
const icpDesignDefinition: TypedProjectionDefinition<unknown, unknown> = {
  ...
  project(raw) {
    ...
    return projectModelResultEnvelope(result, {
      ...
      companyAttributeEntries: projectFactEntries(
        field(result.data, "company_attributes"),
        ICP_ATTRIBUTE_KEYS,
      ),
      ...
      buyingCommittee: denseArray(field(result.data, "buying_committee")).map(
        (buyer) => {
          const source = ownDataRecord(
            buyer,
            ["role", "title", "concerns"],
      ...
      qualificationRules: denseArray(
        field(result.data, "qualification_rules"),
      ).map((rule) => {
        const source = ownDataRecord(
          rule,
          ["kind", "field", "operator", "value", "weight", "rationale"],
          ["kind", "field", "operator", "value"],
        );
        return {
          ...
          ...(hasField(source, "weight")
            ? { weight: field(source, "weight") }
            : {}),
          ...(hasField(source, "rationale")
  ...
  restore(projected) {
    const result = projected as {
      data: UnknownRecord & {
        companyAttributeEntries: unknown;
        painPoints: readonly string[];
        triggerSignals: readonly string[];
        exclusions: readonly string[];
        valueProps: readonly string[];
        targetMarkets: readonly string[];
        personas: readonly UnknownRecord[];
        buyingCommittee: readonly UnknownRecord[];
    ...
    return restoreModelResult(result, {
      ...
      company_attributes: restoreFactEntries(
        result.data.companyAttributeEntries,
      ),
          ...
          objectSchema(
            {
              ...
              filterEntries: factEntriesSchema(
                QUERY_FILTER_KEYS,
                RULE_VALUE_SCHEMA,
              ),
    ...
    return projectModelResultEnvelope(result, {
      queries: denseArray(field(result.data, "queries")).map((query) => {
        ...
        return {
          ...
          filterEntries: projectFactEntries(
...
const fitDefinition: TypedProjectionDefinition<unknown, unknown> = {
  ...
  restore(projected) {
    const result = projected as {
      data: UnknownRecord & { reasons: readonly string[] };
      provider: string;
      model: string;
    };
    return restoreModelResult(result, {
...
const companyDefinition: TypedProjectionDefinition<unknown, unknown> = {
  ...
  jsonSchema: modelResultSchema(
    optionalObjectSchema(companyDataProperties, ["isCompanySite"]),
  ),
  project(raw) {
    const rawKeys = [
      "is_company_site",
    ...
    return projectModelResultEnvelope(result, {
      ...
      ...(hasField(result.data, "name")
        ? { name: field(result.data, "name") }
        : {}),
      ...(hasField(result.data, "country")
        ? { country: field(result.data, "country") }
        : {}),
      ...(hasField(result.data, "industry")
        ? { industry: field(result.data, "industry") }
        : {}),
      ...
      ...(hasField(result.data, "evidence")
        ? { evidence: field(result.data, "evidence") }
        : {}),
...
const listDefinition: TypedProjectionDefinition<unknown, unknown> = {
  ...
  project(raw) {
    const result = readModelResult(
      raw,
      ["is_directory", "list_kind", "companies", "has_next_page"],
      ["is_directory", "companies"],
    );
    return projectModelResultEnvelope(result, {
      isDirectory: field(result.data, "is_directory"),
      ...(hasField(result.data, "list_kind")
        ? { listKind: field(result.data, "list_kind") }
        : {}),
      companies: denseArray(field(result.data, "companies")).map((company) => {
        const source = ownDataRecord(
          company,
          ["name", "website", "location", "detail_url"],
          ["name"],
        );
        return {
          name: field(source, "name"),
          ...(hasField(source, "website")
            ? { website: field(source, "website") }
            : {}),
          ...(hasField(source, "location")
            ? { location: field(source, "location") }
            : {}),
          ...(hasField(source, "detail_url")
            ? { detailUrl: field(source, "detail_url") }
            : {}),
        };
      }),
  ...
  restore(projected) {
    ...
    return restoreModelResult(result, {
      ...
      ...(hasField(result.data, "listKind")
        ? { list_kind: result.data.listKind }
        : {}),
      companies: result.data.companies.map((company) => ({
        name: company.name,
        ...(hasField(company, "website") ? { website: company.website } : {}),
        ...(hasField(company, "location")
          ? { location: company.location }
          : {}),
        ...(hasField(company, "detailUrl")
          ? { detail_url: company.detailUrl }
          : {}),
      })),
...
const peopleDefinition: TypedProjectionDefinition<unknown, unknown> = {
    ...
    return projectModelResultEnvelope(result, {
      people: denseArray(field(result.data, "people")).map((person) => {
        ...
        return {
          ...
          ...(hasField(source, "title")
            ? { title: field(source, "title") }
            : {}),
          ...(hasField(source, "email")
            ? { email: field(source, "email") }
            : {}),
          ...(hasField(source, "phone")
            ? { phone: field(source, "phone") }
            : {}),
          ...(hasField(source, "department")
            ? { department: field(source, "department") }
            : {}),
          ...(hasField(source, "seniority")
            ? { seniority: field(source, "seniority") }
            : {}),
          ...(hasField(source, "buying_role")
            ? { buyingRole: field(source, "buying_role") }
            : {}),
  ...
  restore(projected) {
    const result = projected as {
      data: { people: readonly UnknownRecord[] };
      provider: string;
      model: string;
    };
    return restoreModelResult(result, {
      people: result.data.people.map((person) => ({
        ...
        ...(hasField(person, "department")
          ? { department: person.department }
          : {}),
        ...(hasField(person, "seniority")
          ? { seniority: person.seniority }
          : {}),
        ...(hasField(person, "buyingRole")
          ? { buying_role: person.buyingRole }
          : {}),
        ...(hasField(person, "isTargetRole")
          ? { is_target_role: person.isTargetRole }
          : {}),
