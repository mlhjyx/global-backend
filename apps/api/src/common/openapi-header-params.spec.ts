...
function openapiPath(): string {
  ...
  throw new Error('openapi.json not found — 契约文件缺失，请先 node dist/main.js --export-openapi');
}
  ...
  for (const [route, item] of Object.entries(spec.paths)) {
    for (const [method, op] of Object.entries(item)) {
      ...
      for (const [key, names] of byLowerName) {
        if (names.length > 1) {
          offenders.push(`${method.toUpperCase()} ${route} → "${key}": [${names.join(', ')}]`);
        }
      }
    }
  }
...
describe('OpenAPI header 参数大小写唯一性（oasdiff 破坏性门防误红）', () => {
  ...
  it('intake 暴露唯一可选 idempotency-key，响应为 siteId/buildId/generating_demo 且无 mode', () => {
    ...
    expect(operation, 'OpenAPI 缺 POST /api/v1/site-builder/intake').toBeDefined();
    ...
    expect(new Set(result?.required)).toEqual(new Set(['siteId', 'buildId', 'status']));
