...
describe('understanding.activities — extraction and profile persistence', () => {
  ...
  it('returns zero claims for a stub non-array output and never fabricates business facts', async () => {
    ...
    await expect(acts.extractClaims({ workspaceId: 'ws-1', text: 'Acme makes pumps.' })).resolves.toEqual({ claims: [] });
  });
...
describe('understanding.activities — idempotent persistence', () => {
  it('deduplicates claims across pages, creates evidence per page, and records a bounded conflict', async () => {
        ...
        findMany: vi.fn().mockResolvedValue([
          {
            id: 'claim-prior',
            type: 'capability',
            statement: 'Acme builds industrial pumps for chemical plants',
          },
        ]),
        create: claimCreate,
      },
    ...
    expect(sourceCreate.mock.calls[0]?.[0]?.data.ingestKey).toBe(
      'run-understanding-1:https://acme.example/products',
    );
