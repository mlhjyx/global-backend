...
function primeHappyPath() {
  ...
  acts.generateDesignBrief.mockResolvedValue({
    source: "generated",
    designBrief: { digest: "brief-digest" },
...
describe("refurbishWorkflow — happy path（P1 → P3 → P5）", () => {
  it("顺序 begin < ingest < brandProfile < imagePipeline < designBrief < copy < assemble < finalize；不触发补偿", async () => {
    ...
    expect(firstOrder(acts.processImages)).toBeLessThan(
      firstOrder(acts.generateDesignBrief),
    );
    expect(firstOrder(acts.generateDesignBrief)).toBeLessThan(
      firstOrder(acts.generateCopyBundles),
    );
    expect(firstOrder(acts.generateCopyBundles)).toBeLessThan(
    ...
    expect(acts.generateCopyBundles).toHaveBeenCalledWith(
  ...
  it("旧历史 replay（patched=false）保持原命令序列且 finalize 使用兼容摘要", async () => {
    ...
    expect(acts.generateDesignBrief).not.toHaveBeenCalled();
  ...
  it("仅 M1-e-B patch 未记录时保留旧 copy/assemble 命令输入", async () => {
    ...
    expect(acts.generateDesignBrief).not.toHaveBeenCalled();
    expect(acts.generateCopyBundles.mock.calls[0][0]).not.toHaveProperty(
      "designBrief",
    );
    expect(acts.assembleAndBuild.mock.calls[0][0]).not.toHaveProperty(
      "designBrief",
