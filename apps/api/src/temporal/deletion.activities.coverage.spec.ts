...
function prismaFor(tx: Record<string, unknown>): PrismaService {
  const suppressionRecord = tx.suppressionRecord as
    | { createMany?: (args: { data: Record<string, unknown>[] }) => Promise<unknown>; upsert?: unknown }
    | undefined;
  const batch = { data: [] as Record<string, unknown>[] };
  const decorated = {
    ...tx,
    $queryRaw: tx.$queryRaw ?? vi.fn(async () => [{ locked: true }]),
    ...(suppressionRecord && !suppressionRecord.upsert
      ? {
          suppressionRecord: {
            ...suppressionRecord,
            upsert: vi.fn(async ({ create }: { create: Record<string, unknown> }) => {
              batch.data.push(create);
              if (batch.data.length === 1) await suppressionRecord.createMany?.(batch);
              return create;
            }),
          },
        }
  ...
  return {
    withWorkspace: vi.fn(async <T>(_workspaceId: string, fn: (value: unknown) => Promise<T>) => fn(decorated)),
...
describe('deletion activities — locate and freeze', () => {
  it('locates a contact, writes email/person suppression, and returns a PII-free snapshot', async () => {
    ...
    const written = createMany.mock.calls[0]?.[0]?.data as Array<{ type: string; value: string; reason: string }>;
...
describe('deletion activities — erase', () => {
  ...
  it('re-reads a company erase set, suppresses late email, deletes PII/cache, and persists real counts', async () => {
    ...
    const tx = {
      ...
      suppressionRecord: { createMany: suppressionCreate },
    ...
    expect(suppressionCreate.mock.calls[0]?.[0]?.data).toEqual(
      expect.arrayContaining([
