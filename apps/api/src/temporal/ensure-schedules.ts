...
const SPECS = [
  ...
  // Raw Source v2 retention: scrub expired payloads to hash-only receipts; never calls providers.
  {
    id: RAW_RETENTION_SWEEP_SCHEDULE_ID,
    workflowType: RAW_RETENTION_SWEEP_WORKFLOW,
    everyEnv: 'RAW_RETENTION_SWEEP_EVERY',
    everyDefault: '24h',
  },
      ...
      try {
        ...
        await client.schedule.create({
          ...
          spec: {
            intervals: [
              {
                every: (process.env[s.everyEnv] ?? s.everyDefault) as Duration,
              },
            ],
          },
          action: {
