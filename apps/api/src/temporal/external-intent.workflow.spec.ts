...
function resolvedEcho(t: Target): Record<string, unknown> {
  return { ...t, cpvCodes: ['42122000'], buyerCountries: ['DEU'], fdaProductCodes: ['LLZ'] };
}

function ingestSummary(over: Record<string, unknown> = {}): Record<string, unknown> {
  return { tedSpecs: 1, fdaSpecs: 1, fetches: 2, ledgerHits: 0, signalsUpserted: 5, budgetExceeded: false, errors: [], ...over };
}

/** 为 happy path 配好全部活动；测试各自覆盖其所验的那一个（once 覆盖优先于此默认实现）。 */
function primeHappyPath(targets: Target[], live: { ted: boolean; openfda: boolean } = { ted: true, openfda: false }): { ted: boolean; openfda: boolean } {
  acts.listExternalIntentTargets.mockResolvedValue({ targets, tedEnabled: true, openfdaEnabled: true });
...
describe('externalIntentSweepWorkflow — fail-safe 分支', () => {
  it('两 provider 全 disabled → 早返，不触任何下游活动，结果为零 agg', async () => {
    acts.listExternalIntentTargets.mockResolvedValue({ targets: [target('ws-1', 'icp-1')], tedEnabled: false, openfdaEnabled: false });
