...
describe('TemporalClient.checkSystemInfo', () => {
  it('fails closed before the Temporal connection is initialized', async () => {
    await expect(new TemporalClient().checkSystemInfo({ timeoutMs: 10 })).rejects.toThrow(
      /not initialized/i,
    );
  });

  it('issues a bounded getSystemInfo request without exposing server details', async () => {
    const getSystemInfo = vi.fn().mockResolvedValue({ capabilities: { usefulServerInfo: true } });
    const withAbortSignal = vi.fn(async (_signal: AbortSignal, fn: () => Promise<unknown>) => fn());
    const withDeadline = vi.fn(async (_deadline: number, fn: () => Promise<unknown>) => fn());
    ...
    await expect(client.checkSystemInfo({ timeoutMs: 25, signal })).resolves.toBeUndefined();
