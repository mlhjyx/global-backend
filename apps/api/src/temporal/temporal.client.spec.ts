...
describe("TemporalClient.probe", () => {
  it("single-flights concurrent reconnects so one probe cannot close another bootstrap credential", async () => {
    ...
    await expect(
      Promise.all([client.reconnect(), client.reconnect()]),
  ...
  it("uses a bounded control-plane RPC without claiming worker health", async () => {
    ...
    const withDeadline = vi.fn(
      async (_deadline: number, operation: () => Promise<unknown>) =>
        operation(),
    );
    const client = new TemporalClient();
