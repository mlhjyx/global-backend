  ...
  return {
      ...
      try {
        const runOne = async (
          ...
          try {
            ...
            if (r.error) summary.errors.push(signalIngestOutcomeCode(r.provider, r.error));
            ...
            summary.errors.push(safeTemporalErrorCode(err, signalIngestFailureCode(provider)));
    ...
    async projectExternalIntentForIcp(args: ResolvedIntentTarget & {
      ...
      const out: ExternalIntentIcpResult = {
        ...
        ...(args.error
          ? { error: safeErrorCodeSequence(args.error, 'EXTERNAL_TARGET_RESOLUTION_FAILED') }
          : {}),
      };
