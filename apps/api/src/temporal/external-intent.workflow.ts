...
export async function externalIntentSweepWorkflow(
  ...
  try {
    ...
    agg.ingest = {
      ...
      errors: [safeTemporalErrorCode(err, 'EXTERNAL_SIGNAL_INGEST_FAILED')],
    };
  }
  ...
  if (patched('external-intent-recompute-v1')) {
    try {
      ...
      agg.recompute = {
        ...
        companiesCleared: 0,
        truncated: 0,
        error: safeTemporalErrorCode(err, 'EXTERNAL_INTENT_RECOMPUTE_FAILED'),
      };
    }
  }
  ...
  for (let i = 0; i < resolved.length; i++) {
    ...
    try {
      ...
      r = {
        ...
        fdaProductCodes: 0,
        naicsCodes: 0,
        error: safeTemporalErrorCode(err, 'EXTERNAL_INTENT_PROJECTION_FAILED'),
      };
    }
    agg.results.push(r);
    agg.swept += 1;
