...
export class AiSdkOpenAiChatCompletionsAdapter implements NativeModelAdapter {
  ...
  constructor(settings: AiSdkNativeAdapterSettings) {
    ...
    this.provider = createOpenAI({
      ...
      name: "new-api-openai-chat-completions",
    });
  }
  ...
  async execute<OutputValue = string>(
    ...
    try {
      const result = streamText({
        model: this.provider.chat(request.alias),
      ...
      const requestId = readOneApiRequestId(response.headers);
