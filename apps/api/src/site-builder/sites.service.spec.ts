    ...
    site: {
      ...
      updateMany: async ({
        ...
        const row = db.sites.find(
          (s) =>
            s.id === where.id && s.profileVersionId === where.profileVersionId,
        );
...
describe("SitesService（站点列表/详情/向导档案，07 §2）", () => {
  ...
  it("get 未命中 → NotFound", async () => {
    ...
    await expect(service.get(CTX, SITE_ID)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it("patchProfile 站点不存在 → NotFound", async () => {
    const { service } = makeService([]);
    await expect(
      service.patchProfile(CTX, SITE_ID, { brand: {} }, BODY_PRECONDITION),
  ...
  it("stale body token returns stable 409 and does not write", async () => {
    ...
    await expect(
      service.patchProfile(CTX, SITE_ID, { brand: {} }, stale),
  ...
  it("stale If-Match token returns stable 412; two same-base writes have exactly one winner", async () => {
    ...
    const outcomes = await Promise.allSettled([
      ...
      service.patchProfile(
        CTX,
        SITE_ID,
        { contact: { publicEmails: ["b@example.com"] } },
        header,
      ),
    ]);
    expect(
