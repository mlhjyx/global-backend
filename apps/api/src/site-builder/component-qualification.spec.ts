...
const completeArtifacts: Readonly<Record<string, ComponentQualificationArtifact>> = {
...
describe('M1-e-A component qualification gate', () => {
  it('classifies the original ten release components as transitional debt', () => {
    ...
    expect(Object.isFrozen(SITE_SPEC_TRANSITIONAL_RELEASE_COMPONENT_TYPES)).toBe(
      true,
    );
  ...
  it('verifies every registered qualification artifact against checked-in bytes', () => {
    ...
    for (const registered of Object.values(
      ...
      const bytes = readFileSync(resolve(repositoryRoot, registered.repositoryPath));
