...
function makeLauncher() {
  ...
  const temporal = { client: { workflow: { start, getHandle } } } as unknown as TemporalClient;
  return { launcher: new TemporalDemoV0Launcher(temporal), start, describe, getHandle };
}
...
describe('TemporalDemoV0Launcher exactly-once ACK contract', () => {
  ...
  it('closed duplicate（ACK 丢失窗口）经 describe 修复，优先取 execution chain 的 firstRunId', async () => {
    ...
    start.mockRejectedValue(
      new WorkflowExecutionAlreadyStartedError('already started', WORKFLOW_ID, 'demoV0Workflow'),
    );
