...
export function normalizeBuildRequest(
  ...
  if (
    ...
    (options.locales[0] !== "en" ||
      options.locales.some(
        (locale) =>
          !(COPY_GENERATION_LOCALES as readonly string[]).includes(locale),
      ))
  ) {
    unavailable(
      "BUILD_OPTION_UNAVAILABLE",
      "locales must start with source locale en and use the frozen M1-d generation set",
    );
  }
