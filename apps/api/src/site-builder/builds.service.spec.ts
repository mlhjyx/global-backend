...
interface ActiveV2Base {
  ...
  spec: SiteSpecV1_1;
  specVersion: string;
  release: { status: string; manifest: ReleaseManifestV2 };
  copyBundles: Array<{
    locale: string;
...
function activeV2Base(fixture: M1ebGoldenFixture): ActiveV2Base {
  const id = ACTIVE_VERSION_ID;
  ...
  const manifest: ReleaseManifestV2 = {
    ...
    producerToken: "55555555-5555-4555-8555-555555555555",
    artifactPrefix: `sites/${SITE_ID}/releases/33333333-3333-4333-8333-333333333333`,
    ...
    files: [],
    componentLibraryVersion: spec.componentLibraryVersion,
  ...
  return {
    ...
    copyBundles: Object.entries(spec.copyBundleSet.bundles).map(
      ([locale, bundle]) => ({
        locale,
        claimSnapshotId: bundle.claimSnapshot.id,
        taskAttemptId: `copy-attempt-${locale}`,
...
function makeService(
  opts: {
    ...
    activeSpec?: SiteSpec;
    activeBase?: ActiveV2Base;
  } = {},
) {
  const activeSpec = opts.activeBase?.spec ?? opts.activeSpec;
  const db: FakeDb = {
    sites:
        ...
        : [
            {
              id: SITE_ID,
              workspaceId: CTX.workspaceId,
              activeVersionId: activeSpec ? ACTIVE_VERSION_ID : null,
            },
          ],
  ...
  const tx = {
    ...
    siteVersion: {
      findFirst: async () =>
        opts.activeBase ??
        (activeSpec ? { id: ACTIVE_VERSION_ID, spec: activeSpec } : null),
      ...
      updateMany: async ({
        ...
        for (const step of db.steps) {
          if (
            ...
            (!where.status?.in ||
              where.status.in.includes(step.status as string))
          ) {
    ...
    siteBuildBudget: {
      ...
      findUnique: async ({ where }: { where: { buildRunId: string } }) =>
        db.budgets.find((budget) => budget.buildRunId === where.buildRunId) ??
        null,
      updateMany: async ({
        where,
        data,
...
describe("BuildsService.create", () => {
  ...
  it("accepts active page/section targets and freezes their exact base SiteVersion", async () => {
    const pageId = approvedBase.spec.pages[1]!.id;
    const sectionId = approvedBase.spec.pages[0]!.puck.content[0]!.props
      .id as string;
    for (const request of [
      { scope: "page" as const, targetId: pageId },
      { scope: "section" as const, targetId: sectionId },
      {
        scope: "site" as const,
        options: {
          pages: approvedBase.spec.pages.map((page) => page.id),
        },
      },
    ]) {
      const { service, db, launchedInputs } = makeService({
        activeBase: approvedBase,
      });
      await expect(
      ...
      expect(launchedInputs[0]).toMatchObject({
        scope: { ...request, baseVersionId: ACTIVE_VERSION_ID },
      });
    }
  });
  ...
  it("replays a durable partial-build key even if the active pointer later changes", async () => {
    ...
    const request = {
      scope: "page" as const,
      targetId: approvedBase.spec.pages[1]!.id,
