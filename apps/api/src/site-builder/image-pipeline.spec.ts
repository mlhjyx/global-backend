...
describe('M1-c deterministic image policy', () => {
  ...
  it.each([0, -1, 1.5, Number.NaN])(
    ...
    (timeoutMs) => {
      expect(() => new IsolatedImagePipelineRunner(timeoutMs)).toThrow('timeoutMs must be positive');
    },
  );
  ...
  it('rejects excessive, duplicate and malformed plans before starting a child', async () => {
    ...
    await expect(runner.render(input, [validPlan, validPlan])).rejects.toThrow('invalid or duplicate hashes');
    await expect(runner.render(input, [{ ...validPlan, recipeHash: 'not-a-sha256' }])).rejects.toThrow(
