...
describe('DQ-1 契约守卫：demo 生产端符合 @global/contracts 的 SiteSpec 信封', () => {
  it('版本一致：Demo v0 永久显式使用 SiteSpec v1', () => {
    expect(DEMO_SPEC_VERSION).toBe(SITE_SPEC_V1_VERSION);
  });
...
describe('buildDemoSpec（demo v0：行业模板 + 注册信息填充，02 §4 快速通道）', () => {
  ...
  it('sanitizePolish：虚构指征（年限/认证/产能）命中即弃字段，干净文案放行', () => {
    ...
    expect(
      sanitizePolish({
        headline: '   ',
        subhead: 'x'.repeat(501),
        aboutBody: 'a'.repeat(
          QUALIFIED_COMPONENT_CONTENT_BUDGETS.AboutBlock.body + 1,
        ),
      }),
    ).toEqual({});
  });
  ...
  it('超出 AboutBlock 预算的润色文案回退到受限模板', () => {
    const doc = buildDemoSpec({
      siteName: 'Acme',
      intake: INTAKE,
      polish: {
    ...
    const aboutBody = doc.copyBundles.en['about.body'];
    expect(aboutBody).not.toHaveLength(
      QUALIFIED_COMPONENT_CONTENT_BUDGETS.AboutBlock.body + 1,
    );
    expect(() =>
      assertQualifiedComponentContentBudget('AboutBlock', {
        title: doc.copyBundles.en['about.title'],
        body: aboutBody,
      }),
    ).not.toThrow();
  });
  ...
  it('最长合法 intake 与过长 polish 仍产出符合已晋级组件预算的 demo', () => {
    ...
    expect(() =>
      assertQualifiedComponentContentBudget('ProductGrid', {
        ...
        products: [
          {
            name: en['products.p1.name'],
            blurb: en['products.p1.blurb'],
          },
        ],
      }),
    ).not.toThrow();
    ...
    expect(() =>
      assertQualifiedComponentContentBudget('FaqAccordion', {
        title: en['faq.title'],
        items: [{ question: en['faq.q1'], answer: en['faq.a1'] }],
      }),
    ).not.toThrow();
    expect(en['faq.a1'].match(/United States/g)).toHaveLength(1);
  });
});
