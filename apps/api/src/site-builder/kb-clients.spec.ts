...
describe("R2-A2 typed KB dependency errors", () => {
  it("embedding 默认复用统一 New API 地址和 Bearer token", async () => {
    ...
    await expect(new EmbeddingsClient().embed(["hello"])).resolves.toHaveLength(
      1,
    );
  ...
  it("Docling 泛 400 不足以证明文档损坏，按 dependency retryable 处理", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(
        async () => new Response("gateway rejected request", { status: 400 }),
      ),
    );
  ...
  it("embedding 错维度是 typed invalid response，仍可重试而非终结文档", async () => {
    ...
    const error = await new EmbeddingsClient()
      .embed(["hello"])
      .catch((err) => err);
  ...
  it.each([
    ...
    "embedding 拒绝%s，不能把向量静默映射到错误文本",
    async (_label, indexes) => {
  ...
  it.each([
    ...
    "%s 统一产生 typed dependency error，而不是原生 TypeError",
