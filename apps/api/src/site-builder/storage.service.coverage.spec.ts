...
function storageService(env?: Record<string, string | undefined>): StorageService {
...
function clientHarness(env?: Record<string, string | undefined>) {
...
describe('StorageService startup lifecycle admission', () => {
  it('allows explicitly unavailable development storage but not an unavailable strict runtime snapshot', async () => {
    ...
    const strict = storageService({ NODE_ENV: 'production', S3_ACCESS_KEY: undefined });
    await expect(strict.onModuleInit()).rejects.toThrow('S3_ACCESS_KEY is required in production');
  });
