...
export interface DesignSpecEvaluationManifestPrepManifest {
  schemaVersion: typeof DESIGN_SPEC_EVALUATION_MANIFEST_PREP_SCHEMA_VERSION;
  prepId: typeof DESIGN_SPEC_EVALUATION_MANIFEST_PREP_ID;
...
export function assertDesignSpecFixedSourceCommitOnMain(
  repositoryRoot: string,
  fixedCommitSha: string,
): void {
  if (!/^[a-f0-9]{40}$/.test(fixedCommitSha)) {
    throw new Error("design_spec source commit requires a 40-character SHA");
  }
  ...
  try {
    execFileSync(
      ...
      { cwd: realRepositoryRoot, stdio: "ignore" },
    );
  } catch {
    throw new Error(
      "design_spec fixed source commit must be reachable from the prep head",
    );
  }
  try {
    execFileSync(
    ...
    throw new Error(
      "design_spec fixed source commit must already be reachable from origin/main",
    );
  }
}
...
export function buildDesignSpecEvaluationPrepManifest(
  fixedCommitSha: string,
  compiledContracts: CompiledContractsAttestation,
): DesignSpecEvaluationManifestPrepManifest {
  if (!/^[a-f0-9]{40}$/.test(fixedCommitSha)) {
  ...
  const withoutDigest = {
    schemaVersion: DESIGN_SPEC_EVALUATION_MANIFEST_PREP_SCHEMA_VERSION,
    prepId: DESIGN_SPEC_EVALUATION_MANIFEST_PREP_ID,
...
export async function writeDesignSpecEvaluationPrepManifestCreateOnly(
  repositoryRoot: string,
  repositoryRelativePath: string,
  manifest: DesignSpecEvaluationManifestPrepManifest,
  compiledContracts: CompiledContractsAttestation,
): Promise<void> {
  if (
  ...
  const canonicalManifest = buildDesignSpecEvaluationPrepManifest(
