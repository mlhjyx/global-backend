...
export const COPY_SONNET_RECOVERY_ZERO_CALL_PREFLIGHT_OUTPUT_PATH =
  "docs/evidence/site-builder/m1-g-copy-sonnet-recovery-zero-call-preflight-v16.json" as const;
export const COPY_SONNET_RECOVERY_ZERO_CALL_PREFLIGHT_SCHEMA_VERSION =
  "site-builder-copy-sonnet-recovery-zero-call-preflight/2026-08-10-v1" as const;
export const COPY_SONNET_RECOVERY_ZERO_CALL_PREFLIGHT_ARTIFACT_ID =
  "site-builder-copy-sonnet-recovery-zero-call-preflight/2026-08-10-v16-v1" as const;
...
export const COPY_SONNET_RECOVERY_MAXIMUM_LIFETIME_MS =
  24 * 60 * 60 * 1_000;
...
export interface CopySonnetRecoveryObservedRequest {
  method: string;
  authority: "new_api_admin" | "new_api_bearer" | "openox_public";
  path: string;
}
...
function exactArtifactKeys(
  ...
  return (
    ...
    exactKeys(artifact.route, ["channelId", "channelName", "group"]) &&
  ...
  try {
    ...
    const observationsValid = artifact.controlPlaneObservation.requests.every(
      (request) => {
        ...
        ["new_api_admin", "new_api_bearer", "openox_public"].includes(
          authority,
        ) &&
        ["GET", "POST"].includes(method) &&
        (authority !== "openox_public" ||
    ...
    if (
      ...
      artifact.credential.purpose !==
        COPY_SONNET_RECOVERY_CREDENTIAL_PURPOSE ||
