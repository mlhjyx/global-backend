...
const MISSING_TASK_CONTEXT = Object.freeze([
  'audience',
  'brand_voice',
  'prohibited_assertions',
  'cta_policy',
...
const PLAN = {
  ...
  taskContract: {
    ...
    missingContext: MISSING_TASK_CONTEXT,
    status:
  ...
  fixtureAdmission: {
    ...
    repeats: REPEATS,
    status:
      CURRENT_FIXTURE_IDS.length >= REQUIRED_FIXTURE_COUNT
        ? 'READY'
...
export function validateCopyEvaluationV2Plan(plan: unknown): void {
  if (
    JSON.stringify(canonicalize(plan)) !== EXPECTED_PLAN_DIGEST_INPUT
  ) {
