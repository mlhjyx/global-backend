...
export const SAM_DEFAULT_SINCE_DAYS = 120; // Sources Sought 意图窗（与 TTL 一致）
export const SAM_DEFAULT_MAX_RECORDS = 500; // 有界样本（整包 CSV 过滤后取最新 N 条）
...
/**
 * SAM.gov Sources Sought 规范查询（**NAICS 无关**）：整包 CSV 无服务端按码过滤 → 指纹只含 window/窗参数，
 * 全部 ICP 收敛成**每窗口一次下载**（ingest-once 账本天然去重），投影层再逐 ICP 按 NAICS 子树过滤。
 */
export interface CanonicalSamSpec {
  provider: 'samgov';
  kind: 'sources_sought';
  sinceDays: number;
  maxRecords: number;
...
export type CanonicalQuerySpec = CanonicalTedSpec | CanonicalFdaSpec | CanonicalSamSpec;
...
export function canonicalSamSpec(input: { sinceDays?: number; maxRecords?: number }): CanonicalSamSpec {
  return {
    provider: 'samgov',
    kind: 'sources_sought',
    sinceDays: input.sinceDays ?? SAM_DEFAULT_SINCE_DAYS,
    maxRecords: input.maxRecords ?? SAM_DEFAULT_MAX_RECORDS,
  };
}
...
export function queryFingerprint(spec: CanonicalQuerySpec): string {
  let parts: string[];
  if (spec.provider === 'ted') {
    parts = [spec.provider, spec.kind, spec.cpvCodes.join(','), spec.buyerCountries.join(','), String(spec.sinceDays), String(spec.maxRecords)];
  } else if (spec.provider === 'openfda') {
    parts = [spec.provider, spec.kind, spec.productCodes.join(','), spec.applicantCountries.join(','), String(spec.sinceDays), String(spec.maxRecords)];
  } else {
    // samgov：NAICS 无关（整包 CSV 下载一次/窗口，投影层按 NAICS 过滤）
    parts = [spec.provider, spec.kind, String(spec.sinceDays), String(spec.maxRecords)];
  }
