...
import {
  ...
  AlgoliaFairConfig,
  FairExhibitor,
} from "../adapters/trade-fair-algolia";
import {
  searchCompanies,
  listOfficers,
  ChCompanyHit,
  ChOfficer,
} from "../adapters/companies-house";
import {
...
export interface HttpGetOutput {
  status: number;
  ok: boolean;
  /** The bounded Tool result is normalized UTF-8 text; future artifact storage uses this exact media type. */
...
export const httpGetTool: Tool<HttpGetInput, HttpGetOutput> = {
  ...
  idempotencyKey: (i) =>
    `http.get:${stableKey({ url: i.url, method: i.method ?? "GET" })}`,
  durableResultStrategy: {
    kind: "artifact_reference",
    schema: "http-get/v1",
    maxBytes: 3_000_000,
    mediaTypes: [HTTP_GET_ARTIFACT_MEDIA_TYPE],
    privacyClass: "PERSONAL_DATA",
    ttlSeconds: 86_400,
  },
    ...
    try {
      ...
      return {
        data: {
          status: res.status,
          ok: res.ok,
          mediaType: HTTP_GET_ARTIFACT_MEDIA_TYPE,
          text,
...
> = {
  ...
  durableResultStrategy: {
    ...
    schema: CATALOG_RESULT_PROJECTION_SCHEMAS["wikidata.entity"],
  },
  ...
  execute: async (input, ctx) => {
    ...
    if (input.op === "search") {
      return {
        data: {
          search: await wikidataSearchEntity(
            input.name,
            input.limit,
            beforeRequest,
          ),
    ...
    return {
      data: {
        entities: await wikidataGetEntities(
          input.qids,
          input.props,
          beforeRequest,
        ),
...
export const gleifFetchTool: Tool<GleifFetchInput, GleifFetchOutput> = {
  ...
  durableResultStrategy: {
    kind: "typed_projection",
    schema: CATALOG_RESULT_PROJECTION_SCHEMAS["gleif.fetch"],
  },
...
export const tedSearchTool: Tool<TedSearchInput, TedSearchOutput> = {
  ...
  durableResultStrategy: {
    kind: "typed_projection",
    schema: SOURCE_RESULT_PROJECTION_SCHEMAS["ted.search"],
  },
...
export const openFdaSearchTool: Tool<OpenFdaSearchInput, OpenFdaSearchOutput> =
  {
    ...
    durableResultStrategy: {
      kind: "typed_projection",
      schema: SOURCE_RESULT_PROJECTION_SCHEMAS["openfda.search"],
    },
    durableReplayResult: (result) => ({
      data: {
          ...
          ? {
              establishments: result.data.establishments
                ...
                .map((record) => ({
                  ...
                  ownerOperatorNumbers: record.ownerOperatorNumbers.slice(
                    0,
                    64,
                  ),
    ...
    execute: async (input, ctx) => {
      ...
      if (input.kind === "registration") {
        return {
          data: {
            establishments: await searchRegistrations(
              input.params,
              beforeRequest,
            ),
          },
...
> = {
  ...
  durableResultStrategy: {
    kind: "typed_projection",
    schema: CATALOG_RESULT_PROJECTION_SCHEMAS["companies_house.search"],
  },
...
export const inpiRneSearchTool: Tool<InpiRneInput, InpiRneOutput> = {
  ...
  durableResultStrategy: {
    kind: "typed_projection",
    schema: CATALOG_RESULT_PROJECTION_SCHEMAS["inpi_rne.search"],
  },
 ...
 */
export const googlePatentsSearchTool: Tool<
  GooglePatentsInput,
  GooglePatentsOutput
> = {
...
export const tradeFairAlgoliaTool: Tool<
...
> = {
  ...
  durableResultStrategy: {
    kind: "typed_projection",
    schema: CATALOG_RESULT_PROJECTION_SCHEMAS["tradefair.algolia"],
  },
  healthCheck: async () => ({ healthy: true, detail: "algolia" }),
  execute: async (input, ctx) => ({
    data: {
      exhibitors: await queryAlgoliaExhibitors(
        input.cfg,
        input.limit,
        beforeExternalRequest(ctx),
      ),
...
export const mapYourShowFetchTool: Tool<
...
export const samgovSearchTool: Tool<SamSearchInput, SamSearchOutput> = {
  ...
  durableResultStrategy: {
    kind: "typed_projection",
    schema: SOURCE_RESULT_PROJECTION_SCHEMAS["samgov.search"],
  },
  durableReplayResult: (result) => ({
    data: {
      ...(result.data.notices
...
export const DEFAULT_SANCTIONS_UA =
  "GlobalGrowthBackend/1.0 (+sanctions-screening)";
export const sanctionsDownloadTool: Tool<
