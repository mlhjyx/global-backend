...
const context = { workspaceId: 'workspace-test', purpose: ['discovery'] };
...
describe('ted.search 工具边界', () => {
  it('award 分支原样委托参数并只返回 awards 槽位', async () => {
    ...
    expect(adapterMocks.searchAwardNotices).toHaveBeenCalledWith(params);
  ...
  it('contract 分支原样委托参数并只返回 notices 槽位', async () => {
    ...
    expect(adapterMocks.searchContractNotices).toHaveBeenCalledWith(params);
...
describe('openfda.search 工具边界', () => {
  it('registration 分支原样委托参数并只返回 establishments 槽位', async () => {
    ...
    expect(adapterMocks.searchRegistrations).toHaveBeenCalledWith(params);
  ...
  it('510k 分支原样委托参数并只返回 clearances 槽位', async () => {
    ...
    expect(adapterMocks.search510kClearances).toHaveBeenCalledWith(params);
