...
function makeBroker(
  ...
  const { budget, ...deps } = extra ?? {};
  return {
    registry,
    broker: new ToolBroker({
      registry,
      budgetStore: deps.budgetStore ?? new InMemoryBudgetStoreAdapter(budget),
...
describe("ToolBroker — allowedTools 白名单（无超级 Agent 的代码强制）", () => {
  ...
  it("holds product artifact Tools before execute when no governed subject-bound persistence path exists", async () => {
    const execute = vi.fn(async () => ({
      ...
      text: "must never reach a physical wire",
      finalUrl: "https://example.com/",
    }));
    const tool = fakeTool("http.get", 1, execute);
    tool.durableResultStrategy = {
      kind: "artifact_reference",
      schema: "http-get/v1",
      maxBytes: 3_000_000,
  ...
  it("fails closed when no authoritative budget store is injected", async () => {
    ...
    await expect(
      broker.invoke("searxng.search", {}, { workspaceId: "w" }),
    ).rejects.toBeInstanceOf(BudgetStoreUnavailableError);
  });
  ...
  it("fails closed when no authoritative rate-limit store is injected", async () => {
    ...
    const broker = new ToolBroker({
      registry,
      budgetStore: new InMemoryBudgetStoreAdapter(),
    });
    await expect(
      broker.invoke("searxng.search", {}, { workspaceId: "w" }),
  ...
  it("未注册工具 → denied", async () => {
    ...
    await expect(
      broker.invoke("nope.tool", {}, { workspaceId: "w" }),
    ).rejects.toThrow(ToolPolicyDenied);
  });
  ...
  it("声明了 taskContractId 但工具不在该任务 allowedTools → denied", async () => {
    ...
    await expect(
      broker.invoke(
        "searxng.search",
        {},
        { workspaceId: "w", taskContractId: "discovery.qualify_fit" },
      ),
...
describe("ToolBroker — 预算 reserve-then-settle", () => {
  ...
  it("projects the exact Google Patents result and canonical cost facts without a replay hook", async () => {
    ...
    const budgetStore = {
      reserve: vi.fn(async () => ({
        ...
        accountKey: "run",
        operationId: "op",
        estimatedMicrousd: BigInt(2) * 10_000n,
        replay: false,
      })),
      settle,
    } as unknown as BudgetStore;
  ...
  it("converts a first-success replay projector failure into the stable no-second-wire error", async () => {
    ...
    tool.durableReplayResult = () => {
      throw new Error("projection contract unavailable");
    };
    const settle = vi.fn();
    const budgetStore = {
      reserve: vi.fn(async () => ({
  ...
  it("超预算 → BudgetExceededError，工具不执行", async () => {
    ...
    const { broker } = makeBroker(fakeTool("t.expensive", 30, exec), {
      budget,
    });
    budget.open("run1", 20); // 预算 20¢，工具单价 30¢
    await expect(
      broker.invoke("t.expensive", {}, { workspaceId: "w", runId: "run1" }),
  ...
  it("执行开始后的失败保持 reservation，未知物理调用不得退额重试", async () => {
    ...
    budget.open("run1", 10);
    await expect(
      broker.invoke("t.fail", {}, { workspaceId: "w", runId: "run1" }),
...
describe("ToolBroker — Trace + source_policy", () => {
  ...
  it("预算超限也要留 DENIED trace（审计可见）", async () => {
    const traces: { status: string; reason?: string }[] = [];
    const budget = new BudgetLedger();
    const { broker } = makeBroker(fakeTool("t.pricy", 30), {
      budget,
      traceRecorder: (t) => traces.push(t),
    });
    budget.open("run1", 10);
    await expect(
      broker.invoke("t.pricy", {}, { workspaceId: "w", runId: "run1" }),
    ).rejects.toThrow(BudgetExceededError);
    expect(traces).toHaveLength(1);
...
describe("ToolBroker — source_policy fail-closed（收口②：未登记不放行）", () => {
  ...
  it("required + input 提不出域名且无 policyDomain → denied（不静默跳过合规门）", async () => {
    const { broker } = makeBroker(requiredTool(), {
      sourcePolicyReader: async () => ({ suspended: false }),
    });
    await expect(
      broker.invoke(
        "gov.source",
  ...
  it("required + 用途不允许 → denied(purpose_not_allowed)", async () => {
    const { broker } = makeBroker(requiredTool("api.example"), {
      sourcePolicyReader: async () => ({
        suspended: false,
        allowedPurpose: ["outreach"],
      }),
    });
    await expect(
      broker.invoke("gov.source", {}, { workspaceId: "w" }),
  ...
  it("advisory + 未登记域 → 放行（标的公司站点由 robots/DAT-011 兜底）", async () => {
    ...
    const r = await broker.invoke(
      "crawl.subject",
      { url: "https://some-company.example/" },
      { workspaceId: "w" },
    );
    expect(r.data).toEqual({ ok: true });
  });
  ...
  it("advisory + 已登记 SUSPENDED → denied（登记则强制）", async () => {
    ...
    const { broker } = makeBroker(t, {
      sourcePolicyReader: async () => ({ suspended: true }),
    });
    await expect(
      broker.invoke(
        "crawl.subject",
  ...
  it("ctx.purpose：用途门按**本次调用用途**判——域策略只允许 enrichment 时，discovery 调用被拒（TED E2E 抓到的回归）", async () => {
    ...
    await expect(
      broker.invoke(
        "gov.source",
        {},
        { workspaceId: "w", purpose: "enrichment" },
      ),
    ).resolves.toBeDefined();
  });
  ...
  it("ctx.purpose 不在工具声明集内 → 拒（工具不得被用于未声明用途）", async () => {
    ...
    await expect(
      broker.invoke(
        "gov.source",
        {},
        { workspaceId: "w", purpose: "outreach" },
      ),
    ).rejects.toThrow(/purpose/);
  });
