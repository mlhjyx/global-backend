...
import {
  assertDesignSpecV5NativeFeeCardManifest,
  buildDesignSpecV5NativeFeeCard,
} from "../src/site-builder/eval/design-spec-v5-native-fee-card";
...
const HELP = `Usage:
  pnpm --filter @global/api exec tsx scripts/prepare-site-builder-design-spec-v5-native-fee-card.mts \\
    --output=docs/evidence/site-builder/<new-v5-fee-card>.json

This command reads only the committed v5 design_spec manifest and the public
...
export function validateV5EvidenceOutputPath(value: string | null): string {
...
function requiredEvidencePath(): string {
  return validateV5EvidenceOutputPath(option("output"));
}
...
function currentCleanHead(): string {
  ...
  if (dirty)
    throw new Error("v5 fee-card preparation requires a clean worktree");
  const head = execFileSync("git", ["rev-parse", "HEAD"], {
    cwd: REPOSITORY_ROOT,
    encoding: "utf8",
  }).trim();
  if (!/^[a-f0-9]{40}$/.test(head)) {
    throw new Error("v5 fee-card preparation head is invalid");
  }
  return head;
}

export function assertV5FixedSourceReachability(manifest: unknown): void {
  ...
  if (
    ...
    throw new Error("design_spec v5 manifest fixed source commit is invalid");
  }
  for (const revision of ["HEAD", "refs/remotes/origin/main"] as const) {
    try {
      ...
      throw new Error(
        `design_spec v5 fixed source commit must be reachable from ${revision}`,
      );
    }
  }
}

export function assertV5ManifestForPublicPriceRead(manifest: unknown): void {
  assertDesignSpecV5NativeFeeCardManifest(manifest);
  assertV5FixedSourceReachability(manifest);
}
...
async function main(): Promise<void> {
  ...
  if (worktreeManifest !== committedManifest) {
    throw new Error(
      "v5 design_spec manifest differs from the committed HEAD blob",
    );
  }
  const manifest = JSON.parse(committedManifest);
  assertV5ManifestForPublicPriceRead(manifest);
