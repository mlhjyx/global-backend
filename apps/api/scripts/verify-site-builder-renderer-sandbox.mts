...
const fixturePath = path.join(
  repoRoot,
  "apps/site-renderer/fixtures/demo-spec.json",
);
...
try {
  ...
  const env = buildRendererEnv({
    ...
    outDir,
    cacheRoot: path.join(outDir, ".renderer-cache"),
    basePath: "/preview/r1-safety/",
    siteOrigin,
  });
  check(Object.keys(env).length === 9, "Renderer 环境固定为 9 个显式变量");
  ...
  check(
    index.includes("/preview/r1-safety/_astro/"),
    "静态资产路径使用受控 BASE_PATH",
  );
  check(
