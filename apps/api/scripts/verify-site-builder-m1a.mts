...
import {
  ...
  previewRoot,
} from "../src/temporal/site-builder.activities";
...
function requireDevelopmentTargets(): void {
  ...
  for (const name of ["S3_ACCESS_KEY", "S3_SECRET_KEY"] as const) {
    if (!process.env[name]) {
      throw new Error(
        `${name} is required before the M1-a verifier creates fixtures`,
      );
    }
  }
...
async function expectHttp(
  ...
  try {
    ...
    throw new Error(
      `${label}: expected HTTP ${status}, got ${got ?? String(err)}`,
    );
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (
      appRole?.currentUser !== "app_user" ||
      appRole.isSuper ||
      appRole.bypassRls
    ) {
      throw new Error(
        "app connection must be non-superuser app_user without BYPASSRLS",
      );
    }
    ...
    const intakeService = new IntakeService(prisma, {
      launchDemoV0: async ({ buildRunId }) => ({
        firstExecutionRunId: `verify-${buildRunId}`,
      }),
    ...
    const builds = new BuildsService(prisma as never, {
      ...
      cancelRefurbish: async (buildRunId) => {
        // The real launcher resolves only after Temporal compensation has made
        // the terminal state durable. Mirror that contract instead of returning
        // a terminal acknowledgement while leaving the database row active.
    ...
    const b1 = await builds.create(ctxA, siteId, {
      scope: "site",
    ...
    await expectHttp(
      () => builds.create(ctxA, siteId, { scope: "site" }),
      409,
      "同站在飞 409",
    );
    ...
    if (replay.buildId !== b1.buildId)
      throw new Error("idempotency replay miss");
    if (launchedRefurbish.length !== 1)
      throw new Error("replay must not relaunch");
    ...
    const after = await prisma.withWorkspace(wsA, async (tx) => ({
      ...
      version: await tx.siteVersion.findUnique({
    ...
    if (after.site?.status !== "ready")
      throw new Error(`site status ${after.site?.status}`);
    if (
      after.version?.source !== "build" ||
      after.version?.buildStatus !== "succeeded"
    ) {
      throw new Error(
        `version wrong: ${after.version?.source}/${after.version?.buildStatus}`,
      );
    }
    if (
      after.run?.status !== "succeeded" ||
      after.run?.phase !== "P5_publish"
    ) {
      throw new Error(`run wrong: ${after.run?.status}/${after.run?.phase}`);
    }
    const indexHtml = await readFile(
      path.join(previewRoot(), fin.previewSlug, "index.html"),
      "utf8",
    );
    ...
    ok(
      "refurbish",
      `version=${after.version.version} source=build 指针已切 构建 ${buildMs}ms`,
    );
    ...
    if (!comp.site)
      throw new Error("🔴 site was deleted by refurbish compensation!");
    if (comp.site.status !== "ready")
      throw new Error(`site status not restored: ${comp.site.status}`);
    if (comp.site.activeVersionId !== build.versionId) {
      throw new Error("activeVersionId changed by compensation");
    }
    if (comp.run?.status !== "failed")
      throw new Error(`run not failed: ${comp.run?.status}`);
    if (comp.building !== 0)
      throw new Error("building version rows left behind");
    ...
    if (committed.processingStatus !== "queued")
      throw new Error("doc not queued");
    const ingest = await activities.processQueuedKbDocs({
      workspaceId: wsA,
      siteId,
    });
    ...
    if (
      ...
      throw new Error(
        `kb doc wrong: ${JSON.stringify(docs.map((d) => d.status))}`,
      );
    }
    ...
    await prisma.withWorkspace(wsA, (tx) =>
      tx.brandProfile.create({
        data: {
          ...
          siteId,
          version: 1,
          factSheet: { probe: true },
        },
      }),
    );
    const seenByB = await prisma.withWorkspace(wsB, (tx) =>
      tx.brandProfile.count(),
    );
    const seenByA = await prisma.withWorkspace(wsA, (tx) =>
      tx.brandProfile.count({ where: { siteId } }),
    );
    if (seenByB !== 0)
      throw new Error(`🔴 RLS leak: B sees ${seenByB} brand_profile rows`);
    ...
    if (siteId) {
      try {
        const [siteToDelete, assetsToDelete, variantsToDelete] =
          await Promise.all([
            owner.site.findUnique({
              where: { id: siteId },
              select: { slug: true },
            }),
            owner.asset.findMany({
              where: { siteId },
              select: { objectKey: true },
            }),
            owner.assetVariant.findMany({
              where: { siteId },
              select: { objectKey: true },
            }),
          ]);
        previewSlug ??= siteToDelete?.slug;
        for (const row of [...assetsToDelete, ...variantsToDelete])
    ...
    for (const result of await Promise.allSettled([
      ...
      ...(previewSlug
        ? [
            rm(path.join(previewRoot(), previewSlug), {
              recursive: true,
              force: true,
            }),
          ]
    ...
    try {
      // Workspace deletion is intentionally guarded while a Site exists.
      await owner.site.deleteMany({
        where: { workspaceId: { in: [wsA, wsB] } },
      });
      await owner.workspace.deleteMany({ where: { id: { in: [wsA, wsB] } } });
    } catch (error) {
      console.error("M1-a verifier relational cleanup failed:", error);
      process.exitCode = 1;
    }
    for (const result of await Promise.allSettled([
      prisma.$disconnect(),
