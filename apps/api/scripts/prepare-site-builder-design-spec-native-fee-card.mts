...
import { buildDesignSpecNativeFeeCard } from "../src/site-builder/eval/design-spec-native-fee-card";
...
const HELP = `Usage:
  pnpm --filter @global/api exec tsx scripts/prepare-site-builder-design-spec-native-fee-card.mts \\
    --output=docs/evidence/site-builder/<new-fee-card>.json

This command reads the committed design_spec manifest and the public OpenOx
pricing catalog only. It never reads credentials, calls a model, or changes a
model route. The output is create-only and remains NOT_AUTHORIZED for dispatch.
`;
...
function currentCleanHead(): string {
  ...
  if (dirty) throw new Error("fee card preparation requires a clean worktree");
  const head = execFileSync("git", ["rev-parse", "HEAD"], {
    cwd: REPOSITORY_ROOT,
    encoding: "utf8",
  }).trim();
  if (!/^[a-f0-9]{40}$/.test(head)) {
    throw new Error("fee card preparation head is invalid");
  }
  return head;
}
...
export function assertFixedSourceReachability(manifest: unknown): void {
  ...
  if (
    ...
    throw new Error("design_spec manifest fixed source commit is invalid");
  }
  for (const revision of ["HEAD", "refs/remotes/origin/main"] as const) {
    try {
      ...
      throw new Error(
        `design_spec fixed source commit must be reachable from ${revision}`,
      );
    }
  }
}
...
async function main(): Promise<void> {
  ...
  if (worktreeManifest !== committedManifest) {
    throw new Error(
      "design_spec manifest differs from the committed HEAD blob",
    );
  }
  const manifest = JSON.parse(committedManifest);
  assertFixedSourceReachability(manifest);
  ...
  await writeRepositoryJsonCreateOnly(REPOSITORY_ROOT, output, {
    schemaVersion: "site-builder-design-spec-native-fee-card-evidence/v1",
