/**
 * 待办 3 · EPO OPS 发明人身份源 —— 真库真 API 端到端（无 sandbox，CLAUDE.md §5）。
 * 需 postgres 在跑；EPO_OPS_CONSUMER_KEY / EPO_OPS_CONSUMER_SECRET 在 .env（EPO 审批通过后填）。
 * 真欧洲专利公司 = Siemens（DE，海量 EP 专利，稳定出发明人）。
 *
 ...
 *   node --import tsx scripts/verify-epo-ops.mts
 *
 * 四段证明（有界样本）：
 *   A · 真 API：provider.discoverContacts 直打 EPO OPS → 真近期发明人 + personalData + CC BY license
 ...
 *   C · 跨源并：先 seed 同名 Impressum 联系人（decision_maker，带邮箱、无 externalId）→ 再跑 EPO →
 *       Tier 2/3 归一名并进同一行 + identity.merge 证据（兑现待办 2 跨源合并，第 3 个源）。
...
import { EpoOpsInventorProvider } from '../src/discovery/providers/epo-ops.provider';
...
const WS = 'ddddcccc-0000-4000-8000-0000000e0001';
const EPO_DOMAIN = 'ops.epo.org';
const KEY_B = 'vepo-siemens-b.test';
const KEY_C = 'vepo-siemens-c.test';
...
if (!process.env.EPO_OPS_CONSUMER_KEY || !process.env.EPO_OPS_CONSUMER_SECRET) {
  console.error('❌ EPO_OPS_CONSUMER_KEY/EPO_OPS_CONSUMER_SECRET 缺失（.env）——无 creds 无法真打 EPO OPS API。');
  process.exit(2);
}
...
async function main() {
  console.log('\n█ 待办 3 · EPO OPS 发明人身份源（真库真 API）\n');
  await new DiscoveryProviderRegistry().seed(ownerDb); // epo_ops data_provider + ops.epo.org source_policy
  const pol = await ownerDb.sourcePolicy.findUnique({ where: { domain: EPO_DOMAIN } });
  ok(pol?.personalData === true && pol.reviewStatus === 'APPROVED', 'source_policy EPO personalData=true/APPROVED 已 seed');
  await cleanup();

  // ══════════ A · 真 API ══════════
  console.log('\n══ A · 真 API：Siemens(DE) → 近期发明人 ══');
  const provider = new EpoOpsInventorProvider({ broker });
  ...
  ok(res.contacts.every((c) => c.externalIds === undefined), '🔴 无 externalIds（EPO 不走 Tier 0，走归一名并）');
  // 🔴 数据最小化硬自检：结果里绝不出现 residence/地址/国籍字段
  const serialized = JSON.stringify(res.contacts);
  ok(!/residence|"address"|nationality|locality|postal/i.test(serialized), '🔴 结果无 residence/地址/国籍（数据最小化）');

  if (!res.contacts.length) {
    console.log('   ⚠️ 无发明人（数据稀疏/API 契约需校准），跳过 B/C');
    return;
  }
  ...
  const p1 = await persist(coB, 'epo_ops', res.contacts);
  ...
  ok(!b1.points.some((p) => p.type === 'external_id'), '🔴 无 external_id 点（EPO 无稳定人 id）');

  const p2 = await persist(coB, 'epo_ops', res.contacts); // 二次跑
  ...
  // ══════════ C · 跨源并（Impressum 同名 → EPO 归一名并进同一行）══════════
  console.log('\n══ C · 跨源并：先 seed 同名 Impressum 联系人 → EPO 跑 → Tier 2/3 名并进同一行 ══');
  ...
  // EPO 侧：同名发明人
  const epoSame = res.contacts.filter((c) => c.fullName === inventor.fullName);
  const pc = await persist(coC, 'epo_ops', epoSame);
  const cAfter = await readCompany(coC.id);
  ok(cBefore.contacts.length === 1, `Impressum 先落 1 条（=${cBefore.contacts.length}）`);
  ok(cAfter.contacts.length === 1 && pc.merged >= 1, `EPO 同名发明人并进同一行（行数仍 1，merged=${pc.merged}）——跨源合并`);
  ...
  // ══════════ D · §8.8 用途门 ══════════
  console.log('\n══ D · §8.8 用途门：去 source_policy discovery 用途 → EPO 零发明人 ══');
  await ownerDb.sourcePolicy.update({ where: { domain: EPO_DOMAIN }, data: { allowedPurpose: ['enrichment'] } });
  const resDenied = await new EpoOpsInventorProvider({ broker }).discoverContacts(TARGET, { workspaceId: WS });
  ok(resDenied.contacts.length === 0, `§8.8 用途门：去 discovery 用途 → EPO 零发明人（=${resDenied.contacts.length}）`);
  await ownerDb.sourcePolicy.update({ where: { domain: EPO_DOMAIN }, data: { allowedPurpose: ['discovery', 'enrichment'] } });
}
...
try {
  ...
  await ownerDb.sourcePolicy
    .update({ where: { domain: EPO_DOMAIN }, data: { allowedPurpose: ['discovery', 'enrichment'] } })
    .catch(() => {}); // D 段失败也复位策略
  await cleanup();
