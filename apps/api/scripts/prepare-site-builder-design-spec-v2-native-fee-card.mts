...
import {
  assertDesignSpecV2NativeFeeCardManifest,
  buildDesignSpecV2NativeFeeCard,
} from "../src/site-builder/eval/design-spec-v2-native-fee-card";
...
const HELP = `Usage:
  pnpm --filter @global/api exec tsx scripts/prepare-site-builder-design-spec-v2-native-fee-card.mts \\
    --output=docs/evidence/site-builder/<new-v2-fee-card>.json

This command reads only the committed v2 design_spec manifest and the public
...
export function validateV2EvidenceOutputPath(value: string | null): string {
...
function requiredEvidencePath(): string {
  return validateV2EvidenceOutputPath(option("output"));
}
...
function currentCleanHead(): string {
  ...
  if (dirty)
    throw new Error("v2 fee-card preparation requires a clean worktree");
  const head = execFileSync("git", ["rev-parse", "HEAD"], {
    cwd: REPOSITORY_ROOT,
    encoding: "utf8",
  }).trim();
  if (!/^[a-f0-9]{40}$/.test(head)) {
    throw new Error("v2 fee-card preparation head is invalid");
  }
  return head;
}

export function assertV2FixedSourceReachability(manifest: unknown): void {
  ...
  if (
    ...
    throw new Error("design_spec v2 manifest fixed source commit is invalid");
  }
  for (const revision of ["HEAD", "refs/remotes/origin/main"] as const) {
    try {
      ...
      throw new Error(
        `design_spec v2 fixed source commit must be reachable from ${revision}`,
      );
    }
  }
}
...
async function main(): Promise<void> {
  ...
  if (worktreeManifest !== committedManifest) {
    throw new Error(
      "v2 design_spec manifest differs from the committed HEAD blob",
    );
  }
  const manifest = JSON.parse(committedManifest);
  assertV2ManifestForPublicPriceRead(manifest);
  ...
  await writeRepositoryJsonCreateOnly(REPOSITORY_ROOT, output, {
    schemaVersion: "site-builder-design-spec-v4-native-fee-card-evidence/v1",
