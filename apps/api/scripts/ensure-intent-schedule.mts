...
import { INTENT_SWEEP_SCHEDULE_ID } from '../src/temporal/understanding.constants';
...
const contract = PLATFORM_SCHEDULES.find((item) => item.id === INTENT_SWEEP_SCHEDULE_ID);
if (!contract) throw new Error('INTENT_SCHEDULE_CONTRACT_MISSING');
...
if (process.argv.includes('--describe')) {
  ...
  try {
    ...
    console.log(`✓ schedule '${INTENT_SWEEP_SCHEDULE_ID}' created from the code-owned action contract`);
  } catch (e) {
