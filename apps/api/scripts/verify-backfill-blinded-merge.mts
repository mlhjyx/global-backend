 ...
 *
 * 运行前必须提供独立 PII_BACKFILL_* 授权，并显式
 * PII_BACKFILL_ISOLATED_VERIFY=true。脚本不读取 .env，只允许隔离、可丢弃的验证库。
 */
...
import { resolvePiiBackfillVerifierAuthorization } from "../src/compliance/pii-backfill-admission";

const BACKFILL_AUTHORIZATION =
  resolvePiiBackfillVerifierAuthorization(process.env);
const OWNER_URL = BACKFILL_AUTHORIZATION.databaseUrl;
const WS = randomUUID();
const EMAIL = "proc@merge.example";
const SUPPRESSION_EMAIL = "blocked@merge.example";
...
async function cleanup(): Promise<void> {
  // WS is generated by this process and never accepted from caller input.
  await owner.suppressionReleaseDecision.deleteMany({ where: { workspaceId: WS } });
  await owner.suppressionRecord.deleteMany({ where: { workspaceId: WS } });
...
async function main(): Promise<void> {
  if (!process.env.PII_ENCRYPTION_KEY)
    throw new Error("PII_ENCRYPTION_KEY 未配置——无法验证");
  await owner.workspace.create({
    data: { id: WS, name: "backfill-merge-verify" },
  });
  ...
  const suppression = await owner.suppressionRecord.create({
    data: {
      workspaceId: WS,
      type: "email",
  ...
  const counts = await runBackfill(BACKFILL_AUTHORIZATION);
  assert(
    counts.merges === 1,
    `② runBackfill 合并 1 行（merges=${counts.merges}），未抛错`,
  );
  const storedSuppression = await owner.suppressionRecord.findUnique({
    where: { id: suppression.id },
    select: { value: true, reason: true },
  });
  assert(
    storedSuppression?.reason === "unsubscribe" &&
      isEncryptedPii(storedSuppression.value),
    "② suppression email 已加密且法定禁联原因未改变",
  );
  const legacyGone = await owner.canonicalContact.findUnique({
    where: { id: legacy.id },
  });
  assert(legacyGone === null, "② legacy 行已删");
  const survAfter = await owner.canonicalContact.findUnique({
    where: { id: survivor.id },
    select: { dedupeKey: true },
  });
  assert(
    survAfter?.dedupeKey === blinded,
    "② survivor 存活且键仍为 bi:v1:（同一身份收敛为一行）",
  );

  const survPts = await owner.contactPoint.findMany({
    where: { contactId: survivor.id },
  });
  const emails = survPts.filter((p) => p.type === "email");
  assert(
    emails.length === 1,
    `② survivor 仅 1 个 email 点（明文点折叠进密文点，未重复）——实得 ${emails.length}`,
  );
  assert(isEncryptedPii(emails[0].value), "② 保留 email 点是密文（enc:v1:）");
  assert(
    emails[0].status === "VALID",
    "🔴② 验证态保全：折叠后 email 点=VALID（#60 P2，未被默认 UNVERIFIED 覆盖丢分）",
  );
  assert(
    emails[0].verifiedAt?.getTime() === VERIFIED_AT.getTime(),
    "② verifiedAt 一并保全",
  );
  const phones = survPts.filter((p) => p.type === "phone");
  assert(
    phones.length === 1 && isEncryptedPii(phones[0].value),
    "② legacy phone 点改挂到 survivor 并加密（无等价点→改挂）",
  );

  const evOnLegacy = await owner.fieldEvidence.count({
    where: { entityType: "contact", entityId: legacy.id },
  });
  const evOnSurvivor = await owner.fieldEvidence.count({
    where: { entityType: "contact", entityId: survivor.id },
  });
  assert(
    evOnLegacy === 0 && evOnSurvivor === 1,
    "② field_evidence 由 legacy 改挂到 survivor（无孤儿证据）",
  );
  ...
  // ③ 幂等再跑
  const again = await runBackfill(BACKFILL_AUTHORIZATION);
  assert(again.merges === 0, `③ 再跑无合并（merges=${again.merges}）——幂等`);
  assert(again.suppressions === 0, "③ suppression email 重跑不重复加密");
