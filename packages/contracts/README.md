...
- 本地重导出：`pnpm --filter @global/api build && node apps/api/dist/main.js --export-openapi`（在 runtime admission 前进入 Nest preview 元数据模式；无需 `DEPLOYMENT_STAGE`、数据库或 Temporal 配置）。
