...
- 本地重导出：`pnpm --filter @global/api build && DEPLOYMENT_STAGE=development NODE_ENV=test node apps/api/dist/main.js --export-openapi`（无需 DB/Temporal，假 `DATABASE_URL` 即可）。
