...
truth-sync 审计基线（合并前）为 `main@a306ffa`（#124）：#121 已落“intake 无条件触发 demo”的行为，#123/#124 已完成事实安全、隐私、可取消超时与失败保站；intake 的目标 API 契约（`Idempotency-Key`、返回 `buildId`、去 `mode`、修 Swagger/OpenAPI）仍待 closeout。
