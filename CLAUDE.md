# CLAUDE.md — 旧 Claude Code 入口兼容

> 本文件仅保留给旧 Claude Code 集成/链接使用；**当前开发者是 Codex，唯一现行规则/跨会话记忆入口是 [AGENTS.md](AGENTS.md)**。下方镜像内容若与 `AGENTS.md` 或权威文档冲突，一律以后者为准。
...
- **Site Builder M0（当前主线）**：注册引导、`Site`/`SiteVersion`/`BuildRun`、素材直传与 KB、Demo v0 Temporal 流程、Astro 渲染器 10 型组件、SiteSpec 1.0.0 共享类型（DQ-1/#117）和 7 个有界 AI Task 路由已存在。#121 已做无条件触发 demo 的行为修复，#123 已禁虚构身份，#124 已落 businessEmail 隔离/可取消超时/失败保站；**R0 contract closeout（#126）已补齐 intake `Idempotency-Key`、`buildId`、稳定错误码、Temporal 启动证据和 code-first OpenAPI，响应固定为 `{siteId,buildId,status:"generating_demo"}` 且不再暴露 `mode`**。**R1-safety ①+②（2026-07-17）均已完成**：随机 0700/0600 临时 SiteSpec + `finally` 清理、Renderer 固定入口与 7 变量 env allowlist，以及 API/Crawl4AI 双层 egress gate、fake-IP 窄回退和连接钉扎均已真机验证。**R2-A1 Asset 正确性门（2026-07-17）已完成**：commit 先以 attempt+UUID fencing token+lease CAS 认领，再在事务外 hash/copy，canonical 真值与 parked cleanup Outbox 同事务落库后才清 staging；瞬时失败保留 staging 转 `failed_retryable`，P2002 收敛为 `duplicate`，DELETE 改 DB tombstone。**R2-A2 KB 正确性门（2026-07-17）已完成**：单素材 lease/fencing、文档+chunks+Asset ready 原子提交、typed retry/terminal、确定性历史 reconciliation、5 分钟 recovery/告警与人工 redrive 已落，并经开发环境真 PostgreSQL/RLS/MinIO/Docling/BGE 验证。R2-A4 前 cleanup 命令只持久停靠；MF-0 scanner 前 canonical 绝不自动删。
...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）、R1-safety ①+②、**R2-A1–A4、MF0-A/B 与 M1-c** 均已闭环，不把 DOC-12 重新描述为一项代码能力。**下一能力项**：M1-d 多语种事实受限文案；R1-min 原子预览安全门须在 M1-e 可见预览前完成。M1-c 已接纯 Sharp writer/refurbish P2，但没有公开 process/select API 或 Renderer `<picture>`；后两者不能冒充完成。
>
