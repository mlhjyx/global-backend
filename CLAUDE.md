...
> 🔴 **2026-07-13 起获客侧开发暂停**（用户指示，明确通知才恢复）。**当前主线 = 独立站建设（Site Builder）**：设计文档 **00-11 + 新建 13/14** 见 [docs/site-builder/](docs/site-builder/)（00 决策台账 / 01 PRD / 02 架构+决策 / 03 agents / 04 SiteSpec 契约 / 05 部署 / 06 安全 / 07 API / 08 评测 / 09 M1 施工图 / 10 选型 / 11 Readdy / **13 设计智能层领域模型 / 14 MF-0 媒体地基**），承重决策进 [ADR-013~019](docs/adr/registry.md)，as-built=DQ-1 SiteSpec 共享契约 #117（`@global/contracts` 1.0.0）+ 7 有界 AI Task + 渲染器 10 组件。**DOC-12（2026-07-16）已把外部草稿 v3.2 的 362 条实质分发进 00-14 + 权威层，v3.1/v3.2 归档为 dated proposal（12 号非权威）**。工作流程红线=**先研究对标→讨论→用户认可→才提交**。本节以下为获客侧冻结内容。
