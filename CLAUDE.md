...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync（#125）已收口项目级状态、00–14 漂移与接入说明，R0 contract（#126）与 R1-safety ①+②均已闭环，不把 DOC-12 重新描述为一项代码能力。**下一关键路径**：R2-A（拆分 Asset/KB/Profile 正确性状态机）→ MF-0-thin → M1-c 纯 Sharp。R1-min 其余原子预览与未知组件 fail-closed 可并行；严禁把 R2-A 做成 schema/API/Temporal/迁移一体的 mega PR。
>
