...
> 本文件仅保留给旧 Claude Code 集成/链接使用；**当前开发者是 Codex，唯一现行规则/跨会话记忆入口是 [AGENTS.md](AGENTS.md)**。从 §1 开始与 AGENTS.md 同步，如有冲突以 AGENTS.md 为准。
...
cd /global/backend/apps/api && pnpm test               # vitest
```
...
- **Site Builder M0（当前主线）**：注册引导、`Site`/`SiteVersion`/`BuildRun`、素材直传与 KB、Demo v0 Temporal 流程、Astro 渲染器 10 型组件、SiteSpec 1.0.0 共享类型（DQ-1/#117）和 7 个有界 AI Task 路由已存在。截至 truth-sync 审计基线 `a306ffa`（#124）：#121 已做无条件触发 demo 的**行为修复**，#123 已禁虚构身份，#124 已落 businessEmail 隔离/可取消超时/失败保站。⚠️ #121 没有完成目标 API 契约：intake 仍缺 `Idempotency-Key` 与 `buildId`，响应仍有 `mode`，Swagger 仍描述旧诊断分叉；不得把 R0 contract closeout 标成已完成。
...
>
> **DOC-12 状态**：#119/#120 已把主要设计内容分发入活文档；2026-07-16 truth-sync 已收口项目级状态、00–14 漂移与接入说明，不把 DOC-12 重新描述为一项代码能力。**下一关键路径**：① R0 contract closeout（intake `Idempotency-Key`、返回 `buildId`、去 `mode`、修 Swagger/OpenAPI）→ ② R1-safety（临时/构建清理、renderer 子进程 env allowlist）→ ③ R2-A（拆分 Asset/KB/Profile 正确性状态机）→ ④ MF-0-thin → ⑤ M1-c 纯 Sharp。R1-min 其余原子预览与未知组件 fail-closed 可并行；严禁把 R2-A 做成 schema/API/Temporal/迁移一体的 mega PR。
>
