...
- **已先构建**：`pnpm --filter @global/api build`（unit 跑 `dist/`，不含热重载）。
...
> **改代码后**：systemd 跑的是构建产物，`pnpm --filter @global/api build` 后要 `systemctl restart global-api global-worker` 才生效。
