...
ok WRITER "SELECT 1 / (has_function_privilege(session_user,'revoke_platform_execution_authority_v1(UUID, TEXT, TIMESTAMPTZ)'::regprocedure,'EXECUTE')::integer);"
...
ok WRITER "BEGIN; SELECT * FROM ingest_and_admit_platform_execution_budget_run_v2('https://platform-writer-verification.invalid','global-backend:execution-budget','10000000-0000-4000-8000-000000000001','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','execution-budget-grant/v1','platform.acquisition','schedule','acq-sweep','5e960ccef72129aa32bdd9464c9d7b546e5ed6dd7a639caad46df77edea3448e','platform-writer-verification','10000000-0000-4000-8000-000000000001','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','USD','microusd',1,1,1,clock_timestamp(),clock_timestamp(),clock_timestamp()+interval '1 minute','platform.acquisition','schedule','acq-sweep','acq-sweep','5e960ccef72129aa32bdd9464c9d7b546e5ed6dd7a639caad46df77edea3448e','platform-writer-verification','10000000-0000-4000-8000-000000000001','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'); SELECT * FROM revoke_platform_execution_authority_v1((SELECT id FROM execution_budget_authority WHERE issuer='https://platform-writer-verification.invalid' AND jti='10000000-0000-4000-8000-000000000001'::uuid),'platform-writer-verification',clock_timestamp()); SELECT 1 / ((EXISTS (SELECT 1 FROM execution_budget_authority_revocation revocation JOIN execution_budget_authority authority ON authority.id=revocation.authority_id WHERE authority.issuer='https://platform-writer-verification.invalid' AND authority.jti='10000000-0000-4000-8000-000000000001'::uuid))::integer); ROLLBACK;"
ok ADMIN "SELECT 1 / ((NOT EXISTS (SELECT 1 FROM execution_budget_authority WHERE issuer='https://platform-writer-verification.invalid' AND jti='10000000-0000-4000-8000-000000000001'::uuid))::integer);"
